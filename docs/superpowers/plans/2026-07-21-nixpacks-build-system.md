...
**Architecture:** Add a `Builder` type selector to `.tengiz.yaml` (`build.builder: "nixpacks"`). When set, override the framework detection to `FrameworkNixpacks` and dispatch to a new `buildWithNixpacks()` method that runs `nixpacks build` via `os/exec`. Nixpacks produces a Docker image directly — the rest of the pipeline (docker run, proxy, idle, health) remains unchanged. The `Builder` struct gets a setter for `NixpacksConfig` (extra APT packages, start command, etc). Existing callers (CLI deploy, gitdeploy pipeline, preview manager) minimally pass config through.

**Tech Stack:** `nixpacks` CLI (external dependency, `npm install -g nixpacks` or `brew install nixpacks`), Go `os/exec` + `exec.LookPath`, existing `internal/builder` package.
...
- Create a feature branch first: `git checkout -b feat/nixpacks-build-system`
...
In `internal/types/types.go`, replace lines 42-45:
...
- Produces: `FrameworkNixpacks` constant (value `"nixpacks"`)
...
Add imports at top of `builder_test.go`: `"strings"` is already imported.
...
Add imports: `"os/exec"` is already imported. Add `"strings"` to imports.

Replace the `Builder` struct and its methods:
...
Run: `go test ./internal/builder/... -v -run "TestBuildWithNixpacks" -count=1`
Expected: PASS (may skip `TestBuildWithNixpacksDispatches` if nixpacks CLI not installed, but setter test and compile-check pass)

- [ ] **Step 5: Run all builder tests**

Run: `go test ./internal/builder/... -v -count=1`
Expected: PASS (TestBuildCapturesOutput and TestBuildWithDeploymentIDCompiles may skip if docker not available)
...
**Files:**
- Modify: `internal/config/config.go:109-110`
- Test: `internal/config/config_test.go`

**Interfaces:**
- Consumes: extended `BuildConfig` from Task 1
- Produces: merged `Build.Builder` and `Build.NixpacksConfig` from env-specific config to base config
...
- Produces: `builder.Builder` with nixpacks config set and detection overridden when `cfg.Build.Builder == "nixpacks"`

- [ ] **Step 1: Understand the current deploy flow**

In `internal/cli/root.go:173`, `cfg` is loaded via `config.LoadForEnvironment(projectRoot, envFlag)`. At line 187, `builder.Detect(projectRoot)` runs and returns a `Detection`. At line 199, `b := builder.New(dataDir)`. At line 201, `b.Build(...)` is called.

When `cfg.Build.Builder == "nixpacks"`, we need to:
1. Override `detection.Framework` to `FrameworkNixpacks`
2. Call `b.SetNixpacksConfig(cfg.Build.NixpacksConfig)`

- [ ] **Step 2: Write the test**

No pure unit test for CLI (it's in `main` package). The builder tests already cover the dispatch. Verification is via `go vet` and existing tests.

- [ ] **Step 3: Modify `root.go` deploy command**

After line 187 (`detection, err := builder.Detect(projectRoot)`) and the error check, add:
...
- Modify: `internal/preview/manager.go:18-23,46-69,143-151`

**Interfaces:**
- Consumes: for gitdeploy: stored app config with `Build.Builder` and `Build.NixpacksConfig`; for preview: nixpacks config passed into `Manager` struct
- Produces: builder correctly configured for nixpacks when the app's config specifies it

- [ ] **Step 1: Modify `gitdeploy/deployer.go`**

After the `existingApp` lookup block (after line 102, closing brace of `if lookupErr == nil {`), add:
...
func TestPipelineDeployWithNixpacksDetectionOverride(t *testing.T) {
	// Integration test — verify compiled method dispatch
...
In `Create()` (line 61-64), after `detection` is returned but before build, add:
...
In `Update()` (line 143-146), after `detection` is returned but before build, add:
...
- [ ] **Step 5: Write test for preview nixpacks path**
...
func TestManagerSetNixpacksConfig(t *testing.T) {
	...
	rt := runtime.NewStub()
	m := NewManager(t.TempDir(), store, rt)
	if m.nixpacksCfg != nil {
		t.Error("expected nil initially")
	}
...
### Task 7: Run full test suite, verify, and document
...
- Nixpacks support: `build.builder: nixpacks` in `.tengiz.yaml` selects Nixpacks build backend; `build.nixpacks.packages`, `build.nixpacks.apt_packages`, `build.nixpacks.cmd` configure Nixpacks behavior; requires `nixpacks` CLI in PATH
...
- Task 3 covers `buildWithNixpacks()` method, `SetNixpacksConfig()` setter, and dispatch logic in `Build()`
- Task 4 covers config merge of builder fields in `LoadForEnvironment`
- Task 5 covers CLI deploy command wiring
- Task 6 covers gitdeploy and preview pipeline wiring
- Task 7 covers verification and documentation

**2. Placeholder scan:** No TODOs, TBDs, or "add validation" placeholders. Every step has actual code. Test skip on missing CLI is an explicit `t.Skip` with reason.

**3. Type consistency:** `buildWithNixpacks` returns `(string, string, error)` matching `buildWithDockerfile`. `NixpacksConfig` is used consistently everywhere. `FrameworkNixpacks` constant value `"nixpacks"` matches `builder` string in config.
