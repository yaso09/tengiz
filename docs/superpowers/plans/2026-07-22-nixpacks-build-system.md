...
In `internal/types/types.go:42` replace `BuildConfig`:
...
- [ ] **Step 5: Run `go vet` and all types tests**

Run: `go vet ./internal/types/... && go test ./internal/types/... -v -count=1`
Expected: PASS
...
In `internal/builder/builder_test.go`, add a test to verify the constant exists:
...
- [ ] **Step 5: Run `go vet` and all builder tests**

Run: `go vet ./internal/builder/... && go test ./internal/builder/... -v -count=1`
Expected: PASS
...
Replace the `Builder` struct and `Build` method, and add new fields/methods:
...
Add `"strings"` to the import block (already has `"os/exec"` for `LookPath`).

- [ ] **Step 4: Run test to verify it passes**

Run: `go test ./internal/builder/... -v -run "TestBuildWithNixpacks" -count=1`
Expected: PASS (may skip if nixpacks CLI not installed, but the compile check tests pass)

- [ ] **Step 5: Run `go vet` and all builder tests**

Run: `go vet ./internal/builder/... && go test ./internal/builder/... -v -count=1`
Expected: PASS (TestBuildCapturesOutput may skip if Docker unavailable)
...
- [ ] **Step 5: Run all config tests + vet**

Run: `go vet ./internal/config/... && go test ./internal/config/... -v -count=1`
Expected: PASS
...
In `internal/cli/root.go:187-201`, detection runs first, then builder is created and used. When `.tengiz.yaml` has `build.builder: nixpacks`, override the detection framework to `FrameworkNixpacks` and pass the nixpacks config.

- [ ] **Step 2: Modify `root.go` deploy command**

After `cfg, err := config.LoadForEnvironment(...)` (line 173) and `detection, err := builder.Detect(projectRoot)` (line 187), add the override after line 191:
...
- Modify: `internal/preview/manager.go:18-23`

**Interfaces:**
- Consumes: `cfg.Build.Builder`, `cfg.Build.NixpacksConfig` for gitdeploy (goes through stored app config); for preview, detection-based nixpacks
- Produces: builder correctly configured for nixpacks when the app's config specifies it

- [ ] **Step 1: Modify `gitdeploy/deployer.go`**

After the `if lookupErr == nil` block (closing brace at line 102), add:
...
In `Create()` after detection (after line 64):
...
In `Update()` after detection (after line 146):
...
Expected: all existing tests pass (Docker-dependent tests may skip)
...
Read `AGENTS.md` and add the nixpacks option in the builder section:

```markdown
- `nixpacks` — Nixpacks build backend. Set `build.builder: nixpacks` in `.tengiz.yaml`. Requires `nixpacks` CLI installed globally.
```

- [ ] **Step 5: Run the final verification**

Run: `go vet ./... && go test ./... -count=1 && go build -o tengiz .`
Expected: all pass
...
**1. Spec coverage:**
- Task 1 covers `BuildConfig.Builder` and `NixpacksConfig` types — adds the config schema
- Task 2 covers `FrameworkNixpacks` constant — enables detection dispatch
- Task 3 covers `buildWithNixpacks()` method and dispatch logic — core build implementation
- Task 4 covers config merge in `LoadForEnvironment` — env-aware builder selection
- Task 5 covers CLI deploy wiring — manual `tengiz deploy` path
- Task 6 covers gitdeploy and preview wiring — automated deploy paths
- Task 7 covers verification and docs — final validation

**2. Placeholder scan:** No TODOs, TBDs, or "add validation" placeholders. Every step has actual code with exact file paths, commands, and expected output.

**3. Type consistency:** All method signatures use existing `(string, string, error)` pattern. `buildWithNixpacks` matches `buildWithDockerfile` signature. Detection framework override is consistent across all 3 callers (CLI, gitdeploy, preview). `NixpacksConfig` fields align with Nixpacks CLI flags.
