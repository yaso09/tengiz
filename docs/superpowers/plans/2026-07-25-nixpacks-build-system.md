...
In `internal/types/types_test.go`, add:
...
In `internal/types/types.go`, replace the `BuildConfig` struct (line 42-45):
...
Add imports for `"encoding/json"` if not already present (needed for mapstructure tags).
...
Expected: FAIL — `buildWithNixpacks` method not defined, `nixpacksCfg` field missing, `SetNixpacksConfig` not defined

- [ ] **Step 3: Implement `buildWithNixpacks` and update `Builder`**

In `internal/builder/builder.go`, modify the `Builder` struct:
...
Add the setter and the `Build` method changes:
...
Replace the `Build` method (line 21-29):
...
Add the helper and the build method:
...
**Files:**
- Modify: `internal/config/config.go:107-109`
...
- [ ] **Step 1: Modify `root.go` deploy command**

After config is loaded (line 183) and after detection (line 191), add the nixpacks framework override:

In `internal/cli/root.go`, after line 191 (the `detected` fmt.Printf line), add:
...
**Files:**
- Modify: `internal/gitdeploy/deployer.go:38` and `:93-102`
- Modify: `internal/preview/manager.go:18-32`

**Interfaces:**
- Consumes: `cfg.Build.Builder`, `cfg.Build.NixpacksConfig` for gitdeploy; for preview, config passed through Manager
- Produces: builder correctly configured for nixpacks when the app's config specifies it

- [ ] **Step 1: Modify `gitdeploy/deployer.go`**

After the existing app config merge block (after line 102, closing brace of `if lookupErr == nil`), add:

```go
if existingApp != nil && existingApp.Config.Build.Builder == "nixpacks" {
    detection.Framework = builder.FrameworkNixpacks
}
if existingApp != nil && existingApp.Config.Build.NixpacksConfig != nil {
...
- [ ] **Step 3: Modify `preview/manager.go` struct and constructor**

Replace the `Manager` struct (line 18-23):
...
In `Create()` method, after detection (around line 60-80, after the `cloneDir` setup and `Detect` call), add:
...
In `Update()` method (around lines 143-151), after detection, add the same pattern:
...
- [ ] **Step 4: Run `go vet ./...` for final verification**
...
**3. Type consistency:** All method signatures use existing `(string, string, error)` pattern. `buildWithNixpacks` matches `buildWithDockerfile` signature. Detection framework override is consistent across all 3 callers (CLI deploy, gitdeploy, preview).
