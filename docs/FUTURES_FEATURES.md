...
## Volume Cloning (Local & Cross-Server)
- **Source:** Coolify
- **Description:** `VolumeCloneJob` duplicates a named Docker volume — either locally on the same server or across servers via an SSH-tunnelled `tar` stream (remote clone uses multiplexed SSH connections). The source volume is streamed into a staging directory and restored into the target volume. Supports cloning volume data between resources and between servers, preserving persistent data without a full backup/restore cycle.
- **Why add to Tengiz:** Cloning is the fastest path to: `tengiz volume clone myapp-data myapp-staging-data` for environment setups, duplicating an app's data before risky deploys, or migrating a volume to another host (complements SSH Remote Deployment, #73). Tengiz's exec-based runtime can implement this with `docker run --rm -v source:/from -v target:/to alpine tar ...` piped through a local or SSH-tunneled stream. Unlike App Cloning (#123, config copy) this duplicates actual persisted bytes. Low-medium effort, high operational value once volume support exists.
- **Detected:** 2026-08-01

## Scheduled Volume Backups (tar + gpg + S3)
- **Source:** Coolify
- **Description:** `VolumeBackupJob` archives a volume's raw contents with `tar`, optionally gpg-encrypts the archive, and uploads it to a configured S3-compatible destination (AWS S3, Cloudflare R2, Backblaze B2). `ScheduledVolumeBackup` model drives cron-based schedules with retention/pruning, and `VolumeBackupRecoveryJob` downloads an archive and restores it into a new or existing volume via `curl` + `tar` streaming. Execution results are tracked and notifications are sent on success/failure.
- **Why add to Tengiz:** Existing backup features cover database logical dumps (#127, #184) and general state (#120), but not raw volume contents. A volume tar is engine-agnostic — it backs up SQLite files, uploaded assets, search indexes, or any persistent state where a logical dump isn't applicable. `tengiz backup create volume myapp-data` + `tengiz backup restore volume <app> <archive>` gives universal data protection. Implementation: `docker run --rm -v <volume>:/data alpine tar czf - /data` streamed to storage; gpg via a small wrapper or Go `crypto/openpgp`. Complements S3 destinations (#186) which provide the reusable storage target. Medium effort, high data-safety value.
- **Detected:** 2026-08-01

## Proxy Configuration Snapshots & Rollback
- **Source:** Coolify
- **Description:** Before every proxy configuration change, the current proxy config is backed up (up to 10 versions stored server-side). `GetProxyConfiguration` computes an md5 hash of the rendered config and only persists when something actually changed (`last_saved_proxy_configuration`), enabling one-click restore of a previous working proxy config if a change breaks routing.
- **Why add to Tengiz:** Tengiz's proxy (`internal/proxy`) is the single point of failure for all deployed apps — a bad domain/config edit can take every app offline. Versioned snapshots give a recovery path: `tengiz proxy backup ls` / `tengiz proxy restore <id>` reverts a broken change in seconds. Implementation: before `SaveConfiguration()`, write the current routing table (`domains`, routes map, port bindings) to `~/.tengiz/proxy-snapshots/<ts>.json` (keep last 10), then diff against the incoming config. Low effort, high reliability value. Distinct from Config Display (#48) which shows current state — this preserves history.
- **Detected:** 2026-08-01

## Deployment Configuration Drift Detection (Hash & Diff)
- **Source:** Coolify
- **Description:** Every resource stores a `config_hash` computed from its full deployment configuration. `Application::isConfigurationChanged()` re-hashes the current config and diffs it against the hash recorded at last deploy; on drift it warns "deployment configuration changed" and prompts a redeploy. `ConfigurationDiffer`/`ConfigurationDiff` provide a structured diff of what changed (env vars, domains, build settings) so operators know exactly why a redeploy is being suggested.
- **Why add to Tengiz:** Today Tengiz has no way to know that an app's `.tengiz.yaml`, env vars, or domains changed since the last successful deploy. This causes "my config isn't live and I didn't notice" surprises. `tengiz deploy` can warn "config changed since last deploy — deploy to apply" and `tengiz status` can show a drift badge. Implementation: compute a stable hash (JSON-canonical form of AppEntry.Config) at deploy time, store in the app record, re-hash on read. Complements Config Display (#48, shows merged config) with a change signal. Low effort, high correctness value.
- **Detected:** 2026-08-01

## Domain Conflict Detection (FQDN Collision Checks)
- **Source:** Coolify
- **Description:** `checkDomainUsage()` scans all resources (applications, service sub-containers, databases) for an FQDN before it's assigned. If a requested domain is already in use by another resource, the user is shown a conflict modal and the assignment is blocked. Prevents silent route hijacking where a new app's `Host(...)` rule shadows an existing app's traffic in the reverse proxy.
- **Why add to Tengiz:** Tengiz's host-based routing (`extractApp()`) routes by subdomain — two apps with the same effective hostname silently collide, and the last-registered route wins. A pre-assignment check in `tengiz domain add` and `tengiz deploy` (when auto-generating `app.tengiz.local`) that queries the store for conflicting FQDNs turns silent breakage into an early, clear error. Implementation: a `CheckDomainAvailable(domain)` helper over the apps store + domains map used before any route is registered. Low effort, prevents a subtle and common class of production bug.
- **Detected:** 2026-08-01

## Cloud Provider Server Provisioning (Hetzner / DigitalOcean / Vultr)
- **Source:** Coolify
- **Description:** `HetznerService`, `DigitalOceanService`, and `VultrService` create server instances directly through provider APIs: upload SSH keys, choose size/region/image, create cloud-init scripts, launch the instance, poll for SSH readiness, then automatically install Docker + prerequisites. `CloudProviderToken` stores per-provider credentials; `CloudInitScript` provides reusable user-data templates. Server provisioning is fully scriptable via the API.
- **Why add to Tengiz:** For users who want a managed alternative without buying Vercel, provisioning the host is currently a manual step (buy a VPS, install Docker, configure Tengiz). `tengiz server create --provider hetzner --region fra1 --size cx22` would launch a droplet, wait for SSH, run the existing Server Bootstrap (#31/40) logic, and register the host — a one-command "Tengiz instance up" flow. Implementation: `internal/cloud/` package with thin REST clients per provider (Hetzner/DO/Vultr all have simple JSON APIs), token stored encrypted in `~/.tengiz/cloud.json`, cloud-init for Docker pre-install. Medium effort; enables multi-server (#194) onboarding without manual steps.
- **Detected:** 2026-08-01

## Team Roles & Invitations (Multi-Member Management)
- **Source:** Coolify
- **Description:** Coolify's `Team` model supports multiple members with a role hierarchy (`MEMBER < ADMIN < OWNER` via `Role::lt()/gt()`), email/link-based invitations with role selection, team switching, and ~15 policy gates (`canAccessTerminal`, `createAnyResource`, etc.) that map roles to permissions. Invitations are consumed with privilege-escalation guards so an invitee can't grant themselves more than the invitation grants.
- **Why add to Tengiz:** A shared Tengiz host used by multiple developers currently has no concept of "who can do what." Team roles give a lightweight authz layer: `tengiz team invite alice --role member`, `tengiz team switch`, and role checks in the webhook/admin/API paths (member = deploy + logs, admin = config + domains, owner = everything). Distinct from User Groups (#196, permission sets) — this is the organizational unit with membership lifecycle. Complements OIDC/SSO (#157) and 2FA (#204). Implementation: `~/.tengiz/teams.json` with role enum + invite tokens, checks in the deploy/webhook handlers. Low-medium effort, high collaboration value.
- **Detected:** 2026-08-01

## Graceful Shutdown with Configurable Stop Grace Period
- **Source:** Coolify
- **Description:** Containers are stopped via `docker stop --time N` where N (`deploymentStopGracePeriodSeconds`, default 30s, clamped to 1–3600) lets in-flight requests drain before SIGKILL. Per-app configurable stop signal is supported. Combined with `restart: unless-stopped` (`RESTART_MODE`), every deploy/stop drains cleanly instead of killing connections mid-request.
- **Why add to Tengiz:** Scale-to-zero (`idle` package) stops containers aggressively — a cold-stop at 5m idle can cut off in-flight requests and corrupt state (SQLite writes, uploads). A configurable grace period means `idle.Stop()` uses `docker stop --time 10` so the app finishes current work before termination. `.tengiz.yaml`'da `deploy.stop_grace_period: 30`; distinct from Readiness Delay (#27, startup) — this is shutdown-side. Complements zero-downtime deploys by making the *stop* half of the lifecycle graceful too. Low effort (one Docker flag), high correctness value for stateful workloads.
- **Detected:** 2026-08-01

## Manual Port Mappings (Host:Container Bindings)
- **Source:** Coolify
- **Description:** `ports_mappings` lets users bind arbitrary host ports to container ports (e.g., `8080:80`, `5432:5432`) for app and database containers, converting a UI-entered `host:container` string into the Docker `ports` array. This is distinct from Coolify's internal service discovery — it exposes containers directly on the host network, which is required for non-HTTP services and external tooling.
- **Why add to Tengiz:** Tengiz auto-allocates ports 9000–9999 for proxy routing, but users currently can't say "I want this app reachable on port 5432" for a database or "3000:3000" for a dev server. `tengiz config set myapp ports "5432:5432"` (or `.tengiz.yaml` `ports:`) gives direct host exposure. Complements Port Mapping Protocol Selection (#111, TCP/UDP choice) with the actual binding config; distinct from the automatic proxy port allocation. Implementation: pass `-p host:container` in `runtime.Create()`, skip auto-allocation when explicit mappings exist, validate conflicts against `ports.json`. Low effort, unblocks DB/API/tooling use cases.
- **Detected:** 2026-08-01

## Build-Time vs Runtime Environment Variable Classification
- **Source:** Coolify
- **Description:** Each environment variable carries `is_buildtime` and `is_runtime` flags controlling when it's injected. `NEXT_PUBLIC_*` style vars are marked build-time (baked into the image during build), while DB URLs and secrets are runtime-only (never present in build logs or image history). Buildpack control prefixes (`NIXPACKS_`, `RAILPACK_`) are auto-detected and excluded from cleanup. Preview vars (`is_preview`) sync automatically to PR deployments.
- **Why add to Tengiz:** Today `tengiz config set` values are injected at runtime only, and build-time values must go through `--build-arg` (#14). A per-var classification lets users declare `tengiz config set myapp NEXT_PUBLIC_API_URL ... --build` and `--runtime` independently, giving precise control over which secrets touch the build phase. `.tengiz.yaml`'da `env: { NEXT_PUBLIC_X: { value: "...", build: true }, DB_URL: { value: "...", build: false } }`. Implementation: extend AppConfig.Env values with a scope flag; builder reads scoped build vars. Low effort, improves security (runtime secrets stay out of build) and DX. Complements Build-Time Secrets (#46/192).
- **Detected:** 2026-08-01

## Platform API IP/CIDR Allowlisting
- **Source:** Coolify
- **Description:** Instance settings expose an allowed IP/CIDR list; requests to the admin surface (API, webhooks, terminal) from non-allowlisted addresses are rejected. Combined with per-IP auth rate limiting, this restricts the control plane to trusted networks even before credentials matter.
- **Why add to Tengiz:** The webhook server, admin API, and proxy are public-facing. An IP/CIDR allowlist means a home-lab or office-hosted Tengiz only accepts control-plane traffic from known ranges — defense-in-depth on top of API keys (#180) and auth rate limiting (#58). `.tengiz.yaml`'da `admin.allowed_ips: ["10.0.0.0/8", "203.0.113.7"]` (empty = allow all). Implementation: a middleware over `net.ParseCIDR` on the admin/webhook listeners. Different from Proxy Security Middleware (#135) which protects *app* traffic — this protects the *platform* API. Low effort, high security value for self-hosters.
- **Detected:** 2026-08-01

## Consistent Deterministic Container Naming
- **Source:** Coolify
- **Description:** `is_consistent_container_name_enabled` makes container names deterministic (`{app}-{stable-hash}`) so every restart reuses the same name instead of appending random suffixes. Combined with label escaping (`is_container_label_escape_enabled` / `is_container_name_escape_enabled`) that sanitizes invalid characters, container references become stable across restarts — which matters for service discovery, `docker exec` scripts, log aggregation, and cross-container networking.
- **Why add to Tengiz:** Tengiz's scale-to-zero cycle stops and restarts containers; if each restart produces a different container name, scripts, health checks, and log drains that reference `tengiz-myapp-<hash>` break silently. A deterministic name derived from app name (+env) guarantees `tengiz exec myapp` and `docker inspect tengiz-myapp` always work. Implementation: hash app+env into the suffix (`tengiz-<app>-<env>-<sha1[:8]>`) in `runtime.ContainerName()`. Complements the existing `ContainerName(name, env)` helper. Very low effort, removes a class of flaky-integration bugs.
- **Detected:** 2026-08-01

## Database Init Script Injection
- **Source:** Coolify
- **Description:** Managed database containers accept a user-defined init script (SQL/SQLite statements or shell) that runs on first container start — e.g., creating schemas, seeding roles, or applying extensions. `ValidCloudInitYaml`-style validation guards the payload. Databases also expose per-engine health checks (`HasDatabaseHealthCheck`) exposed through the DB proxy for readiness gating.
- **Why add to Tengiz:** When Tengiz provisions managed DBs (#63/47), app teams often need `CREATE EXTENSION`, `CREATE USER`, or seed data applied automatically. `tengiz db init <app> --script ./init.sql` mounts the script into the container's init directory (`/docker-entrypoint-initdb.d/` for Postgres/MySQL), which native images run on first boot. Complements database provisioning with schema setup and Database Connection String Auto-Injection (#183). Low effort (volume mount + optional env flag), high convenience value once managed DBs exist.
- **Detected:** 2026-08-01

## Custom Healthcheck Override (Command + Expected Response Matching)
- **Source:** Coolify
- **Description:** Beyond the standard HTTP health check (path, port, method, interval, timeout, retries, start period), users can supply a `custom_healthcheck` — an arbitrary command run in the container — and the HTTP check supports matching on an expected response body/status code. If the custom command or response match fails within the window, the deployment is marked unhealthy and (in rolling updates) rolled back.
- **Why add to Tengiz:** Tengiz's implemented health check (#4) covers HTTP GET on a path. But some apps need: a CLI health probe (`pg_isready`, `curl` internal endpoint), response-body assertions (e.g., `{"status":"ok"}`), or a non-HTTP protocol check (gRPC, TCP). `.tengiz.yaml`'da `healthcheck: { command: "pg_isready -U postgres", expect: "accepting connections", retries: 5 }`. Implementation: `docker exec <container> <cmd>` for command checks + `strings.Contains` on the HTTP response body for expectations, feeding the existing health package's restart/rollback path. Low effort, extends the existing health system to non-HTTP workloads.
- **Detected:** 2026-08-01

## Preview-Scoped Environment Variables & Fork/Untrusted PR Gating
- **Source:** Coolify
- **Description:** Preview deployments support env vars flagged `is_preview` that are auto-synced to PR environments (distinct from production values), so a PR can point at staging DBs without touching prod config. Additionally, preview auto-creation is gated for untrusted sources — fork PRs and PRs from unknown author associations are skipped or require approval (`ProcessGithubPullRequestWebhook` gating), preventing resource abuse and secret leakage to untrusted code.
- **Why add to Tengiz:** Tengiz's implemented preview system (#2) currently uses the same env vars as production — a PR pointing at the prod database is dangerous. Preview-scoped vars (`tengiz preview env set <app> <pr> DB_URL=...`) isolate PR environments. And gating auto-preview creation on fork/untrusted PRs (a `preview.fork_prs: skip|require-approval` setting in `.tengiz.yaml`) prevents arbitrary public-PR authors from spinning up containers on the host. Complements Webhook Event Filtering (#50) with an additional security dimension. Low effort for var scoping, low-medium for gating; high security value for public repos.
- **Detected:** 2026-08-01

## Per-App SSO Gate (OAuth2-Proxy-as-a-Service Forward-Auth)
- **Source:** Dokploy
- **Description:** Deploys the `quay.io/oauth2-proxy/oauth2-proxy` container as a shared auth service (port 4180), then wraps each app domain in a forward-auth middleware. Every request to the app is sent to `/oauth2/auth`; on 401 the paired `errors` middleware rewrites the response into a 302 redirect to the sign-in page (`errors.status: ["401-403"]`, `statusRewrites: {401: 302}`). Authenticated identity headers (`X-Auth-Request-User`, `X-Auth-Request-Email`, `X-Auth-Request-Preferred-Username`, `Authorization`) are copied into the upstream request. OIDC-only (clientId/clientSecret/issuer), email-domain allowlist (`OAUTH2_PROXY_EMAIL_DOMAINS`), and a single central auth-domain router (`/oauth2/*`) shared by all apps. Cookie secret is deterministically derived via HMAC-SHA256 from the master auth secret + per-server salt — no random secret to persist. Enabling SSO on a domain is guarded: the auth proxy must be running first, else `PRECONDITION_FAILED`.
- **Why add to Tengiz:** Existing "OIDC/OAuth Single Sign-On (#157)" covers platform/admin login; this is *per-app* SSO that gates user-facing app traffic — the equivalent of Vercel Password/SSO Protection. It's a ready-made implementation: a `tengiz forward-auth enable` command that starts the oauth2-proxy container (via `os/exec`, matching Tengiz's Docker approach), plus per-domain forwardAuth middleware in `internal/proxy` that calls `/oauth2/auth` and injects identity headers. Complements the implemented Basic Auth (#27) — SSO is the team-friendly upgrade. The deterministic HMAC cookie secret (no storage) and the 401→302 errors-middleware trick are directly portable to Go's `httputil.ReverseProxy`.
- **Detected:** 2026-08-01

## Structured Access-Log Pipeline (JSON Access Logs + Query API)
- **Source:** Dokploy
- **Description:** Strongly-typed, line-delimited JSON access log (`LogEntry`, 48 fields: client addr, downstream/origin status + size + duration, request method/host/path/protocol, router/service names, timestamps, user-agent) with a full query API: substring search on host, status-class filtering (info/success/redirect/client/server → 1xx–5xx), date-range filtering, arbitrary-field sort, and cursor pagination with totalCount. Aggregates entries into per-hour request counts for traffic-over-time charts. Memory-safe streaming reader (first 500 valid lines via readline, `readAll` mode for full). Scheduled rotation (cron, default daily) truncates to last 1000 lines and signals the proxy with `kill -USR1 1` so it reopens the log file — the logrotate dance automated in-app.
- **Why add to Tengiz:** Tengiz's proxy has zero traffic observability. Existing roadmap items are different: "GoAccess (#125)" is a log-file analytics container, "Built-in Platform Analytics (#81)" is JS/Web Vitals injection. This is a structured access log + `tengiz access-logs` query command built into the Go proxy: write JSON Lines per request to `~/.tengiz/access.log`, then filter/search/aggregate in Go (`encoding/json` + `sort`). Hourly aggregation feeds a future traffic dashboard. Low effort (middleware in `internal/proxy`), no external deps, and directly enables `tengiz access-logs myapp --status 5xx --since 1h`.
- **Detected:** 2026-08-01

## Magic Domains via sslip.io Wildcard DNS (Zero-DNS-Config Preview URLs)
- **Source:** Dokploy
- **Description:** Generates one-click random public domains as `{projectName}-{6-hex}-{ip-slug}.sslip.io`, leveraging sslip.io's wildcard DNS (any subdomain resolves to the embedded IP). No real DNS setup required. `generateRandomDomain()` uses `randomBytes(3).toString("hex")`, slugs the IP (`.`/`:` → `-`), and truncates the project name to the 40-char label limit. A second helper builds wildcard-style domains `{appName}-{serverDomain}`. Server IP comes from the server record or platform settings.
- **Why add to Tengiz:** Currently every Tengiz app gets `app.tengiz.local` which only works if the operator runs their own DNS/wildcard. sslip.io gives instantly-accessible public preview URLs with zero DNS configuration — ideal for preview deployments (#2), demos, and testing webhooks from the internet. `tengiz preview deploy` could auto-generate `{app}-{pr}-{ip}.sslip.io` and print a working URL immediately. Low effort (string generation + proxy host routing), high UX value. Complements Domain Conflict Detection (#209) and the domain validation below.
- **Detected:** 2026-08-01

## DNS Validation on Domain Add (Resolve A-Records + CDN Detection)
- **Source:** Dokploy
- **Description:** `validateDomain()` resolves the domain's A records (`dns.resolve4`) and validates they point at the expected server IP before accepting the domain. If the IP matches a known CDN edge range (Cloudflare/Fastly/BunnyCDN/Arvancloud via `detectCDNProvider`), the domain is accepted but the user is warned that the origin IP is masked by the CDN. Returns structured `{isValid, resolvedIp, error, isCloudflare, cdnProvider}`. Companion: IDN → punycode conversion (`тест.рф` → `xn--e1aybc.xn--p1ai`) applied to routing rules since proxies require ASCII.
- **Why add to Tengiz:** Distinct from "Domain Conflict Detection (#209)" (FQDN collision) and "CDN Provider Detection (#136)" (trusted IP extraction) — this validates at *add-time* that a custom domain actually points at the Tengiz server, catching DNS misconfiguration before users wonder why their domain 404s. `tengiz domain add myapp.com` resolves A records, warns if not pointed at the server IP, and converts IDNs to punycode for the proxy's host matching. Low effort (Go `net.LookupHost`), high correctness value for the custom-domain workflow.
- **Detected:** 2026-08-01

## File Mounts (Declarative Config-File Injection into Containers)
- **Source:** Dokploy
- **Description:** Declarative "file mounts": users store file *contents* (not just volume paths) that are written to disk as files at deploy time (`echo "<base64>" | base64 -d > path`) and bind-mounted into the container. Full CRUD (`createFileMount`/`updateFileMount`/`deleteFileMount`), with each service type mapping to its own base files path. Distinct from Patches (#118, build-time source overrides) — these are runtime config files injected into the running container.
- **Why add to Tengiz:** Apps routinely need config files that don't exist in the repo or are environment-specific: `nginx.conf`, `Caddyfile`, `application.properties`, `.npmrc`, cert files. Today a Tengiz user must build them into the image or manage host bind-mounts manually. `tengiz mount file myapp /etc/nginx/nginx.conf --content "$(cat nginx.conf)"` (or `.tengiz.yaml` `file_mounts:`) writes the content and adds `-v` to `runtime.Create()`. Complements the implemented Persistent Storage (#7, volume paths) with content-based mounts. Low effort (base64 write + volume flag), high convenience value.
- **Detected:** 2026-08-01

## Drop-Upload ZIP Deployment (Drag & Drop with Security Guards)
- **Source:** Dokploy
- **Description:** Accepts a ZIP upload as a deployment source. Detects and strips a single root folder, extracts to the code directory with two security checks: path-traversal validation (`readValidDirectory` guards against `../` escapes) and rejection of symlink/device/FIFO zip entries (`isDangerousNode` checks file mode bits). Remote servers receive files via SFTP. Enables a web-UI "drag & drop your folder" deploy with no git repo required.
- **Why add to Tengiz:** Gives non-git users a deployment path (designers, prototype builders, quick fixes) and powers a future web dashboard upload. `tengiz deploy --archive app.zip` extracts (with traversal + symlink guards — critical since malicious archives can overwrite arbitrary host files), then runs the normal framework detection + build. Go's `archive/zip` + `path.Clean`/`filepath.Rel` checks make this ~100 lines. Complements git-based deploy (#5) and webhook deploy (#13) as a third source type. Low effort, meaningful for the "one binary, two interfaces" vision.
- **Detected:** 2026-08-01

## HTTP/3 (QUIC) Support on the Proxy TLS Entrypoint
- **Source:** Dokploy
- **Description:** The TLS proxy entrypoint also listens on a UDP port with an advertised HTTP/3 port (`entryPoints.websecure.http3.advertisedPort`), exposing QUIC/HTTP3 through the proxy alongside HTTP/1.1 and HTTP/2. Port binding exposes the UDP port.
- **Why add to Tengiz:** HTTP/3 reduces latency on high-loss connections and is increasingly expected by browsers for production sites. With built-in Let's Encrypt (#65) on the roadmap, adding `http3: true` to the proxy TLS config gives Tengiz a modern edge. Go's `golang.org/x/net/http3` provides a QUIC implementation; the proxy would serve `http3.Server` on a UDP port alongside the existing `httputil.ReverseProxy`. `.tengiz.yaml`'da `proxy.http3: true`. Low-medium effort, a differentiator most Vercel alternatives lack.
- **Detected:** 2026-08-01

## Consistent Volume Snapshots via Service Quiescing (turnOff + flock Lock)
- **Source:** Dokploy
- **Description:** Before a volume snapshot, the runner can scale the service to zero (`docker service update --replicas=0` for Swarm, `docker stop` for compose) to guarantee a consistent snapshot, then restores replicas after. A cross-platform lock (`flock` on a per-app lock path, with a `mkdir` spin-loop fallback) prevents concurrent backups of the same service. This pairs with the volume-tar backup mechanism (#208) to produce crash-consistent snapshots of stateful volumes.
- **Why add to Tengiz:** A raw `tar` of a live volume (SQLite, Postgres data dir) can capture a torn/corrupt state. Quiescing the service first — via Tengiz's own scale-to-zero infra (`idle` package already stops containers) — makes volume backups (#208) and the built-in datastore trustworthy. `tengiz backup volume myapp --consistent` stops the app, snapshots, restarts. The `flock` guard fits Go's `syscall.Flock`. Low-medium effort on top of existing volume-backup + idle machinery, high data-integrity value.
- **Detected:** 2026-08-01

## Per-Domain Certificate Strategy & Custom Proxy Entrypoint
- **Source:** Dokploy
