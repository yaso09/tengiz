...
## Volume & Directory Backups (tar.gz Archive Engine per Storage Target)
- **Source:** Coolify
- **Description:** New in Coolify 4.1.2 — a general-purpose file/volume backup engine (previously only DB backups existed). A `ScheduledVolumeBackup` schedule attaches to any Docker named volume OR directory (`LocalFileVolume`). On each run it launches an ephemeral helper container that runs `tar -czf - -C /volume . > <archive>` on the server filesystem (`/data/coolify/backups/volumes/...`), verifies the archive is non-empty, and writes an execution record with status/size/duration. Configurable: cron/human frequency, S3 upload (via server-side MinIO client, never through the control plane), `disable_local_backup`, optional **stop-containers-during-backup** (gracefully `docker stop`s app containers that mount the volume, archives, then restarts them) and per-location retention. **"Backup Now"** triggers an immediate run.
- **Why add to Tengiz:** Tengiz stops/restarts containers constantly (scale-to-zero, zero-downtime deploys), so stateful volumes are its riskiest resource. A `tengiz backup volume create --app myapp` that archives any volume/directory as tar.gz via `docker run --rm <helper> tar -czf - .` is two `os/exec` calls and gives every app crash-proof file-level backups — complementing the DB-specific backups already on the roadmap (#127). Consistent archives (stop containers → tar → restart) combined with the existing scale-to-zero model (cold container = free backup window) makes this genuinely low-effort and high-value.
- **Detected:** 2026-08-09

## Volume Backup Retention Engine (Count / Age / Max-Storage-GB)
- **Source:** Coolify
- **Description:** After each backup, `VolumeBackupJob::removeExpiredBackups()` evaluates three independent retention dimensions, separately for local storage and S3: **Amount** (keep only N most recent), **Days** (delete archives older than N days) and **Max storage (GB)** (drop oldest when cumulative size exceeds a decimal-GB limit, e.g. `0.001` = 1 MB). Any limit >0 activates cleanup for that location; dimensions are OR'd ("whichever fires first"). Local archives are removed server-side, S3 objects via the storage client, then the execution row is pruned. Pure, testable logic (~40 lines).
- **Why add to Tengiz:** Self-hosted disk fill-up is the #1 failure mode for single-server PaaS. Existing road-map items mention retention only vaguely (S3 backup features). A pure `retention(executions, limits) → toDelete` function is trivially implementable in Go and prevents: runaway backup bloat (count rule), stale archives (age rule), and unbounded disk/S3 cost (GB rule). Config: `.tengiz.yaml`'da `backups.retention.amount: 5`, `backups.retention.days: 30`, `backups.retention.max_gb: 10`. The same engine serves both DB and volume backups.
- **Detected:** 2026-08-09

## Volume Cloning with Data Copy (Same-Server & Cross-Server)
- **Source:** Coolify
- **Description:** When cloning a Project/Environment/App/DB/Service, persistent volume records are replicated and — when `cloneVolumeData` is toggled — the actual data is copied: **same-server** via `docker volume create <target>` + an alpine container running `cp -a /source/. /target/` + `chown -R 1000:1000`; **cross-server** a tar `tar czf /clone/volume-data.tar.gz .` streamed over SSH (`instant_scp`) to the destination server where it's extracted. The resource is stopped during the copy and restarted after; temp dirs cleaned in `finally`.
- **Why add to Tengiz:** Existing "App Cloning" records (#123) copy *configuration* only. Cloning real volume data unlocks the most common enterprise request: copy a production app + its database to a staging environment with working data, or migrate volumes between hosts. For Tengiz, same-server clone is `docker run --rm -v src -v dst` copy; cross-server reuses the planned SSH remote deployment (#105) with `docker cp`/tar. A `tengiz clone --with-data myapp staging` workflow differentiates Tengiz from config-only cloners.
- **Detected:** 2026-08-09

## Preview Deployment Volume Isolation (Per-PR Volume Suffix)
- **Source:** Coolify
- **Description:** Each persistent volume has an `is_preview_suffix_enabled` flag (default on). During PR preview deployments, the volume name gets a suffix based on the PR id (`myapp-volume` → `myapp-volume-pr-42`), so each preview gets its **own isolated data**, preventing previews from clobbering production data. The suffix logic is applied consistently in the compose generator so `depends_on` and compose references all use the suffixed name.
- **Why add to Tengiz:** Tengiz already has Preview Deployments (#2, implemented), but currently a preview reuses the same volume mounts — a PR that runs migrations against shared state can destroy production data. Per-PR volume suffixing (`{name}-pr-{id}`) is a string-naming function in the deploy pipeline and completely eliminates the classic "preview wiped production volume" incident. It's also the correct default for the scale-to-zero + volumes model: preview data stays scoped, isolated, cleanable with the preview container.
- **Detected:** 2026-08-09

## Scoped Shared Environment Variables (Team/Project/Environment/Server)
- **Source:** Coolify
- **Description:** `SharedEnvironmentVariable` model with a `type` enum: `team` / `project` / `environment` / `server` and unique-key constraints per scope (`key`+`project_id`+`team_id`, etc.). Any app env value can reference a shared value via `{{team.KEY}}`, `{{project.KEY}}`, `{{environment.KEY}}`, `{{server.KEY}}` placeholders — resolved at deploy/run time with proper scope precedence. `is_multiline`, `is_literal`, `is_shown_once` (value displayable only once then masked), `comment` fields, and values are encrypted. Server-scoped variables resolve to the destination server.
- **Why add to Tengiz:** Existing "Variable Resource (Global Interpolation)" (#53/#70) covers one global registry; Coolify proves a hierarchical scope model (team → project → environment → server) is what multi-app/multi-env users actually need. This is a natural extension of Tengiz's existing `variables:` support and `.tengiz.{env}.yaml` merge: define `{{environment.DATABASE_URL}}` once per environment instead of copying into every app. Go implementation: env resolution walks scope chain (server → env → project → team) with nearest-wins, values in `~/.tengiz/shared-vars.json`. Solves configuration duplication across the multi-env (#1) and multi-project (#112) roadmaps.
- **Detected:** 2026-08-09

## Secret Redaction from Logs & Echoed Output (`REDACTED`)
- **Source:** Coolify
- **Description:** Shared variables and secrets flagged `is_shown_once` (or marked secret) are automatically **redacted from everything emitted during operations**: `ExecuteRemoteCommand` runs a `redact_sensitive_info()` that regex-replaces the decrypted values in all SSH/command output with `REDACTED`, they are excluded from logs, API serialization and deployment snapshots. Sensitive URLs are also redacted (`redactedUrlForLog`). Prevents secret leakage through remote command echo, deploy logs, or `echo $SECRET` style output capture.
- **Why add to Tengiz:** Tengiz exposes surfaces where secrets can leak today: `tengiz logs` output isn't filtered, deploy hook output is captured to build records, and the secret interpolation system (#54) substitutes values into env vars whose values may get echoed by apps. A post-hoc redaction layer on the `deploy.go` and `logs` output streams — replacing known secret values (from `secrets-{env}.json`) before printing — is a small, high-security-impact feature. Marks Tengiz as security-hygiene-aware by default rather than treating secrets as "trust the app not to log them".
- **Detected:** 2026-08-09

## SSRF Protection for Outbound Webhook/S3 Connections (DNS Pinning + IP Validation)
- **Source:** Coolify
- **Description:** `SafeWebhookUrl` rule validates any user-supplied URL before Coolify connects to it: rejects non-http(s) schemes, trailing-dot hosts, `localhost`/`*.local`/`*.internal` hostnames, and any resolved IP in loopback/private/link-local/CGNAT/reserved ranges (full IPv4+IPv6 CIDR tables, incl. `169.254.0.0/16` metadata + `::ffff:` mappings). The actual HTTP call uses **DNS pinning** (`CURLOPT_RESOLVE`/`mc --resolve`) so the connection can't be re-pointed after validation, plus `allow_redirects=false`. Operator override via `webhook_allowed_internal_hosts` + `webhook_allow_localhost` instance settings; custom DNS server resolution configurable.
- **Why add to Tengiz:** Tengiz's webhook receiver (#13, implemented), outgoing webhook payloads (#121), and planned S3 backups all make outbound HTTP requests to URLs users/CI control. A malicious repo or misconfigured URL could otherwise reach the Tengiz's own loopback service or cloud metadata endpoints (SSRF). Implementing validation + DNS pinning in Go (`net.Dialer` with pinned IP/SNI) is well-understood. Cross-references the existing HMAC webhook security (#40) and rate limiting (#100) — this closes the still-open SSRF door in the webhook integration.
- **Detected:** 2026-08-09

## Cloud VPS Auto-Provisioning (Hetzner/Vultr/DigitalOcean + Cloud-init ISO)
- **Source:** Coolify
- **Description:** Coolify provisions entire servers from cloud providers without any manual setup: a `CloudProviderToken` (Hetzner/Vultr/DigitalOcean) + optional `CloudInitScript` (bash or cloud-config user-data) → wizard creates the VPS, writes a local server row with a **placeholder IP** so the resource shows up immediately, `ServerCloudProviderStatusCheckJob` polls provider state and backfills the real public IP (`backfillPlaceholderIp`), then `ValidateAndInstallServerJob` SSH's in, auto-installs Docker + prerequisites, and registers it. Includes incremental server status polling, firewall/private-network/SSH-key wiring per provider API, and "also destroy the provider resource" on deletion.
- **Why add to Tengiz:** The biggest gap for a single-binary PaaS is "I need a server to run on". Today that's a manual cloud console + SSH dance. Adding cloud auto-provisioning (`tengiz server provision --provider hetzner --token ...` → placeholder entry → poll → auto-install Docker then first deploy) turns Tengiz into a truly turnkey Heroku/Vercel replacement and cross-cuts the SSH-based remote deployment (#105), Server Bootstrap (#31) and Cloudflare tunnel roadmap items. Directly extends the server-management story the project already wants to build.
- **Detected:** 2026-08-09

## Team Membership Roles & Invite Links (Owner/Admin/Member)
- **Source:** Coolify
- **Description:** Multi-user teams with a `team_user` pivot table carrying a `role` (member/admin/owner via `Role` enum). Members only read; admins/owners mutate (policy applied to every resource). **Invite links** (email-based or shareable link) with a 30-day expiry and privilege-escalation guards (can't invite someone at a role you don't hold); accepting auto-creates the user + resets password. Role changes (`makeAdmin`/`makeOwner`/`makeReadonly`) and removals immediately **revoke all personal access tokens** for that team (`RevokeUserTeamTokens`), and the guard prevents demoting the last owner.
- **Why add to Tengiz:** Multi-developer deployment is already partially on the roadmap (OIDC/SSO #157, API keys #180, RBAC groups #198 — but none cover *human team membership*). Tengiz has no notion of "who may deploy". A minimal `tengiz team add <app> --member b@x.com` + role tiers + invite link revocation gives real team SAAS-grade collaboration: owner mints, members-only read, admins deploy. `Invite link + token revocation on role change` closes the security loop when people join/leave. Fits well with the existing encrypt/AES store for invite state.
- **Complements rather than duplicates** the earlier "User Group Resource (RBAC)" (#196) which is about permission sets; this is the human membership + lifecycle half.
- **Detected:** 2026-08-09

## API Token Expiry Warnings & Auto-Revocation on Role Change
- **Source:** Coolify
- **Description:** Personal Access Tokens are **team-scoped** (`team_id` even on a Sanctum token) with an abilities set (`read`/`write`/`write:sensitive`/`deploy`/`read:sensitive`/`root`). Middleware downgrades members: a non-admin/owner using a `write`/`deploy`/`root` token gets 403 with "please revoke this token", and sensitive data responses are masked without `read:sensitive`. Token creation defaults to 30-day expiry. A cron job (`ApiTokenExpirationWarningJob`) emails owners 24h before a token expires. Any team role change / removal / team deletion **hard-deletes all that team's tokens** immediately.
- **Why add to Tengiz:** Existing "Granular Scoped API Keys" (#180) defines permission levels but not the *token lifecycle safety*: expiry-warning + revocation cascade on role change. For Tengiz this means a CI token that auto-revokes when a dev leaves the organization, and no phantom tokens with `deploy` privilege outliving the account. Add: `tengiz token create --expiry 30d`, a background `token-expiry-warning` check, and revocation hooks in team role changes. Important because the webhook + `TENGIZ_TOKEN` CI story relies on tokens staying current.
- **Detected:** 2026-08-09

## Dynamic Proxy Configuration Files (Custom Traefik/Caddy/YAML Hot-Reload)
- **Source:** Coolify
- **Description:** Arbitrary custom `*.yaml`/`*.caddy` files written into `proxy/dynamic/` on the server. Traefik watches that directory (`providers.file.directory`) and hot-loads new routers/middlewares without restarting the proxy. The UI covers YAML auto-formatting/reserialization and preserves pre-existing Traefik CLI arguments (`extractCustomProxyCommands`) so operator-configured flags (e.g. `--entrypoints.web.proxyProtocol.trustedIPs` for Cloudflare) aren't lost on config regeneration. Additionally, per-app custom container labels are passed straight through (`custom_labels`), with the special `coolify.traefik.middlewares` label letting users attach their own named middlewares to any app router.
- **Why add to Tengiz:** Tengiz's Go `ReverseProxy` can't accept arbitrary labels, but the same *extensibility contract* applies: an "extra routes/middleware" config directory (e.g. `~/.tengiz/proxy-enabled.yaml` hot-reloaded) plus explicit per-app custom middleware support via `.tengiz.yaml` gives power users escape hatches for CORS, auth, redirects, headers without rebuilding. Combined with the planned "Custom HTTP Headers per URL Path" (#60 security) and "Proxy Security Middleware" (#135), dynamic config file/dir support is the plug-and-play complement. Low effort (fs.Watch + parse), high lock-in-avoidance.
- **Detected:** 2026-08-09

## www↔non-www Redirect (Built-in RedirectTypes)
- **Source:** Coolify
- **Description:** `RedirectTypes` enum (`WWW` / `NON_WWW` / `BOTH`) on the Application model controls **automatic www↔non-www canonicalization** at the proxy. For a host that starts with `www.`, a redirectregex middleware rewrites `^(http|https)://www\.(.+)$` → `${1}://${2}`; for a non-www host, the reverse. This is done per-route in the proxy's label/policy config (not in app code).
- **Why add to Tengiz:** www↔non-www canonical redirects are the single most common domain hygiene need, and every production site behind Tengiz's proxy will eventually want it. Tengiz's existing "URL Redirect & Rewrite Rules" (#134) is a manual rules system; adding a built-in `www`/`non-www` selector (`proxy.redirect: both` in `.tengiz.yaml`) is a 10-line middleware (`req.Host` check + 301). Cheap SEO correctness, complements custom domains (implemented) and force-HTTPS (#52).
- **Detected:** 2026-08-09

## Default Catch-All Proxy Route (503 or Redirect for Unknown Hosts)
- **Source:** Coolify
- **Description:** `Server::setupDefaultRedirect()` writes a catch-all proxy route with an explicit **low priority** (e.g. `priority = -1000`) matching `PathPrefix('/')` and pointing to a `noop` service — either replying **503** (when proxy is not connected) or redirecting via a `redirectRegex` to a configurable URL (`proxy.redirect_url`, `proxy.redirect_enabled`). New/hostless apps or unknown `.tengiz.local` hostnames get a clean response instead of an obscure proxy error.
- **Why add to Tengiz:** Tengiz's proxy currently errors or 404s in confusing ways for unknown hosts and during cold-start downtime. A default catch-all route (HTTP 503 with custom body, or a 301 redirect to a configured URL like the app's canonical domain) is a small middleware in `proxy.go` that makes port-in-use, wrong-host, and pre-deploy states unsurprising. Pairs with the existing Error Pages (#35) roadmap item — the catch-all is the fallback when no app route matches.
- **Detected:** 2026-08-09

## Container Name Consistency & Custom Internal Names
- **Source:** Coolify
- **Description:** `is_consistent_container_name_enabled` + `custom_internal_name` (unique-per-server generated) on apps. When enabled, the container is given a **stable, user-chosen internal name** across all deploys rather than Coolify's normal random-uuid suffix (`app-{uuid}`). This makes the container addressable by the same hostname on every restart, which is what makes external routers/DNS/scripts attach reliably, and unlocks true zero-downtime "traffic switching" on restart when a name must be stable externally.
- **Why add to Tengiz:** Tengiz's zero-downtime deploys (implemented) and scale-to-zero cold starts both destroy and recreate containers. If a container's name is stable (`tengiz-myapp` per the existing naming convention), then: (a) DNS records/firewalls/scripts referencing `tengiz-myapp` keep working, (b) `docker inspect` + networking still point to the "same" container across restarts, and (c) inter-container `--link`/`depends_on` composition doesn't break. Currently name stability across all paths is assumed but not guaranteed by a documented setting. A `container.name` + `container.consistent: true` section in `.tengiz.yaml` clarifies the contract to users.
- **Detected:** 2026-08-09

## Container DNS/Stale Detection + Multiplexing Reuse (Exec Optimizations)
- **Source:** Coolify
- **Description:** `Server::muxFilename()` / `SshMultiplexingHelper` reuses a single SSH control connection across many Docker operations (mux file keyed by host, refresh on key-change → massively fewer SSH/TCP handshakes); `CleanupStaleMultiplexedConnectionsJob` prunes dead mux files. The Sentinel agent runs **push-based** metrics (heartbeat updates `sentinel_updated_at` on every push, skip SSH-based reachability checks when live) and uses **hash-diffing** of the container list to avoid re-dispatching update jobs when nothing changed.
- **Why add to Tengiz:** These are performance/reliability patterns directly applicable to Tengiz's exec-based Docker client: batch consecutive `docker ps`/`inspect` per second into a short-lived SSH (or a cached `docker` connection) with a small TTL — instead of spawning an `os/exec` for every query; and keep a cached runtime snapshot with hash-diffing so `tengiz ps`/polling is cheap. For scale-to-zero + cold-start frequency, process-spawn per request is real overhead on single nodes. Port is `runtime.Manager` caching a 1s TTL snapshot.
- **Detected:** 2026-08-09

## Deployment Tasks (Phased app.json + Procfile Lifecycle Scripts)
- **Source:** Dokku
- **Description:** Dokku's `deployment-tasks` runs image-scoped scripts at precise phases of a deploy: `app.json` `scripts.dokku.predeploy` runs after the image is built but before containers are scheduled (**changes are committed to the image** — for bundling assets, installing binaries); `scripts.dokku.postdeploy` runs after containers are scheduled (no image commit — for signalling Slack / coordinating a load balancer); `scripts.postdeploy` runs **once only**, after the app is created, never on subsequent deploys (seed data, OAuth client setup); and the Procfile `release` process runs after build, before scheduling (CDN upload, cache priming, DB migrations). A failed task aborts the deploy without affecting running containers. `app-json:set <app> appjson-path .dokku/app.json` relocates the manifest for monorepos.
- **Why add to Tengiz:** The existing Pre-Deploy Hooks (#9) and Extended Hook System (#24) run host-side commands. Deployment tasks run *inside the built image* at four distinct phases with three different persistence semantics — a genuinely different contract that covers migration runners, asset uploads and one-time bootstrapping. `.tengiz.yaml` can model this as `deploy.tasks: { prebuild, release, predeploy, postdeploy, postdeploy_once }` executed by the existing deploy pipeline between image build and container schedule. Small to build: run `docker run <image> <cmd>` at the right pipeline stage.
- **Detected:** 2026-08-09

## Cron Task Lifecycle Management (Concurrency Policy, Suspend/Resume, On-Demand Run)
- **Source:** Dokku
- **Description:** Dokku's `cron` plugin (beyond defining schedules in `app.json`) adds per-task operational controls: `cron:list`, `cron:report`, `cron:suspend <app> <id>`, `cron:resume <app> <id>` (per-task maintenance without deleting), and `cron:run <app> <id> [--detach]` for manual on-the-fly invocation. Each task carries a `concurrency_policy` of `allow` / `forbid` (skip if already running) / `replace` (kill the old one first). App-level and global `maintenance` flags suspend all tasks at once, and `mailto`/`mailfrom` route cron failure emails. Tasks run in one-off containers and are validated at deploy time (malformed shell operators reject the deploy).
- **Why add to Tengiz:** The existing Scheduled Tasks / Cron Jobs feature covers scheduling only. Production teams need to: pause a nightly job during a release, force-run a stuck job, and avoid overlapping long-running jobs (the classic email-duplication bug). `.tengiz.yaml` `cron:` entries get `concurrency_policy` and `maintenance` fields; `tengiz cron list|run|suspend|resume <app> <id>` completes the surface. Concurrency control is the differentiator — `robfig/cron` plus a per-task in-flight lock in `~/.tengiz/`.
- **Detected:** 2026-08-09

## Zero-Downtime Container Retirement (wait-to-retire Graceful Drain)
- **Source:** Dokku
- **Description:** After a zero-downtime deploy, Dokku does **not** immediately stop old containers. Traffic switches to the new containers, old ones receive `SIGTERM` (to drain in-flight connections and flush buffers), and are left running for a configurable `wait-to-retire` grace period (default 60s) before being stopped; a second `SIGTERM` is sent, then `SIGKILL` after `stop-timeout` (default 30s). This mirrors Heroku's dyno shutdown behavior. Configured via `checks:set <app> wait-to-retire 30` and `ps:set <app> stop-timeout 60`.
- **Why add to Tengiz:** Tengiz's implemented zero-downtime deploy replaces the container but has no configurable drain window. Long-lived HTTP connections (SSE, file uploads, WebSockets) get cut instantly if the old container is killed the moment traffic flips. A `deploy.wait_to_retire` (default 60s) + `deploy.stop_timeout` in `.tengiz.yaml` gives app operators control over graceful shutdown, with a background goroutine handling the delayed stop. Distinct from readiness checks (#28) which gate the *new* container — this governs the *old* one's exit.
- **Detected:** 2026-08-09

## One-off Task Container Lifecycle Management (Detached Run, List, Logs, Stop, TTL)
- **Source:** Dokku
- **Description:** Beyond `run`, Dokku manages one-off task containers as first-class objects: `run:detached <app> <cmd>` (background container, returns ID immediately, `--force-tty` to attach later), `run:list [--format json]` (all run containers per app with state/command/created), `run:logs <app> [--container <id>]` (stream one-off logs), `run:stop <app> | --container <id>` (terminate a stuck task), and `--ttl-seconds N` / `--no-tty` flags. One-off containers are removed at process exit and reaped after a max TTL (default 24h) to prevent leaked daemons.
- **Why add to Tengiz:** `tengiz run` (implemented) starts a task and cleans up on exit, but a runaway `npm run migrate` or a background `run:detached` worker can't be listed, logged, or killed — they leak containers until manual `docker rm`. Adding `tengiz run:list|logs|stop`, a `--ttl` flag and a periodic reaper goroutine in `runtime.Manager` turns one-off tasks from a fire-and-forget command into an operable surface. Essential companion to cron (each scheduled job is a one-off container) and migrations.
- **Detected:** 2026-08-09

## App-to-App Service Discovery via Network Aliases
- **Source:** Dokku
- **Description:** Dokku's `network` plugin attaches containers to user-defined networks at **three phases** with different semantics: `initial-network` (attached at container creation, for resources the app needs on boot), `attach-post-create` (attached after create but before start, for the app to reach peers), and `attach-post-deploy` (attached to a running container after it passes healthchecks, so peers can reach *it*). Every attached container gets a stable DNS alias `APP.PROC_TYPE` (e.g. `node-js-app.web`) for inter-app load-balanced routing, with an optional custom `tld` (e.g. `svc.cluster.local`) appended for Kubernetes-style fully-qualified names. `network:create/destroy/list --dokku-managed` manages the networks, and `bind-all-interfaces` controls whether containers bind `0.0.0.0` or only the internal bridge.
- **Why add to Tengiz:** The existing Custom Docker Network (#30) attaches one network; it can't express *when* a container joins (pre/post-healthcheck matters for cross-app DB handshake ordering) or give apps a stable hostname to talk to each other. Tengiz apps today must guess peer IPs via the proxy. `.tengiz.yaml` `network: { initial, attach_post_create, attach_post_deploy, tld }` plus the `docker network connect` call at the right pipeline phase gives web+worker+db compositions a real service-discovery story — the foundation for accessory services (#62) and managed databases (#63).
- **Detected:** 2026-08-09

## Explicit Port Mapping & gRPC Proxying (scheme:host:container)
- **Source:** Dokku
- **Description:** Dokku decouples host/container port mapping from the proxy via `ports:add/set/remove/clear <app> <scheme>:<host-port>:<container-port>` (e.g. `http:80:5000`, `grpc:443:5000`). Schemes are `http`, `https`, `grpc`, `grpcs` (UDP/TCP for non-HTTP proxies). Port maps are **auto-detected** from Dockerfile `EXPOSE` directives (one listener per exposed port) or defaulted to `http:80:5000` for buildpacks, and can be manually overridden to expose a second port, change the proxy port, or serve non-standard ports without editing the Dockerfile. `ports:report --ports-map-json` surfaces the effective map machine-readably.
- **Why add to Tengiz:** Tengiz hardcodes host-port 9000+ → single container port with HTTP-only proxying (per Port Mapping Protocol Selection, #111, which covers tcp/udp publishing). It cannot expose a second HTTP port or proxy gRPC traffic — and gRPC requires HTTP/2 on the upstream, which the Go `httputil.ReverseProxy` can do. `.tengiz.yaml` `ports: [{scheme: http, host: 8080, container: 3000}, {scheme: grpc, host: 8443, container: 50051}]` with EXPOSE auto-detection in the builder makes Tengiz's proxy suitable for gRPC microservices and multi-port apps. Small change: build the proxy listener set from the port map instead of a single fixed port.
- **Detected:** 2026-08-09

## Archive-Based Deployment (Tar/Zip via URL or stdin)
- **Source:** Dokku
- **Description:** `dokku git:from-archive <app> <url|--> [--archive-type tar|zip]` initializes or updates an app from a pre-built archive — a binary jar, a Go binary, or a GitHub release tarball at a specific commit — without a git repo or buildpack source. The archive is extracted and built as if it were the pushed source tree; feeding the same archive again is a no-op (no changes detected). Custom author info can be recorded. Archives are **validated before extraction** to block path-traversal (`..`), absolute paths, and symlink escapes, with `archive-max-size` (1 GiB default) and `archive-max-files` (10k default) anti-tar-bomb limits; `-` reads from stdin.
- **Why add to Tengiz:** Tengiz's `deploy` only takes a local directory. Pre-built artifacts (Java jars, Go/Elixir releases, GitHub release zips) currently force users to download+extract manually or keep source in the repo. `tengiz deploy --archive <url> [--archive-type zip]` reuses the existing build+deploy pipeline — download, safety-extract to a temp dir, then framework-detect. The security validation matters on a public-facing server: it makes CI-fed binary deploys safe. Distinct from Explicit Image Deploy (#13, whole pre-built image) — this builds a normal image from an archived source tree.
- **Detected:** 2026-08-09

## Batch Config Updates (--no-restart, --encoded, multi-var sets)
- **Source:** Dokku
...
## Server-Side Command Audit Log (`audit` — Performer-Attributed, host-local `audit.log`)
- **Source:** Kamal (`cli/main.rb#audit`, `commands/auditor.rb`)
- **Description:** Every mutating command (deploy, boot, start/stop, exec, prune, lock acquire, proxy reboot, accessory lifecycle, registry login) is recorded to a per-service `audit.log` on each server under the run directory, with `[recorded_at] [performer] [destination] [version] [service@version]` tags. The **performer** is resolved from git user email (falls back to `whoami`), so multi-developer teams know exactly who ran what, when, and against which destination + app version. `kamal audit` tails the last 50 lines from any server.
- **Why add to Tengiz:** The generic "Event Logging & Audit Trail" (#7) covers Tengiz's own slog/JSON lines; Kamal's auditor is a **server-side, per-service, human-attributed** command ledger that tells ops exactly "who deployed this app, to which env, at which version". Implementing as per-app `~/.tengiz/audit/<app>.log` lines (append `recorded_at`, `performer` = git config email, env, version) plus `tengiz audit <app>` is a small `os.OpenFile(O_APPEND)` + one command. Directly supports the "who can deploy" story (#149/#180).
- **Detected:** 2026-08-09

## Persistent Proxy Run Options (`proxy boot_config set/get/reset`)
- **Source:** Kamal (`cli/proxy.rb#boot_config`, `configuration/proxy/boot.rb`, `commands/proxy.rb#boot_config`)
- **Description:** The kamal-proxy container's launch arguments (publish ports, log max-size, metrics port, extra docker options, image, registry/repository, image version, run command) are **persisted to files** per host (`options`, `image`, `image_version`, `run_command`) when set via `kamal proxy boot_config set`. `get` prints the effective persisted options; `reset` clears them back to defaults. This guarantees a recreated (reboot/upgrade/remote-destroyed) proxy comes back with the same runtime options without re-supplying flags.
- **Why add to Tengiz:** The roadmap's proxy records describe *routing* config (path prefix, timeouts, headers — #76/#135). This is the missing **launch-persistence half**: the settings a proxy *starts with*. For Tengiz the proxy is a Go system in the same binary, but the concept of persisting user-supplied proxy launch options (`~/.tengiz/proxy.json` — ports, TLS, docker options, `--expose` metrics) and restoring them on restart makes the "Gelişmiş Proxy" (#76) state durable, not just recomputed per-process. Cheap (config.Store already persists JSON), avoids config-drift after reboots.
- **Detected:** 2026-08-09

## Exec Command Modes: reuse / interactive / detached / raw for one-off runs
- **Source:** Kamal (`cli/app.rb#exec`)
- **Description:** One-off `exec` distinguishes several run contracts beyond a fresh temp container: `--reuse` (run the command in the **currently running** container instead of starting a new one — shares its process state and env), `--interactive/-i` (attach a TTY shell), `--detach` (launch job in a background one-off container and return), and `--raw` (stream unmodified stdout, no instrumentation). Env overlay via `--env KEY=VALUE`. Mutually-exclusive combos are validated (detach+interactive etc. rejected).
- **Why add to Tengiz:** Tengiz's implemented `run` (one-off process executor) always spins up a new container. `--reuse` is a genuinely different contract — executing in the long-lived container is the fastest way to run one-liners (rake, migrations, db shell) with the already-warm env, while `--detach` aligns with the planned detached one-off lifecycle and `--raw`/`--interactive` complete the debugging surface. Small arg parsing + a `runtime.ExecInContainer` vs `Run` selection.
- **Detected:** 2026-08-09

## Sensitive-Value Redaction in Logs/Arguments (`sensitive()` / `[REDACTED]`)
- **Source:** Kamal (`utils.rb#sensitive/redacted`, config output)
- **Description:** Secrets and any `sensitive: true` attributes are wrapped so that **human-visible output, error messages, and generated command arguments** render them as `KEY=[REDACTED]` instead of the plain value. This applies to env var writes, build args, `docker run` lines, debug output, and the `kamal config` dump — secrets can never accidentally leak into CI logs or shell history through Tengiz's own printing.
- **Why add to Tengiz:** Tengiz logs config, env, and docker commands today; secret values flowing into `echo`/error strings are a real leak vector for `config.Store`-backed env and the AES-256 secrets. A small `redact` wrapper applied at every print/format boundary (slog attribute helper + `fmt.Sprintf` wrapper) means "show me the deploy command" never dumps `DATABASE_URL=postgres://...`. Ties directly into Config Display (#48) and the secrets provider work.
- **Detected:** 2026-08-09

## Full Platform Teardown (`remove` — proxy + app + accessories + registry)
- **Source:** Kamal (`cli/main.rb#remove`)
- **Description:** One `remove` command reverses an entire `setup`: it removes the kamal-proxy, the app's containers + images, all accessories, and the registry session — with `--confirmed` needed to proceed. Prevents the "I uninstalled but half the containers linger" state after a failed manual cleanup.
- **Why add to Tengiz:** Tengiz has granular `rm`/`stop` but no single teardown that mirrors its one-command setup. `tengiz teardown --confirmed` (remove app, its images, accessory containers, proxy/state) gives a reset story isomorphic to setup — with the "protected service deletion" (#185) confirmation story applied. Uses only existing `runtime.RemoveImage/KeepLastN` + `config.Store` ops; adds the `--force` guard.
- **Status:** Planned
- **Detected:** 2026-08-09
