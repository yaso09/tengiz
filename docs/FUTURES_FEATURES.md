...
✅ Implemented, ⬜ Pending. Each feature evaluated on **Impact (I)** (user reach + criticality for Vercel alternative), **Effort (E)** (complexity given current architecture), **Alignment (A)** (fit with Go, CLI-first, Docker exec, scale-to-zero).

### P0 — Critical (Must-Have for Vercel Alternative)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 1 | **Nixpacks Build System** ⬜ | Critical | Medium | Perfect | #1 bottleneck: only 6 frameworks supported. Nixpacks unlocks hundreds (Ruby, Rust, PHP, Java, Elixir). Single `builder` package integration. Every user benefits. |
| 2 | **Automatic SSL/TLS (Let's Encrypt)** ⬜ | Critical | Medium | Good | No production deployment works without HTTPS. `golang.org/x/crypto/acme/autocert`. External proxy workaround is unacceptable for a PaaS. |
| 3 | **Secrets Management** ⬜ | High | Med-High | Perfect | No platform deploys real apps without encrypted DB passwords, API keys. Vault/1Password/Doppler integration. Production security fundamental. |
| 4 | **Docker Housekeeping** ⬜ | High | Low | Perfect | Disk space is #1 production issue on single-server. Label-based `docker system prune`. `tengiz cleanup`. Required for long-running instances. |
| 5 | **Event Logging & Audit Trail** ⬜ | High | Low | Perfect | Who deployed what, when? Multi-developer audit trail via `log/slog` + JSON Lines. Required for team deployments. |
| 6 | **Pre-Deploy Hooks** ⬜ | High | Low | Perfect | Migration runner before deploy is table stakes. `.tengiz.yaml` `pre_deploy` command list. Failed hook aborts deploy. |
| 7 | **Custom Build Commands** ⬜ | High | Low | Perfect | Framework detection override for custom toolchains, monorepos, non-standard setups. `commands.install/build/start` in `.tengiz.yaml`. |
| 8 | **Monorepo Support (Base Directory)** ⬜ | High | Low | Perfect | Turborepo/Nx/Lerna users are a large, growing segment. `base_dir` override for framework detection. |
| 9 | **Build Arguments from Env** ⬜ | High | Low | Perfect | NEXT_PUBLIC_*, NPM_TOKEN need `--build-arg` passthrough. Critical for all framework builds that use env at build time. |
| 10 | **Notification System** ⬜ | High | Medium | Perfect | Production ops require alerts for deploy failures, SSL expiry, disk filling. Discord/Slack/Telegram/Email. |
| 11 | **REST API + OpenAPI Spec** ⬜ | High | High | Good | Unlocks CI/CD integration, programmatic access, web UI. Must-have for platform growth. High effort but essential investment. |
| 12 | **Container Registry Integration** ⬜ | High | Low-Med | Perfect | Build → push pipeline. Image storage for rollback + multi-node. Prerequisite for KeepLastNImages to work across servers. |
| 13 | **Commit Status Reporting** ⬜ | High | Low | Perfect | Report deploy result to GitHub/GitLab commit status API. Green checkmark/red X on PRs. Closes the git feedback loop. |
| 14 | **App Auto-Creation on Git Push** ⬜ | High | Low | Perfect | Zero-setup: `git push tengiz main` auto-creates app. Every PaaS does this (Heroku, Vercel, Railway). Removes `tengiz create` step. |
| 15 | **Zero-Downtime Deploy Health Checks** ⬜ | High | Low | Perfect | App-level health verification before traffic migration. Auto-rollback on check failure. Without this, ZDD is dangerous. |
| 16 | **One-Line Install Script** ⬜ | High | Low | Perfect | `curl -fsSL https://tengiz.dev/install.sh \| bash`. Cross-compile binaries, detect platform, verify checksum. Adoption friction. |
| 17 | **Headless CI/CD Mode** ⬜ | High | Low | Perfect | `TENGIZ_TOKEN` + `--headless` for GitHub Actions, GitLab CI. Non-interactive, no TTY. Required for any CI integration. |
| 18 | **Server Reboot Recovery** ⬜ | High | Low | Perfect | Auto-restart all apps after host reboot. Systemd unit + `ps:restore` on boot. Without this, server crash = manual recovery. |
| 19 | **App Report (Detailed Status)** ⬜ | High | Low | Perfect | `tengiz ps` is too minimal. Need deploy history, image tags, env vars, resource limits, domains in one command. |

### P1 — High (Production-Ready Platform)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 20 | **Container Restart Policy** ⬜ | High | Low | Perfect | Per-app Docker restart policy: `no/always/unless-stopped/on-failure`. Crash behavior control. Essential for production reliability. |
| 21 | **HMAC-Signed Webhook Payloads** ⬜ | High | Low | Perfect | Verify GitHub/GitLab webhook signatures. Prevent unauthorized deploy triggers. Security gap in current webhook implementation. |
| 22 | **Well-Known Paths Automatic Handling** ⬜ | High | Low | Perfect | `/.well-known/` for ACME HTTP-01, Apple Universal Links, security.txt. Required for SSL domain verification. |
| 23 | **Background Monitoring Scheduler** ⬜ | High | Low | Perfect | Proactive health checks, disk usage tracking, container status monitoring. Feeds alert system. Without it, issues only detected reactively. |
| 24 | **Auth Rate Limiting** ⬜ | High | Low | Perfect | Per-IP brute force protection on auth/webhook endpoints. 5 attempts/300s window. Webhook endpoints are public-facing. |
| 25 | **Readiness Delay & Deploy Timeouts** ⬜ | Medium | Low | Perfect | Per-operation timeouts for deploy/drain/stop. Accommodates both fast Go apps (100ms) and slow Node.js (30s). Prevents false failures. |
| 26 | **Webhook Event Filtering** ⬜ | Medium | Low | Perfect | Branch/tag/path filtering. `--only-branch main`, `--ignore-paths docs/*`. Prevents wasteful deploys from non-critical pushes. |
| 27 | **Config Display Command** ⬜ | Medium | Low | Perfect | `tengiz config` shows merged config after env override + template resolution. Critical for debugging config inheritance. |
| 28 | **Deploy Source Metadata Recording** ⬜ | Medium | Low | Perfect | Record GIT_SHA, branch, deploy source (git-hook/cli/webhook). Foundation for audit, rollback-by-SHA, commit status API. |
| 29 | **Prometheus Metrics** ⬜ | Medium | Low | Perfect | Proxy HTTP metrics: request count, latency, error rate, cold start count. Grafana + alerting foundation. |
| 30 | **Maintenance Mode** ⬜ | Medium | Low | Perfect | Proxy draining for planned maintenance. `tengiz maintenance:on --message "Upgrading..."`. Production operations need this. |
| 31 | **Build-Time Secrets (Docker Build Secrets)** ⬜ | Medium | Low | Perfect | NPM_TOKEN, signing keys via `--secret` (not `--build-arg`). Excluded from image history. Security for build phase. |
| 32 | **Stale Container Detection** ⬜ | Medium | Low | Perfect | Detect orphaned old-version containers after ZDD deploy. Auto-cleanup or report. Safety net for deploy cleanup failures. |
| 33 | **App Deploy Tokens** ⬜ | Medium | Low | Perfect | CI/CD authentication without user credentials. `tengiz token create --app myapp`. Token rotation. Complements headless mode. |
| 34 | **Deploy Lock** ⬜ | Medium | Low | Perfect | Prevent concurrent deploy collisions in team environments. File-based lock + `--lock-wait`. Safety for multi-developer teams. |
| 35 | **Config Export/Import** ⬜ | Medium | Low | Perfect | Disaster recovery and app migration. Export env vars as shell/dotenv/Docker args/JSON. Multiple format support. |
| 36 | **Explicit Image Name Deploy** ⬜ | Medium | Low | Perfect | Deploy pre-built images (Postgres, Redis, CI/CD output) without build step. `tengiz deploy --image nginx:alpine`. |
| 37 | **Node.js Multi-Core Scaling (PM2/Cluster)** ⬜ | Medium | Low | Perfect | 4-8x performance improvement for Node.js apps via PM2 cluster mode. `.tengiz.yaml` toggle. Major perf win. |
| 38 | **Custom Docker Network** ⬜ | Medium | Low | Perfect | Isolated networks for multi-service apps. `docker run --network` flag. Foundation for accessory services. |
| 39 | **Error Pages** ⬜ | Medium | Low | Perfect | Cold start or container-down UX: user-friendly error pages instead of raw HTTP errors. Proxy middleware. |
| 40 | **HTTP Basic Auth (Staging Protection)** ⬜ | Medium | Low | Perfect | Password-protect staging/pre-production environments. Proxy middleware. Essential for multi-env setups. |
| 41 | **Server Bootstrap** ⬜ | Medium | Medium | Perfect | `tengiz server init` + `tengiz setup` — one-command Docker + Tengiz installation. Complements one-line install. |
| 42 | **Container Retention Policy** ⬜ | Medium | Low | Perfect | Rollback companion. Keep N old containers, prune rest. Default: retain 5. Complements KeepLastNImages. |
| 43 | **Full System Backup & Restore** ⬜ | Medium | Medium | Perfect | `tengiz backup create` archives `~/.tengiz/` state. `tengiz backup restore` for DR. |
| 44 | **Extended Hook System** ⬜ | Medium | Low-Med | Perfect | Pre-build (secret injection), post-deploy (notifications), app-boot (cache warming). Rich hook env context. |
| 45 | **Private Registry Authentication** ⬜ | Medium | Low | Perfect | GHCR, GitLab Registry, AWS ECR pull support. Enterprise prerequisite for custom base images. |
| 46 | **Per-Container Resource Metrics** ⬜ | Medium | Low | Perfect | Live CPU/memory/network from `docker stats`. `tengiz ps --stats` or `tengiz stats <app>`. |
| 47 | **Scheduled Deployments** ⬜ | Medium | Low | Perfect | Cron-based auto-deploy. Nightly rebuilds, dependency updates. Built on `robfig/cron`. |
| 48 | **Environment Variable Locking** ⬜ | Medium | Low | Perfect | Prevent accidental `tengiz config unset DATABASE_URL`. Locked vars require confirmation. |
| 49 | **Granular Docker Prune Operations** ⬜ | Medium | Low | Perfect | Per-category prune: containers/networks/images/volumes/buildx cache. Surgical disk management. |
| 50 | **Variable Resource (Global Interpolation)** ⬜ | Medium | Low | Perfect | Define `[[database_url]]` once, reference across all apps. Eliminates env var duplication. |
| 51 | **Secret Interpolation System** ⬜ | Medium | Low | Perfect | `[[secret.name]]` syntax. AES-GCM encrypted storage. No external vault needed for single-server. |
| 52 | **Parallel Bulk Operations** ⬜ | Medium | Low | Perfect | `tengiz rebuild --all`, `tengiz restart --all` with `--parallelism N`. Goroutine pool. |
| 53 | **Custom HTTP Headers per URL Path** ⬜ | Medium | Low | Perfect | Per-path Cache-Control, CORS, security headers. Vercel/Netlify `_headers` style. Proxy middleware. |
| 54 | **KEDA-based Autoscaling** ⬜ | Med-High | High | Perfect | Scale 0→N based on HTTP rate, queue depth, CPU. Transforms idle timer into real serverless. Game-changer for scale-to-zero. |
| 55 | **Embedded Serverless Functions (goja)** ⬜ | High | High | Perfect | Biggest differentiator. Dockerless <10ms function runtime. TypeScript → Go `goja`. No other Docker-based alt has this. |
| 56 | **Accessory Services (Sidecar Containers)** ⬜ | Medium | Medium | Perfect | Postgres/Redis/Search alongside app. Scale-to-zero only affects app, not accessories. Required for stateful apps. |
| 57 | **Managed Database Provisioning** ⬜ | High | Very High | Good | Vercel Postgres/KV equivalent. `tengiz db create postgres --name mydb`. Very high effort but high impact. |
| 58 | **One-Click Service Templates** ⬜ | High | High | Good | 361 Docker Compose templates (WordPress, N8N, MinIO). `tengiz service create <template>`. |
| 59 | **Build Pipeline with Auto-Versioning** ⬜ | Medium | Medium | Perfect | Source → versioned image → multi-registry push. Auto-tag: semver/commit-sha/timestamp. |
| 60 | **Build-to-Deploy Trigger Chain** ⬜ | Medium | Medium | Perfect | Build completes → linked deployment auto-redeploys. Full CI/CD without external tools. |
| 61 | **GitOps / Declarative ResourceSync** ⬜ | High | High | Perfect | Declare apps/env/domains as git-managed YAML. `tengiz sync` reconciles. IaC for Tengiz. |
| 62 | **Herokuish Buildpacks** ⬜ | Medium | Medium | Perfect | Heroku's entire buildpack ecosystem. Zero-config migration from Heroku. Complements Nixpacks. |
| 63 | **Multi-Architecture Builds (Buildx)** ⬜ | Medium | Medium | Perfect | `linux/amd64 + linux/arm64` multi-arch manifests. Apple Silicon → Intel server. |
| 64 | **Remote Docker Builder** ⬜ | Medium | Medium | Perfect | SSH-based remote buildx builder. Offload heavy builds from production server. |
| 65 | **App Renaming** ⬜ | Low | Low | Perfect | `tengiz rename <old> <new>`. Full state migration. Currently only rm + redeploy. |
| 66 | **Custom Docker Options** ⬜ | Low-Med | Low | Perfect | Power user escape hatch: `--shm-size`, `--sysctl`, `--cap-add` for any Docker flag. |
| 67 | **App Repository Lifecycle Management** ⬜ | Medium | Low | Perfect | Per-app git repo management: `git:lock/unlock/status`. Prevent pushes during maintenance. |
| 68 | **Custom Image Repository Naming** ⬜ | Medium | Low | Perfect | Go template-based image naming. `ghcr.io/myorg/{{ .AppName }}`. Collision avoidance. |
| 69 | **Local Development Emulator** ⬜ | High | Medium | Perfect | `tengiz emulator start` runs full platform stack locally. Hot-reload. Match Vercel local dev experience. |
| 70 | **Client SDK Ecosystem** ⬜ | High | Medium | Perfect | `@tengiz/core` npm package for auth/datastore/storage. Makes platform services feel built-in. |

### P2 — Medium (Significant Differentiators)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 71 | **Process Scaling (Multi-Container)** ⬜ | Medium | High | Good | HA + background workers (Sidekiq, Celery). Combines with idle timeout for serverless model. |
| 72 | **Server Monitoring** ⬜ | Medium | Medium | Perfect | Disk, container states, backup status. `tengiz status` + threshold alert. |
| 73 | **Scheduled Tasks / Cron Jobs** ⬜ | Low-Med | Medium | Perfect | Vercel Cron Jobs equivalent. `.tengiz.yaml` `cron:` + `robfig/cron`. `docker exec` execution. |
| 74 | **Force HTTPS Redirect** ⬜ | Medium | Low | Perfect | HTTP→HTTPS 301 redirect in proxy. `.tengiz.yaml` `force_https: true`. |
| 75 | **Advanced Proxy Configuration** ⬜ | Medium | Medium | Perfect | Path prefix, buffering, timeout, X-Forwarded-* header control. Production-grade proxy. |
| 76 | **Pattern-Based Watch Paths** ⬜ | Medium | Low-Med | Perfect | `tengiz deploy --watch` with glob patterns. Auto-redeploy on file changes. `fsnotify`. |
| 77 | **WebSocket Support Per App** ⬜ | Medium | Low | Perfect | Per-app WebSocket toggle in proxy. `.tengiz.yaml` `proxy.websocket: true`. |
| 78 | **Event-Driven Data Hooks (Trigger System)** ⬜ | Medium | Medium | Perfect | `container:start`, `deploy:success`, `idle:timeout` events. Makes Tengiz programmable. |
| 79 | **Container Snapshot System** ⬜ | Medium | Low | Perfect | `docker commit`-based stateful snapshot. Risk mitigation before risky deploys. |
| 80 | **Built-in Platform Analytics** ⬜ | Medium | Medium | Perfect | HTML injection + Web Vitals tracking. SQLite storage. Vercel Analytics-level feature. |
| 81 | **Built-in Authentication Service** ⬜ | High | High | Perfect | Platform-level auth-as-a-service. Google/GitHub/Passkey. Proxy auth intercept + header injection. |
| 82 | **Built-in NoSQL Datastore** ⬜ | High | High | Perfect | Zero-config document store. Embedded SQLite + proxy `/__tengiz/db/` API. |
| 83 | **Built-in File/Blob Storage** ⬜ | High | High | Perfect | Platform-level asset hosting. URL-based access control. Upload/serve/delete API. |
| 84 | **Framework Plugins (Next.js/Vite)** ⬜ | Medium | Medium | Perfect | `@tengiz/nextjs` npm package. Auto-inject env + API routes. Differentiator from Coolify/Dokku. |
| 85 | **Build Precompression** ⬜ | Medium | Low | Perfect | Gzip/Brotli pre-compression at build time. Zero-CPU asset serving. |
| 86 | **Staged Deployments (Change Sets)** ⬜ | Medium | Medium | Perfect | `tengiz deploy --no-apply` → `tengiz changes apply <id>`. Deploy on Friday, apply on Monday. |
| 87 | **Project Scaffolding with Starter Templates** ⬜ | Medium | Medium | Perfect | `tengiz create <template>`. React/Vite/Next.js/Go API templates. Minutes to first deploy. |
| 88 | **Change Approval Workflow** ⬜ | Medium | Medium | Perfect | Submit → Review → Apply. Team governance for deployments. |
| 89 | **Procfile Support** ⬜ | Medium | Low | Perfect | Heroku-style process type definition. Zero-config Heroku migration. |
| 90 | **Docker Compose Import** ⬜ | Medium | Medium | Perfect | Convert existing `docker-compose.yml` to Tengiz containers. Multi-service deploy. |
| 91 | **Global/Per-App Property Cascade** ⬜ | Medium | Medium | Perfect | Global defaults → app overrides. Reduce per-app config overhead for multi-app instances. |
| 92 | **Per-Process-Type Resource Limits** ⬜ | Medium | Low | Perfect | Separate CPU/memory for web/worker/scheduler. Extends existing resource limits. |
| 93 | **Build Tracking with Retention** ⬜ | Medium | Medium | Perfect | Structured deploy history: JSON records, status tracking, build logs retention. |
| 94 | **Git Provider OAuth App Integration** ⬜ | Medium | High | Perfect | One-click GitHub/GitLab App connection. Auto-configure webhooks + deploy keys. |
| 95 | **Magic Environment Variables** ⬜ | Medium | Medium | Perfect | Auto-generated service URLs, DB connection strings, passwords for linked services. |
| 96 | **Centralized Multi-Server Management** ⬜ | Medium | High | Perfect | Control-plane model: one Tengiz instance manages remote Docker hosts via SSH. |
| 97 | **MCP Server for AI Integration** ⬜ | Medium | Medium | Perfect | Model Context Protocol server. AI assistants can query/list/manage Tengiz via natural language. |
| 98 | **Concurrency Control (Operation Locking)** ⬜ | Medium | Low | Perfect | File-based mutex per app for state-modifying operations. Prevents corrupting state files. |
| 99 | **Docker Network & Volume CRUD** ⬜ | Medium | Low | Perfect | `tengiz network create/ls/rm`, `tengiz volume create/ls/rm`. No raw Docker CLI needed. |
| 100 | **Build Cache Management & Git GC** ⬜ | Medium | Low | Perfect | `tengiz cleanup --cache --gc`. Per-app build cache + git repo pruning. |
| 101 | **Server Security Hardening** ⬜ | Medium | Medium | Perfect | UFW firewall, Fail2ban, Docker TLS during `tengiz server init`. |
| 102 | **Database Connection String Auto-Injection** ⬜ | Medium | Medium | Perfect | Auto-inject `DATABASE_URL` when DB accessory linked to app. |
| 103 | **Manual SSL Certificate Management** ⬜ | Medium | Medium | Perfect | Import/generate/inspect/remove SSL certs. Enterprise cert management. |
| 104 | **Pluggable Event/Trigger Architecture** ⬜ | Medium | High | Perfect | 40+ lifecycle trigger points. Community plugin ecosystem. |
| 105 | **Alert System with Severity Levels** ⬜ | Medium | Medium | Perfect | 5 severity levels, resolved/unresolved tracking, auto-pruning. |
| 106 | **Two-Factor Authentication (2FA)** ⬜ | Medium | Medium | Perfect | TOTP-based 2FA for admin accounts. `github.com/pquerna/otp`. |

### P3 — Lower (Niche / Enhancement / Enterprise)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 107 | **SSH Remote Deployment** | Medium | High | Good | Multi-server support. High effort against single-node design. |
| 108 | **Role-Based Server Groups** | Medium | Medium | Good | Web/worker/job role separation. Different cmd/env per role. |
| 109 | **Redeploy** | Low | Low | Perfect | Skip bootstrap/prune steps, just build/push/restart. |
| 110 | **Rolling Boot / Canary Deployment** | Low | High | Low | Multi-server only. Gradual deployment to limit blast radius. |
| 111 | **Encryption at Rest** | Medium | Medium | Perfect | AES-256 encryption of env vars in `apps.json`. Enterprise security. |
| 112 | **Safe Volume Deletion** | Low | Low | Perfect | `tengiz volume rm` cross-app check. Protect shared volumes. |
| 113 | **Port Mapping Protocol Selection** | Low-Med | Low | Perfect | TCP/UDP/both protocol selection. Non-HTTP services (DNS, gRPC, DB). |
| 114 | **Project-Based App Organization** | Low | Low | Perfect | `tengiz project create`, `tengiz ps --project`. Group related apps. |
| 115 | **App Tags** | Low | Low | Perfect | `tengiz tag add myapp staging`. `tengiz ps --tag`. Ad-hoc grouping. |
| 116 | **Pre-Install Env Validation (tengiz doctor)** | Low | Low | Perfect | Docker version, port availability, disk space check before deploy. |
| 117 | **Git Commit Hash Auto-Injection** | Low | Low | Perfect | `TENGIZ_COMMIT_SHA` env var. Shown in `tengiz ps --verbose`. |
| 118 | **Root Domain Change** | Low | Low | Perfect | `tengiz proxy --domain production.com`. Atomic domain + SSL update. |
| 119 | **Interactive Env Prompts** | Low | Low | Perfect | TTY prompts for required env vars on first deploy. `"generator": "secret"`. |
| 120 | **Patches (Build-Time File Overrides)** | Low | Low-Med | Perfect | Environment-specific `.env`, `robots.txt` injected during build. |
| 121 | **Cloudflare Tunnel Support** | Low-Med | Medium | Perfect | Zero-trust exposure via Cloudflare edge. `cloudflared` CLI wrapper. |
| 122 | **S3-Compatible Backup Storage** | Medium | Medium | Perfect | Database backups to S3. Scheduled + retention policy. |
| 123 | **Outgoing Webhook Payloads** | Low | Low | Perfect | POST deploy events to external URLs. CI/CD pipeline integration. |
| 124 | **Custom Compose Overrides** | Low | Low | Perfect | `docker-compose.override.yml` merge support for templates. |
| 125 | **App Cloning** | Low | Low | Perfect | `tengiz apps:clone <old> <new>`. Full config copy for staging/preview. |
| 126 | **Build Queue with Dedup** | Low | Low | Perfect | Per-app channel-based queue. Last-one-wins dedup for rapid-fire CI/CD. |
| 127 | **GoAccess Real-Time Log Analytics** | Low | Low | Perfect | Optional analytics container. `tengiz analytics enable` → dashboard. |
| 128 | **Container Real-Time Metrics** | Medium | Low | Perfect | `docker stats` live CPU/memory/network. `tengiz ps --stats`. |
| 129 | **Automated Database Backups** | Medium | Medium | Perfect | `docker exec <container> pg_dump`. Cron-based, S3 storage. |
| 130 | **SSH Key Management** | Medium | Medium | Perfect | Per-server SSH key pairs. `tengiz ssh-key generate/add/list/remove`. |
| 131 | **Rate Limiting** | Medium | Low | Perfect | Webhook/API rate limiting. `golang.org/x/time/rate`. HTTP 429. |
| 132 | **Service Template Registry** | Medium | Medium | Perfect | Central template registry with CDN auto-update. |
| 133 | **Log Drains (External Log Streaming)** | Medium | Medium | Perfect | Axiom, New Relic, Loki log forwarding. Structured metadata. |
| 134 | **AI-Powered Deployment Assistant** | Medium | Low | Perfect | `tengiz ai "deploy WordPress with Redis"` → LLM-generated compose. |
| 135 | **GPU Passthrough (NVIDIA/CUDA)** | Medium | Medium | Perfect | `--gpus all` flag. AI/ML workloads (Ollama, vLLM). |
| 136 | **URL Redirect & Rewrite Rules** | Medium | Low | Perfect | Per-app 301/302 redirects, URL rewrites at proxy level. |
| 137 | **Proxy Security Middleware** | Medium | Low | Perfect | IP allow/deny (CIDR), security headers (HSTS, CSP), per-app basic auth. |
| 138 | **CDN Provider Detection** | Medium | Low | Perfect | Cloudflare/Fastly IP range detection. Correct client IP extraction. |
| 139 | **Email Notification Engine** | Medium | Low | Perfect | SMTP-based alerts for deploy failure, SSL expiry, backup notification. |
| 140 | **Real-Time WebSocket for Deploy Logs** | Medium | Medium | Perfect | Live deploy log streaming. `tengiz deploy --stream`. Web UI foundation. |
| 141 | **Lambda Builder (Docker-Based FaaS)** | Low | Medium | Perfect | AWS Lambda-compatible functions on Tengiz. `lambda.yml` manifest. |
| 142 | **Container Entering (tengiz enter)** | Low | Low | Perfect | `tengiz enter <app>` → `docker exec -it`. Interactive debug shell. |
| 143 | **Trace/Debug Mode** | Low | Low | Perfect | `--debug` flag → slog LevelDebug. Verbose logging across all packages. |
| 144 | **Git-Sync Deployment** | Low | Low | Perfect | `tengiz deploy --sync <repo> --interval 5m`. Pull-based deployment. |
| 145 | **Railpack Builder** | Low | Low | Perfect | Alternative build system alongside Nixpacks/CNB. |
| 146 | **Null Builder** | Low | Low | Perfect | Skip build permanently. Pre-built images only. |
| 147 | **Failed Deploy Logs** | Low | Low | Perfect | `tengiz logs --failed <app>`. Retrieve last failed deploy logs. |
| 148 | **Vector Log Shipping** | Low | Medium | Perfect | Log aggregator companion container. Loki/Datadog/Axiom sinks. |
| 149 | **Config Validation** | Low | Low | Perfect | `tengiz config validate`. Pre-deploy config sanity check. |
| 150 | **Git-Based Image Version Tagging** | Low | Low | Perfect | Auto-tag images with git commit SHA. `tengiz-<app>:<sha>`. |
| 151 | **SSH Key Management for Deploy Access** | Low | Low | Perfect | Per-developer SSH key deploy access control. |
| 152 | **Web Dashboard (Admin UI)** | High | High | Good | Highest impact for non-CLI users but high effort and deviates from CLI-first. |
| 153 | **NetData Integration** | Low | Low | Perfect | Real-time system monitoring companion container. |
| 154 | **Platform Self-Health Check** | Low | Low | Perfect | Background goroutine + `/healthz` endpoint. Auto-restart on failure. |
| 155 | **Self-Hosted Docker Registry** | Low | Low | Perfect | Built-in `registry:2` container. `tengiz registry enable`. |
| 156 | **Service Update Strategy** | Low | Low | Perfect | `startFirst` vs `stopFirst` deploy strategy. Resource-constrained environments. |
| 157 | **Persistent Docker BuildKit Cache** | Low | Low | Perfect | Per-app build cache volume. 60-90% build time reduction. |
| 158 | **TypeScript Action Automation (Deno)** | Medium | Medium | Perfect | Embedded TS runtime for platform automation. Custom deploy logic, webhook transforms. |
| 159 | **OIDC/OAuth Single Sign-On** | Medium | Medium | Perfect | Google/GitHub OAuth + generic OIDC. Team auth for shared servers. |
| 160 | **Output/Telemetry Loggers** | Low | Medium | Good | OpenTelemetry/file logger. Centralized log collection (Loki, Datadog). |
| 161 | **CLI Alias Tanımlama** | Very Low | Very Low | Perfect | `.tengiz.yaml` `aliases:` section for command shortcuts. |
| 162 | **Alternative ACME Providers** | Low | Low | Perfect | ZeroSSL, BuyPass, Google. Let's Encrypt rate limit bypass. |
| 163 | **Staging Mode for SSL Testing** | Low | Low | Perfect | ACME staging endpoints for rate-limit-free SSL testing. |
| 164 | **Pluggable Multi-Scheduler (Docker → K3s)** | Low | Very High | Good | Scheduler abstraction. Huge architectural change. |
| 165 | **Pluggable Reverse Proxy** | Low | High | Good | nginx/Caddy/HAProxy/Traefik backend option. |
| 166 | **Custom Build Server** | Low | High | Good | Separate build/deploy servers. SSH + registry push/pull. |
| 167 | **Self-Upgrade / Auto-Update** | Low | Low-Med | Perfect | `tengiz upgrade`. GitHub Releases binary download. |
| 168 | **app.json Manifest (Heroku Compatible)** | Low | Medium | Perfect | Zero-config Heroku migration manifest. Merge with `.tengiz.yaml`. |
| 169 | **Git Submodules & Git LFS Support** | Low | Low | Perfect | `git submodule update --init --recursive` + Git LFS during clone. |
| 170 | **App-Level Lifecycle Data Hooks** | Medium | Medium | Perfect | Data change triggers: `onSetDoc`, `onDeleteDoc`. General trigger system. |
| 171 | **Docker Logging Konfigürasyonu** | Low | Low | Perfect | Per-app Docker log driver config (json-file, loki, syslog). |
| 172 | **Asset Path / Asset Bridging** | Low | Low | Perfect | Zero-downtime deploy asset co-existence for hash-based filenames. |
| 173 | **Advanced Docker Build** | Low | Low | Perfect | Multi-arch build, build cache, Docker driver config. |
| 174 | **Server Exec (Host-Level Commands)** | Low | Low | Perfect | `tengiz server exec "df -h"`. Controlled host access without SSH. |
| 175 | **Version Targeting** | Low | Low | Perfect | Deploy/exec specific previous version by tag or version ID. |
| 176 | **Cloud Native Buildpacks (pack CLI)** | Low | Low | Perfect | Heroku-style buildpacks via `pack` CLI. Alternative to Nixpacks. |
| 177 | **App Images Command** | Low | Low | Perfect | List Docker images per app: tag, creation time, size. |
| 178 | **System Stats Recording** | Low | Low | Perfect | Historical per-container CPU, memory, network stats. JSON Lines. |
| 179 | **Image Digest Change Detection** | Low | Low | Perfect | Auto-redeploy when tracked image digest changes. |
| 180 | **Procedure Automation** | Low | Low | Perfect | Multi-step workflows: deploy → migrate → healthcheck → notify. |
| 181 | **Image Digest Pinning** | Low | Low | Perfect | Pin deployment to specific SHA digest. Deterministic deploys. |
| 182 | **Granular Scoped API Keys** | Low | Low | Perfect | API keys with Read/Write/Execute permissions per resource type. |
| 183 | **Stack/Compose Lifecycle Management** | Low | Medium | Perfect | First-class Compose stack resource with full lifecycle. |
| 184 | **Database Backup Import/Upload** | Low | Low | Perfect | `tengiz backup import` from external SQL dump files. |
| 185 | **Protected Service Deletion** | Low | Low | Perfect | Confirmation, backup-before-delete, linked resource protection. |
| 186 | **Reusable External Storage Destinations** | Low | Low | Perfect | First-class S3 destination management for backups. |
| 187 | **Platform Admin Settings** | Low | Low | Perfect | Centralized server URL, Docker config, default resource limits. |
| 188 | **Per-App Proxy Toggle** | Low | Low | Perfect | `tengiz proxy:disable/enable <app>`. Internal-only services. |
| 189 | **Linux Capabilities Management** | Low | Low | Perfect | Per-app `--cap-add`/`--cap-drop`. Principle of least privilege. |
| 190 | **Multi-Channel Alerters** | Low | Low | Perfect | Slack/Discord/Pushover/ntfy with severity-based formatting. |
| 191 | **Docker Swarm Resource Management** | Low | High | Perfect | Multi-node HA via Docker Swarm mode. Lighter than K3s. |
| 192 | **Git Provider Account Management** | Low | Low | Perfect | Store GitHub/GitLab tokens for private repo clone access. |
| 193 | **Docker Registry Account Management** | Low | Low | Perfect | Multi-registry auth management for push/pull. |
| 194 | **WebSocket Interactive Terminal** | Low | Medium | Perfect | PTY allocation + WebSocket streaming. Beyond simple `docker exec`. |
| 195 | **Batch Operations Across Resource Types** | Low | Low | Perfect | Filter by tag/project/status, concurrent execution. |
| 196 | **Private Asset Access Tokens** | Low | Low | Perfect | Token-gated private file URLs. Shareable without auth. |
| 197 | **Granular Per-Operation Rate Limiting** | Low | Medium | Perfect | Per-operation-type limits: reads vs writes, uploads vs downloads. |
| 198 | **Collection Memory Type Configuration** | Low | Low | Perfect | Ephemeral (Go map) vs persistent (SQLite) per datastore collection. |
| 199 | **Wildcard Domain Support** | Medium | Low | Perfect | Auto-generated subdomains via `*.tengiz.local`. Zero-config DNS. |
| 200 | **sslip.io Fallback Domains** | Medium | Low | Perfect | `<app>-<ip>.sslip.io` zero-config DNS for instant URLs. |
| 201 | **www/non-www Redirect Management** | Low | Low | Perfect | Per-app canonical domain redirect. SEO standard. |
| 202 | **Domain Conflict Detection** | Low | Low | Perfect | Prevent duplicate domain assignment across apps. |
| 203 | **Memory Swap Limit / Swappiness** | Low | Low | Perfect | `--memory-swap` and `--memory-swappiness` Docker flags. |
| 204 | **CPU Set Pinning / CPU Shares** | Low | Low | Perfect | `--cpuset-cpus` and `--cpu-shares` for fine-grained CPU control. |
| 205 | **Custom Docker Labels Per App** | Low | Low | Perfect | User-defined Docker labels for external tool integration. |
| 206 | **Shared Environment Variables Across Apps** | Low | Low | Perfect | Define once, reference across multiple apps. |
| 207 | **Database Init Scripts** | Low | Low | Perfect | Custom SQL on first database container start. |
| 208 | **Config Override System** | Low | Low | Perfect | Runtime config overrides without rebuild. `~/.tengiz/override.yaml`. |
| 209 | **Verbose/Quiet CLI Flags** | Low | Low | Perfect | `--verbose`/`--quiet` global flags mapping to slog levels. |
| 210 | **Container Exec with Reuse/Raw Modes** | Low | Low | Perfect | `--reuse` runs in existing container, `--raw` for clean output. |
| 211 | **Docker Container Inspect** | Low | Low | Perfect | `tengiz inspect <app>` shows full container metadata. |
| 212 | **Docker Image CRUD Management** | Low | Low | Perfect | `tengiz image ls/inspect/prune/tag` standalone commands. |
| 213 | **Onboarding Keys** | Low | Medium | Perfect | Auto-enrollment keys for server agents. |
| 214 | **Custom Alerter Webhook Endpoint** | Low | Low | Perfect | Extensible alert routing via HTTP POST to custom URL. |
| 215 | **Event Stream WebSocket** | Low | Medium | Perfect | Real-time platform events for external integration. |
| 216 | **OIDC/JWT CI/CD Auth** | Low | Medium | Perfect | Keyless deploy auth via GitHub Actions OIDC JWTs. |
| 217 | **Rich Query System for Datastore** | Medium | Medium | Perfect | Pagination, ordering, filtering, batch operations. |
| 218 | **Proposal-Based CDN Deployment** | Low | Medium | Perfect | Atomic static asset updates via staged proposals. |
| 219 | **Platform Resource Auto-Management** | Medium | Medium | Perfect | Self-preservation monitoring: disk, Docker health, port exhaustion. |
| 220 | **User Usage Tracking & Metering** | Low | Low-Med | Perfect | Per-user operation counting for billing and rate limits. |
| 221 | **Multi-Domain Auth Origin Management** | Low | Low | Perfect | Dynamic `.well-known/alternative-origins` for custom domains. |
| 222 | **Resource Filtering via Wildcard/Regex** | Low | Low | Perfect | Advanced CLI listing with name, tag, wildcard, regex filters. |
| 223 | **Global Schedule Resource** | Low | Medium | Perfect | Unified cron engine above existing scheduled tasks/deployments. |
| 224 | **Wallet & Credits Billing System** | Low | High | Perfect | Usage-based billing for Tengiz Cloud offering. |
...
## File Browser / Directory Tree Browsing (Debug Deployments)
- **Source:** Dokploy
- **Description:** Dokploy's server-side `readDirectory` function provides a full directory tree browser for both local and remote (SSH) server paths. Users can recursively browse directories, view file contents, and inspect deployment artifacts directly from the platform. Useful for: verifying build output, checking config files, debugging failed deployments, inspecting log files on disk. Directory listing returns file names, sizes, modification times, and types.
- **Why add to Tengiz:** Debugging failed deployments requires inspecting build artifacts, config files, and log files on disk. Currently users must SSH into the server and use raw shell commands. `tengiz fs ls/read <app> <path>` and `tengiz fs tree <app>` provide app-aware file browsing scoped to the app's build/deploy directory. Implementation: Go `os/readdir` wrapper for local files, SSH-scoped for remote servers. Filesystem access scoped to `~/.tengiz/builds/<app>/` and app working directories for security. Low effort, high debugging value. Complements Failed Deploy Logs (#145) and Build Logs (#9) by adding filesystem-level inspection.
- **Detected:** 2026-07-18

## Archive Deploy (Upload Archive)
- **Source:** Dokploy
- **Description:** Dokploy supports `sourceType: "drop"` — deploy an application by uploading a zip or tarball archive (tar.gz, tar.bz2, .zip) directly through the UI or API. No git repository needed. The archive is extracted, framework detection runs on the extracted content, and the app is deployed normally. Enables quick deploys from local builds, CI/CD artifacts, and prototyping without git setup.
- **Why add to Tengiz:** Not all deployment sources are git repositories. CI/CD pipelines produce build artifacts (zip/tarball), designers may have static site builds, and quick prototypes don't need git. `tengiz deploy --archive ./build.tar.gz` extracts the archive, detects the framework, and deploys. Complements existing git-based deploy (#5) and webhook deploy (#13) with a third source type. Implementation: `archive/tar` and `archive/zip` extraction to a temp directory, then existing deploy pipeline runs normally. Low effort, enables CI/CD artifact deployment and quick prototyping.
- **Detected:** 2026-07-18

## libsql (Turso) Managed Database Support
- **Source:** Dokploy
- **Description:** Dokploy supports libsql (the open-source SQLite-compatible database that powers Turso) as a first-class managed database type alongside PostgreSQL, MySQL, MariaDB, MongoDB, and Redis. libsql offers SQLite compatibility with replication support, branching, and edge deployment capabilities. Dokploy provisions it as a Docker container with configurable versions, ports, credentials, and resource limits — same as other database types.
- **Why add to Tengiz:** Tengiz's planned Managed Database Provisioning (#63) currently lists PostgreSQL, MySQL, MongoDB, Redis, MariaDB, KeyDB, Dragonfly, and Clickhouse. Adding libsql provides a lightweight, SQLite-compatible alternative that is ideal for: single-node applications, embedded use cases, development/testing environments, and apps that need maximum simplicity. libsql containers are extremely lightweight (~15MB image, minimal RAM), making them perfect for Tengiz's scale-to-zero model. Implementation: add libsql to the managed DB type enum, configure its Docker image (`ghcr.io/tursodatabase/libsql-server:latest`), port mapping (default 8080), and credentials. The same backup, init script, and connection string auto-injection features apply. Low-medium effort (follows the same pattern as existing DB types), provides a complementary lightweight option alongside heavier PostgreSQL/MySQL managed databases. Particularly valuable for Tengiz's embedded serverless functions runtime (#50) and built-in NoSQL datastore (#1) — libsql serves as a persistent SQL backend for functions that need more than the ephemeral Go-map store.
- **Detected:** 2026-07-18

## Buildpack Management (Multi-Buildpack Ordering & Version Pinning)
- **Source:** Dokku
- **Description:** Per-app buildpack management with index-based ordering, version pinning via `REPO#SHA`, and `.buildpacks` file support. `buildpacks:add <app> <url> [--index N]` inserts a buildpack at a specific position. `buildpacks:remove <app> <url>` removes by URL or position. `buildpacks:list` shows ordered buildpacks. `buildpacks:set <app> <property> <value>` configures per-buildpack properties. Supports both Herokuish and Cloud Native Buildpacks with the same management interface. `.buildpacks` file at repo root is auto-detected and parsed on deploy.
- **Why add to Tengiz:** Tengiz currently selects a single builder backend (Dockerfile, Nixpacks, herokuish). Many real-world apps need multiple buildpacks composed in order — e.g., Node.js core + pgbouncer sidecar + nginx caching buildpack. Version pinning ensures reproducible builds. `.buildpacks` file is a Heroku/Dokku convention that simplifies migration from those platforms. Complements the existing Herokuish Buildpacks (#68) and Cloud Native Buildpacks (#174) features: those are the engines, this is the management layer. `.tengiz.yaml`'da `buildpacks:` listesi ile yapılandırılır: `buildpacks: [{url: "heroku/nodejs", version: "v238"}, {url: "https://github.com/heroku/heroku-buildpack-pgbouncer"}]`.
- **Detected:** 2026-07-18

## Port Management System (Full CRUD with Scheme-Based Mapping)
- **Source:** Dokku
- **Description:** Comprehensive per-app port mapping management beyond simple container/host port pairs. `ports:add <app> <scheme>:<host-port>:<container-port>` (schemes: http, https, grpc, grpcs). `ports:remove`, `ports:set`, `ports:clear`, `ports:list`. Auto-detects `EXPOSE` from Dockerfile as default ports. `ports:report` shows all mappings per app. Multiple ports per app supported (e.g., HTTP :80 + gRPC :50051). Scheme-based naming enables protocol-specific proxy handling.
- **Why add to Tengiz:** Existing Port Mapping Protocol Selection (#111) covers TCP/UDP protocol choice. Dokku's port management adds: scheme-based mapping (http/https/grpc), full CRUD lifecycle, EXPOSE auto-detection from Dockerfiles, and reporting. This is needed for multi-port apps (web + API + gRPC) and protocol-aware proxy routing. `.tengiz.yaml`'da `ports:` listesi: `ports: [{scheme: "http", host_port: 80, container_port: 3000}, {scheme: "grpc", host_port: 50051, container_port: 50051}]`. The proxy can then handle gRPC connections differently from HTTP. Low effort (structured config + Docker port flags), high value for polyglot/non-HTTP workloads.
- **Detected:** 2026-07-18

## Image Tag Management (Named Tags for Deploy Promotion)
- **Source:** Dokku
- **Description:** Manual Docker image tag creation and deployment by tag name, independent of auto-versioning. `tags:create <app> <tag>` creates a named tag (e.g., `v1.0`, `stable`, `canary`) pointing to the currently deployed image. `tags:deploy <app> <tag>` deploys the tagged image (rebuilds container from that image without source rebuild). `tags:destroy <app> <tag>` removes the tag. `tags:report` lists all tags per app. Enables: deploy promotion (`tags:deploy myapp stable`), canary rollback, environment promotion (dev→staging→prod tag chain), and manual version labeling.
- **Why add to Tengiz:** Existing Git-Based Image Version Tagging (#148) auto-tags with commit SHA. Manual tag management provides a higher-level semantic layer: promote a specific image version through environments (`stable → canary → production`), roll back to a specific named release, and label important deployment milestones. Different from both Build Pipeline auto-versioning (#129) which generates tags automatically, and rollback (#8) which reverts to previous deploy — tag management is a manual curation layer for deployment orchestration. Implementation: `docker tag` wrapper + `~/.tengiz/tags/<app>/<tag>.json` for tag metadata. `.tengiz.yaml`'da `deploy.tags: { "stable": true }` marks a deploy to also tag as `stable`.
- **Detected:** 2026-07-18

## Git Auth & Private Repository .netrc Credential Management
- **Source:** Dokku
- **Description:** Credential management for cloning private git repositories at deploy time. `git:auth <app> <host> <username> <password>` stores .netrc credentials per-host (github.com, gitlab.com, self-hosted). `git:auth-status <app>` shows configured hosts (without exposing passwords). `git:allow-host <app> <host>` adds SSH host key verification for private repos over SSH. `git:generate-deploy-key <app>` generates an SSH deploy key pair for the app's git repository. Credentials used during `git clone` in the deploy pipeline — enables deployment from private repos without manual SSH key configuration.
- **Why add to Tengiz:** Existing features cover SSH Key Management (#128, #149) and Git Provider Account Management (#200) conceptually, but Dokku's approach is more focused on the deploy pipeline's credential needs: .netrc for HTTPS-based clone, SSH known_hosts management, and deploy key generation. This is the practical "make private repo cloning work" layer that sits below account management. Implementation: `~/.tengiz/netrc` file (only readable by tengiz) injected via `GIT_ASKPASS` or `--config` during git clone. SSH keys stored in `~/.tengiz/ssh/` with automatic `ssh-keyscan` for known_hosts. Low effort (file-based credential store + git config injection), high practical value for private repo deployments.
- **Detected:** 2026-07-18

## Git Deploy Branch Configuration (Per-App Deploy Trigger Branch)
- **Source:** Dokku
- **Description:** Per-app and global configuration of which git branch triggers a deployment. `git:set <app> deploy-branch <branch>` overrides the default branch (typically `master`/`main`). Global default also configurable. When a git push or webhook event arrives, Tengiz checks if the pushed branch matches the configured deploy branch. Non-matching branches are ignored. Enables: `main` branch deploys to production, `develop` branch deploys to staging, feature branches ignored or deployed as preview environments.
- **Why add to Tengiz:** Existing Webhook Event Filtering (#50) covers which webhook events trigger deployment. Deploy branch configuration is different — it determines WHICH branch in the repository is the deployment source, regardless of push events. Without this, every git push triggers a build from that branch. With deploy branch configured, Tengiz always deploys from the specified branch (e.g., `main`) and ignores pushes to other branches. This is essential for git flow workflows. `.tengiz.yaml`'da `git.deploy_branch: main` ile yapılandırılır. Low effort (string config + branch check logic), high workflow value for teams.
- **Detected:** 2026-07-18

## storage:exec (Temporary Container with Storage Mount)
- **Source:** Dokku
- **Description:** Run one-off commands in a temporary container with a named storage volume mounted. `storage:exec <storage-name> [-- <cmd>...]` creates a temporary container from a minimal image (busybox/alpine), mounts the named storage entry (host path or Docker volume), executes the command, and removes the container on exit. Enables: inspecting storage contents (`storage:exec mydata -- ls -la /storage`), reading/writing files in shared volumes, debugging persistent data without running the full app container, and running data management tasks.
- **Why add to Tengiz:** Existing `tengiz run` (#4) executes commands in the app's image. storage:exec is different — it runs in a minimal container with ONLY the storage mount, making it ideal for: inspecting shared volumes between apps, debugging storage issues without app code interference, and running storage-specific maintenance tasks. Implementation: `docker run --rm -v <storage>:/storage busybox <cmd>`. Low effort (Docker CLI passthrough), high debugging value. Complements Persistent Storage (implemented) and Container Entering (#111).
- **Detected:** 2026-07-18

## Plugin Management System (Install/Update/Uninstall Plugins from Git)
- **Source:** Dokku
- **Description:** Full plugin lifecycle management system built on git-based distribution. `plugin:install <url> [--committish <ref>]` clones a plugin repo and registers it. `plugin:update <name>` pulls latest changes. `plugin:uninstall <name>` removes the plugin. `plugin:disable/enable <name>` toggles without removal. `plugin:list` shows installed plugins with version, status, and description. `plugin:trigger <name> <trigger>` manually runs a plugin trigger. Plugins are shell scripts placed in `~/.tengiz/plugins/<name>/` following a trigger-based architecture — each plugin implements functions like `post-deploy`, `pre-build`, `post-config-set` by providing executable files in named subdirectories. The core dispatches lifecycle events by scanning plugin directories and running matching executables with JSON context on stdin.
- **Why add to Tengiz:** Existing Pluggable Event/Trigger Architecture (#191) describes the trigger system concept. Plugin Management adds: distribution mechanism (git-based install), lifecycle (update/uninstall/disable), discovery (list with metadata), and community ecosystem potential. This is what made Dokku extensible with 100+ community plugins. For Tengiz, a plugin system means users can extend the platform without waiting for core features: new builders, custom health checks, secret backends, CI/CD integrations. Implementation: `os/exec` calling plugin scripts from `~/.tengiz/plugins/<name>/<trigger>/` with JSON context. Plugin discovery: scan `~/.tengiz/plugins/*/manifest.json`. Plugin registry: `tengiz plugin discover` fetches a community plugin index. High effort but transformative for ecosystem growth — the difference between a tool and a platform. Medium priority (P1) because the foundation (event triggers #191) must exist first.
- **Detected:** 2026-07-18

## Nginx Config Validation (Pre-Apply Validation Pattern)
- **Source:** CapRover
- **Description:** Before reloading nginx configuration, CapRover validates the generated config with `nginx -t`. If validation fails, the old config remains active and the error is reported. This prevents misconfigured proxy updates from taking down all apps. For Tengiz's Go proxy, the equivalent is: validate route table changes (port conflicts, duplicate routes, unresolvable targets) before applying them, with atomic rollback on failure.
- **Why add to Tengiz:** A misconfigured proxy update (duplicate route, invalid port, unreachable target) can disrupt all apps. Tengiz's proxy currently applies route changes immediately without validation. Adding a pre-apply validation step — check route uniqueness, verify port bindings, test container reachability — prevents downtime from misconfiguration. CapRover's `validateNginxConfigAndReload()` in `src/user/system/LoadBalancerManager.ts` is the reference. Implementation: a `validate()` method on the proxy's route table that checks for conflicts before `Register()`/`Unregister()` commits changes. Low effort, high reliability value.
- **Detected:** 2026-07-18

## Deployment Progress / Job Tracking (Real-Time Deploy Stages)
- **Source:** CapRover
- **Description:** CapRover tracks deployment progress through distinct stages (source validation, build, push, health check, proxy update) with real-time status reporting via the web UI. Each stage reports percentage progress, log output, and error details. The client (web dashboard or CLI) polls or receives push updates. Failed stages trigger immediate rollback with the failure point highlighted.
- **Why add to Tengiz:** Currently `tengiz deploy` is a black box — users see docker build output then wait. No visibility into which stage is running, how long it's taking, or where it failed. Real-time progress tracking enables: `tengiz deploy --progress` showing `[3/6] Health check...`, parallel build phase monitoring, and build-to-deploy trigger chain observability. Implementation: structured JSON log events per deploy stage written to a shared buffer, readable via `tengiz logs --deploy <app>`. For future web UI, exposed via WebSocket. Medium effort (structured event stream), high UX value. Complements existing Build Tracking (#124).
- **Detected:** 2026-07-18

## Max Version History Config (Per-App Retention Limit)
- **Source:** CapRover
- **Description:** CapRover stores `maxVersionHistory` (default 50) in `CaptainConstants.ts`, controlling how many deployment versions are retained per app. Older versions are pruned automatically. This is configurable per-app and globally. Combined with the existing KeepLastNImages mechanism, it limits disk usage from stale images and deployment records.
- **Why add to Tengiz:** Tengiz's `KeepLastNImages(ctx, appName, n)` hardcodes `n=5` in `gitdeploy/deployer.go`. A configurable `max_version_history` in `.tengiz.yaml` (`deploy.retention: 20`) lets users control the tradeoff between rollback depth and disk usage. Different from Container Retention Policy (#22, keeps old containers) — this controls image + deployment record retention. Low effort: move the hardcoded `5` to a config field, default to 10. High operational value for production instances with frequent deploys.
- **Detected:** 2026-07-18

## Feature Flags System (Dynamic Feature Toggles)
- **Source:** CapRover
- **Description:** CapRover's `FeatureFlags.ts` provides dynamic feature toggles for gradual rollout of new features. Toggles are checked at runtime in key decision points. Allows enabling/disabling features without code changes or redeploys. Useful for: A/B testing new deploy strategies, disabling experimental features in production, canary-feature rollout to specific app groups.
- **Why add to Tengiz:** As the feature surface grows, the ability to toggle features without redeploying the entire Tengiz binary becomes important. A `features_flags.yaml` or env-var-based toggle system enables: gradual rollout (enable for staging first), instant kill-switch for problematic features, and user-configurable behavior without config file changes. Implementation: `internal/feature/` package with a `map[string]bool` loaded from `TENGIZ_FEATURE_*` env vars or `~/.tengiz/features.yaml`. Middleware in CLI commands checks feature gates. Low-medium effort, enables safer feature rollout. Complements Pluggable Event Architecture (#191) as a foundation for a mature platform.
- **Detected:** 2026-07-18

## Config Override System (Runtime Config Overrides Without Rebuild)
- **Source:** CapRover
- **Description:** CapRover supports a `config-override.json` file that overrides internal constants at runtime without rebuilding the Docker image. Overrides apply to: port numbers, default values, timeouts, feature flags, and internal paths. This enables server operators to customize behavior in deployment-specific ways without maintaining a fork. The override file is read at startup and merged with built-in defaults.
- **Why add to Tengiz:** Currently all Tengiz constants (port ranges 9000-9999, default idle timeout 5m, health check intervals, deploy timeouts) are hardcoded. A config override system at `~/.tengiz/override.yaml` or `.tengiz.override.yaml` per-project would let operators: change the port range to avoid conflicts, adjust timeouts for slow/fast apps, override default builder selection, and customize internal paths. Implementation: `viper` already loaded — just add a second config file with lower priority that gets merged into the existing config. Complements Config Display (#48) and Config Validation (#147). Low effort, power-user friendly.
- **Detected:** 2026-07-18

## App Description Field (Human-Readable App Metadata)
- **Source:** CapRover
- **Description:** Each CapRover app has an optional human-readable description field (`IAppDef.description`). Displayed in the web dashboard as secondary text under the app name. Used in notification payloads and commit status reports to provide context about the app's purpose. Not the same as app name — it's free-form text like "Production API server for user authentication".
- **Why add to Tengiz:** Flat app names like `myapp` lack context. A description field in `AppEntry` (`description: "Main content API server (Node.js, connects to Postgres)"`) helps operators manage multiple apps. Displayed in `tengiz ps --verbose` and exported in config dumps. Implementation: `string` field in `AppConfig` and `AppEntry`, optional in `.tengiz.yaml`. Low effort, high organizational value for multi-app instances. Complements existing App Report (#10) and App Tags (#113).
- **Detected:** 2026-07-18

## Third-Party Template Repositories (External One-Click App Sources)
- **Source:** CapRover
- **Description:** CapRover allows adding external one-click app template repositories beyond the built-in catalog. Users add repository URLs via the UI or API (`oneclick/repositories` endpoint). Each repository provides its own set of app templates (WordPress, databases, analytics, etc.). The built-in repository is the CapRover official catalog at `https://github.com/caprover/one-click-apps`. Extends the one-click app ecosystem with community-contributed templates.
- **Why add to Tengiz:** Existing One-Click Templates (#64) and Service Template Registry (#130) cover a single central registry. Third-party repositories enable: community template contributions, private/internal template repositories for enterprise users, specialized template collections (AI/ML apps, data pipelines), and vendor-specific service catalogs. Implementation: a `repositories` list in `.tengiz.yaml` (`service.repositories: ["https://github.com/myorg/tengiz-templates"]`), fetched on `tengiz service list --refresh`. Each repository is a Git repo with a `templates/` directory. Medium effort (git clone + parse + merge), extends the platform ecosystem.
- **Detected:** 2026-07-18

## Nginx Status Metrics (Proxy Health & Performance Monitoring)
- **Source:** CapRover
- **Description:** CapRover exposes nginx status metrics: active connections, accepted connections, handled connections, reading/writing/waiting connection counts. Accessed via `getInfo()` on the LoadBalancerManager. These metrics provide a quick health snapshot of the proxy layer: connection backlog indicates load, reading/writing ratio shows request processing health, waiting connections reveal keepalive behavior.
- **Why add to Tengiz:** Tengiz's Go proxy currently has zero metrics exposure. Basic proxy health metrics (active connections, request count, errors) enable: `tengiz proxy status` health check, Grafana dashboard integration via existing Prometheus metrics (#26), quick debugging of traffic issues (connection leaks, high backlog). Implementation: `sync/atomic` counters in the proxy's `ServeHTTP` for active connections, total requests, errors. Exposed via `GET /__tengiz/metrics` on the admin port. Low effort (atomic counters + handler), fills gap between no observability and full Prometheus. Complements Prometheus Metrics (#26) which is more comprehensive.
- **Detected:** 2026-07-18

## Verbose/Quiet CLI Global Flags (Output Verbosity Control)
- **Source:** Kamal
- **Description:** Kamal supports `--verbose` and `--quiet` global flags that control SSHKit output verbosity. `--verbose` enables detailed logging (command execution, host output, timing), useful for debugging deployment pipelines and CI visibility. `--quiet` suppresses non-essential output, ideal for cron jobs and automated scripts where only errors should appear. The verbosity level propagates through all subcommands (deploy, rollback, logs, exec) — no per-command flag needed.
- **Why add to Tengiz:** Cobra already supports persistent flags — just plumb them through to the logging framework. Silent output for cron/deployment scripts (`--quiet` for non-interactive CI runs), detailed debugging for troubleshooting (`--verbose` for field-level diagnostics). Maps to `slog.LevelDebug` (verbose) and `slog.LevelError` (quiet). Low effort (~1h), improves both CI integration and developer debugging experience.
- **Detected:** 2026-07-18

## Container Exec with Reuse/Raw Modes (Kamal-Style One-Off Commands)
- **Source:** Kamal
- **Description:** Kamal's `app exec` supports three execution modes beyond the standard temp-container approach: `--reuse` runs the command inside the existing running container (no new container created — faster, shares process namespace), useful for debugging (`exec --reuse rails console`) and live inspection. `--raw` strips terminal control characters, produces clean stdout for piping into other tools or file redirection. `--interactive` enables TTY allocation with stdin attachment. Default mode (no flags) creates a new ephemeral container from the app image, runs the command, and removes it on exit. Command runs with the same env vars, secrets, and volume mounts as the running app.
- **Why add to Tengiz:** Existing `tengiz run` (#4) always creates a temporary container. Adding `--reuse` enables live debugging of running containers without spawning a separate process. Adding `--raw` makes output script-friendly (no escape sequences, clean JSON pipes). `tengiz run <app> <cmd> --reuse` runs in the running container (fast, shares state), `tengiz run <app> <cmd> --raw` produces clean output for tooling. Differentiates from `tengiz enter` (#140) which is specifically for interactive shell access — exec is for running arbitrary commands with fine-grained output control. Low effort (Docker CLI flag passthrough + TTY control), fills a gap for both debugging and scripting use cases.
- **Detected:** 2026-07-18

## Docker Container Inspect (Deep Configuration Inspection)
- **Source:** Komodo
- **Description:** Komodo provides full Docker container inspection via `inspect_container()` returning complete container metadata: network settings (IP, ports, DNS, networks), mounts (source, destination, mode, propagation), environment variables, full container config (entrypoint, cmd, working dir, user, labels), state (status, running, paused, restart count, exit code, started/finished timestamps), health check status (failing streak, log output), resource limits (CPU, memory, pids limit), and image details (digest, tags, OS/arch). Komodo's CLI `km container inspect` and UI both surface this data. `bin/periphery/src/docker/container.rs` — `inspect_container()` function.
- **Why add to Tengiz:** Currently `tengiz ps` shows only app name, status, port, and basic state. There's no way to see: what env vars are actually set (for debugging config injection), what volumes are mounted and where, what network the container is on, what health check results look like, or what resources the container is limited to. A `tengiz inspect <app>` command fills this diagnostic gap. Implementation: `docker inspect tengiz-<appname> --format '{{json .}}'` parsed into a Go struct and displayed as a formatted table or JSON (`--json` flag). Particularly valuable for: verifying config injection (env vars), debugging network issues (IP, DNS), checking volume mounts, and diagnosing health check failures. Low effort (Docker CLI passthrough + formatted output), high debugging value.
- **Detected:** 2026-07-18

## Docker Image CRUD Management (Standalone CLI Commands)
- **Source:** Komodo
- **Description:** Komodo provides full Docker image management as standalone operations: list images (tags, digests, size, created time, in-use status), inspect images (config, architecture, layers, env), remove images (single tag, force delete), prune unused images (with filtering), and tag images. `bin/periphery/src/docker/image.rs` (207 lines) implements `list_images()`, `inspect_image()`, `remove_image()`, `image_prune()` with structured output. The UI at `ui/src/pages/docker/image.tsx` shows image details in a table with search, sort, and per-image action menus.
- **Why add to Tengiz:** Tengiz currently has no standalone image management — images are only managed implicitly during deploy and cleanup. Users can't: list all images stored on the system and their sizes, inspect image details (layers, env, entrypoint), prune unused images without cleaning everything, or manually tag images for version management. `tengiz image ls [--app myapp]` shows images (filtered by app), `tengiz image inspect <image>` shows full config, `tengiz image prune` removes unused images (respecting KeepLastNImages). Different from existing "App Images Command" (#124) which shows per-app version history — this is system-wide image management. Complements Granular Docker Prune Operations (#159). Low effort (wrapper around `docker image` CLI commands).
- **Detected:** 2026-07-18

## Onboarding Keys (Auto-Enrollment for Server Agents)
- **Source:** Komodo
- **Description:** Komodo's onboarding key system (`bin/core/src/api/write/onboarding.rs`, 241 lines) generates time-limited, scoped keys for automatically registering new Periphery agents with the Core server. Each `OnboardingKey` has: expiry timestamp (keys auto-expire), optional tags (auto-applied to registered servers), `copy_server` (replicate config from existing server), `create_builder` (auto-create a Builder resource for the new server). The onboarding flow: generate key on Core → run `periphery --onboarding-key <key>` on target server → Periphery contacts Core with key → Core validates, registers server, applies tags, copies config. This eliminates manual server registration — essential for fleet management.
- **Why add to Tengiz:** The existing "Multi-Server Architecture with Periphery Agent" (#142) describes the distributed model but doesn't address the setup friction. Onboarding keys reduce adding a new server from a multi-step manual process to: `tengiz server onboarding-key create --expires 24h` on Core → `tengiz agent --key <key>` on new server. The one-time key pattern prevents unauthorized server enrollment (key expires). Auto-configuration (tags, builder creation, config copy from template) ensures consistent setup across servers. Implementation: generate a random token (32 bytes from `crypto/rand`), store with metadata in `~/.tengiz/onboarding/`, Periphery agent sends it on first connection. Critical for any multi-server deployment workflow.
- **Detected:** 2026-07-18

## Custom Alerter Webhook Endpoint (Extensible Alert Routing)
- **Source:** Komodo
- **Description:** Beyond built-in alerter channels (Slack, Discord, Pushover, ntfy), Komodo supports custom HTTP webhook endpoint alerters. Alerter configuration specifies a target URL. Komodo POSTs structured JSON alert payloads to the URL with: alert type, severity level, resource type, resource ID, title, description, timestamp, and resolved/unresolved status. External services can receive and process alerts in any way. `example/alerter/` provides a complete reference implementation as an Axum web server (Rust) that receives and logs alerts. Users can write alerters in any language as long as they accept POST JSON.
- **Why add to Tengiz:** The existing "Multi-Channel Alerters" (#156) covers major messaging platforms, but there will always be users with custom alert destinations: custom dashboards, internal incident management systems, PagerDuty/Opsgenie webhook integrations, SMS gateways, or homegrown monitoring. A custom webhook alerter makes the system fully extensible without modifying Tengiz core. `.tengiz.yaml`'da `alerters.custom: [{name: "pagerduty", url: "https://events.pagerduty.com/v2/enqueue", headers: {Authorization: "Bearer ..."}}]`. Implementation: generic HTTP POST handler in the alerter system, JSON payload format documented in `tengiz docs alerters`. Low effort (generic HTTP POST with JSON), infinite extensibility.
- **Detected:** 2026-07-18

## Event Stream WebSocket (Real-Time Platform Events for External Integration)
- **Source:** Komodo
- **Description:** Komodo provides a WebSocket-based event stream that external services can subscribe to for real-time platform events. The `update_logger` example (`example/update_logger/`) demonstrates a complete consumer: connects to the Core WebSocket, receives structured update events (deploy start/complete/fail, build status change, container lifecycle), and logs them. Each event contains: target resource type, operation type (create/update/delete/execute), timestamp, user who triggered it, status, and associated log output. This enables building external integrations (Slack bots, metrics dashboards, audit logging services) without polling.
- **Why add to Tengiz:** The existing "Outgoing Webhook Payloads" (#113) covers event-triggered HTTP POSTs to predefined URLs. A WebSocket event stream is complementary — it enables: real-time dashboards that update without polling, external CI/CD status boards, custom analytics pipelines, and third-party tools that integrate with Tengiz without needing webhook configuration per-app. Implementation: WebSocket endpoint at `/ws/events` on the proxy admin port, event types mirror the existing hook/trigger system, JSON-encoded messages with type-tagged enums. Go's `gorilla/websocket` + channel-based fan-out pattern. Medium effort (WebSocket server + event bus), enables a rich ecosystem of external integrations.
- **Detected:** 2026-07-18

## Periphery-Scoped Git & Registry Providers (Server-Level Credentials)
- **Source:** Komodo
- **Description:** Komodo supports both global (Core-level) and Periphery-scoped (server-level) git provider and Docker registry configurations. `periphery.config.toml` can contain `[[git_provider]]` and `[[docker_registry]]` sections specific to that Periphery agent. This allows different servers to have different credentials: server A (dev) accesses private GitHub repos via one token, server B (prod) via another. Core uses whichever provider is applicable to the target server during deployment. `bin/core/src/api/write/provider.rs` (486 lines) handles both global and Periphery-scoped provider CRUD.
- **Why add to Tengiz:** The existing "Git Provider Account Management" (#151) and "Docker Registry Account Management" (#152) describe global provider management. Periphery-scoped configuration is essential in multi-server setups: different servers may need different credentials (dev vs prod tokens), some servers may not need certain providers, and security-conscious deployments prefer server-local credentials over global ones. Implementation: per-server provider config in the server's state entry, resolved at deploy time with fallback to global config. `.tengiz.yaml`'da `servers.<name>.git_providers:` veya `servers.<name>.registry_providers:` ile yapılandırılır. Medium effort (scoped config + resolution logic), required for production multi-server deployments.
- **Detected:** 2026-07-18

## CLI Generic Resource Create/Update Commands
- **Source:** Komodo
- **Description:** Komodo's CLI (`km create` and `km update`) provides generic commands for managing all resource types: `km create server`, `km create deployment`, `km create build`, `km create stack`, `km create procedure`, `km create action`, `km create sync`, `km create builder`, `km create alerter`. Each accepts resource-specific flags and JSON/YAML input. `km update <resource-type> <id>` modifies existing resources. `bin/cli/src/command/create/` has type-specific subcommands (5+ files). This contrasts with Komodo's UI management — CLI create/update enables scripting, CI/CD, and infrastructure-as-code without the web UI.
- **Why add to Tengiz:** Currently Tengiz's CLI is app-centric: `tengiz deploy`, `tengiz config set`, `tengiz domain add`. There's no generic resource model — every feature needs its own dedicated command. As Tengiz adds more resource types (accessories, stacks, builds, procedures, backups, etc.), a generic `tengiz create` and `tengiz update` pattern becomes essential. Implementation: a `tengiz create <type> [flags]` command pattern using Cobra's subcommand system, where each resource type registers its fields. Supports `--json`/`--yaml` for programmatic input. Empowers the GitOps workflow: `tengiz apply -f app.yaml` applies a YAML manifest. High effort but critical architectural investment as the resource model expands. Complements GitOps (#116) by providing the CLI interface for declarative management.
- **Detected:** 2026-07-18

---

## OIDC/JWT CI/CD Automation Authentication (Keyless Deploy Auth)
- **Source:** Juno
- **Description:** Juno's `authenticate_automation()` API verifies OIDC JWTs from GitHub Actions workflows for secure, keyless deployment authentication. Supports: JWT issuer validation (tokens.actions.githubusercontent.com), audience checking, JTI (JWT ID) replay attack prevention via cached nonce tracking, branch-level restrictions (deploy only from specific refs), workflow metadata extraction (run number, attempt, commit SHA, actor), and provider certificate refresh scheduling with JWKS caching. Automation controllers have scoped permissions: `Write`, `Submit`. No long-lived secrets needed — each deployment session authenticates via ephemeral OIDC tokens from the CI provider.
- **Why add to Tengiz:** Existing "Headless CI/CD Mode" uses long-lived tokens (TENGIZ_TOKEN) which are static secrets that can leak, expire, or be stolen. OIDC-based authentication is more secure: tokens are ephemeral (per-workflow-run), automatically scoped to the specific repo/branch/ref, and verified against the provider's JWKS endpoint. This is the industry standard for CI/CD security (AWS, GCP, Vercel all support OIDC). Implementation: Go OIDC verification middleware (`coreos/go-oidc/v3`) in the webhook handler, branch extraction from the JWT's `sub` claim or custom claims, JTI cache for replay prevention. Complements Headless CI/CD Mode (#17, token-based) and HMAC-Signed Webhook Payloads (#40, webhook signing) — all three form a layered CI/CD security model. `.tengiz.yaml`'da `ci.oidc.providers: [{issuer: "https://token.actions.githubusercontent.com", allowed_repos: ["myorg/*"]}]` ile yapılandırılır. Medium effort, high security value.
- **Detected:** 2026-07-18

## Rich Query System for Document Datastore (Pagination, Ordering, Filtering, Batches)
- **Source:** Juno
- **Description:** Beyond basic CRUD (`set_doc`/`get_doc`/`del_doc`), Juno's datastore supports: pagination (`start_after` cursor, `limit`), ordering (`Keys`, `CreatedAt`, `UpdatedAt`, ascending/descending), matcher-based filtering (`Equal`, `NotEqual`, `GreaterThan`, `LessThan`, `Between`), batch operations (`set_many_docs`, `del_many_docs`, `get_many_docs`), and filtered deletes (`del_filtered_docs` with query-like conditions). Combined with owner-based access control, this provides a database-level query capability without external SQL or indexing services.
- **Why add to Tengiz:** The recorded "Built-in NoSQL Datastore" describes the concept but not the query sophistication. Without pagination and filtering, a built-in datastore is only useful for simple key-value lookups. With rich queries, it becomes viable for real applications (user lists, content feeds, filtered collections). Implementation: extend the embedded datastore (SQLite or Go-map) with index structures for sortable fields, cursor-based pagination via offset/keyset, and a matcher expression tree evaluator. The proxy's `/__tengiz/db/` API exposes these as query parameters. `.tengiz.yaml`'da `db.collections.users.indexes: [created_at, email]` enables indexed queries. Transforms the datastore from a toy to a production-capable embedded database. Medium-high effort, critical for the NoSQL Datastore to be genuinely useful.
- **Detected:** 2026-07-18

## Proposal-Based CDN Deployment (Atomic Static Asset Updates)
- **Source:** Juno
- **Description:** Juno's CDN proposal system provides atomic static asset updates: `init_proposal` creates a proposal with a unique ID, `submit_proposal` batches asset upload operations (init/upload/commit proposal asset chunks) into the proposal, and `commit_proposal` activates all assets atomically or `reject_proposal` discards them. Proposals are listable and countable for history. Proposal assets are stored in a staging area during submission and only moved to production on commit. A failed or partial upload doesn't leave the site in a broken state.
- **Why add to Tengiz:** This is fundamentally different from the recorded "Staged Deployments" (container change sets). Proposal-based CDN deployment is about atomic static asset updates: uploading a new version of a single-page app, updating 200+ static files atomically so users never see a half-deployed site. For Tengiz, this means: `tengiz deploy --assets ./dist/` creates a proposal, uploads all files to a staging directory, and atomically swaps the asset root. Implementation: a staging directory (`~/.tengiz/proposals/<app>/<id>/`), atomic rename-on-commit, and proxy middleware that serves from the active asset root. Complements existing static file hosting and zero-downtime deploy (#1) — zero-downtime for containers, proposal-based for assets. Medium effort, ensures asset consistency during deploys.
- **Detected:** 2026-07-18

## Platform Resource Auto-Management (Self-Preservation Monitoring)
- **Source:** Juno
- **Description:** Juno's Mission Control module continuously monitors the platform's own resource consumption (cycles/credits balance) with configurable thresholds. When resources drop below threshold, it automatically procures more (auto-funding). Monitoring history is tracked for capacity planning. Configurable monitoring strategies per segment. This is distinct from monitoring user apps — it's the platform monitoring and managing its own health and resource needs.
- **Why add to Tengiz:** A production Tengiz instance has its own resource constraints: disk space for images/volumes, available port range (9000-9999), Docker daemon health, and system memory. Currently there's no mechanism for Tengiz to monitor its own health and take corrective action. Implementation: a background goroutine that checks: disk usage (prune if >80%), Docker daemon responsiveness (restart proxy if unresponsive), port exhaustion (clean up orphaned allocations), and memory pressure. Alerts on detected issues and auto-remediation for known failure modes. `.tengiz.yaml`'da `self_health.disk_threshold: 85%`, `self_health.auto_prune: true`, `self_health.monitor_interval: 60s` ile yapılandırılır. Complements Platform Self-Health Check (#152, passive HTTP check) with active resource management. Medium effort, essential for unattended production operation.
- **Detected:** 2026-07-18

## Wallet & Credits Usage-Based Billing System
- **Source:** Juno
- **Description:** Juno implements a complete billing system: account credits awarded on signup (initial free tier), ICP/ICRC token payment support, payment acknowledgment/completion/refund lifecycle, fee configuration per service type (Satellite, Orbiter, etc.), rate limiting for cost control, withdraw/payout functionality, and token bucket rate configs. The billing system ties every resource operation to usage tracking — deployments, storage, bandwidth, and compute are all metered.
- **Why add to Tengiz:** Usage-based billing transforms Tengiz from a self-hosted tool into a potential SaaS platform. Even for self-hosted instances, usage tracking helps operators understand resource consumption patterns. Implementation: a `tengiz billing` command family and usage recording in `~/.tengiz/usage/` as daily rollup JSON. Start with read-only usage reporting (`tengiz billing usage --app myapp --since 7d`), add credit system later. `tengiz plan set --app myapp --limits '{"storage": "1GB", "bandwidth": "100GB"}'` enables plan enforcement. High effort, but opens the path to a Tengiz Cloud offering. Complements User Usage Tracking (#202, below) which provides the raw metering data.
- **Detected:** 2026-07-18

## User Usage Tracking & Metering (Per-User Operation Counting)
- **Source:** Juno
- **Description:** Per-user, per-collection operation counting: tracks document create/set/delete counts by user identity, storage upload/download sizes, and function execution counts. Usage data feeds billing calculations and plan enforcement (rate limits, storage caps). Internal hooks auto-track every operation. No external analytics service needed — all tracking is built into the platform's data layer.
- **Why add to Tengiz:** The billing system needs raw metering data. User usage tracking provides: per-app operation counts (how many reads/writes/storage operations), per-user attribution (which developer triggered what), daily/weekly/monthly rollups for trend analysis, and enforcement data for rate limits and plan caps. Implementation: atomic counters in the datastore layer (`sync.Map` + periodic flush to JSON), keyed by `{app, user, operation, date}`. Rollup cron job aggregates daily data and prunes raw counters beyond retention. Exposed via `tengiz usage <app> --period 24h`. Low-medium effort on top of existing datastore/API infrastructure, provides the measurement foundation for billing and rate limiting. Complements Wallet & Credits Billing System (#201) and Granular Per-Operation Rate Limiting (#133).
- **Detected:** 2026-07-18

## User Invitation Code System (Beta/Access Management)
- **Source:** Juno
- **Description:** Juno implements invitation codes for controlled user registration. Each code has a configurable credit allocation (free credits on signup), single-use or multi-use mode, and optional expiry. Code redemption is tracked to prevent reuse. Rate limiting on signup prevents free-tier abuse. Invitation codes enable beta programs, referral systems, and invite-only feature rollouts.
- **Why add to Tengiz:** As Tengiz gains multi-user capabilities (SSO, RBAC, API keys), controlled team onboarding becomes important. Invitation codes enable: beta access for early testers, team member onboarding without exposing the registration endpoint publicly, referral programs for community growth, and controlled staging/production access separation. Implementation: `tengiz invitation create [--uses 10] [--credits 100]` generates a code stored in `~/.tengiz/invitations.json`. Registration flow checks the code before creating a user account. Low-medium effort, useful for any multi-user or SaaS deployment of Tengiz. Complements OIDC/SSO (#128) as an additional onboarding path.
- **Detected:** 2026-07-18

## Platform Module Upgrade System (Independent Service Upgrades)
- **Source:** Juno
- **Description:** Juno's modular architecture allows independent upgrades of platform services (Satellite, Console, Mission Control, Observatory, Orbiter). Upgrades follow a proposal pattern: new module version is staged, verified, and then committed. Release metadata tracks available versions and changelogs. CDN-based proposal assets distribute new module binaries. Version history enables rollback of individual module upgrades without affecting others. The upgrade dock in the Console UI manages pending upgrades.
- **Why add to Tengiz:** Tengiz is a single binary — upgrading means replacing the whole thing. As features grow (multiple platform services: proxy, auth, datastore, storage, analytics, functions), independent service upgrades become valuable: patch the auth module without restarting the proxy, roll back a broken analytics upgrade without affecting running apps. Implementation: a plugin-like module system where each platform service is a separate Go interface implementation loaded at startup. Module manifests in `~/.tengiz/modules/` specify version, binary path, and checksum. `tengiz module update <name>` downloads and activates a new version. `tengiz module rollback <name>` reverts. High effort but architecturally transformative — moves from monolithic binary to composable platform. Complements Self-Upgrade (#138, binary-only upgrade) with service-level granularity. Long-term architectural direction for multi-service platform evolution.
- **Detected:** 2026-07-18

## Multi-Domain Auth Origin Management (Dynamic `.well-known/alternative-origins`)
- **Source:** Juno
- **Description:** Juno dynamically generates and maintains `/.well-known/alternative-origins` files for multi-domain authentication. When custom domains are added to a satellite (app), the auth system auto-discovers them and registers them as valid authentication origins. This enables users to authenticate from any of their app's custom domains without manual origin configuration. The well-known file is certified (signed) for authenticity verification. External alternative origins can also be configured manually, supplementing auto-discovered domains.
- **Why add to Tengiz:** The recorded "Well-Known Paths Automatic Handling" covers generic `/.well-known/` support for SSL, app association, and verification. This specific feature extends that to auth — when a user adds a custom domain, the auth provider (Google, GitHub) needs to know it's a valid redirect origin. Dynamic origin management automates this: `tengiz domain add myapp.com` auto-updates the auth provider's allowed origins. Implementation: after adding a domain in the proxy and store, update the auth module's `allowed_redirect_origins` list and regenerate the well-known origins file. This bridges the existing "Custom Domain Management" (implemented) and "Built-in Authentication Service" (recorded) — domains and auth must be domain-coordinated. Low effort (hook domain add → update auth config), eliminates a common auth misconfiguration pain point.
- **Detected:** 2026-07-18

## Resource Filtering via Wildcard/Regex (Advanced CLI Listing)
- **Source:** Komodo
- **Description:** Komodo's CLI `km list` command supports filtering resources by multiple criteria: name (exact or substring), tag (multiple tags, AND logic), wildcard patterns (`prod-*`), and regex patterns (`^web-.*-api$`). Applied to any resource type (deployments, servers, stacks, builds, procedures, actions). `bin/cli/src/command/list.rs` (1217 lines) implements a full filtering engine with combinable filters and colored output. The filter syntax follows Unix conventions: `--name myapp*`, `--name-regex '^web.*'`, `--tag production --tag critical`.
- **Why add to Tengiz:** As the number of managed apps and resources grows, finding specific items becomes hard. `tengiz ps --filter "name=prod-*"` or `tengiz ls deployments --tag staging` makes large-scale management practical. Combinable filters (name + tag + status) enable precise queries. Implementation: Go glob matching (`path.Match`) for wildcards, `regexp` for regex. Filters applied post-query before display. Extends the existing `--tag` flag for ps with a more powerful filter system. Low-medium effort, high operational value for multi-app instances.
- **Detected:** 2026-07-18

## Global Schedule Resource (Unified Cron/Schedule Engine)
- **Source:** Komodo
- **Description:** Komodo has a first-class `Schedule` resource type (`bin/core/src/schedule.rs`, 388 lines) that can trigger any operation type on a schedule. Schedule rules defined with cron expressions or English-like syntax (`"every day at 3am"`, `"every 30 minutes"`). Each schedule references a target resource (procedure, action, or raw execution) and its parameters. Schedules are managed via the same CRUD lifecycle as other resources — created, updated, deleted, listed. The scheduler engine runs in the background, evaluates cron expressions at startup (catch up on missed runs), and dispatches executions. Supports timezone-aware scheduling.
- **Why add to Tengiz:** Existing features cover "Scheduled Tasks / Cron Jobs" (#54, container commands), "Scheduled Deployments" (#53, deploy pipeline), and "Scheduled Builds" (build execution). Komodo's approach unifies all scheduling behind a single engine. A `tengiz schedule` command family enables: `tengiz schedule create --cron "0 3 * * *" --target deploy:myapp` for nightly deploys, `tengiz schedule create --every 30m --target procedure:health-check` for periodic maintenance. Implementation: `robfig/cron` library (already referenced), schedule records in `~/.tengiz/schedules.json`, background goroutine checks every 30s. Schedule records reference target resources by type+ID. Medium effort (unified engine above existing cron features), eliminates scheduling fragmentation.
- **Detected:** 2026-07-18
