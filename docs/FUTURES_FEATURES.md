...
## Multi-Tenancy (Organization → Project → Environment → Service Hierarchy)
- **Source:** Dokploy
- **Description:** Hierarchical multi-tenancy model: Organization (top-level tenant, user mgmt, billing, SSO, global settings) → Project (logical grouping of related services, shared env vars, access control) → Environment (isolation layer per deploy stage: prod/staging/dev, env vars that override project level) → Service (actual workloads: apps, databases, compose stacks). Each level inherits and can override variables from its parent. Access control maps to the hierarchy — users can be granted project-level or environment-level access without exposing the entire infrastructure. Dokploy's `member` resource ties users to organizations with role-based permissions. All resources (servers, registries, SSH keys, certificates, S3 destinations) are scoped to organizations.
- **Why add to Tengiz:** Tengiz currently uses a flat model — apps exist independently with `--env` providing basic environment isolation. There is no concept of organization, project grouping with shared env vars, multi-user access control, or hierarchical variable inheritance. This prevents Tengiz from being used in team/agency/multi-tenant scenarios. Adding a Dokploy-style hierarchy would: (1) enable team collaboration with RBAC (developers deploy to staging, only admins deploy to production), (2) reduce config duplication via inherited project/environment variables, (3) provide resource isolation between tenants on a shared Tengiz instance, (4) allow agencies to manage multiple client projects in one Tengiz instance. Implementation: 4-level resource tree in `types/` (`Organization`, `Project`, `Environment`, `Service`), variable inheritance engine that resolves `${{project.VAR}}` and `${{environment.VAR}}` syntax, permission middleware in proxy and CLI. Start with 2 levels (Project → Service) and extend upward to Organization and Environment. Compatible with existing `AppEntry` — add `ProjectID` and `EnvironmentID` fields. The `store` package needs scoping by organization. High effort but transforms Tengiz from a single-user tool into a multi-team platform.
- **Detected:** 2026-07-25

## Concurrent Builds (Per-Server Build Concurrency Management)
- **Source:** Dokploy
- **Description:** Dokploy manages build concurrency with per-server queue partitions. Each server (local, remote, build) has its own independent queue. Within a server's queue, up to N builds can run in parallel (configurable concurrency). Multiple deployments of the SAME application are always serialized (FIFO) regardless of concurrency setting — preventing collisions on source directories and container names. Default concurrency is 1 (one build at a time). OSS users can increase to max 2; Enterprise users have unlimited concurrency. Concurrency is configured per-server, so build servers with more resources can have higher concurrency than the main server. Deployments queue wait until slots are available. Queued deployments can be cancelled.
- **Why add to Tengiz:** Existing "Build Queue with Deduplication" (#124) and "Parallel Bulk Operations" (#53) cover related but different concerns. Dokploy's concurrent builds addresses a specific operational bottleneck: when deploying to multiple apps simultaneously, the Docker daemon can be overwhelmed by concurrent builds, causing slowdowns or failures. Per-server concurrency gives operators precise control: (1) prevents Docker daemon resource exhaustion from too many concurrent builds, (2) same-app serialization prevents build collisions without broad serialization of ALL builds, (3) different concurrency for build servers vs deploy servers enables optimal resource utilization. Implementation: a per-server goroutine pool (`chan struct{}` semaphore) in the deploy pipeline, a queue store (`~/.tengiz/queue.json`) for pending deployments, per-app channel-based serialization (mutex per app name). `tengiz config set server.concurrency 3` CLI command. Higher priority than general parallel operations — controls resource usage rather than just speed. Complements the existing per-deploy flow with a capacity management layer.
- **Detected:** 2026-07-25

## Dockerfile EXPOSE Directive Parsing (Auto-Detect Container Port)
- **Source:** Dokku
- **Description:** Auto-detect container ports by parsing `EXPOSE` directives in the Dockerfile. If the Dockerfile declares `EXPOSE 3000`, Tengiz uses that as the internal port — no manual `port:` config needed. Falls back to framework defaults only when no EXPOSE is present. Respects `EXPOSE` over framework auto-detection, since the Dockerfile author explicitly chose that port.
- **Why add to Tengiz:** Currently Tengiz requires manual `port: 3000` config or framework defaults (Next.js=3000, Go=8080). When a user provides a Dockerfile with `EXPOSE 5000`, Tengiz should honor it automatically. Eliminates the "container started but port mismatch" failure mode. Implementation: `builder.go` parses `EXPOSE \d+` regex from Dockerfile during detection, stores it in `BuildConfig.InternalPort`. Low effort (~30 lines), high UX value. Complements existing framework auto-detection.
- **Detected:** 2026-07-25

## `ps:rebuild` Command (Rebuild Without Git Push)
- **Source:** Dokku
- **Description:** Rebuild and restart an app from its existing source code without needing a new git push or source change. Retriggers the full build pipeline (detect → build → deploy) using the last deployed source. Useful after: config changes (env vars, docker-options, domains), resource limit adjustments, base image updates, or failed deploy retries.
- **Why add to Tengiz:** Currently Tengiz requires a full `tengiz deploy .` (or git push) to rebuild, even for trivial config changes. `tengiz rebuild <app>` re-runs the build pipeline from the cached source. Essential for: reapplying env vars after `tengiz config set`, retrying failed deploys without git push, picking up updated base images. Implementation: reuses existing deploy pipeline, stores source reference (git SHA, archive path) in deployment record. Low effort (reuse existing deploy code), high operational value.
- **Detected:** 2026-07-25

## Buildpack URL Management (Per-App Custom Buildpack Pinning)
- **Source:** Dokku
- **Description:** Per-app custom buildpack URL management: `tengiz buildpacks:add/set/remove <app> <url>` to specify which buildpack(s) to use. Supports pinned buildpack versions (`heroku/buildpacks:22`), custom forks (`github.com/user/buildpack`), multi-buildpack via `.buildpacks` file, and `BUILDPACK_URL` env var for single-pack override. Each app can have its own buildpack stack, independent of the platform default.
- **Why add to Tengiz:** Existing Herokuish and Nixpacks support runtime selection but not per-app buildpack URL pinning. Teams need to: pin specific buildpack versions for reproducible builds, use custom forks with patches, or set different buildpacks per environment. Implementation: `tengiz buildpack set <app> <url>` stores URL in `AppEntry.Config.BuildpackURL`. Builder inspects this flag during the build phase. Low effort (string + conditional in builder), fills a gap between "use default buildpack" and "provide custom Dockerfile."
- **Detected:** 2026-07-25

## `app.json` Script Phases (Heroku-Structured Lifecycle Hooks)
- **Source:** Dokku
- **Description:** Heroku-style `app.json` `scripts` section defining structured lifecycle phases: `scripts.predeploy` (runs before new container starts, in a temporary container from the new image), `scripts.postdeploy` (runs after new container is accepting traffic), `scripts.release` (runs during deploy in a temporary container — failure blocks deploy). Each phase has distinct semantics: predeploy/postdeploy are informational, release is authoritative and gates the deployment.
- **Why add to Tengiz:** Existing pre-deploy hooks (#9) cover arbitrary shell commands. The `scripts.release` phase is unique — it runs in a throwaway container from the new image and its exit code gates deployment. This is how Heroku runs migrations safely: if migration fails, the old app stays running. Implementation: extend existing hook system with phase-aware runners. Release phase uses `runtime.RunOneOff()` and gates the deploy pipeline. `.tengiz.yaml`'da `scripts.release: "bundle exec rails db:migrate"` ile yapılandırılır. Low-medium effort, critical for database-backed app deploys.
- **Detected:** 2026-07-25

## Port Scheme Management (http/https/grpc Scheme Metadata)
- **Source:** Dokku
- **Description:** Multi-scheme port mapping with metadata: `tengiz ports:add <app> http:80:5000 https:443:5000 grpc:8080:5000`. Each port mapping includes a scheme (http, https, grpc, grpcs) that affects proxy backend generation. Commands: `ports:add`, `ports:remove`, `ports:set`, `ports:report`, `ports:clear`. Proxy backends use scheme metadata for proper protocol handling (TLS termination, gRPC support, HTTP/2 negotiation).
- **Why add to Tengiz:** Tengiz currently supports simple HTTP port mapping. Production apps may need: HTTP + HTTPS on different ports, gRPC endpoints (scheme affects proxy routing), or multiple services on different ports within the same container. Implementation: `[]PortMapping` struct with `{Scheme, HostPort, ContainerPort}` in `AppEntry`. Extends existing port allocation with scheme support. Low-medium effort, unblocks gRPC and multi-protocol service deployments.
- **Detected:** 2026-07-25

## Plugin Management Commands (Install/Update/Uninstall/List)
- **Source:** Dokku
- **Description:** First-class plugin management: `tengiz plugin install <repo-url>` downloads and registers a plugin, `tengiz plugin update <name>` updates it, `tengiz plugin uninstall <name>` removes it, `tengiz plugin list` shows installed plugins. Plugins hook into lifecycle triggers (pre-build, post-deploy, etc.). Dokku has 100+ community plugins for databases, SSL, monitoring, CI/CD.
- **Why add to Tengiz:** Existing "#191 Pluggable Event/Trigger Architecture" describes the extensibility model. Plugin Management Commands are the CLI interface to that system — without them, installing a plugin requires manual file manipulation. Implementation: `tengiz plugin install gh:user/repo` clones to `~/.tengiz/plugins/<name>/`, registers trigger hooks, validates the plugin manifest. Low-medium effort once the plugin architecture exists.
- **Detected:** 2026-07-25

## Deploy-Time Programmable Hooks (JavaScript Functions)
- **Source:** CapRover
- **Description:** Store JavaScript functions in the app definition (`preDeployFunction` in `IAppDef`) that run before each deploy and can programmatically modify the container/service creation parameters. The function receives the current app config and the Docker update object, and returns a modified update object — enabling dynamic configuration at deploy time. CapRover uses `require-from-string` to dynamically load and execute the JS function. Scripts can modify environment variables, volumes, network config, resource limits, or any part of the container spec based on runtime conditions (time of day, external API state, previous deploy status).
- **Why add to Tengiz:** Existing pre-deploy hooks (#9) run shell commands before deploy but cannot programmatically alter the deploy configuration itself. Deploy-time programmable hooks enable: dynamically injecting secrets from an internal vault API, adjusting resource limits based on time-of-day traffic patterns, conditionally enabling/disabling features per deploy, and complex migration logic requiring state inspection. `.tengiz.yaml`'da `pre_deploy_script:` bölümü ile küçük bir JS/Go fonksiyonu depolanabilir. Implementation: embed `goja` (pure Go JS runtime) to execute user-defined transform functions on the deploy config struct before it's passed to `runtime.Run()`. Function signature: `function(appConfig, deploySpec) { return modifiedSpec; }`. Low-medium effort, high power-user value. Different from pre-deploy hooks (shell commands) — these are programmatic config transformations with return values.
- **Detected:** 2026-07-25

## Custom Certbot/LetsEncrypt Command Rules per Domain Pattern
- **Source:** CapRover
- **Description:** Per-domain-pattern custom Certbot command rules (`CertbotCertCommandRule` with `domain` pattern and `command` template). Rules match domains via glob patterns (`*.example.com`, `*` for all) and define custom Certbot commands with template substitution (`${domainName}`, `${subdomain}`). Enables: alternative ACME providers (ZeroSSL, BuyPass, Google Trust Services), custom certificate profiles (ECC vs RSA key types, specific chain preferences), private CA integration for internal domains, and rate-limit-bypassing staging mode per domain group.
- **Why add to Tengiz:** Existing "#160 Alternative ACME Providers" and "#161 Staging Mode for SSL Testing" cover the general concept but lack per-domain-pattern command customization. Production environments often need: different certificate types per domain (wildcard `*.app.com` vs single `api.app.com`), private/internal CAs for `.internal` domains, staged rollout of new ACME providers. `.tengiz.yaml`'da `ssl.rules:` listesi: `{ domain: "*.example.com", provider: "zerossl", key_type: "ecc" }`. Implementation: pattern-matching middleware in the certificate provisioning pipeline, template rendering for provider-specific CLI commands. Low-medium effort, enterprise SSL flexibility.
- **Detected:** 2026-07-25

## Per-App Custom Error Pages Directory
- **Source:** CapRover
- **Description:** Per-app configurable custom error pages directory (`customErrorPagesDirectory` in `IServerBlockDetails`). Each app can have its own set of error pages (404.html, 502.html, 503.html, 504.html) served by the proxy when the app container is unreachable, cold-starting, or returning errors. Different from global error pages — this is per-app, so each app can brand its error experience. CapRover serves these via the nginx proxy block's `error_page` directive.
- **Why add to Tengiz:** Existing "#21 Error Pages (Özel Hata Sayfaları)" describes global proxy-level error pages for cold-start/container-down scenarios. Per-app error pages add brand-level customization: `app1` shows startup company branding on 502, `app2` shows enterprise support portal on 503. `.tengiz.yaml`'da `error_pages.dir: ./error-pages` veya `error_pages.404: ./custom-404.html`. Implementation: Tengiz proxy reads per-app error page paths from route config, serves them when backend is unavailable. Low effort (file serving middleware), useful for multi-tenant and white-label deployments.
- **Detected:** 2026-07-25

## Path-Based HTTP Basic Auth (Granular Access Control)
- **Source:** CapRover
- **Description:** HTTP basic authentication applied to specific URL paths within an app, not the entire app (`httpBasicAuthPath` in `IServerBlockDetails` alongside `httpBasicAuth`). Enables protecting admin panels (`/admin/*`), API endpoints (`/api/internal/*`), or staging previews while keeping the rest of the app publicly accessible. CapRover's nginx config template renders a conditional `auth_basic` directive per location block based on these fields.
- **Why add to Tengiz:** Existing "#32 HTTP Basic Auth (Staging Koruması)" applies basic auth to the entire app. Path-based basic auth is more granular: protect `/admin` routes behind a password while serving `/public` and `/api` without auth. Useful for: admin panels on production apps, internal API documentation, staged feature previews, and shared hosting scenarios. `.tengiz.yaml`'da `basic_auth.paths: [{ "/admin/*": { user: "admin", pass: "secret" } }]`. Implementation: URL path matching middleware in the proxy that checks Authorization header against configured credentials before forwarding. Low effort (header check + path matcher), important for production apps needing partial auth coverage.
- **Detected:** 2026-07-25
