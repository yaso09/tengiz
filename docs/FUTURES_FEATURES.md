...
## SCIM 2.0 User Provisioning (Automatic User Lifecycle from IdP)
- **Source:** Dokploy
- **Description:** SCIM 2.0 provisioning endpoints (`/scim/*`) via better-auth's SCIM plugin. Each organization gets a unique bearer token (`scimToken`) used by an identity provider (Okta, Azure AD, JumpCloud) to automatically create, update, deactivate, and group-provision users. Provisioned users are auto-verified, assigned the org's default role, and deactivation (`active: false`) maps to user banning. Token generation/listing/deletion managed per-org (`scim.generateToken`, `scim.listProviders`, `scim.deleteProvider`). `sso.ts` + `scim.ts` schemas; `lib/auth.ts` lines 447-460.
- **Why add to Tengiz:** Enterprise adoption requires automatic user lifecycle management. Manual account creation doesn't scale for orgs with 50+ users rotating in/out. SCIM lets Okta/Azure AD push user changes into Tengiz automatically — no per-user admin work. Complements the existing User Group RBAC (#196) and OIDC/SSO (#157): SSO authenticates, SCIM provisions, groups authorize. For a Go implementation, SCIM is a small set of standard endpoints (`/Users`, `/Groups`, `/ServiceProviderConfig`) with bearer-token auth over JSON — the schema (`scim_provider` with providerId + scimToken + organizationId) maps directly to `~/.tengiz/scim.json`. Medium effort, high enterprise value. Differentiates Tengiz from Coolify/Dokku's manual-invite model.
- **Detected:** 2026-08-18

## SSO Forward-Auth Gateway (App-Level SSO Protection)
- **Source:** Dokploy
- **Description:** Per-app forward-auth that gates an application's domains behind an SSO provider. `forward_auth_settings` deploys an `oauth2-proxy` service on a central auth domain (baseDomain + authDomain) wired to an OIDC provider. Domains with `forwardAuthEnabled` route through the proxy's `/oauth2/auth` middleware — unauthenticated users are 401→302 redirected to `/oauth2/sign_in`; authenticated requests pass `X-Auth-Request-User`, `X-Auth-Request-Email`, and `Authorization` headers to the app. Cookie secret HMAC-derived from the platform auth secret; email domain allowlist restricts access. `services/proprietary/forward-auth.ts`, `utils/traefik/forward-auth.ts`, `db/schema/forward-auth.ts`.
- **Why add to Tengiz:** Existing OIDC/OAuth SSO (#157) covers admin login to the platform, but deployed apps have no way to be gated behind an identity provider. Forward-auth turns "deploy an internal tool" into "deploy an internal tool that requires Google login" — a top use case for internal dashboards, admin panels, and staging environments. Tengiz's Go proxy is perfectly positioned: instead of a separate oauth2-proxy container, a forward-auth middleware can be embedded directly (`/oauth2/*` routes + cookie session check + `X-Auth-Request-*` header injection). `.tengiz.yaml`'da `auth.forward: { provider: google, email_domains: [company.com] }`. Pairs with HTTP Basic Auth (#32, username/password) — forward-auth is the SSO version. Medium-high effort (OIDC flow implementation), but unlocks a distinct enterprise capability no other Docker-based PaaS offers natively.
- **Detected:** 2026-08-18

## DNS Provider Management (Cloudflare / Route53 Zone & Record CRUD)
- **Source:** Dokploy
- **Description:** Org-scoped DNS provider entities (`dns_provider` table) with pluggable `DnsClient` implementations: Cloudflare (bearer API token, paginated zones/records, idempotent `upsertRecord` via PUT/POST) and AWS Route53 (access key + secret, `ListHostedZones`/`ChangeResourceRecordSets` UPSERT with trailing-dot normalization). CRUD operations: list zones, list/create/update/delete records (A/CNAME, name, content, ttl), `testConnection` (minimal list call), secret masking (`********` on read, preserved on update). `services/dns-provider.ts`, `services/cdn.ts`, `utils/dns/{cloudflare,route53,index,types}.ts`, `db/schema/dns-provider.ts`.
- **Why add to Tengiz:** Today users add a custom domain in Tengiz, then manually create the A/CNAME record at their DNS provider. DNS provider integration automates the whole flow: `tengiz domain add myapp.com --dns-provider cf` validates the zone, creates the DNS record, and (with ACME DNS-challenge, #161/#65) issues a wildcard cert — a genuinely serverless-feeling experience. It also enables the DNS validation + CDN-detection warnings (`services/cdn.ts` IP ranges for Cloudflare/Fastly/Bunny/Arvancloud) already sketched in #136. Go's `cloudflare-go` and `aws-sdk-go-v2` make this a well-trodden integration. `.tengiz.yaml`'da `dns_providers: [{name: cf, type: cloudflare, api_token: "..."}]`. Medium effort (two provider SDKs + record diff logic), high automation value — complements Managed Databases (#63) and auto-subdomains for a fully hands-off platform.
- **Detected:** 2026-08-18

## Auto-Generated Wildcard Subdomains (sslip.io / Free Subdomains)
- **Source:** Dokploy
- **Description:** `generateRandomDomain()` and `generateWildcardDomain()` auto-provision free subdomains without user-owned DNS: `{project}-{hash}-{ip-slug}.sslip.io` (project name truncated to 40 chars to respect the 63-char DNS label limit) or a configurable `*.sslip.io` wildcard for preview deployments. The domain is derived from the web-server IP or a specific remote server IP (127.0.0.1 fallback in dev), so it resolves without any DNS configuration — instant `https://myapp.hash.1.2.3.4.sslip.io` access. `services/domain.ts`, `templates/index.ts`, `services/preview-deployment.ts`.
- **Why add to Tengiz:** One of Vercel's most loved properties is the instant `*.vercel.app` URL — no DNS setup required. Tengiz currently requires users to either use `<app>.tengiz.local` (only works on the host) or configure a real domain. With sslip.io wildcards, any Tengiz deployment gets a publicly reachable URL the moment `tengiz deploy` completes: `https://{app}-{hash}.{server-ip}.sslip.io`. This dramatically lowers time-to-first-deploy and is the natural default for preview deployments (implemented) which currently need a manually configured wildcard base. Implementation is a pure string function (`fmt.Sprintf("%s-%s.%s.sslip.io", app, hash, ipSlug)`) + proxy host-wildcard matching. Low effort, very high UX value. Complements DNS Provider Management (real domains) with an instant zero-config tier.
- **Detected:** 2026-08-18

## Layered Environment Variable Inheritance (Project → Environment → App)
- **Source:** Dokploy
- **Description:** Three-level env layering: every project has its own encrypted `env`, every environment (staging/prod/dev within a project) has its own encrypted `env` and `isDefault` flag, and every app/compose/database resolves env by merging service env + project env + environment env with interpolation (`{{project.X}}`, `{{environment.X}}`, and self-references `{{X}}`). Unresolved references raise errors before deploy. `duplicateEnvironment` copies an environment's env into a new one. Env is encrypted at rest at every level (AES-256-GCM). `db/schema/environment.ts`, `db/schema/project.ts`, `utils/docker/utils.ts` (`prepareEnvironmentVariables`).
- **Why add to Tengiz:** Tengiz's `--env` flag handles per-env config files, but there's no inheritance or sharing — common secrets/URLs must be duplicated across every app and env, and a change requires touching N configs. Layered inheritance means: set `DATABASE_URL` once at project level, reference it in all apps via `{{project.database_url}}`, override per-environment. Existing Variable Resource (#53) and Secret Interpolation (#54) cover global values; this adds the hierarchical scoping layer. Complements Project-Based App Organization (#112, grouping) with a real config-resolution model. `.tengiz.yaml`'da `project.env:` + `environment.env:` sections merged in `config.LoadForEnvironment`. Medium effort (merge + interpolation order), eliminates a real operational pain for multi-app instances.
- **Detected:** 2026-08-18

## Multi-Tenant Organizations with Member Roles & Invitations
- **Source:** Dokploy
- **Description:** Full organization model: `organization` (name, slug, logo, metadata, defaultRole, ownerId) owns every resource; `member` links users to orgs with roles (owner/admin/member + custom) and fine-grained boolean permissions (`canCreateProjects`, `canCreateServices`, `canDelete*`, `canAccessToDocker`, `canAccessToAPI`, `canAccessToGitProviders`, `canAccessToSSHKeys`, `canAccessToTraefikFiles`) plus per-resource access lists (`accessedProjects/Environments/Services/GitProviders/Servers`). `invitation` records email-based invites with role, status, expiry, and inviter. All resource lookups verify `organizationId` against the session's active org (IDOR protection). `db/schema/account.ts`, `services/permission.ts`, `lib/access-control.ts`.
- **Why add to Tengiz:** A shared Tengiz server today is either single-admin or "everyone with CLI access has full control". For teams this is a gap: members need deploy access without Docker/settings access, and new teammates should join via invitation rather than shell access. This is the concrete data model behind the existing User Group RBAC (#196): groups define permission sets, organizations provide the tenant boundary + invitation flow. Even in a CLI-first design, `tengiz org create/invite/member add` is natural. The access-control statement engine (`(resource, action)` checks) maps cleanly to Go middleware. High effort (auth stack + permission checks across all commands), but foundational for any multi-developer deployment of Tengiz. Pairs with SCIM (auto-provision) and SSO (auth).
- **Detected:** 2026-08-18

## Container & Volume File Browser (List/Read/Write/Delete/Upload)
- **Source:** Dokploy
- **Description:** Browse and edit files inside running containers (`services/docker.ts`) and inside named Docker volumes (`services/docker-volume.ts`). Container ops use `docker exec ls -1Ap` (list), `docker exec cat` (read, base64-encoded, 512KB cap with truncation flag), write, delete, and `docker cp`-based upload (temp file + docker cp). Volume ops mount the volume read-only into a `busybox` sidecar at `/__volume`. Both validate container IDs and destination paths against strict regexes to block shell metacharacters; images without shell utilities get a friendly error. Volume listing + per-volume sizes come from `docker system df -v`.
- **Why add to Tengiz:** Debugging a running app often requires inspecting files: "is the `.env` actually correct?", "why is this log file growing?", "what's in the uploaded volume?". Today users must `docker exec` and know exact container names/paths. A `tengiz fs ls/read/write <app> <path>` and `tengiz volume browse <volume>` command family gives app-aware, validated file access without shell access — also a security improvement (no arbitrary command execution, just bounded file ops). Fits the exec-based runtime (`os/exec` on `docker exec`/`docker cp`) perfectly. Low-medium effort, high debugging value. Complements Container Entering (#140, interactive shell) and Volume File Browser pairs with Volume Backups (browse before snapshot).
- **Detected:** 2026-08-18

## Docker Volume Backups (tar Volume → S3 with In-Use Restore Guard)
- **Source:** Dokploy
- **Description:** Cron-scheduled backups of named Docker volumes (`volume_backup` table): mount the volume read-only into an `ubuntu` container, `tar cvf` to `/tmp`, then upload via rclone to the configured S3 destination under `<app>/<prefix>/<volume>-<timestamp>.tar`. Optional `turnOff` flag scales the service to 0 replicas during backup for consistency (preserving the replica count to restore). `flock`-based lock prevents concurrent backups; `keepLatestCount` retention prunes old archives. Restore (`utils/volume-backups/restore.ts`) downloads the tar, checks whether the volume exists and whether ANY container (including stopped, via `docker ps -a --filter volume=`) is using it — refusing restore while in use with a clear message — then recreates and extracts the volume.
- **Why add to Tengiz:** Tengiz's S3-Compatible Backup (#120) and Automated Database Backups (#127) cover database dumps, but there's no way to back up raw volume data (SQLite files, uploaded media, cache) — which is exactly what scale-to-zero apps with Persistent Storage (#110, implemented) lose on container teardown. A `tengiz backup volume <app> --volume mydata` command family with the in-use restore guard is a self-contained, high-value addition. The `docker run --rm -v <vol>:/data ubuntu tar czf` pattern matches the existing exec-based runtime; S3 upload can reuse the Destination entity (#186) already on the roadmap. Medium effort, closes a real data-loss gap for stateful Tengiz apps.
- **Detected:** 2026-08-18

## Full-Context Rollback Snapshots (Env/Mounts/Ports/Resources)
- **Source:** Dokploy
- **Description:** Beyond image-tag rollback, Dokploy's `rollbacks.ts` stores a `fullContext` JSON snapshot of the ENTIRE application at each deployment point: env vars (with project/env layering), all mounts (volume/bind/file), ports, networks, healthcheck, restart policy, resource limits, command args, and registry auth. Each rollback record has an incrementing `version`, an image tagged `<app>:v<version>`, and a dedicated `rollbackRegistryId`. Restore (`rollbackApplication`) reconstructs the Swarm service from the snapshot with force-update; deleting a rollback also `docker image rm`s the snapshot image to reclaim disk. `rollbackActive` flips image pulls to the rollback registry.
- **Why add to Tengiz:** Tengiz's Rollback (implemented) restores the previous image but not the config state at that point. If an app's env vars, resource limits, or domains changed between deploys, image-only rollback produces a config/image mismatch. Full-context snapshots make rollback deterministic: "restore exactly what was running at deploy v3" including its env and mounts. For Tengiz this means serializing `AppEntry` (already JSON-persisted) into `~/.tengiz/rollbacks/<app>/<version>.json` at each deploy, tagging the image `<app>-v<version>`, and having `tengiz rollback <app> --version <n>` reconstruct the full `AppEntry` before restarting. Medium effort — mostly reuse of existing store/deploy code, high reliability value. Distinct from Container Snapshot System (#80, `docker commit` state) — this is config-state rollback.
- **Detected:** 2026-08-18

## Preview Deployment Author Permission Gate (Block Non-Collaborators)
- **Source:** Dokploy
- **Description:** For GitHub-triggered preview deployments, `previewRequireCollaboratorPermissions` (default true) checks the PR author's collaborator permission level on the repository (`write`, `maintain`, or `admin`) before creating the preview. Blocked PRs get a structured security-comment notification on the PR ("Preview Deployment Blocked — Security Protection") explaining why, with dedup detection to avoid duplicate comments. The check runs via the GitHub API (`checkUserRepositoryPermissions`) before any preview resource is created. `services/github.ts` lines 200-348, `utils/providers/github.ts`, `services/preview-deployment.ts`.
- **Why add to Tengiz:** Tengiz's Preview Deployments (implemented) automatically spin up ephemeral environments for every PR — but if the webhook is public-facing, a malicious PR from an untrusted fork executes code on the Tengiz host (build + container run). That's a real supply-chain risk: preview deployments run arbitrary repo code. Gating preview creation on the PR author being a repo collaborator is a critical security control. For Tengiz: in `gitdeploy`/`preview` webhook handling, call the GitHub API with the stored provider token, check the author's `permission` field, and skip + comment if below `write`. Also naturally pairs with the Collaborator permission gate for the existing HMAC webhook validation (#40). Low effort (one API call), closes a genuine security hole in the most automated feature.
- **Detected:** 2026-08-18

## Static SPA Deployment (Publish Directory + Generated Nginx Config)
- **Source:** Dokploy
- **Description:** For apps with a `publishDirectory` (and optionally `isStaticSpa`), the static builder generates a `.dockerignore`, a production `nginx.conf` (with `try_files ... /index.html` SPA fallback when `isStaticSpa`), and a `Dockerfile` (`FROM nginx:alpine`, `COPY <publishDir> .`), then builds. For Nixpacks-built apps it goes further: builds with nixpacks, then `docker create` + `docker cp` extracts the publish directory output and hands it to the static builder — so any framework's build output becomes a fast, tiny nginx static serve. `utils/builders/static.ts`, `utils/builders/nixpacks.ts`.
- **Why add to Tengiz:** Tengiz's framework detection supports `static` but currently only as "serve the directory as-is" — no artifact packaging, no SPA fallback, and no way to run a frontend build then serve its output. A real Vercel-like static deploy needs: build step (npm build/vite build) → copy publishDirectory → serve via nginx with SPA `try_files`. This closes the gap between Tengiz's static detection and production-grade static hosting, and the nixpacks-export pattern means Next.js static export, Vite, SvelteKit, and Astro all get optimized nginx serving automatically. `.tengiz.yaml`'da `static.publish_directory: dist` + `static.spa: true`. The builder package already generates Dockerfiles — adding nginx templates is a natural extension. Low-medium effort, high value for the fastest-growing app category.
- **Detected:** 2026-08-18

## Inline File Mounts (Config File Injection without Repo Changes)
- **Source:** Dokploy
- **Description:** A `file` mount type alongside `bind` and `volume`: store file content in the DB, materialize it to `<APPLICATIONS_PATH>/<app>/files/<filePath>` on the host (base64-decoded, `mkdir -p` parent dirs) and bind-mount it into the container at `mountPath`. Creating/updating/deleting a file mount writes/removes the host file immediately; content changes take effect on next deploy. Polymorphic across all service types via `serviceType` (application, postgres, mysql, compose, etc.). `services/mount.ts`, `utils/docker/utils.ts` (`generateFileMounts`).
- **Why add to Tengiz:** Many apps need config files injected that don't belong in git: `nginx.conf`, `php.ini`, `application.yml`, `replicaset.conf`. Today users must either commit secrets to the repo, patch files at build (#118), or bind-mount a host path (requiring host filesystem access). Inline file mounts let `tengiz volume add-file <app> --path /etc/nginx/nginx.conf --content "..."` inject config declaratively from the Tengiz state store — no git, no host access. Complements Persistent Storage (implemented, volume mounts) and Patches (build-time) with a runtime config mechanism. Low-medium effort: store content in AppEntry, write file before container start, pass `--volume` flag. Small but high-value ergonomic win.
- **Detected:** 2026-08-18

## Per-Domain Path Routing & Rewriting (stripPath / internalPath)
- **Source:** Dokploy
- **Description:** Each domain record supports `path` (default `/`) producing a `Host(...) && PathPrefix(...)` proxy rule, plus `stripPath` (remove the public path prefix before forwarding) and `internalPath` (prepend an internal path after stripping) — so `/blog` public traffic can be rewritten to `/v2` internally. Validation enforces: `stripPath` requires a non-root path; `internalPath` must start with `/`. Each domain also has per-domain `port` (target backend port, decoupled from app default), `https` toggle, `certificateType`, `enabled` flag (instant apply/remove from proxy), `customEntrypoint`, and IDN/punycode conversion. `db/schema/domain.ts`, `utils/traefik/middleware.ts`, `utils/docker/domain.ts`.
- **Why add to Tengiz:** Tengiz's domain model is single host → single app with no path control. Real deployments need: serve one app at `myapp.com/blog` while another handles `/`, map `api.myapp.com/v1` to an internal path, or disable a domain without deleting it. Per-domain path routing turns the proxy from a hostname router into a proper router. The existing Gelişmiş Proxy Konfigürasyonu (#76) covers buffering/timeouts; this adds the routing surface. Implementation: extend `proxy` routing to match `Host + PathPrefix`, apply `StripPrefix`/`Rewrite` middleware (Go `httputil` has `ReverseProxy.Director` for this), and per-domain port in the routes map. Low-medium effort, unlocks real-world multi-app hosting patterns.
- **Detected:** 2026-08-18

## Docker Event Stream (Container Activity Feed)
- **Source:** Dokploy
