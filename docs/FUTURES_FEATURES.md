...
## Application Forward Authentication (SSO Login Gate for Deployed Apps)
- **Source:** Dokploy
- **Description:** Beyond admin/operator SSO, Dokploy protects deployed applications themselves behind SSO login gates using oauth2-proxy + Traefik forward-auth middleware. When enabled, accessing the app's URL redirects to an SSO login page (Google, GitHub, OIDC). Only authenticated users can access the app. No app code changes needed — the proxy layer handles auth entirely. Per-app toggle, configurable providers, session management with configurable TTL, and optional group-based access control. Dokploy's `/api/application/auth` endpoints (3 files) manage the full lifecycle: enable/disable, provider selection, session configuration. HTTP basic auth (#27) is simpler; forward auth is the enterprise-grade version with full SSO integration.
- **Why add to Tengiz:** Currently deployed Tengiz apps are fully public (no auth). For internal tools, staging environments, or enterprise apps, operators need to control who can access each app. Forward auth at the proxy layer means: (1) zero app code changes — any app gets SSO protection automatically, (2) consistent auth experience across all apps, (3) per-app access control (engineering team can access dev apps, only admins access prod). This is distinct from existing OIDC/SSO (#157), which covers platform admin authentication — this is about auth for deployed user apps. Implementation: an oauth2-proxy companion container managed by Tengiz, or a Go forward-auth middleware in the proxy. `.tengiz.yaml`'da `auth.forward.enabled: true`, `auth.forward.provider: google`, `auth.forward.allowed_domains: [example.com]`. Medium-high effort but transforms Tengiz from "deployment tool" to "enterprise platform."
- **Detected:** 2026-07-24

## FIFO Deployment Queue with Per-Server Concurrency Control
- **Source:** Dokploy
- **Description:** Dokploy implements a FIFO deployment queue per server (`services/queue.ts`): deployments are ordered sequentially per service, queued deployments can be cancelled, and concurrent build count is configurable per server (default: 1, OSS max: 2, Enterprise: unlimited). The queue prevents resource oversubscription and ensures fair ordering. Status tracking per queued deployment: queued → running → success/failed/cancelled. Log output preserved even for cancelled deployments. Cancel button available in UI for any queued deployment.
- **Why add to Tengiz:** Existing "Build Queue with Dedup (#124, CapRover)" covers build deduplication. Dokploy's approach adds: (1) per-server concurrency limit (don't overwhelm single-server Docker daemon with 10 concurrent builds), (2) explicit queue cancellation, (3) FIFO ordering per service (important for CI/CD push chains where commit order matters), (4) status transparency (queue position, estimated wait time). For Tengiz, this is especially important when combined with `--all` (#55 Parallel Bulk Operations) — 20 concurrent builds on a single Docker daemon is destructive. Implementation: per-app Go channel with configurable buffer + worker count, cancellation via context cancellation, status in deploy records. `.tengiz.yaml`'da `build.concurrency: 2` (server-level) and `build.concurrency_per_app: 1` (app-level). Medium effort, important for production stability.
- **Detected:** 2026-07-24

## Tailscale Networking Integration (Zero-Trust Private Network Exposure)
- **Source:** Dokploy
- **Description:** Dokploy integrates with Tailscale for private networking: apps can be exposed exclusively via the Tailscale network (no public ports needed). Dokploy's `utils/tailscale.ts` handles: Tailscale auth key management, subnet routing configuration, DNS settings for custom domains, and container exposure via Tailscale serve/funnel. This enables zero-trust app deployment where HTTPS is handled by Tailscale's cert provisioning, no open firewall ports needed beyond the Tailscale wireguard connection.
- **Why add to Tengiz:** Existing "Cloudflare Tunnel Support (#119, Dokploy)" covers Cloudflare's zero-trust approach. Tailscale is a different (arguably simpler) model: no CDN dependency, built on WireGuard, great for internal tools and teams. Tengiz could offer `tengiz tailscale enable --app myapp` which: (1) ensures Tailscale is installed on the host, (2) registers the app in Tailscale serve/funnel, (3) configures Tailscale DNS for the app's domain. No port forwarding, no firewall config, no public IP needed. Implementation: `tailscale` CLI wrapper (matches Tengiz's `os/exec` pattern) + `tags` file for app-to-tailscale-name mapping. `.tengiz.yaml`'da `network.tailscale.enabled: true`, `network.tailscale.funnel: false` (funnel = public via Tailscale edge, false = private only). Medium effort, growing demand as Tailscale adoption increases for zero-trust networking.
- **Detected:** 2026-07-24

## Environment Duplication (Clone Entire Environment with All Services)
- **Source:** Dokploy
- **Description:** Dokploy's multi-tenant hierarchy (Organization → Project → Environment → Service) supports duplicating an entire environment — cloning all services, configurations, env vars, domains, and cross-service connections from one environment to a new one. Useful for: creating staging from production with a known state, spinning up dev environments identical to production for testing, and creating hotfix environments from a snapshot of production. The clone preserves service definitions, env var values (optionally redacting secrets), network topology, and resource limits. Users optionally select which services to include.
- **Why add to Tengiz:** Existing "App Cloning (#123, Dokku)" clones a single app. Environment duplication clones an entire multi-service deployment — all apps, their env vars, domains, resource limits, and cross-service connections. For Tengiz, this enables: `tengiz environment clone prod staging` creates an exact copy of production for testing, or `tengiz environment snapshot prod hotfix-20260724` for pre-hotfix backup. Implementation: traverse all AppEntries in the source environment, deep-copy with `-clone` suffix, update port allocations, connect to same networks/volumes. More than bulk app cloning — preserves the relational links between services (e.g., accessory connections to their consumers). Medium-high effort, high value for team deployments.
- **Detected:** 2026-07-24

## Whitelabeling / Custom Platform Branding
- **Source:** Dokploy
...
## One-Command Full Platform Setup (tengiz setup)
- **Source:** Kamal
- **Description:** `kamal setup` bootstraps servers (Docker install, network creation, run directory), pushes environment variables, deploys all accessory services (databases, caches), and deploys the app in a single atomic command. Uses the deploy lock to prevent concurrent operations. Combines what would be 5+ separate commands into one: `kamal server bootstrap` + `kamal accessory boot` + `kamal deploy`. Performs health verification at each stage and rolls back on failure.
- **Why add to Tengiz:** Currently Tengiz users must manually: install Docker, create networks, configure env vars, start accessories, and deploy. A one-command `tengiz setup` dramatically reduces time-to-first-deploy. Fits Tengiz's CLI-first model perfectly — one command, zero config, full stack running. Complements Server Bootstrap (#40) and Redeploy (#33) — `setup` is the full first-time bootstrap, `redeploy` is the optimized subsequent path. `.tengiz.yaml` contains all the info needed for setup (env, accessories, app config).
- **Detected:** 2026-07-24

## Clean Build Mode (Git Clone to Temp Dir for Reproducible Builds)
- **Source:** Kamal
- **Description:** When building from a git repo and no custom build context is set, Kamal clones the repository to a temporary directory instead of building from the working directory. This produces clean, reproducible builds that ignore uncommitted changes, local config files, build artifacts, and `node_modules`. The resulting Docker image reflects exactly what is committed, not what is on the developer's machine. Falls back to working directory build when git is unavailable or `--local` is specified.
- **Why add to Tengiz:** A common developer mistake is deploying with uncommitted changes, dirty `node_modules`, or local config files. Clean builds guarantee the deployed image matches the repo state — critical for CI/CD and team environments. Implementation: in `deploy.go`, if the source is a git repo, `git clone --depth 1` to a temp dir (under `~/.tengiz/builds/` for caching), then build from there. `.tengiz.yaml`'da `build.clean: true` default, `--local` flag to override. Complements Git-Based Image Version Tagging (#112) — clean builds + SHA tagging = fully traceable, reproducible deployments.
- **Detected:** 2026-07-24

## Tengiz Auto-Injected Platform Environment Variables
- **Source:** Kamal
- **Description:** Kamal automatically injects `KAMAL_CONTAINER_NAME`, `KAMAL_VERSION`, `KAMAL_HOST`, `KAMAL_DESTINATION` into every app container. These provide the running app with context about where and how it's deployed — the container name, the deployment version (git SHA), which host it runs on, and which environment destination. No user configuration needed. Apps can use these vars for: telemetry tagging, environment detection, logging context, status page display.
- **Why add to Tengiz:** Currently Tengiz only injects `TENGIZ_COMMIT_SHA` (if configured). Apps have no way to know: which environment they're running in (dev/staging/prod), their own container name, or the Tengiz version managing them. Auto-injected vars enable app-level environment detection without config. Implementation: inject `TENGIZ_APP_NAME`, `TENGIZ_ENV`, `TENGIZ_DEPLOY_ID`, `TENGIZ_CONTAINER_NAME`, `TENGIZ_VERSION` as `-e` flags in `runtime.Create()`. Low effort (string passthrough), high developer experience value. Complements Git Commit Hash Auto-Injection (#89) and Multi-Environment Support (#11).
- **Detected:** 2026-07-24

## SSH Agent Forwarding During Docker Build
- **Source:** Kamal
- **Description:** Kamal supports forwarding the local SSH agent socket to the Docker build process via `builder.ssh: ssh-agent`. This enables `RUN --mount=type=ssh` in Dockerfiles, allowing secure access to private Git repositories, npm registries, and package managers during the build without embedding SSH keys in the image or using `--build-arg` (which leaks into image history). The SSH connection is ephemeral — only available during the build step, not in the final container.
- **Why add to Tengiz:** Many projects have private npm packages (`@myorg/design-system`), private Go modules, or internal Python packages that require SSH access during `go mod download`, `npm install`, or `pip install`. Without SSH forwarding, users must either: make packages public (security risk), embed SSH keys (image security risk), or use `--build-arg` with tokens (visible in image history). Implementation: `docker build --ssh default=$SSH_AUTH_SOCK` in `builder.go`, gated by `.tengiz.yaml`'da `build.ssh: true`. Low effort (Docker CLI flag passthrough), high value for private dependency workflows. Complements Build-Time Secrets (#55) for a complete build-time security model.
- **Detected:** 2026-07-24

## Build Provenance and SBOM Attestations
- **Source:** Kamal
- **Description:** Kamal supports Docker Buildx attestation generation: provenance attestations (recording how the image was built, with what source, builder version, build config) and SBOM (Software Bill of Materials — listing all packages and dependencies in the image). Attestations are attached to the image manifest and are verifiable via `docker buildx imagetools inspect`. Configurable via `builder.provenance` and `builder.sbom` with mode options: `disabled`, `true` (default buildx behavior), `max` (full detail). SBOM formats: SPDX, CycloneDX.
- **Why add to Tengiz:** Supply chain security is increasingly mandatory for enterprise deployments. Provenance attestations prove where and when an image was built. SBOM provides a dependency inventory for vulnerability scanning. For Tengiz: `tengiz sbom <app>` extracts the SBOM from the latest image. Implementation: `--attest type=provenance,mode=max --attest type=sbom` flags in `docker buildx build`, configurable via `.tengiz.yaml`'da `build.attestations: { provenance: true, sbom: true }`. Medium effort (buildx-specific flags + output parsing), high enterprise adoption value. Complements Build Pipeline (#129) and Image Digest Pinning (#75).
- **Detected:** 2026-07-24

## Accessory Exec (One-Off Commands in Sidecar Containers)
- **Source:** Kamal
- **Description:** Beyond running commands in the main app container (`kamal app exec`), Kamal supports `kamal accessory exec <name> <cmd>` for executing commands inside accessory containers (Postgres, Redis, etc.). Supports interactive (`-i`), new container, existing container, and raw output modes. Enables: `tengiz accessory exec db "pg_dump -U myapp mydb > /tmp/backup.sql"`, `tengiz accessory exec redis "redis-cli INFO"`, or `tengiz accessory exec cache -i sh` for interactive debugging.
- **Why add to Tengiz:** Accessories (PostgreSQL, Redis, Meilisearch) are managed by Tengiz but users frequently need to: run database operations, check cache memory usage, perform manual backups, or debug connectivity issues. Currently users must `docker exec` with the Tengiz container name prefix. A `tengiz accessory exec` command provides app-aware, named access without knowing Docker container naming conventions. Implementation: `runtime.Exec()` wrapper around `docker exec`, or `docker run --rm` with the accessory's image. `.tengiz.yaml`'da `accessories[].exec_enabled: true` toggle. Low effort, completes the Accessory Services (#45) feature family.
- **Detected:** 2026-07-24

## Per-Role Deep Configuration Specialization (Cmd, Env, Labels, Docker Options, Proxy, Logging)
- **Source:** Kamal
- **Description:** Kamal roles (`web`, `workers`, `jobs`) each get fully independent configuration: different `cmd` (start command), different `env` (environment variables, clear and secret), different `labels` (Docker labels), different Docker options (restart policy, capabilities, volumes), different proxy configuration (port, SSL, health check settings), different logging configuration (log driver, max-size, max-file). Roles inherit base config but can override any field. `.tengiz.yaml`'da `roles:` altında her rol ayrı ayrı yapılandırılır.
- **Why add to Tengiz:** Single-cmd-per-app model is too limited. A web app might need: `web` process (port 3000, health check, proxy exposed), `worker` process (no ports, no proxy, background job processing), `cron` process (no ports, no proxy, scheduled job runner). Each needs different env (workers don't need `PORT`), different Docker options (worker needs more memory), and different proxy settings (only web should be exposed). Extends the existing Role-Based Server Groups (#35) with actual configuration depth. Implementation: `AppEntry.Roles` map with per-role configs, `runtime.Run()` iterates roles and starts one container per role with role-specific args. Medium effort, unlocks polyglot/polyprocess deployments.
- **Detected:** 2026-07-24

## Proxy Debug Mode (Toggleable Verbose Request/Response Logging)
- **Source:** Kamal
- **Description:** Kamal-proxy supports a `debug` mode that enables detailed request/response logging: every HTTP request URL, method, headers, status code, upstream target, response duration, and error details printed to stdout. Toggleable at proxy boot via config (`proxy.debug: true`) or at runtime. Does not require redeploy — proxy reconfiguration is hot-reloadable. Debug mode is resource-intensive (logs every request) so it's disabled by default and intended for troubleshooting.
- **Why add to Tengiz:** Debugging proxy routing issues (wrong app serving, headers not forwarded, WebSocket not working) is currently a guessing game — Tengiz's proxy is a black box. Proxy debug mode outputs structured logs for every request showing: which route matched, where traffic was forwarded, response status and timing, cold-start latency, header modifications. Implementation: toggleable `slog.LevelDebug` logging in the proxy's `ServeHTTP` handler, configurable via `.tengiz.yaml`'da `proxy.debug: true` or CLI flag `tengiz proxy --debug`. Low effort (log statements added to existing proxy handler), high debugging value. Complements Trace/Debug Mode (#114) which covers platform-wide debug logging.
- **Detected:** 2026-07-24

## Runtime Duration Reporting (Deploy/Setup Command Timing)
- **Source:** Kamal
- **Description:** After every `kamal deploy`, `kamal setup`, and `kamal rollback`, Kamal prints the total runtime in seconds: `"Finished all deployment steps (45.2s)"`. This provides immediate feedback on deploy performance, helps users detect regressions (deploy went from 30s to 120s? something changed), and gives a baseline for optimization. The timer spans the entire operation from start to finish, including all sub-steps (build, push, pull, boot, health check, proxy update, prune).
- **Why add to Tengiz:** Users currently have no sense of deploy duration — no feedback on whether a 3-minute deploy is normal or concerning. Runtime reporting provides: immediate perf feedback, baseline for detecting regressions, cold-start timing visibility, and data for performance optimization. Implementation: `time.Now()` at operation start, `time.Since()` at end, printed via `slog.Info(...)`. Logged to audit trail for historical analysis. Low effort (two lines of Go per command), high UX value. Complements Event Logging & Audit Trail (#7) with duration metrics.
- **Detected:** 2026-07-24
