...
| — | **Granular Env Var Flags** ⬜ | Yüksek | Düşük | Mükemmel | Per-var runtime/buildtime/preview/is_required flags. Critical for build-time secrets, preview isolation, and env var lifecycle management. |
| — | **Config Snapshot + Deployment Diff (`tengiz plan`)** ⬜ | Yüksek | Düşük | Mükemmel | SHA256 config hash per deploy. `tengiz plan` shows what changed. Like `terraform plan` for deployments. |
| — | **Crash-Loop Detection & Max Restart Limit** ⬜ | Yüksek | Düşük | Mükemmel | Track restart count per app in sliding window. Stop auto-restart after threshold. Prevents infinite crash loop thrashing. |
| — | **SPA Mode** ⬜ | Yüksek | Düşük | Mükemmel | 404 → index.html fallback for client-side routing. Essential for React Router/Vue Router/Angular apps. |
...
| — | **Multi-Scope Shared Env Vars** ⬜ | Yüksek | Düşük-Orta | Mükemmel | Team/Project/Environment/Server scoped vars with `{{type.KEY}}` interpolation. Eliminates env var duplication across apps. |
| — | **Auto-Generated Webhook Secrets** ⬜ | Orta | Düşük | Mükemmel | Auto-generate unique HMAC secrets for GitHub/GitLab/Bitbucket/Gitea on app create. Display once, then mask. |
| — | **Database Health Check Trait** ⬜ | Orta | Düşük | Mükemmel | DB-specific probes (pg_isready, redis-cli PING) separate from HTTP app health checks. Essential for managed DB reliability. |
| — | **Configurable Stop Grace Period** ⬜ | Orta | Düşük | Mükemmel | Per-app Docker stop timeout. Fast apps stop quickly, slow apps finish cleanup. SIGTERM→SIGKILL delay. |
| — | **Config Hash Change Detection** ⬜ | Orta | Düşük | Mükemmel | Compute config hash pre-deploy. Skip rebuild if only config changed. Env-only changes skip build step. |
...
| — | **Service FQDN/URL Auto-Injection** ⬜ | Orta | Düşük | Mükemmel | Auto-inject `SERVICE_FQDN_<NAME>` / `SERVICE_URL_<NAME>` for compose services. Zero-config inter-service discovery. |
| — | **Preview URL Template** ⬜ | Orta | Düşük | Mükemmel | Customizable preview FQDN: `{{pr_id}}.{{domain}}`, `{{random}}.{{domain}}`. Team-defined preview URL conventions. |
| — | **DB Public Port with Timeout** ⬜ | Orta | Düşük | Mükemmel | Temporary public DB access on configurable port with auto-close timeout. Secure external DB tool connections. |
| — | **Per-Image Service Extra Fields** ⬜ | Orta | Düşük-Orta | Mükemmel | Image-specific config fields for one-click templates. WordPress: site title/admin email. MinIO: root user/bucket. |
| — | **Disable Build Cache Per-App** ⬜ | Orta | Düşük | Mükemmel | `build.no_cache: true` toggle for deterministic builds. Fresh dependency validation. Troubleshoot cache issues. |
| — | **Docker Cleanup Execution History** ⬜ | Orta | Düşük | Mükemmel | Per-run cleanup record: what deleted, space reclaimed, duration. `tengiz cleanup --history` for audit. |
| — | **Buildpack Control Var Prefixes** ⬜ | Düşük-Orta | Düşük | Mükemmel | Filter NIXPACKS_*/RAILPACK_* vars from user env display. Prevents build config polluting app env. |
| — | **GPU Config Per App (Detailed)** ⬜ | Orta | Orta | Mükemmel | Per-app GPU count, device IDs, driver, options. Beyond simple `--gpus all`. AI/ML workload precision. |
| — | **14-Event Notification Toggles** ⬜ | Orta | Düşük | Mükemmel | Per-event-type notification control: deploy_success/failure, backup, disk_usage, server_reachable, etc. |
...
## Granular Environment Variable Flags (Runtime/Buildtime/Preview Separation)
- **Source:** Coolify
- **Description:** Each env var gets typed flags: `is_runtime` (available in running container), `is_buildtime` (passed as Docker `--build-arg`), `is_preview` (scoped to preview deployments only), `is_required` (must have value before deploy), `is_shown_once` (one-time display, e.g. auto-generated passwords), `is_literal` / `is_multiline` (controls Docker escape handling), and `order` (custom sort position). Coolify's `EnvironmentVariable` model has all these fields, enabling fine-grained lifecycle control per variable. Build-time `NPM_TOKEN` is not available at runtime; production-only `DATABASE_URL` is not leaked to preview environments.
- **Why add to Tengiz:** Today all env vars are treated identically — passed at runtime only. This breaks: build-time secrets (NPM_TOKEN not available during `docker build`), preview isolation (DATABASE_URL leaked to PR environments), and variable order (no control over Docker run argument sequence). Implementation: extend `AppConfig.Env` from `map[string]string` to a struct slice with flag fields. Runtime vs buildtime flag maps to `--build-arg` vs `-e` in Docker run. Preview flag controlled via existing preview deployment pipeline. Backward-compatible: plain `KEY: value` syntax defaults to runtime-only. Low-medium effort, high impact for both security and build correctness. Complements Build Arguments from Env (#14) by making the distinction explicit per-variable rather than bulk.
- **Detected:** 2026-07-29

## Multi-Scope Shared Environment Variables (Team/Project/Environment/Server Cascade)
- **Source:** Coolify
- **Description:** Shared environment variables organized in a 4-level cascade: `team` (visible to all apps in team), `project` (visible to all apps in project), `environment` (staging/production shared vars), `server` (host-wide vars). Apps reference shared vars via `{{type.KEY}}` syntax (e.g. `{{team.DATABASE_URL}}`, `{{environment.API_KEY}}`). Resolution order: app-level overrides environment-level, which overrides project-level, which overrides team-level. Coolify's `SharedEnvironmentVariable` model stores the `type` field with these four scopes. The `realValue()` resolver on `EnvironmentVariable` replaces `{{type.KEY}}` references at deploy time.
- **Why add to Tengiz:** Eliminates env var duplication across apps in the same team/project/environment. The `[[secret.NAME]]` interpolation already exists for secrets (#54) — this extends the same pattern to non-secret shared vars with a scope cascade. Deployment: "staging" env has one `DATABASE_URL` defined at environment scope, all staging apps inherit it. New apps automatically get correct staging config with zero configuration. Implementation: add `SharedVariable` store (JSON file `~/.tengiz/vars-{env}.json` with type scope). The existing `ResolveInterpolations` in `secrets` package (which handles `[[secret.NAME]]`) can be extended to also resolve `{{scope.KEY}}` references. Medium effort, high operational value for multi-app installations.
- **Detected:** 2026-07-29

## Config Snapshot + Deployment Diff (`tengiz plan`)
- **Source:** Coolify
- **Description:** Before each deployment, Coolify computes a SHA256 hash of the entire application configuration (FQDN, git repo, branch, build pack, env vars, resource limits, etc.) and stores it in `ApplicationConfigurationSnapshot`. The `ConfigurationDiffer` service produces a structured diff between the current config and the last successful deploy's snapshot. The diff shows exactly what changed: FQDN differences, env var additions/removals/modifications, build pack changes, resource limit adjustments. Coolify's `pendingDeploymentConfigurationDiff()` method returns this as a human-readable comparison. This is like `terraform plan` for deployments.
- **Why add to Tengiz:** Currently every `tengiz deploy` silently applies whatever config exists — users don't know what changed until the deploy fails or produces unexpected results. A `tengiz plan` command that shows: "DATABASE_URL will change (masked), NPM_TOKEN will be added, memory limit changed from 512M to 1G" before actually deploying. Implementation: compute a config hash before deploy, store it on the deployment record. `tengiz plan` compares current config against last successful deploy's snapshot. The diff is JSON-formatted with added/removed/changed sections, displayed as a table in CLI. Low effort (SHA256 of serialized AppConfig + structured diff), high production safety value. Complements Deploy Lock (#15) and Change Approval Workflow for a governance-friendly deploy pipeline.
- **Detected:** 2026-07-29

## Crash-Loop Detection with Max Restart Limit
- **Source:** Coolify
- **Description:** Coolify tracks `restart_count`, `max_restart_count`, `last_restart_at`, and `last_restart_type` per app. When an app exceeds `max_restart_count` (default 10) within a time window, it's marked as stopped and not automatically restarted. The `stoppedAfterRestartLimit()` method detects this state. This prevents infinite crash loops where a buggy container restarts every second, consuming CPU and filling logs. Coolify records the restart reason (e.g. `crash`) in `last_restart_type` for debugging.
- **Why add to Tengiz:** The existing health check + auto-restart (#4) can create infinite restart loops if the app has a startup bug. Each restart is fast (a few seconds) so it can cycle hundreds of times unnoticed, thrashing the Docker daemon and filling logs. Crash-loop detection: tracks restart count per app in a sliding window (N restarts in M minutes). After threshold, stop the container and mark the app as `crashed` instead of `running`. The app stays stopped until user intervention (`tengiz start myapp`). A crash event is emitted to the notification system. Implementation: augment `health.Restarter` with a restart counter stored in `AppEntry.State`. Threshold and window configurable in `.tengiz.yaml` (`crash_detection.max_restarts: 10`, `crash_detection.window: 5m`). Low effort, essential production reliability feature.
- **Detected:** 2026-07-29

## SPA Mode (Single Page Application Client-Side Routing)
- **Source:** Coolify
- **Description:** Coolify's `ApplicationSetting.is_spa` flag enables SPA mode: when an HTTP request matches no static file, it serves `index.html` instead of returning 404. This enables client-side routing (React Router, Vue Router, SvelteKit SPA mode, Angular routing) where the JS router handles path rendering. Single toggles: `is_spa: true` in app settings.
- **Why add to Tengiz:** Most modern frontend frameworks use client-side routing. Without SPA mode, refreshing `/dashboard` or navigating directly to `/settings` returns a 404 error from the proxy because no `dashboard.html` exists — the path should be handled by `index.html` with client-side routing. Implementation: proxy middleware that checks if the app has SPA mode enabled. If the requested path is not a static file (no extension or no file on disk), rewrite the path to `/index.html` before forwarding to the container. For statically-built frameworks, serve `index.html` directly from the built output. `.tengiz.yaml`'da `spa: true` toggle. Low effort (path-rewrite middleware in proxy), critical for modern SPA frameworks. Complements Static Framework Detection — Tengiz already detects Vite/Next.js, SPA mode completes the deployment story.
- **Detected:** 2026-07-29

## Auto-Generated Webhook Secrets (4 Git Providers)
- **Source:** Coolify
- **Description:** When an application is created, Coolify auto-generates unique webhook secrets for each of the 4 supported git providers (GitHub, GitLab, Bitbucket, Gitea) via the `manual_webhook_secret_*` fields (`manual_webhook_secret_github`, `manual_webhook_secret_gitlab`, `manual_webhook_secret_bitbucket`, `manual_webhook_secret_gitea`). Secrets are generated during the model's `creating` boot event using secure random generators. Auto-generated secrets are shown once at creation time and then masked. Users can manually regenerate them later.
- **Why add to Tengiz:** Existing "#40 HMAC-Signed Webhook Payloads" covers verification but not secret provisioning. Manually generating and configuring webhook secrets is error-prone and often skipped (users use weak or default secrets). Auto-generation ensures every app has cryptographically strong, unique secrets for each provider with zero user effort. Implementation: during `tengiz init` or first deploy, generate 4 secrets via `crypto/rand` and store them in `AppEntry`. Display once in CLI output. Webhook handler selects the correct secret based on `X-GitHub-Event` vs `X-Gitlab-Event` vs `X-Hub-Signature` origin detection. Low effort (Go `crypto/rand` + HMAC storage), closes a security gap in the webhook pipeline.
- **Detected:** 2026-07-29

## Database Health Check Trait (DB-Specific Health Probes)
- **Source:** Coolify
- **Description:** Coolify has a reusable `HasDatabaseHealthCheck` trait used by all database models (Postgres, MySQL, MongoDB, Redis, etc.). Each database type has a type-appropriate health check: PostgreSQL uses `pg_isready` or `SELECT 1`, Redis uses `redis-cli PING`, MongoDB uses `mongosh --eval "db.adminCommand('ping')"`, MySQL uses `mysqladmin ping`. Health checks verify that the database engine is running AND accepting connections (not just that the port is open). Results affect the database's `status` attribute. Separate from application health checks which probe HTTP endpoints.
- **Why add to Tengiz:** Database health must be verified differently from application health. `pg_isready` checks postgres readiness (not just port open), `redis-cli PING` verifies Redis is responding (not just TCP connect). A generic HTTP health check against a database container's port will always succeed even if the database engine is down (Docker exposes the port as long as the process is alive). Tengiz's existing health check system (#4) is HTTP-based — it probes `GET /health` on the app container. Database containers don't serve HTTP. Implementation: extend the health check system with a `HealthCheckType` enum: `http` (existing), `tcp` (port dial), `command` (`docker exec`). Database accessories use `command` type with `pg_isready` / `redis-cli PING` / `mysqladmin ping`. `.tengiz.yaml`'da `healthcheck.type: command`, `healthcheck.command: pg_isready -U postgres`. Low-medium effort, essential for managed database reliability.
- **Detected:** 2026-07-29

## Configurable Stop Grace Period (Per-App Docker Stop Timeout)
- **Source:** Coolify
- **Description:** Coolify's `ApplicationSetting.stop_grace_period` defines how long Docker waits between sending SIGTERM and SIGKILL during container shutdown. The `stopGracePeriodSeconds()` method returns the value in seconds with min/max validation (default 10s, min 1s, max 120s in dev mode). A separate `deploymentStopGracePeriodSeconds()` applies during zero-downtime deployment rollouts. Different apps need different stop grace periods: a Go HTTP server shuts down cleanly in 5s, a Rails app with active jobs needs 30s+ to finish processing.
- **Why add to Tengiz:** Hardcoded Docker stop timeout causes either: unnecessary slowness (waiting 30s for a Go app that stops in 1s) or truncated graceful shutdowns (SIGKILL before Rails finishes active requests). Per-app configurable `stop_grace_period` lets fast apps stop quickly (faster deploy cycles for zero-downtime) and slow apps complete cleanup (no interrupted database migrations or lost jobs). Implementation: `docker stop --time <N>` flag in `runtime.Stop()`. The value from `.tengiz.yaml` (`container.stop_grace_period: 30`) is passed through. During zero-downtime deploy, the old container uses this value for draining. Low effort (passing an integer CLI flag), high reliability impact for stateful and worker apps.
- **Detected:** 2026-07-29

## Config Hash Change Detection (Skip Rebuild on Unchanged Config)
- **Source:** Coolify
- **Description:** Coolify's `Application.isConfigurationChanged()` method computes a hash of the application's configuration (FQDN, git repo, branch, build pack, env vars, etc.) and compares it against the last deploy's `config_hash`. If nothing changed, the deploy can skip unnecessary rebuilds. The `legacyConfigurationHash()` computes the hash, and `markDeploymentConfigurationApplied()` updates it after a successful deploy. This is also called for all database models (Postgres, Redis, MySQL, etc.).
- **Why add to Tengiz:** Users often run `tengiz deploy` when only env vars changed, or run `tengiz rebuild` when the source hasn't changed. Without change detection, every deploy triggers a full rebuild even when only configuration changed. Config hash detection enables: env-var-only changes skip build (just restart container with new env), repeated `tengiz deploy` on unchanged repo is a no-op (no build, no restart), and the hash itself is a deploy identifier for rollback and audit. Implementation: compute SHA256 of `AppEntry.Config` (env, limits, domains, etc.) before deploy. Compare with stored hash in `AppEntry`. If only env vars changed, use `docker run` with existing image. If nothing changed, skip entirely. Low-medium effort, significant build time savings for config-only deploys.
- **Detected:** 2026-07-29

## Preview URL Template Customization
- **Source:** Coolify
- **Description:** Coolify's `ApplicationPreview` model has two FQDN generation methods: `generate_preview_fqdn()` (for regular apps) and `generate_preview_fqdn_compose()` (for Docker Compose services). The `Application` stores a `preview_url_template` field (default `{{pr_id}}.{{domain}}`). Templates support placeholders: `{{pr_id}}` (Pull Request number), `{{domain}}` (app's primary domain), `{{random}}` (random string for uniqueness). Users can customize: `pr-{{pr_id}}-{{random}}.{{domain}}` or `{{pr_id}}.staging.{{domain}}`.
- **Why add to Tengiz:** Preview deployment FQDNs are currently generated with a fixed scheme. Some teams need: numeric-only preview URLs for scripts (`42.tengiz.local`), environment-qualified previews (`pr-42.staging.tengiz.local`), or randomized previews for security-by-obscurity (`a8x3.tengiz.local`). A customizable template gives teams control over their preview URL convention. Implementation: add `preview_url_template` field to `AppConfig`. Template processing in `preview.Create()` using Go's `text/template` with `pr_id`, `domain`, `random` functions. Default preserves current behavior. Low effort, high usability value for teams with established URL conventions.
- **Detected:** 2026-07-29

## Database Public Port with Timeout (Temporary External Access)
- **Source:** Coolify
- **Description:** Coolify database models (Postgres, Redis, MySQL, MongoDB, etc.) share fields `is_public`, `public_port`, and `public_port_timeout`. When enabled, the database is exposed on the host's public IP on a configurable port (e.g., `5433`). The `public_port_timeout` automatically closes the port after a configurable duration (e.g., 60 minutes). This provides temporary external access for: running database migrations from an external CI/CD runner, connecting with desktop database tools (TablePlus, DataGrip), debugging from a developer's laptop, or ad-hoc data exports/imports. After the timeout, `is_public` is automatically set to `false` and the port is closed.
- **Why add to Tengiz:** Managed databases behind Tengiz's proxy are only accessible from within the Docker network. For legitimate external access (migrations, admin tools), users currently must SSH tunnel or reconfigure Docker networking. A temporary public port with automatic timeout provides secure, auditable external access without permanently exposing the database. Implementation: `tengiz db expose <app> --port 5433 --timeout 30m` runs a `docker run -p` or updates container port mapping, sets an `time.AfterFunc` timer to close it. Safety: require `--confirm` flag, log all expose events, only allow from configured IP ranges. Medium effort, high operational value for database management workflows.
- **Detected:** 2026-07-29

## Service FQDN/URL Auto-Injection for Compose Services
- **Source:** Coolify
- **Description:** Coolify automatically injects `SERVICE_FQDN_<NAME>` and `SERVICE_URL_<NAME>` environment variables for every service in a Docker Compose stack. Each service in a compose file gets a predictable environment variable containing its fully qualified domain name and URL within the Tengiz network. For example, a service named `api` gets `SERVICE_FQDN_API=api.appname.tengiz.local` and `SERVICE_URL_API=https://api.appname.tengiz.local`. This lets services discover each other without hardcoded URLs or manual configuration. The `boot saving` event on Application cleans up stale `SERVICE_*` vars when the build pack changes.
- **Why add to Tengiz:** Multi-service apps (web + api + db) need to know each other's addresses. Currently each service must be manually configured with the other services' hostnames/URLs. Auto-injection makes compose-based deployments zero-config for inter-service communication. Implementation: after starting all containers in a compose/accessory group, Tengiz enumerates them and injects `SERVICE_FQDN_<NAME>` / `SERVICE_URL_<NAME>` as env vars on each container. Names are sanitized (uppercase, underscore). Complements Magic Environment Variables (#96) — those handle service-to-app linking; this handles service-to-service linking. Low-medium effort, high value for compose deployments.
- **Detected:** 2026-07-29

## Per-Image Service Extra Fields (One-Click Template Customization)
- **Source:** Coolify
- **Description:** When deploying one-click service templates, Coolify renders image-specific extra configuration fields beyond the generic Docker Compose template. Each supported image (MinIO, WordPress, Plausible, RabbitMQ, Ghost, Directus, GitLab, Supabase, Kong, etc.) defines its own set of extra fields: admin username/password, custom ports, domain names, feature toggles, license keys, database names. Coolify's `Service.extraFields()` method returns these per-image fields with validation rules and labels. The fields are model-specific — WordPress asks for site title and admin email, MinIO asks for root user and bucket name, RabbitMQ asks for default user and vhost.
- **Why add to Tengiz:** Existing "#64/#104 One-Click Service Templates" covers deploying compose-based services but treats every image identically — it just runs the Compose file with env var overrides. Per-image extra fields make templates truly usable: users deploying WordPress get prompted for `WORDPRESS_SITE_TITLE`, `WORDPRESS_ADMIN_EMAIL`, `WORDPRESS_ADMIN_PASSWORD` with validation and sensible defaults. Implementation: a `TemplateField` struct (name, type (string/password/number/boolean/select), label, description, default, required, validation regex). Each service template includes a `fields: []` section in its YAML definition. CLI: `tengiz service create wordpress` shows an interactive prompt (or `--field title=MySite` for headless). Medium effort, transforms raw templates into truly usable one-click deployments.
- **Detected:** 2026-07-29

## Disable Build Cache Per-App (Deterministic Builds)
- **Source:** Coolify
- **Description:** Coolify's `ApplicationSetting.disable_build_cache` per-app toggle for Docker build caching. When enabled, Docker's `--no-cache` flag is passed to `docker build`, forcing every instruction to execute fresh. Useful for: deterministic CI/CD builds where cached layers could mask bugs, builds that pull dynamic content (git dependencies, package registries), and troubleshooting build issues where cache invalidation is suspected.
- **Why add to Tengiz:** Build caching is enabled by default for speed, but sometimes causes problems: old cached layers produce stale output, corrupted cache layers cause mysterious build failures, or builds need to validate against fresh dependency trees. A per-app `build.no_cache: true` toggle in `.tengiz.yaml` gives users control. Implementation: pass `--no-cache` to `docker build` in `builder.go` when the flag is set. Extends the existing app settings pattern. Low effort (boolean flag passthrough), high value for CI/CD reliability.
- **Detected:** 2026-07-29

## Docker Cleanup Execution History (Track Reclaimed Space)
- **Source:** Coolify
- **Description:** Coolify tracks every Docker cleanup execution in `DockerCleanupExecution`: what was deleted (images, containers, volumes, networks, build cache), how much disk space was reclaimed, and when it happened. The `DockerCleanupJob` records each run. `ServerSetting` controls: `docker_cleanup_frequency`, `docker_cleanup_threshold`, `delete_unused_volumes`, `delete_unused_networks`. Historical data enables trend analysis ("how much space do we reclaim weekly?") and audit ("did a cleanup accidentally delete something important?").
- **Why add to Tengiz:** Existing "#6 Docker Housekeeping" and "#56 Granular Docker Prune Operations" cover the cleanup mechanics but not the audit/history layer. Without execution history, users don't know what was cleaned or how much space was reclaimed — making it impossible to tune cleanup policies or investigate accidental deletions. Implementation: each cleanup run records to `~/.tengiz/cleanup_history.json` as timestamped entries with: categories cleaned, space reclaimed per category, total reclaimed, duration. `tengiz cleanup --history` or `tengiz cleanup --history-last` shows the last N entries. Low effort (append JSON log), high operational visibility value.
- **Detected:** 2026-07-29

## Buildpack Control Variable Prefixes (NIXPACKS_*, RAILPACK_* Filtering)
- **Source:** Coolify
- **Description:** Coolify's `EnvironmentVariable` model has a constant `BUILDPACK_CONTROL_VARIABLE_PREFIXES` (containing `NIXPACKS_`, `RAILPACK_`). The `scopeWithoutBuildpackControlVariables()` and `isBuildpackControlKey()` methods separate buildpack-control env vars from user env vars. The computed attribute `isBuildpackControl` returns true for vars like `NIXPACKS_METADATA`, `NIXPACKS_BUILD_CMD`, `RAILPACK_DATABASE_URL`. This separation prevents user-facing env var listings from being polluted with internal buildpack configuration.
- **Why add to Tengiz:** Nixpacks and other build systems inject their own env vars to control behavior (e.g., `NIXPACKS_INSTALL_CMD`, `NIXPACKS_BUILD_CMD`, `NIXPACKS_START_CMD`). These are not user secrets — they're build system configuration — but they appear alongside user env vars in `tengiz config show` and `tengiz env list`, confusing users. Prefix-based classification: filter them out from normal env var display, group them under a `Build Configuration` section, and automatically handle conflicts (user-set `NIXPACKS_*` overrides build system defaults). Implementation: maintain a `BuildControlPrefixes` list. In env var display and export commands, treat matching vars as build config (not user env). Low effort, improves UX for any app using Nixpacks or similar builders.
- **Detected:** 2026-07-29

## GPU Configuration Per App (NVIDIA/CUDA Passthrough Details)
- **Source:** Coolify
- **Description:** Coolify's `ApplicationSetting` has detailed GPU fields: `is_gpu_enabled` (toggle), `gpu_driver` (e.g., `nvidia`), `gpu_count` (number of GPUs, `all` for all available), `gpu_device_ids` (specific GPU UUIDs, comma-separated), `gpu_options` (extra `--gpus` flags like `capabilities=gpu,compute,utility`). Dokploy's GPU setup pipeline additionally checks: NVIDIA driver version via `nvidia-smi`, CUDA version, `nvidia-container-runtime` installation, Docker daemon configuration with nvidia as default runtime, and Swarm GPU registration.
- **Why add to Tengiz:** Existing "#133 GPU Passthrough" is a basic concept. Coolify's model is production-ready with per-app device selection and driver options. AI/ML workloads (Ollama, vLLM, LocalAI, Stable Diffusion) need fine-grained GPU control: assign specific GPUs to specific apps (GPU 0 for web app, GPU 1 for batch inference), control GPU count for multi-GPU inference, and specify compute capabilities. Implementation: `.tengiz.yaml`'da `gpu.enabled: true`, `gpu.count: 1`, `gpu.devices: ["GPU-<uuid>"]`, `gpu.options: "capabilities=gpu"`. `runtime.Run()` passes `--gpus '"device=GPU-<uuid>"'` flag. A `tengiz gpu status` command reports detected GPUs, drivers, and availability (following Dokploy's diagnostic pattern). Medium effort, critical for AI deployment workflows.
- **Detected:** 2026-07-29

## 14-Event Notification Toggles (Granular Per-Event Alert Control)
- **Source:** Coolify
- **Description:** Coolify's `WebhookNotificationSettings` model has 14 individually toggleable notification events: `deployment_success`, `deployment_failure`, `status_change` (container up/down), `backup_success`, `backup_failure`, `scheduled_task_success`, `scheduled_task_failure`, `docker_cleanup_success`, `docker_cleanup_failure`, `server_disk_usage` (threshold exceeded), `server_reachable`, `server_unreachable`, `server_patch` (OS patch available), `traefik_outdated` (proxy outdated). Each event can be independently enabled/disabled, allowing users to receive only the notifications they care about (e.g., enable deployment_failure but disable deployment_success to reduce noise).
- **Why add to Tengiz:** The current notification system (#5) has binary enable/disable. Production teams need surgical control: enable "deploy failure" and "disk usage" alerts but disable "container stopped" and "scheduled task success" to avoid noise. Without 14-event granularity, users either get spammed by all events or miss critical alerts by disabling everything. Implementation: extend `notify.Manager` with an event-type filter per channel. Events are typed (string enum). Each channel's config includes `events: [deploy_failure, disk_usage]` or `events: all`. `.tengiz.yaml`'da `notifications.discord.events: [deploy_failure, disk_usage]`. Low-medium effort (event type enum + filter middleware), high operational value for production teams.
- **Detected:** 2026-07-29
