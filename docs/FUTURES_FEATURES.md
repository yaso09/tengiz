...
## Volume Data Cloning (Clone App + Copy Volume Contents)
- **Source:** Coolify
- **Description:** When cloning an application, database, or service to another destination/server, the user can optionally copy the contents of all persistent volumes. `VolumeCloneJob` (`app/Jobs/VolumeCloneJob.php`) handles both cases: same-server clone via a helper Alpine container running `cp -a` + `chown -R 1000:1000`, and cross-server clone by tarring the source volume into a temp dir, copying it with instant SCP, then creating the target volume and extracting. The source resource is stopped during the copy and restarted afterwards to guarantee consistency. Cloning also replicates tags, env vars, scheduled backups, file storages, and scheduled tasks (with regenerated volume names/FQDNs).
- **Why add to Tengiz:** Existing "App Cloning" (#123) copies configuration only. Real environment duplication (staging from production, branch previews) requires copying actual data — databases, uploaded files, generated caches. Volume data clone makes `tengiz apps:clone` production-useful: stop source → copy volume data → start source. For Tengiz, `docker run --rm -v <srcvol>:/from -v <dstvol>:/to alpine cp -a` covers same-host; the tar+SCP path extends to the future SSH remote deployment (#105). `.tengiz.yaml`'da `clone.volumes: true`. Complements App Cloning (#123) and Container Snapshot (#80) — snapshot captures one container's state, clone duplicates an entire resource with its data.
- **Detected:** 2026-07-31

## Scheduled Volume & Directory Backups (Tar Archives to S3)
- **Source:** Coolify
- **Description:** `ScheduledVolumeBackup` (`app/Models/ScheduledVolumeBackup.php`) schedules backups of both named Docker volumes and host directories (`LocalPersistentVolume` OR `LocalFileVolume` with `is_directory=true`). `VolumeBackupJob` (`app/Jobs/VolumeBackupJob.php`) runs an Alpine helper container with a read-only mount, tars the source (`tar -czf - -C /volume .`), saves to local storage, and optionally uploads to S3 via the MinIO `mc` client. Key capabilities: `stop_during_backup` (detects containers using the volume via `docker inspect` mount scan, stops them, archives, and restarts them with a state-file recovery mechanism), independent retention policies for local vs S3 (max count / max age in days / max cumulative size in GB), `disable_local_backup` (delete local archive after successful S3 upload), per-backup timeout, and a recovery job (`VolumeBackupRecoveryJob`) that restarts stopped containers (20 retries/60s backoff) and cleans up partial S3 uploads. Full execution history with status, size, and download route.
- **Why add to Tengiz:** Existing "Automated Database Backups" (#127) covers `pg_dump`/`mysqldump` — application-specific and requires a DB. Volume backups are universal: any stateful app's data (SQLite files, uploaded content, generated caches) lives in volumes, and scale-to-zero makes volume data the most vulnerable asset. `tengiz backup create volume --app myapp` with `stop_during_backup` guarantees a consistent archive even for non-database state. The S3 upload + retention + container recovery pipeline maps directly onto Tengiz's S3-Compatible Backup (#120) and Reusable Destinations (#186). `.tengiz.yaml`'da `backup.volumes: [{volume: data, schedule: "0 3 * * *", save_s3: true, retention: {count: 7}}]`.
- **Detected:** 2026-07-31

## Managed File Mounts (Encrypted Content File Volumes)
- **Source:** Coolify
- **Description:** `LocalFileVolume` (`app/Models/LocalFileVolume.php`) models a file or directory bind-mount whose content the platform can manage: content is stored encrypted in the DB and pushed to the server on save (`ServerStorageSaveJob`), read back for in-browser editing (with `[binary file]` / `[file too large]` placeholders for >5MB files), and written with configurable `chmod`/`chown`. Three modes: managed file (content editable), directory (auto-`mkdir -p`), and `is_host_file` bind-only (platform never touches the source file). `is_based_on_git` refreshes content from the preserved git repo after each deploy; `is_preview_suffix_enabled` appends `-pr-N` to the path for preview deployments; read-only mounts are auto-detected from compose `:ro` syntax.
- **Why add to Tengiz:** Tengiz's "Persistent Storage" (#7) only supports named volume/host-path mounts — no way to manage config files (`.env`, `nginx.conf`, `robots.txt`) that live on the host. Managed file mounts give CLI users the equivalent of `tengiz file set myapp /etc/app/config.yml --content "..."` with chmod/chown, plus git-based refresh so files are re-synced after each deploy. The preview-suffix behavior keeps PR environments isolated. This bridges Patches (#118, build-time) with runtime file management. Implementation: `tengiz file` command family + encrypted JSON storage in `~/.tengiz/files/` + a pre-run write step in `runtime.Run()`. Low-medium effort, high value for config-heavy apps.
- **Detected:** 2026-07-31

## Cloud Provider Server Provisioning (Hetzner/Vultr/DigitalOcean)
- **Source:** Coolify
- **Description:** End-to-end cloud server provisioning from within the platform using stored provider API tokens. Team-scoped `CloudProviderToken` models (encrypted at rest, live-validated against each provider's API before save, undeletable while servers reference them). Multi-step server creation wizards (`app/Livewire/Server/New/ByHetzner.php`, `ByVultr.php`, `ByDigitalOcean.php`) pick location/region, OS image, server type, existing provider SSH keys, firewalls, networks, IPv4/IPv6, backups, monitoring, and a cloud-init script. `HetznerService`/`VultrService`/`DigitalOceanService` are rate-limit-aware API clients; `ServerCloudProviderStatusCheckJob` polls the provider to sync server state and backfill assigned public IPs. Full REST API catalog (locations, images, plans, ssh-keys, firewalls, networks) and a server-provisioning endpoint.
- **Why add to Tengiz:** Server Bootstrap (#31) installs Docker on an existing host; this feature provisions the host itself. `tengiz server create --provider hetzner --type cx32 --image ubuntu-24.04` spins up a server, installs the SSH key, and automatically runs the Tengiz bootstrap — zero-to-deployable-host in one command. For a self-hosted Vercel alternative, "provision a server" removes the last manual cloud-console step. `~/.tengiz/providers.json` stores tokens encrypted (reusing the `encrypt` package), and the cloud status sync feeds the SSH Remote Deployment (#105) feature. Cloud-init script support (below) makes the provisioned servers pre-configured for Tengiz. High value for the single-node → multi-node growth path.
- **Detected:** 2026-07-31

## Reusable Cloud-Init Server Scripts
- **Source:** Coolify
- **Description:** Team-scoped, encrypted cloud-init scripts (`app/Models/CloudInitScript.php`, `app/Livewire/Security/CloudInitScripts.php`) that can be applied when provisioning new cloud servers. `ValidCloudInitYaml` rule accepts three forms: bash scripts starting with `#!`, `#cloud-config` YAML, and plain YAML. Script content is injected as the provider's `user_data` field on Hetzner/Vultr/DigitalOcean server creation. Admin-only policy, UUID-based management, and full CRUD via `tengiz cloud-init`-style commands.
- **Why add to Tengiz:** Provisioned servers need repeatable initialization (packages, firewall, storage mounts, Docker config). Cloud-init scripts make server setup declarative and reproducible — pair with Server Security Hardening (#182, UFW/Fail2ban) so `tengiz server create` provisions a hardened, Tengiz-ready host automatically. `tengiz cloud-init create --name hardened-ubuntu --script ./init.sh`, referenced by `tengiz server create --cloud-init hardened-ubuntu`. Complements Server Bootstrap (#31) and Cloud Provider Provisioning (above). Low effort (store scripts encrypted, pass as `user_data` when provisioning).
- **Detected:** 2026-07-31

## Multi-Team Organization with Role Hierarchy & Invitations
- **Source:** Coolify
- **Description:** Users belong to multiple teams via a role pivot with a three-level hierarchy: member < admin < owner (`app/Enums/Role.php`). Members are read-only for most resources; admins/owners manage members, invitations, keys, and tokens. Team invitations (`app/Models/TeamInvitation.php`) work via email or shareable link, with auto-account creation and a magic auth link (`/auth/link`) for new users, role-targeted invites, expiry (auto-delete + unverified-account cleanup), and privilege-escalation blocking (members can't invite admins, admins can't invite owners). Role changes/removals revoke the user's API tokens for that team. Teams own projects, servers, private keys, shared env vars, S3 storages, tags, and git sources. Per-team server limits enforce subscription tiers.
- **Why add to Tengiz:** "User Group Resource (RBAC)" (#196) covers groups inside a single org; Coolify's model is separate multi-tenant teams each owning their own resources — the same as a shared-host PaaS serving several teams/companies. `tengiz team create`, `tengiz team invite --email x@y.com --role admin`, `tengiz team members` give a multi-tenant deployment story on one server. Magic-link onboarding removes password friction. Token revocation on role change is an important security guarantee to replicate. Complements OIDC/SSO (#157) for auth and the alert/audit system for shared-host accountability. Medium effort (state in `~/.tengiz/teams.json` + CLI auth checks).
- **Detected:** 2026-07-31

## Scoped Shared Environment Variables (Team/Project/Environment/Server)
- **Source:** Coolify
- **Description:** `SharedEnvironmentVariable` (`app/Models/SharedEnvironmentVariable.php`) defines env vars at four scopes — `team`, `project`, `environment`, `server` — resolved into app env vars via `{{scope.KEY}}` placeholder interpolation (e.g. `{{server.SERVER_UUID}}`, `{{project.API_URL}}`). Values are encrypted, support multiline/literal/shown-once semantics, and can be bulk-edited in "normal"/"dev" views. Server scope has reserved keys (`COOLIFY_SERVER_UUID`, `COOLIFY_SERVER_NAME`) that can't be deleted. Resolution happens at read/deploy time, so a change to one shared var propagates to every app referencing it. A `serverOverride` variant supports build-server-specific values.
- **Why add to Tengiz:** Existing "Variable Resource (Global Interpolation)" (#53) covers a single global namespace. Scoped variables match how real teams organize: team-wide credentials, per-project API endpoints, per-environment database URLs, per-server machine identifiers. `.tengiz.yaml`'da `shared_vars.team: {SERVER_URL: "https://tengiz.local"}` ve `shared_vars.project: {DB_URL: "postgres://..."}`; apps reference them with `{{scope.KEY}}`. Combined with per-app env (#19) and secrets (#54) this completes the interpolation stack. Medium effort: an `interpolate` pass in the deploy pipeline resolving `{{scope.KEY}}` from the appropriate store files.
- **Detected:** 2026-07-31

## Preview-Scoped Env Vars & Configurable Preview URL Templates
- **Source:** Coolify
- **Description:** Two coupled preview-deployment capabilities. (1) **Preview env var mirroring**: when a non-preview env var is created on an Application, an `is_preview=true` copy is auto-created with the same value (overridable per-preview), so PR deployments can receive different values (e.g. staging DB vs production DB) without breaking base config. `is_preview` env vars are injected only into PR preview containers. (2) **Configurable preview URL template**: `ApplicationPreview` builds preview FQDNs from a Go-style template supporting `{{pr_id}}`, `{{random}}` (short random slug), and `{{domain}}` placeholders — e.g. `https://{{pr_id}}-myapp.{{domain}}` or `https://myapp-{{random}}.preview.example.com`.
- **Why add to Tengiz:** Tengiz's Preview Deployments are implemented (#2) but env vars are shared between main and preview deployments, and the preview URL is a fixed convention. PR environments almost always need different env values (feature flags, test credentials, scaled-down resources); without isolation, a misconfigured var can corrupt a preview DB or leak production secrets into ephemeral environments. `tengiz config set myapp DATABASE_URL --preview "postgres://test:...@localhost:5432/ci"` adds a preview-scoped override. URL templates let operators keep PR domains behind their own wildcard (`*.preview.example.com`) instead of the default `pr-N.app.tengiz.local`. `.tengiz.yaml`'da `preview.env_overrides:` ve `preview.url_template: "{{pr_id}}.{{domain}}"`. Low effort — both are small additions to the existing preview pipeline.
- **Detected:** 2026-07-31

## Push-Based Monitoring Agent with Heartbeat Liveness (Sentinel-style)
- **Source:** Coolify
- **Description:** Sentinel is a sidecar monitoring container (`coollabsio/sentinel`) deployed on each managed server with the host Docker socket mounted and `--pid host`, exposing a metrics API on `:8888`. It pushes container snapshots to the control plane every ~60s (`/api/v1/sentinel/push`), authenticated by an AES-encrypted per-server token. The platform exposes live CPU/memory charts (`app/Livewire/Server/Charts.php`, 5s polling, LTTB downsampling for series >1000 points) and per-container metrics via `/api/container/{uuid}/{type}/history`. Crucially, the Sentinel heartbeat (`sentinel_updated_at`) acts as proof-of-life: `ServerManagerJob` skips SSH connection checks when Sentinel is live, avoiding per-minute SSH round-trips. `CheckAndStartSentinelJob` self-heals the agent (restarts if stale/outdated version).
- **Why add to Tengiz:** Existing monitoring features (#26 Prometheus, #72 System Stats, #151 NetData) are poll-based and co-located with Tengiz itself. A push-based agent changes the architecture for the future multi-server model (#105/#194): remote hosts can't be polled via local `docker stats`, but an agent pushing metrics + heartbeat gives centralized, SSH-free observability. Even single-server, the heartbeat proves "server is alive" without SSH. For Tengiz's scale-to-zero, per-container metrics enable smarter idle decisions (scale down based on real traffic, not just idle timer). Implementation: `tengiz agent` subcommand or companion container pushing JSON metrics to the proxy at `/__tengiz/monitor`, stored in `~/.tengiz/stats/`. Reuses the Go `runtime` exec pattern to start the agent container.
- **Detected:** 2026-07-31

## Server OS Patch & Update Checking
- **Source:** Coolify
- **Description:** `ServerPatchCheckJob` (`app/Jobs/ServerPatchCheckJob.php`, weekly on Sunday midnight) runs an OS-update check on each server: detects the distribution from `/etc/os-release`, maps distros to package managers (apt, dnf, zypper, pacman, apk), runs the appropriate update-list command, and parses results into structured package lists (total update count, per-package new/current version, repo, architecture). The team is notified only when updates are available. `app/Actions/Server/CheckUpdates.php` contains the distro-aware logic.
- **Why add to Tengiz:** A self-hosted platform is responsible for host security, and unpatched servers are the #1 vulnerability vector. Tengiz operators currently have no visibility into pending OS updates without SSH-ing in. `tengiz server patches` lists available updates; a scheduled check (reusing the cron infrastructure from #42) notifies via the existing notification system when patches are pending. For a Go binary, the check is pure `os/exec` (`apt list --upgradable`, `dnf check-update`, etc.) — no daemon needed. `.tengiz.yaml`'da `server.patch_check: {schedule: "0 0 * * 0", notify: true}`. Complements Server Security Hardening (#182) and Server Monitoring (#73). Low-medium effort, high security value.
- **Detected:** 2026-07-31

## Configuration Diff & Change Impact Analysis (Build vs Redeploy)
- **Source:** Coolify
- **Description:** `ConfigurationDiffer`/`ConfigurationDiff` (`app/Services/DeploymentConfiguration/`) diff the previous deployment's configuration snapshot against the current config before each deploy. Each config item carries an `impact` tag (`build` vs `redeploy`), a `sensitive` flag (values compared as HMAC-SHA256 hashes and displayed redacted as `••••••••`), and a diff mode. The diff produces typed changes (`added`/`removed`/`changed`) with line-level diffing for multi-line values (compose files, env blocks), ignores auto-introduced defaults, and classifies the result: any `build`-impact change → full rebuild required; only `redeploy`-impact changes → lightweight container restart. `ConfigurationDiff::requiresBuild()` vs `requiresRedeploy()` drives the deploy pipeline decision.
- **Why add to Tengiz:** This directly optimizes the most common Tengiz workflow: changing an env var shouldn't trigger a full Docker build, only a container redeploy. Today a config change's rebuild-vs-redeploy decision isn't explicit. With diff analysis, `tengiz deploy` skips `docker build` when only runtime config changed — dramatically faster iteration. It also powers a `tengiz config diff` command showing exactly what changed since the last deploy (with secrets redacted), and feeds "App Report" (#10) and audit logging (#7). `~/.tengiz/deployments/` already stores per-deploy records; adding a config hash/snapshot + differ is incremental. Distinguish from Config Display (#48, shows resolved config) — this shows WHAT CHANGED and its impact.
- **Detected:** 2026-07-31

## Hardened Backup Upload (Chunked Upload + Content Validation)
- **Source:** Coolify
- **Description:** `UploadController.php` (`/upload/backup/{databaseUuid}`) accepts database backup files for restore using chunked uploads (up to 10 GiB, via pion/laravel-chunk-upload). `DatabaseBackupFileValidator.php` enforces an extension allowlist (`sql, gz, zip, tar, tar.gz, tgz, dump, bak, bson, archive, bz2, xz, dmp`), rejects dangerous extensions (`.sh`, `.php`, `.exe`), validates magic bytes against the declared file type, and scans SQL content for PostgreSQL `COPY ... PROGRAM` command-injection patterns. Upload is per-resource authorized; downloads of backups are admin/owner-only.
- **Why add to Tengiz:** "Database Backup Import/Upload" (#184) records the feature at a high level but not its security posture. Restore is one of the most dangerous operations in a PaaS — a malicious or malformed SQL file can execute host commands via `COPY ... PROGRAM`, and a disguised executable (`.sql.gz` actually being a script) can own the container. Chunked uploads matter for multi-GB production dumps. For the future web dashboard (#150) and `tengiz backup upload <app> <file>` CLI, implementing validation (magic-byte sniffing with Go's `net/http` DetectContentType, SQL command-injection regex scan) is a low-effort but critical hardening step. This complements Database Backup Import (#184) and Protected Service Deletion (#185).
- **Detected:** 2026-07-31

## Dockerfile HEALTHCHECK Auto-Detection
- **Source:** Coolify
- **Description:** `Application::parseHealthcheckFromDockerfile()` parses the `HEALTHCHECK` instruction from a project's Dockerfile and maps it into the application's health check settings (path/port/interval/timeout/retries/start_period) when no explicit health check is configured. This lets apps declare their own health semantics in their Dockerfile and have the platform honor them automatically — no manual configuration.
- **Why add to Tengiz:** Tengiz's Container Health Check (#4, implemented) requires manual config or defaults. Many Dockerfiles already declare a `HEALTHCHECK`; parsing it means `tengiz deploy` immediately gets correct health semantics (e.g. a Node app's `HEALTHCHECK CMD curl -f http://localhost:3000/health`) with zero config. Implementation is a small parser in `internal/config/` or `internal/health/` reading the Dockerfile during framework detection, feeding the existing health checker and zero-downtime deploy (#1) checks. Low effort, removes a manual step and improves cold-start reliability for apps that declare their own checks.
- **Detected:** 2026-07-31

## Threshold-Triggered Docker Cleanup with Execution History & Rollback Protection
- **Source:** Coolify
- **Description:** `DockerCleanupJob` (`app/Jobs/DockerCleanupJob.php`) runs on a per-server cron schedule but only actually cleans when root disk usage is above a configurable threshold (default 10%) — a "cleanup on demand" model that avoids wasted work. Each run creates a `DockerCleanupExecution` record (status, message, structured `cleanup_log` per command+output, finished_at) with a downloadable full log and real-time status via WebSocket. Cleanup protects rollbacks: per-app `docker_images_to_keep` (default 2) retains the N newest regular images, the currently-running image is never deleted, PR/build images are pruned, and non-app images are cleaned via a shell one-liner excluding app repos and current infra images. Notifications fire on success/failure; a staleness indicator detects a dead cleanup scheduler.
- **Why add to Tengiz:** "Docker Housekeeping" (#6) and "Granular Docker Prune" (#56) cover the prune commands but not the operational layer: threshold-triggering (don't clean an under-utilized disk), execution history with downloadable logs (audit + debugging), and explicit rollback protection (never delete the running image; keep N versions per app). On a scale-to-zero server with frequent deploys, these guarantees matter — a blind `docker system prune` can delete images needed for Rollback (#8). `.tengiz.yaml`'da `cleanup: {threshold: 10, schedule: "*/10 * * * *", keep_images: 2}`. Reuses the cron infrastructure (#42) and the notification system (#5). Medium effort, high operational safety value.
- **Detected:** 2026-07-31

## Advanced Env Var Semantics (Literal/Multiline/Shown-Once/Required/Runtime-Buildtime)
- **Source:** Coolify
- **Description:** `EnvironmentVariable` (`app/Models/EnvironmentVariable.php`) supports per-variable flags beyond key/value: `is_literal` (no shell escaping, wrapped in single quotes), `is_multiline` (multi-line values wrapped correctly), `is_shown_once` (locked secret — value visible only once at creation, thereafter only deletable/re-addable), `is_required` (enforcement of non-empty required vars), `is_runtime` / `is_buildtime` (control whether a var is injected into the running container, the build step, or both), and `is_preview`. Values are encrypted at rest, key names normalized/validated, JSON object/array values are passed unescaped, and buildpack-control keys (`NIXPACKS_*`, `RAILPACK_*`) are filtered from the normal env list.
- **Why add to Tengiz:** Tengiz env management (#19, implemented) is flat `KEY: value`. Adding flags improves correctness and security: `is_runtime: false` + `is_buildtime: true` cleanly implements "Build Arguments from Env" (#14) per-variable (no duplicate config); `is_shown_once` gives the "generate a secret, show it once" UX that prevents secrets lingering in shell history/state; `is_required` catches a missing `DATABASE_URL` before deploy instead of at runtime. `.tengiz.yaml`'da `env: {DATABASE_URL: {value: "...", buildtime: false, shown_once: true, required: true}}`. The literal/multiline handling fixes the common JSON/quote-escaping bugs. Low effort (extend the existing AppConfig.Env struct + a few validation rules), high correctness value.
- **Detected:** 2026-07-31

## Forward-Auth SSO Proxy (OAuth2-Proxy Per-Domain App Protection)
- **Source:** Dokploy
- **Description:** Deploys an `oauth2-proxy` container (`quay.io/oauth2-proxy/oauth2-proxy:v7.6.0`) on each server that acts as a centralized forward-authentication proxy. An OIDC provider (Google, GitHub, Keycloak, etc.) is configured once per organization, then individual app domains are toggled on/off for SSO protection. OIDC config (clientId, clientSecret, scopes, skipDiscovery) is stored per-provider; the proxy generates an HMAC-derived cookie secret, sets cookie domains/whitelist domains from the base domain, supports email-domain allowlists, and a per-domain `forwardAuthEnabled` flag. A `/oauth2/callback` redirect URL is exposed for the provider. `setup/forward-auth-setup.ts` + `services/proprietary/forward-auth.ts` implement the full lifecycle (settings, deploy, status check with 4s timeout, enable/disable per domain).
- **Why add to Tengiz:** Existing OIDC/SSO (#157) protects Tengiz's *admin* interface. Forward-auth is different: it protects the *deployed apps* themselves — put a staging or internal app behind SSO login without writing any auth code. This generalizes the HTTP Basic Auth (#32) feature into a real identity layer: users log in with their Google/GitHub account, and Tengiz's proxy checks the session cookie before routing. For a Go proxy this is a middleware that validates a cookie signed by Tengiz's own OIDC flow (no separate oauth2-proxy container needed — Tengiz can be the IdP client directly). `.tengiz.yaml`'da `auth.forward: {issuer, client_id, client_secret, email_domains: [example.com]}` ve per-domain `app.domain.forward_auth: true`. High value: it turns any Tengiz app into an SSO-protected internal tool.
- **Detected:** 2026-07-31

## SCIM User Provisioning (Enterprise IdP Sync)
- **Source:** Dokploy
- **Description:** Integrates the SCIM 2.0 protocol (`@better-auth/scim`) into the platform's auth stack so enterprise identity providers (Okta, Entra ID/AD, OneLogin) can automatically create, update, disable, and remove user accounts and group memberships. `scim_provider` table stores a unique provider ID and a SCIM token per organization; the token is used by the IdP to call the platform's `/scim` endpoints. Token generation is gated behind an enterprise license flag (`enableEnterpriseFeatures`). Better-auth handles the standard SCIM Users and Groups resources, filters, and pagination.
- **Why add to Tengiz:** Tengiz currently has no user/team management at all. When teams adopt SSO (#157), the remaining chore is manually creating/deactivating accounts for every hire/departure. SCIM automates the lifecycle: a terminated employee is auto-disabled in Tengiz within minutes via the IdP. This is the standard enterprise provisioning contract (Okta/Entra both speak SCIM 2.0). Implementation in Go is modest: an HTTP handler at `/__tengiz/scim/v2/` (Users/Groups endpoints), a `scim` package, and `~/.tengiz/scim-tokens.json` storing org-scoped tokens. Complements OIDC SSO (#157) and RBAC (#196) for a complete enterprise auth story.
- **Detected:** 2026-07-31

## Docker Network Import & Drift Sync
- **Source:** Dokploy
- **Description:** `network.ts` (359 lines) treats Docker networks as first-class platform resources with a management layer on top of plain `docker network`. Key capability: `findNetworksToSync()` compares the platform's registered networks against the actual Docker daemon and returns two lists — `importable` (networks existing in Docker but not yet registered, filtered out from reserved names like `bridge`/`host`/`ingress`) and `missing` (registered networks that no longer exist in Docker). `importDockerNetworks()` bulk-imports selected daemon networks with full IPAM config (subnet, gateway, ipRange), driver, internal/attachable flags, and IPv4/IPv6 toggles. Networks support both `bridge` and `overlay` drivers (Swarm), and apps can attach to multiple networks via `networkIds` plus a `detachDokployNetwork` option.
- **Why add to Tengiz:** Existing Docker Network CRUD (#102) creates/manages networks from scratch, but real deployments often have pre-existing networks (created by another tool, a VPN, or a legacy stack). Import-and-sync is the missing operational workflow: `tengiz network sync` lists what exists in Docker vs what Tengiz knows, `tengiz network import <name>` adopts an existing network with its full IPAM settings. Drift detection (`missing`) catches networks deleted behind Tengiz's back — a classic single-server failure mode. For Tengiz's exec-based runtime this is a thin `docker network inspect/list` wrapper plus persistence in `~/.tengiz/networks.json`. Low effort, high operational value.
- **Detected:** 2026-07-31

## Compose Resource Suffixing (Name Randomization for Isolated Stacks)
- **Source:** Dokploy
- **Description:** `randomizeSpecificationFile()` (in `utils/docker/compose.ts`) rewrites an entire `docker-compose.yml` so every resource — service names, volumes, networks, configs, and secrets — gets an app-supplied suffix appended (e.g. `db` → `db-4f3a9c1d`). A YAML parse/stringify pass handles `maxAliasCount: 10000` for large files. Compose resources have a `randomize` boolean and a `suffix` string; when randomization is on, each deploy of the same compose file produces isolated, collision-free resource names. This is what lets multiple tenants or preview environments run the identical compose file on one host without name clashes.
- **Why add to Tengiz:** Tengiz's compose story (Docker Compose Import, Stack Lifecycle) deploys each compose under the app name, but two environments (staging + prod, or two previews) using the same compose file collide on `db`, `redis`, and named volumes. Suffixing solves this cleanly: `.tengiz.yaml`'da `compose.randomize: true` + `compose.suffix: "pr42"` rewrites service/volume/network names before `docker compose up`. This is also the natural companion to preview deployments (#2) for compose-based apps. Implementation is a small YAML transform in the compose package (`gopkg.in/yaml.v3` round-trip), then pass the rewritten file to the existing compose runner. Low-medium effort, unlocks true multi-env compose.
- **Detected:** 2026-07-31

## Preview Deployment PR Comments (GitHub Status Table)
- **Source:** Dokploy
- **Description:** When a preview deployment is created, Dokploy posts a markdown table comment on the pull request (`createPreviewDeploymentComment`, `getIssueComment` in `services/github.ts`): `| Name | Status | Preview | Updated (UTC) |` with a clickable preview URL. The same comment is *updated in place* (`updateIssueComment`) as the build transitions initializing → running → success/error, so reviewers always see the live state of the preview environment on the PR without opening Dokploy. Comment ID is stored on the preview deployment record (`pullRequestCommentId`).
- **Why add to Tengiz:** Tengiz's Preview Deployments (#2, implemented) and Commit Status Reporting (#38, pending) create the environment and report build state via the status API, but neither surfaces the *preview URL* in the PR conversation itself. A single pinned comment with the live preview link + status table is the UX Vercel/Netlify reviewers are used to — no digging through commit statuses. For GitHub, the REST API is two calls: create comment once, PATCH it on status change. `.tengiz.yaml`'da `preview.pr_comment: true`. Small addition on top of the existing gitdeploy/preview pipeline, high collaboration value.
- **Detected:** 2026-07-31

## Host-Level Scheduled Scripts (Root Cron on the Server)
- **Source:** Dokploy
- **Description:** `services/schedule.ts` extends the schedule model with two schedule types beyond app/container scoping: `server` and `dokploy-server`. These run arbitrary shell scripts **on the host as root** (not inside a container), wired through the Docker Scheduler app. Scripts are base64-encoded into a script file on the host, `chmod +x`'d, and a PID + Schedule ID header is prepended to every script (`echo "PID: $$ | Schedule ID: ..."`). Host-level schedules are restricted to owner/admin roles for security. The cron execution itself runs via the scheduled Docker service.
- **Why add to Tengiz:** Existing Scheduled Tasks/Cron Jobs runs `docker exec` commands *inside* containers — limited to what the image provides. Host-level schedules cover host maintenance: `apt update && apt upgrade`, log rotation, disk-space cleanup, backup hooks, and TLS cert renewal cron. This also pairs with Server Exec (#172, one-shot host commands) to make it scheduled/recurring: `tengiz schedule add --type host --cron "0 3 * * *" --script "df -h > /var/log/disk.log"`. `tengiz server exec` already provides the shell plumbing; adding a cron runner (robfig/cron) that executes those same host commands on a schedule is low incremental effort. Role-gating (owner/admin only) is a critical security control to replicate.
- **Detected:** 2026-07-31

## Container File Upload (docker cp into a Running Container)
- **Source:** Dokploy
- **Description:** `uploadFileToContainer()` in `services/docker.ts` copies a local file into a running container. It validates the container ID against a strict `[a-zA-Z0-9.\-_]+` regex and the destination path against a no-shell-metacharacter regex, writes the file to a temp path on the host, runs `docker cp <temp> <container>:<dest>`, and removes the temp file. Content is passed base64-encoded to avoid shell-escaping bugs, and the temp filename is sanitized (`dokploy-upload-<ts>-<name>`). Also supports uploading into containers on remote servers via `execAsyncRemote`.
- **Why add to Tengiz:** This is the missing "hotfix / config inject" primitive for operators: drop a `.env`, `config.yml`, or TLS key into a running container without a redeploy. Distinct from Managed File Mounts (#158, host-side bind mounts) and Patches (#118, build-time) — this is runtime, ad-hoc, and requires no config file. For Tengiz: `tengiz cp <app> ./settings.json /app/settings.json` → resolves container by label, `docker cp` with the same regex validation. Note: file is lost on container recreate, so it complements (not replaces) volumes/mounts. Low effort (existing exec pattern + regex guards from Dokploy), good ops value.
- **Detected:** 2026-07-31

## Domain DNS Validation (Pre-Deploy A-Record Check)
- **Source:** Dokploy
- **Description:** `validateDomain()` in `services/domain.ts` verifies a custom domain before it's used: strips protocol/path, resolves the domain via DNS A-record lookup (`dns.resolve4`), and returns the resolved IPs. If an expected server IP is provided, it checks the domain actually points to that IP and reports a clear mismatch error (`Domain resolves to 1.2.3.4 but should point to 5.6.7.8`). If the domain resolves to a CDN provider (Cloudflare/Fastly/Bunny), it returns the provider name and a warning instead of failing — because CDN-fronted domains legitimately won't point at the raw server IP.
- **Why add to Tengiz:** Custom domain management (implemented) currently adds domains with zero verification — a typo'd domain silently 404s until someone notices. `tengiz domain add myapp.com --verify` resolves the A record, warns on CDN-backed domains, and hard-fails on mismatch when an expected IP is known (`.tengiz.yaml`'da `domain.expected_ip` from the server IP). Reuses the CDN Provider Detection (#136) tables to avoid false failures for Cloudflare-fronted apps. Zero new dependencies (Go `net.Resolver`), low effort, and it gives users the "wait, it doesn't point here yet" feedback loop during onboarding.
- **Detected:** 2026-07-31

## Per-App Multi-Registry Split (Build / Deploy / Rollback Registries)
- **Source:** Dokploy
- **Description:** The application model carries **three separate registry references**: `registryId` (registry used to pull/run the app image), `buildRegistryId` (registry the *built* image is pushed to during build), and `rollbackRegistryId` (registry where rollback images are stored/tagged). `rollbacks.ts` uses the rollback registry to tag prior images (`app:v<version>`), performs a `docker login` to that registry before the Swarm update, and rewrites the image reference via `getRegistryTag()` so the rollback pulls from the dedicated registry. During preview deploys, all three registries are intentionally nulled to force fresh builds.
- **Why add to Tengiz:** Today Tengiz has one implicit registry (local daemon / Docker Hub). Separating the three roles enables real workflows: build on a build server and push to a private GHCR (`build.registry`), deploy a pre-pulled image from a local registry (`run.registry`), and keep rollback images in a long-lived archive registry instead of local disk that gets pruned. This composes with Custom Image Repository Naming (#141), Container Registry (#29), and Rollback (#8, implemented) — rollback becomes "pull the tagged image from the rollback registry" instead of relying on the local daemon still holding it. `.tengiz.yaml`'da `registry: {build: ghcr.io/org/app:build, run: localhost:5000/app, rollback: s3.example/rollback}`. Medium-low effort (three fields + tag/push plumbing in builder), high architectural value.
- **Detected:** 2026-07-31

---

## Explicit Port Mapping Management (Dokku `ports` plugin)
- **Source:** Dokku
- **Description:** Dokku's standalone `ports` plugin (replaced the legacy `proxy:ports-*` commands) manages explicit host→container port mappings in `scheme:host-port:container-port` format. Schemes: `http`, `https`, `grpc`, `grpcs` (plus internal `__internal__` for non-persisted maps). Commands: `ports:add`, `ports:set`, `ports:remove` (supports host-port wildcard removal), `ports:clear`, `ports:list`, `ports:report --ports-map-json/--ports-map-detected`. `ports:report --ports-map-detected` shows the **auto-detected** port map inferred from Dockerfile `EXPOSE` directives during deploy (defaulting to `http:80:5000`), with `https:443` auto-added when a cert exists. `map-detected` property controls whether EXPOSE-derived mappings are adopted. (`plugins/ports/src/commands/commands.go`, `plugins/ports/functions.go`)
- **Why add to Tengiz:** Tengiz auto-allocates ports in the 9000-9999 range and hardcodes HTTP routing on the internal port. Real apps need explicit, durable port mappings (grpc services, non-HTTP TCP backends, apps exposing multiple ports), and the EXPOSE-based auto-detection removes manual `port` config for the common case. `tengiz ports set myapp http:80:3000` gives operators deterministic, inspectable port assignments that survive redeploys. This extends (not duplicates) the Port Mapping Protocol Selection (#111, TCP/UDP) — that covers protocol; this covers the mapping lifecycle + scheme + auto-detection. Implementation: a `ports` section in `AppEntry` + `docker run -p` plumbing in `runtime.Run()`, with a Dockerfile `EXPOSE` parser in the builder. Low-medium effort.
- **Detected:** 2026-07-31

## Per-App VHost Toggle (Random High Port Exposure)
- **Source:** Dokku
- **Description:** `domains:disable <app>` / `domains:enable <app>` toggle VHOST support per app (sets the `NO_VHOST` env var). When VHOST is disabled, the app is **not** exposed via hostname-based routing at all — instead Dokku binds it to a random high host port for direct access, avoiding hostname collisions for non-web services. `domains:report` exposes `--domains-app-enabled`, `--domains-app-vhosts`, `--domains-global-vhosts`, `--domains-global-enabled`. (`plugins/domains/subcommands/disable`, `plugins/domains/subcommands/enable`, `plugins/domains/report.go`)
- **Why add to Tengiz:** Tengiz routes every app through the proxy via `<app>.tengiz.local` with a 9000-9999 port. There's no way to run a service *outside* the hostname model — e.g. an internal Redis, a metrics endpoint, or a legacy TCP app — while still getting a stable published port. `tengiz domains disable myapp` turns off proxy routing and publishes a dedicated host port (with a deterministic range instead of Dokku's random one). This is distinct from Per-App Proxy Toggle (#189, proxy:disable keeps the app on the network unproxied) — VHost toggle is about *routing model* (hostname vs port). `.tengiz.yaml`'da `domains.enabled: false`. Low effort (a boolean + port-publish branch in deploy), fills a real gap for non-HTTP services.
- **Detected:** 2026-07-31

## Global Default Domains (Server-Wide Domain Inheritance)
- **Source:** Dokku
- **Description:** `domains:add-global` / `domains:set-global` / `domains:remove-global` / `domains:clear-global` manage server-wide default domains. New apps auto-inherit `<app>.<global-domain>` subdomains on creation, and `domains:reset <app>` clears an app's domain overrides and re-applies the global-configured defaults. Global and app domains are separate lists that merge at routing time. (`plugins/domains/subcommands/add-global`, `plugins/domains/subcommands/reset`, `plugins/domains/functions.go`)
- **Why add to Tengiz:** Today every Tengiz app's domain must be configured individually (`tengiz domain add myapp.example.com`). On a platform hosting dozens of apps, a global default (`*.apps.tengiz.local` or a wildcard production domain) means new apps get a working URL with zero domain config — the Heroku/Vercel "your app is live at <name>.vercel.app" experience. `tengiz domain set-global apps.example.com` + `domains:reset` also provides a clean "give me my default URL back" recovery command when an app's domains drift. Complements implemented Custom Domain Management (#19). Low effort: a `globalDomains` list in the store merged into the proxy's `p.domains` map. High UX value for multi-app instances.
- **Detected:** 2026-07-31

## Service Discovery via Network Aliases (Custom Internal TLD)
- **Source:** Dokku
- **Description:** Dokku's `network:set <app>|--global <property>` exposes network properties beyond basic attach: `tld` (custom internal DNS TLD, e.g. `svc.cluster.local`), `bind-all-interfaces` (bind container to `0.0.0.0` vs internal-only), `initial-network` (the network used at `docker run --network`, with global default), and **`attach-post-create` / `attach-post-deploy`** (attach app containers to additional user networks with **network aliases** `<app>.<process-type>.<tld>` for inter-container DNS). `network:rebuild <app>` / `network:rebuildall` resync IP/PORT state files for running containers. `network:create` marks networks attachable + label-managed (`com.dokku.network-name`); `network:exists`, `network:info --format json`, `network:list --dokku-managed` complete the tooling. (`plugins/network/subcommands.go`, `plugins/network/functions.go`, `plugins/network/triggers.go`)
- **Why add to Tengiz:** Tengiz's Custom Docker Network (#30) and Network CRUD (#102) cover attaching to a network, but not *service discovery* — the ability for apps to address each other by stable DNS names. Once Accessory Services (#62) and managed DBs (#63) exist, apps need to find them without hardcoding IPs. `network.tld: svc.cluster.local` + `attach-post-deploy` aliases give Tengiz apps `myapp.web.svc.cluster.local`-style names. For Tengiz's single-host proxy, this maps to creating a user-defined bridge network per app group and passing `--network-alias` at `docker run` — plus a local DNS/interpolation helper in the deploy pipeline. `.tengiz.yaml`'da `network: {tld: "svc.tengiz.local", attach: ["tengiz-apps"]}`. Medium effort, high value as the foundation for multi-service apps.
- **Detected:** 2026-07-31

## Durable Per-App Deploy Lock (apps:lock)
- **Source:** Dokku
- **Description:** `apps:lock <app>` creates a persistent per-app lock file that **blocks deploys** until released; `apps:locked <app>` probes the lock state (exit 0/error), and `apps:unlock <app>` removes it (warning "a deploy may be in progress" first). The lock is durable app state queried by other plugins (the scheduler skips deploy while locked), not a transient mutex. `apps:set --global disable-autocreation true` is the global opt-out for auto-app-creation on git push. (`plugins/apps/subcommands.go:135-150,257`, `plugins/apps/apps.go:5-14`)
- **Why add to Tengiz:** The recorded Deploy Lock (#15) is Kamal's *transactional* concurrency lock (acquire/release around a single deploy). Dokku's `apps:lock` is a *durable maintenance flag*: an operator locks an app before a long-running migration or an investigation, and every deploy attempt fails fast with a clear message until unlocked. These are complementary — #15 prevents two concurrent deploys colliding; apps:lock prevents *any* deploy during maintenance. `tengiz app lock myapp --reason "db migration"` + `tengiz deploy` → "myapp is locked (db migration)". Pairs with App Repository Lifecycle Management (#51, git:lock) but is app-level and blocks the full deploy pipeline, not just pushes. Low effort (a state file + a check in the deploy pipeline).
- **Detected:** 2026-07-31

## One-Off Run Container Lifecycle (detached/list/logs/stop/retire + TTL)
- **Source:** Dokku
- **Description:** Beyond foreground `dokku run`, the `run` plugin manages the full one-off container lifecycle: `run:detached [-e KEY=VALUE] [--ttl-seconds N] <app> <cmd>` (background, no TTY), `run:list [--format json|stdout] <app>`, `run:logs <app|--container NAME> [-t tail] [-n N] [-q]` (container must match `<app>.run.*`), `run:stop <app|--container NAME>`, and **`run:retire [<app>]`** which stops every run container that exceeded its TTL deadline (default 86400s). Flags: `--ttl-seconds`, `--concurrency-policy allow|forbid|replace`, `--cron-id` (associates the run with a cron task), `--no-tty`/`--force-tty`. (`plugins/run/internal-functions:116-244`)
- **Why add to Tengiz:** Tengiz's one-off `tengiz run` (#36, implemented) is foreground-only — a command starts, streams output, and the container is removed on exit. Long-running or fire-and-forget jobs (data imports, ETL, seed scripts, cron-linked tasks) need a managed lifecycle: detached execution, list/query, log retrieval after the fact, explicit stop, and **automatic TTL-based expiry** (`run:retire`) so orphaned job containers don't accumulate on disk. TTL expiry aligns perfectly with Tengiz's scale-to-zero philosophy — ephemeral compute should self-immolate. `tengiz run --detached --ttl 3600 myapp "python manage.py import"` + `tengiz run list myapp`. Implementation: extend the existing `run` command with the subcommand family + a `~/.tengiz/runs.json` registry + a retire sweep. Low-medium effort, high ops value.
- **Detected:** 2026-07-31

## Container Retirement & Garbage Collection (ps:retire)
- **Source:** Dokku
- **Description:** `ps:retire [<app>]` (all apps when omitted) retires old containers + old images via `scheduler-retire` and expired one-off run containers via `scheduler-run-retire`. Protected by a global `flock` (exit 137 if busy). Under the hood, the docker-local scheduler registers retired containers (`scheduler-register-retired`) with a configurable age and runs age-based GC: containers older than N hours get removed, old app images are pruned (`DOKKU_SKIP_IMAGE_RETIRE` to opt out), and the currently-running image is never deleted. (`plugins/ps/subcommands.go:118`, `plugins/scheduler-docker-local/triggers.go`)
- **Why add to Tengiz:** Tengiz has Housekeeping (#6), Granular Prune (#56) and Stale Container Detection (#47) planned, but no single cron-able "retire everything old" command that understands the *deployment model*: it must never delete the running image, must keep rollback candidates, and must also clean up expired one-off run containers. `ps:retire` is that primitive — a safe, idempotent sweep operators can run on a schedule (or via the cron infrastructure #42) without fear of breaking rollback. `tengiz retire --app myapp --image-age 7d` merges Container Retention Policy (#22) with run-container GC into one command. Implementation: label-based `docker ps -a`/`docker images` scan + age comparison, reusing the existing `runtime` exec layer. Low effort, high disk-safety value.
- **Detected:** 2026-07-31

## Git Deploy Key & Credential Management (git:auth / git:allow-host / git:generate-deploy-key)
- **Source:** Dokku
- **Description:** The `git` plugin manages the git-layer credentials a PaaS needs: `git:generate-deploy-key` generates the platform's ed25519 SSH deploy key (used for git-push deploys and private-repo clones), `git:public-key` prints it, `git:allow-host <host>` adds a private git host (GitHub Enterprise, self-hosted GitLab) to `known_hosts` via `ssh-keyscan -t rsa`, and `git:auth <host> [<user> <pass>]` configures netrc credentials (password may be read from stdin, `chmod 600` enforced) with `git:auth-status <host>` verifying the entry. `git:set <app>` properties include `archive-max-files`, `archive-max-size`, `deploy-branch`, `keep-git-dir`, `rev-env-var` (configurable revision env var name, default `GIT_REV`). (`plugins/git/internal-functions:8,61,74,107`, `plugins/git/subcommands/set`)
- **Why add to Tengiz:** Tengiz's git-based deploy (#5, implemented) and webhooks (#13, implemented) need credentials to clone *private* repos and to accept git-push deploys, but there's no CLI surface to generate/manage the deploy key or register private hosts. Recorded Git Provider Account Management (#200) is token-based (GitHub/GitLab tokens); Dokku's model is the complementary SSH/netrc layer: one platform key added to the provider once, then all clones/pushes use it. `tengiz git keygen`, `tengiz git allow-host git.example.com`, `tengiz git auth github.com <user>` complete the git-ops story. Implementation: `crypto/ssh` keygen + `ssh-keyscan` + netrc file management, all via the existing `os/exec` pattern. Low effort, unblocks private-repo deploys.
- **Detected:** 2026-07-31

## Deploy from Docker Image Tar (git:load-image)
- **Source:** Dokku
- **Description:** `git:load-image <app> <image> [--build-dir <dir>]` reads a **Docker image tar from STDIN** (`cat | docker load`), verifies the named image exists in the daemon, then updates the app's git repo from it and triggers a deploy with deploy source recorded as `docker-image`. `--build-dir` selects a subdirectory of the image to use as the build context. Related input paths: `git:from-image <image>` (image already in the daemon), `git:from-archive [--archive-type tar|tar.gz|zip]` (archive read from stdin with top-level folder stripping). (`plugins/git/internal-functions:19,143,189`, `plugins/git/subcommands/load-image`)
- **Why add to Tengiz:** Recorded Explicit Image Name Deploy (#13) pulls a named image from a registry. `git:load-image` is the missing *offline/CI* input path: a CI pipeline that built an image and `docker save`'d it can ship the tar directly to the Tengiz host via stdin — no registry required, no daemon-to-daemon network. This is how air-gapped deploys and heavyweight-artifact transfers work (`ssh tengiz@host tengiz git load-image myapp myimage:1.2.3 < image.tar`). Complements the tar/archive deploy already used for non-git sources. Implementation: `docker load` piped through `os/exec` + the existing deploy pipeline. Low effort, closes the last image-ingest gap.
- **Detected:** 2026-07-31

## Cron Task Management (list/suspend/resume/run + mailto + concurrency policy)
- **Source:** Dokku
- **Description:** Dokku's `cron` plugin manages scheduled tasks declared in `app.json` `cron:` entries: `cron:list [--global|<app>] [--format json]` (ID / Schedule / Concurrency / Maintenance / Command; IDs are base36 hashes of app+command+schedule), `cron:run <app> <cron_id> [--detach]` (manual on-demand run through the scheduler with concurrency policy + TTL env), `cron:suspend`/`cron:resume` (per-task maintenance toggle without deletion), and `cron:set <app>|--global mailfrom/mailto/maintenance` (**email the cron output**). Each task has `concurrency_policy: allow|forbid|replace` controlling overlapping runs, and commands are shell-tokenization-validated at deploy time. (`plugins/cron/subcommands.go`, `plugins/cron/cron.go`)
- **Why add to Tengiz:** Scheduled Tasks/Cron (#74) is recorded at the "execute a command on a schedule" level. The missing management layer is what operators actually touch: list what's scheduled, run a task manually (`--detach` fits `tengiz run`), suspend a task during maintenance, and get cron output emailed. The `concurrency_policy: replace` (kill previous run before starting new) and `forbid` (skip if still running) semantics prevent the classic overlapping-cron catastrophe — critical for migrations and ETL. `tengiz cron list`, `tengiz cron suspend <app> <id>`, `.tengiz.yaml`'da `cron: [{id: nightly, schedule: "0 3 * * *", cmd: "backup", concurrency: "forbid", mailto: "ops@x.com"}]`. Low-medium effort on top of the planned cron executor.
- **Detected:** 2026-07-31

## Named Storage Volumes & Volume Exec (k8s/PVC-style volumes)
- **Source:** Dokku
- **Description:** Beyond host-path bind mounts, the `storage` plugin manages **named volumes as first-class resources**: `storage:create <name> [<path>] [--scheduler docker-local|k3s] [--size] [--access-mode] [--storage-class-name] [--namespace] [--chown] [--reclaim-policy] [--annotation k=v] [--label k=v]`, `storage:destroy`, `storage:info [--format json]`, `storage:set`, `storage:list-entries [--scheduler]`, `storage:wait <name>`, `storage:migrate [--all]`, and **`storage:exec <name> [--image alpine:3] [--as-user <uid>] [-- <cmd>]`** — a throwaway container mounting the volume at `/data` to inspect or modify volume contents directly. On the k3s scheduler these map to PVC/PV with storage classes. (`plugins/storage/src/subcommands/subcommands.go:22-134`)
- **Why add to Tengiz:** Persistent Storage (#7, implemented) only mounts existing host paths / named volumes via `--volume`. Named volumes as a managed resource give CLI users: guaranteed lifecycle (create once, reference by name across redeploys), k8s/PVC parity for the future scheduler abstraction (#162), and — most valuable — `storage:exec` to debug *what's actually in a volume* without constructing `docker run --rm -v` one-liners. With scale-to-zero, volume debugging is a daily task ("did my data persist through the cold start?"). `tengiz storage create myapp-data`, `tengiz storage exec myapp-data -- ls -la /data`. Implementation: a `~/.tengiz/volumes.json` registry + `docker volume create` wrappers + a `storage exec` helper. Low-medium effort, closes the volume-management gap.
- **Detected:** 2026-07-31

## Rich Volume Mount Modifiers (phase, process-type, subpath, readonly, options)
- **Source:** Dokku
- **Description:** `storage:mount <app> <name|path> <container-dir> [--container-dir] [--phase deploy|run] [--process-type] [--volume-subpath] [--volume-readonly] [--volume-chown] [--volume-options "Z,noexec,nosuid"]` attaches a volume to a **specific phase** (deploy vs run) and **process type**, with subpath mounting, read-only flag, ownership, and SELinux `:Z`/`noexec`/`nosuid` options. The legacy colon form also supports `host:container:ro`. (`plugins/storage/src/subcommands/subcommands.go:150-170`, `plugins/storage/storage.go`)
- **Why add to Tengiz:** Tengiz's volume mounts are all-or-nothing: mounted for every process, always read-write, root-owned. Production apps need granularity: a `worker` process should mount a scratch volume read-only while `web` gets read-write; config volumes should be `noexec`; and `:ro` configs are a security best practice. `--phase` scoping means the deploy/build step doesn't inherit runtime mounts (or vice-versa). `.tengiz.yaml`'da `volumes: [{name: data, mount: /app/data, ro: true, process: "web", options: "noexec"}]`. Implementation: pass-through of `:ro`/`:Z`/options to `docker run -v` in `runtime.Run()`, plus filtering mounts by process type at container-creation time. Low effort, meaningful security/correctness gain.
- **Detected:** 2026-07-31

## Network & GPU Resource Limits with Reservation (resource plugin)
- **Source:** Dokku
- **Description:** The `resource` plugin goes beyond CPU/memory: `resource:limit [--process-type]` supports `cpu`, `memory`, `memory-swap`, **`network`**, **`network-ingress`**, **`network-egress`**, and **`nvidia-gpu`** limits; plus **`resource:reserve`** / `resource:reserve-clear` for guaranteed minimums (reservation), `resource:limit-clear`, and `resource:report` with dynamic `--resource-<key>` flags. Limits/reserves are per-process-type. (`plugins/resource/src/commands/commands.go`, `plugins/resource/resource.go`)
- **Why add to Tengiz:** Recorded Resource Limits (#6, implemented) covers CPU/memory only. Two gaps: (1) **network bandwidth caps** — on a shared single server, one app saturating the NIC starves every other app; per-container ingress/egress limits (`--network` rate) prevent noisy-neighbor bandwidth abuse. (2) **reservation vs limit** — current Tengiz only sets hard `--memory`/`--cpus` limits but no guaranteed minimums; `--memory-reservation`/`--cpuset`-style reserves ensure latency-sensitive apps get their share even under load. The nvidia-gpu resource limit complements GPU Passthrough (#133) by capping GPU allocation per app. `.tengiz.yaml`'da `resources: {limits: {network: "100mbps"}, reserves: {memory: "256M"}}`. Low-medium effort (extra `docker run` flags + config fields).
- **Detected:** 2026-07-31

## Registry Login Credential Store & Image Repo Template
- **Source:** Dokku
- **Description:** The `registry` plugin manages **registry credentials and naming**: `registry:login [--global|--password-stdin] [<app>] <server> <username>` / `registry:logout [--global] [<app>] <server>` persist per-app Docker credentials in `/var/lib/dokku/config/registry/<app>/config.json`, and `registry:set <app>|--global` properties include `image-repo`, **`image-repo-template`** (e.g. `registry.example.com/{{ APP }}:{{ TAG }}`), `push-on-release`, `push-extra-tags`, `server`. `registry:report` shows computed values (`--registry-computed-image-repo`, `--registry-computed-server`). (`plugins/registry/src/commands/commands.go`, `plugins/registry/functions.go`)
- **Why add to Tengiz:** Recorded Container Registry Integration (#29) covers push + extra tags, and Private Registry Auth (#20) mentions pulling, but neither provides a **credential lifecycle** (`registry login/logout`, per-app vs global) nor the **Go-template image naming** (`{{ APP }}`/`{{ TAG }}`). Dokku's `image-repo-template` is a cleaner abstraction than Custom Image Repository Naming (#52): one template string with variable substitution replaces per-deploy naming logic. For Tengiz: `tengiz registry login ghcr.io myuser --password-stdin` + `tengiz registry set myapp --global image-repo-template "ghcr.io/org/{{ APP }}:{{ TAG }}"`. Credentials stored encrypted via the existing `encrypt` package. Low effort, closes the registry-auth gap that CI/CD pushes depend on.
- **Detected:** 2026-07-31

## Ordered Buildpack List Management (buildpacks CLI)
- **Source:** Dokku
- **Description:** The `buildpacks` plugin manages an **ordered, multi-buildpack list** per app via CLI: `buildpacks:add [--index N]`, `buildpacks:remove [--index N]`, `buildpacks:set [--index N]` / `buildpacks:set --replace <app> <bp> [<bp>...]` (replace whole list), `buildpacks:list`, `buildpacks:clear`, `buildpacks:detect [--branch]` (run herokuish detection), and `buildpacks:set-property [--global] stack <value>` (build stack). The `.buildpacks` file in the source workdir is rewritten in place; resolution order: `.buildpacks` → `app.json` → properties → auto-detection. (`plugins/buildpacks/src/subcommands/subcommands.go`)
- **Why add to Tengiz:** Herokuish Buildpacks (#68) is recorded at the engine level but not the *list management* surface. Real polyglot apps chain buildpacks in order (e.g. node + python with a final custom BP), and the only current mechanism is editing `.buildpacks` by hand. A CLI gives operators: positional ordering (`--index`), atomic replacement (`set --replace`), a `list` for debugging "what order will this build in?", and a global `stack` default. `tengiz buildpacks add myapp heroku/nodejs --index 1`. Implementation: parse/write `.buildpacks` + store per-app overrides in `~/.tengiz/buildpacks.json`, consumed by the herokuish builder when built. Low effort on top of #68.
- **Detected:** 2026-07-31

## Generated Proxy Config Inspection & Validation (nginx:show-config / validate-config / access-logs)
- **Source:** Dokku
- **Description:** Dokku's `nginx-vhosts` plugin exposes the generated proxy configuration as a debuggable artifact: `nginx:show-config <app>` prints the rendered `nginx.conf` for an app, `nginx:validate-config [<app>] [--clean]` validates all app configs (optionally removing invalid ones), `nginx:access-logs <app> [-t]` / `nginx:error-logs <app> [-t]` tail **proxy-level** logs (distinct from container logs), and `nginx:reload/start/stop` manage the proxy daemon. Per-app custom templates (`nginx-conf-sigil-path`, `disable-custom-config`) allow overrides. The `nginx:set <app> <property>` family (30+ props) tunes `client-max-body-size`, proxy connect/read/send timeouts, buffering, HSTS (+ max-age/include-subdomains/preload), access/error log paths+formats, `bind-address-ipv4/ipv6`, and `x-forwarded-*` header values. (`plugins/nginx-vhosts/command-functions`, `plugins/nginx-vhosts/subcommands/set`, `plugins/nginx-vhosts/src/nginx-property/nginx-property.go`)
- **Why add to Tengiz:** Tengiz's proxy is a Go `httputil.ReverseProxy` — invisible and un-debuggable. When routing misbehaves ("why does POST fail?", "why is upload rejected?"), operators have no way to see the *effective* routing config, validate it, or inspect proxy-level access/error logs. `tengiz proxy show-config <app>` dumps the resolved routing table (hosts → container → headers/timeouts) as YAML, `tengiz proxy validate` checks all apps for conflicts, and `tengiz proxy access-logs <app>` tails request logs — the three tools that make the proxy a debuggable component. `proxy.client_max_body_size: 100m` and `proxy.hsts: true` map cleanly to the Go middleware. This deepens (not duplicates) the Gelişmiş Proxy Config (#76) and Custom Headers (#60) entries with inspection + tuning. Low effort — Tengiz already owns the proxy, so it's data plumbing, not config-file generation.
- **Detected:** 2026-07-31

## Per-Process-Type Deploy Checks (disable/skip/run + wait-to-retire)
- **Source:** Dokku
- **Description:** The `checks` plugin's zero-downtime verification is granular: `checks:disable <app> [proctypes]`, `checks:enable <app> [proctypes]`, `checks:skip <app> [proctypes]` (skip checks for specific process types, comma list or `_all_`), `checks:run <app> [proctypes]` (manually run pre-flight checks, supports `proctype.containerIndex` targeting), and `checks:set <app>|--global wait-to-retire <seconds>` — a grace period (default 60s) between the new container passing checks and the old container being retired, during which the old version keeps serving. (`plugins/checks/subcommands/{disable,enable,skip,run,set}`)
- **Why add to Tengiz:** Zero-Downtime Deploy Health Checks (#28) is recorded at the app level. Production reality is per-process: a worker process may not expose an HTTP health endpoint at all, so checks should be disabled for it while `web` gets full HTTP validation. `checks:skip myapp worker` prevents false deploy failures on non-HTTP processes. The **`wait-to-retire` grace period** is a subtle but important addition to zero-downtime: after the new container is healthy, hold the old one for N seconds to catch immediate post-migration failures (connection pools, warm-up) before traffic fully switches. `tengiz checks skip myapp worker` + `.tengiz.yaml`'da `deploy.checks.wait_to_retire: 30`. Low effort (config + timing in the deploy pipeline), high reliability value.
- **Detected:** 2026-07-31

## K3s Cluster Lifecycle & Helm Chart Management (scheduler-k3s concrete surface)
- **Source:** Dokku
- **Description:** Beyond the scheduler abstraction, Dokku's `scheduler-k3s` plugin provides the concrete Kubernetes cluster surface: `scheduler-k3s:initialize [--server-ip] [--taint-scheduling] [--ingress-class nginx|traefik]` (bootstraps a single-node K3s), `cluster:add [--profile|--role|--server-ip|--kubelet-args] <ssh://host>` / `cluster:list [--format json]` / `cluster:remove <node-id>` / `profiles:*`, `ensure-charts [--force]` (installs cert-manager, longhorn, traefik/ingress-nginx, KEDA, vector), `show-kubeconfig`, `preview <app>` (Helm dry-run unified diff with `--show-secrets`), `charts:set <chart>.<property>` + `charts:report`, per-resource `annotations:set`/`labels:set` (scoped by `--process-type`/`--resource-type`), `autoscaling-auth:set <trigger>` (KEDA TriggerAuthentication), and app properties `deploy-timeout` (default 300s), `rollback-on-failure`, `namespace`, `image-pull-secrets`, `kustomize-root-path`, `shm-size`. Deploys render a per-app Helm chart → kustomize → `helm upgrade --install --wait` with auto-rollback. TLS via cert-manager ClusterIssuers (letsencrypt-prod/stag). (`plugins/scheduler-k3s/src/commands/commands.go`, `plugins/scheduler-k3s/subcommands.go`, `plugins/scheduler-k3s/chart.go`)
- **Why add to Tengiz:** Pluggable Multi-Scheduler (#162) is recorded as an interface abstraction; this is the concrete target implementation. Even if Tengiz never ships full k3s, these concepts transfer: `deploy-timeout` + `rollback-on-failure` belong in the Docker scheduler too (extending Readiness Delay & Timeouts #27), and the Helm-chart-per-app + kustomize-overrides model is a clean template for Tengiz's own config rendering. `tengiz cluster init` gives the growth path to multi-node HA with zero CLI changes. Record this so the scheduler work has a concrete reference. Very high effort — flag as architectural, not near-term.
- **Detected:** 2026-07-31

## Interactive CLI Shell (tengiz shell REPL)
- **Source:** Dokku
- **Description:** `dokku shell` launches an interactive `dokku>` REPL with command history (`~/.dokku_history`), builtins `clear`/`quit`/`exit`, line editing (ships an inputrc), and passes every other command straight to the `dokku` CLI. It's a thin wrapper, but it changes the operator UX: repeatable multi-command sessions (deploy → check status → tail logs) without re-typing `tengiz` and with up-arrow history. (`plugins/shell/subcommands/default`)
- **Why add to Tengiz:** Tengiz is CLI-first; a REPL is a cheap but real UX win for long operator sessions. `tengiz shell` drops the user into `tengiz>` with history, tab-completion wiring, and passthrough to the Cobra command tree. It also foreshadows an interactive admin mode once the REST API/Web UI (#8/#150) land. Implementation: Go's `github.com/chzyer/readline` or `golang.org/x/term` + a read-eval-loop that execs the root command with args. Distinct from Container Entering (#140, `docker exec -it`) — this is host-side, not in-container. Trivial effort, nice developer-experience polish.
- **Detected:** 2026-07-31

## Builder Property System & Per-Builder Config Paths (monorepo build contexts)
- **Source:** Dokku
- **Description:** The `builder` plugin centralizes build configuration: `builder:set <app> <property> (<value>)` with properties **`build-dir`** (subdirectory used as the build context — monorepo support), `selected` (which builder: dockerfile/herokuish/nixpacks/pack/railpack/lambda/null), `skip-cleanup`, and read-only `detected`. Each builder plugin contributes its own config-file path property: `dockerfile-path` (dockerfile builder), `nixpackstoml-path`, `projecttoml-path` (pack/CNB), `railpackjson-path`, `lambdayml-path` — i.e. the manifest is not assumed to live at the repo root. `builder:report` shows per-app computed values; triggers `builder-detect`/`builder-get-property`/`builder-set-property` drive detection. (`plugins/builder/`, `plugins/builder-dockerfile/`, `plugins/builder-nixpacks/` etc.)
- **Why add to Tengiz:** Monorepo Support (base_dir, #11) is recorded as a single `base_dir` override. Dokku's model is more flexible: **per-builder manifest paths** mean a monorepo can have `apps/web/Dockerfile`, `apps/api/nixpacks.toml`, and `services/fn/lambda.yml` each detected and built with the right config without a single global base_dir. `builder.build_dir: apps/web`, `builder.nixpacks_path: services/api/nixpacks.toml` — plus `skip-cleanup` (keep intermediate build containers for debugging, complementing Build Logs #9) and the `detected` report showing *why* a builder was chosen. This generalizes the existing `builder` package's `detect.go` into a settable, inspectable property store. Low-medium effort, high monorepo value.
- **Detected:** 2026-07-31

---

## Namespace-Based Multi-Tenancy (Fully Isolated Environments)
- **Source:** CapRover
- **Description:** CapRover supports per-namespace isolation on a single instance. Each namespace gets its own `ServiceManager` (cached per namespace), its own `AppsDataStore`, its own `Authenticator` with a namespace-derived encryption key (`secret + namespace`), and its own JWT signing context. Tokens, salts, and data from one namespace are cryptographically unusable in another. Every API route and datastore is namespace-scoped. (`src/user/ServiceManager.ts:36,50-71`, `src/user/Authenticator.ts` constructor, `src/datastore/DataStoreProvider.ts`)
- **Why add to Tengiz:** Tengiz's current `--env` flag is config-level (`.tengiz.{env}.yaml` merge) but not security isolation — all envs share the same `~/.tengiz/` state store and Docker namespace. CapRover's namespace model gives hard multi-tenant isolation on one host: separate state files, separate encryption salts, separate auth contexts. This enables a service-provider model where one Tengiz instance serves multiple teams/companies without cross-tenant leakage. Complements Multi-Team Organization (#196) with a data-level isolation boundary rather than just RBAC. Implementation: a `namespace` parameter threaded through Store/Proxy/CLI, with per-namespace state directories (`~/.tengiz/<namespace>/`).
- **Detected:** 2026-07-31

## Pre-Deploy Function (Programmatic Docker Service Mutation)
- **Source:** CapRover
- **Description:** Beyond shell-command pre-deploy hooks, CapRover's `preDeployFunction` is a user-supplied JavaScript function compiled at deploy time (`requireFromString`) that receives `(captainAppObj, dockerUpdateObject)` and can **programmatically mutate the Docker service spec** before the service update is applied. `createPreDeployFunctionIfExist` compiles it and `ensureServiceInitedAndUpdated` invokes it inside the update pipeline. (`src/user/ServiceManager.ts:578-605,978-1001`)
- **Why add to Tengiz:** Existing Pre-Deploy Hooks (#15) run shell commands before deploy. A programmatic mutation hook is more powerful — it lets deploy logic inspect and alter the actual container spec (add labels, rewrite env conditionally, inject mounts based on logic). For Tengiz's exec-based runtime, this maps to a Go template or an embedded scripting hook that can modify the `docker run` argument slice before execution. `.tengiz.yaml`'da `deploy.pre_deploy_fn:` olarak tanımlanır. Useful for teams with complex, conditional container customization that static config can't express.
- **Detected:** 2026-07-31

## Docker Secrets Integration (Container-Level Encrypted Secrets)
- **Source:** CapRover
- **Description:** CapRover manages Docker native secrets: `ensureSecret` creates a base64-encoded secret if absent, `ensureSecretOnService` idempotently attaches a secret to a service (only updating when not already attached, to avoid restarts), and `checkIfServiceHasSecret` verifies attachment. The `captain-salt` secret is created via Docker secrets and mounted into the Captain service at `/run/secrets/`, then used as the encryption salt for all sensitive data. (`src/docker/DockerApi.ts:1108-1247`, `src/user/system/CaptainManager.ts` initialize step 8-9)
- **Why add to Tengiz:** Tengiz's Secrets Management (#32/#54) uses AES-GCM encrypted JSON files in `~/.tengiz/`. Docker native secrets offer a complementary distribution mechanism: secrets delivered to the container filesystem (`/run/secrets/`) rather than env vars, which some apps (and security reviewers) prefer. For an exec-based runtime this is `docker run --secret` flag passthrough plus a `tengiz secret attach/detach <app>` command family. Also useful for the platform's own encryption key — storing the master key as a Docker secret (rather than `~/.tengiz/.key`) survives host reboots and manual key file loss.
- **Detected:** 2026-07-31

## Atomic Proxy Config Regeneration with Validation & Rollback
- **Source:** CapRover
- **Description:** CapRover's LoadBalancerManager regenerates the nginx config atomically and safely: writes `.fut`, swaps to `.conf` via `.bak` rename sequence, validates with `nginx -t`, reloads via SIGHUP, and coalesces concurrent reload requests into one pass (`reloadInProcess` flag + request queue). On nginx validation failure during an app update, the previous app definition is restored and the LB reloaded — a full rollback of the config change. (`src/user/system/LoadBalancerManager.ts:69-241,782-838`)
- **Why add to Tengiz:** Tengiz's Go reverse proxy rebuilds routes in-memory with no validation or rollback — a bad config change (duplicate domain, invalid header) silently breaks routing until noticed. Implementing write-then-validate-then-activate semantics for the proxy's route table (backup current table → apply new → health-check → revert on failure) makes proxy updates crash-safe. Complements Generated Proxy Config Inspection (#223) and Config Validation (#147). This is the operational safety layer for zero-downtime routing changes.
- **Detected:** 2026-07-31

## Self-Expiring Backup Download Links (Time-Limited Download Tokens)
- **Source:** CapRover
- **Description:** CapRover backup downloads are secured by short-lived JWTs: `getDownloadToken(filename)` issues a token with a 2-minute expiry and the `DownloadRouter` authorizes only matching tokens. Each generated backup file also self-deletes after 2 hours (`fs.removeSync` timeout). Backups are named with timestamp + leader IP + sanitized hostname (`caprover-backup-<YYYY_MM_DD-HH_mm_ss>-<epoch>-ip-<ip>-host-<hostname>.tar`). (`src/routes/download/DownloadRouter.ts`, `src/user/system/BackupManager.ts`, `src/user/Authenticator.ts` DOWNLOAD_TOKEN)
- **Why add to Tengiz:** Full System Backup (#23) records the create/restore cycle but not secure distribution. A `tengiz backup create --share` that prints a time-limited download link (with expiry + single-use token) lets operators hand backups to teammates or CI without exposing the whole state directory. Aligns with the token-based auth already planned (#37 tokens, #16 API keys). Low effort: HMAC-signed URL with `expires` claim validated by the HTTP server.
- **Detected:** 2026-07-31

## Deploy Input Source Abstraction (Git / Uploaded TAR / Inline Captain-Definition)
- **Source:** CapRover
- **Description:** CapRover's `IImageSource` model supports three mutually exclusive deploy inputs: `uploadedTarPathSource` (pre-uploaded TAR archive), `captainDefinitionContentSource` (inline captain-definition JSON + git hash), and `repoInfoSource` (git repo with credentials). `extractContentIntoDestDirectory` handles all three uniformly — tar extraction, git clone, or direct file write — before the build. (`src/models/IImageSource.ts`, `src/user/ImageMaker.ts:305-369`)
- **Why add to Tengiz:** Tengiz currently deploys from a local directory or git push. Supporting **inline content deploy** (`tengiz deploy --definition '<json>'`) enables API/CI-driven deploys without a checkout or file transfer, and **uploaded TAR deploy** matches the existing tar-upload webhook pattern. Unifying these behind one source abstraction makes the deploy pipeline (currently `deploy.go`) provider-agnostic and paves the way for future sources (Dockerfile URL, registry image). Low-medium effort, high CI/CD flexibility value.
- **Detected:** 2026-07-31

## Curated Runtime Template Catalog (templateId in captain-definition)
- **Source:** CapRover
- **Description:** CapRover's `captain-definition` supports a `templateId` field referencing curated runtime templates like `node/18`, `php/8`, `python-django/3.11`, `ruby-rack/3.2`. `TemplateHelper.getDockerfileContentFromTemplateTag` resolves these to `FROM <image>:<ver><suffix>` plus template `postFromLines` (framework-specific setup lines). `convertCaptainDefinitionToDockerfile` requires exactly one of `templateId`, `imageName`, `dockerfilePath`, or `dockerfileLines` — an enforced, validated contract. (`src/models/ICaptainDefinition.ts`, `src/user/TemplateHelper.ts`, `src/user/ImageMaker.ts:386-431`)
- **Why add to Tengiz:** Framework auto-detection (#2) infers the runtime from source files; a `templateId` is the explicit alternative — users declare `template: node/18` and get a known-good, versioned base image + setup. This is complementary to Nixpacks (#20): templates give deterministic, pin-able runtimes while Nixpacks gives zero-config detection. `.tengiz.yaml`'da `build.template: node/20` ile kullanılır. The enforced "exactly one source" validation (schemaVersion 2 contract) also improves config validation (#147) by rejecting ambiguous build definitions early.
- **Detected:** 2026-07-31

## Orphaned SSL Certificate Detection & Webroot Reconciliation
- **Source:** CapRover
- **Description:** CapRover's CertbotManager tracks certificate hygiene: `ensureAllCurrentlyRegisteredDomainsHaveDirs()` runs `certbot certificates`, parses "Certificate Name:" entries, and provisions webroot dirs for every registered cert. `logExpiringOrphanedCertificates(activeDomains)` scans certbot renewal `.conf` files and flags certificates expiring within 48h that are no longer used by any active domain (orphaned), logging them for manual cleanup. (`src/user/system/CertbotManager.ts:22-36,291-352`)
- **Why add to Tengiz:** Let's Encrypt (#37) issues certs but doesn't handle lifecycle hygiene: certs for removed domains or changed root domains linger, renew unnecessarily, and eventually expire. Automatic detection of orphaned certs (compared against the proxy's active domain list) plus webroot reconciliation prevents renewals of unused certs and surfaces cleanup candidates. This complements Manual SSL Management (#188) and Staging Mode (#161) with an operational sweep — `tengiz certs orphan-check` or auto-notification. Low effort: parse existing cert store + compare against proxy route table.
- **Detected:** 2026-07-31

## Healthcheck Barrier (Primary-Role Gating for Multi-Role Boot)
- **Source:** Kamal
- **Description:** Kamal's deploy boot coordinates multiple roles/hosts via a barrier (`lib/kamal/cli/healthcheck/barrier.rb`). Only the first primary-role container "opens" the barrier: it boots first, waits for its healthcheck (or readiness delay), and only then releases queued non-primary roles (workers/jobs) to boot. If the primary container fails to become healthy, the barrier "closes", other roles are never booted, and container logs + `docker inspect` health output are fetched for debugging. Boot order groups are sliced via `boot.limit` (count or %) with `boot.wait` delays between groups and optional `parallel_roles`.
- **Why add to Tengiz:** The recorded Role-Based Server Groups (#106) and per-process containers would silently boot workers before the web app is ready (connection pools, migrations, warm-up). A healthcheck barrier guarantees dependency order: web first, everything else after it's healthy — the difference between a clean multi-process boot and random startup failures on every deploy. For Tengiz this is a boot-stage state machine in the deploy pipeline: `web` container healthy → release `worker`/`cron` containers. Complements Readiness Delay & Deploy Timeouts (#27) and Rolling Boot (#108). Low effort once healthchecks exist (#4, implemented).
- **Detected:** 2026-07-31

## Accessory Config File & Directory Management (Host-Side Upload with Permissions)
- **Source:** Kamal
- **Description:** Kamal accessories (`lib/kamal/configuration/accessory.rb`, `lib/kamal/cli/accessory.rb`) support `files:` and `directories:` declarations beyond volumes: `files` upload local files to the host (with configurable `mode` chmod + `owner` chown, e.g. `nginx.conf`, `my.cnf`) before the container starts, rendered through ERB at deploy time when the file path ends in `.erb`. `directories` create host directories with mode/owner. Uploaded files are bind-mounted into the accessory container, giving each accessory its own host-side config directory (`$PWD/<service>/...`).
- **Why add to Tengiz:** Accessory Services (#62) covers running sidecar containers (Postgres/Redis) but not configuring them. Real databases need a `postgresql.conf`, `redis.conf`, or TLS key on the host before boot — today that means manual `scp`/`docker cp`. `tengiz accessory file myapp /path/my.cnf --mode 0644 --owner 999` gives CLI-driven config management with chmod/chown, and per-environment config via templates (`.erb` → Go `text/template`). Complements Managed File Mounts (#158, app-level) and Patches (#118, build-time). Low-medium effort: an upload step in the accessory boot pipeline + permission plumbing.
- **Detected:** 2026-07-31

## Secrets via Docker Env-File Injection & Secret Aliasing
- **Source:** Kamal
- **Description:** Kamal splits env delivery into two mechanisms: clear env vars passed as `docker run -e`, and secrets written to a per-app/role env file (`~/.kamal/<service>/env/roles/<name>.env`, chmod 0600) injected via `--env-file` (`lib/kamal/env_file.rb`, `lib/kamal/configuration/env.rb`). `env.secret` entries support aliasing — `APP_KEY:PRODUCTION_KEY` maps the container variable `APP_KEY` to the stored secret `PRODUCTION_KEY`, so multiple apps can reference one stored value under different names. Values are docker-env-file-escaped (UTF-8 preserved, quotes/`#` handled).
- **Why add to Tengiz:** Tengiz's Secrets Management (#32/#54) injects secrets as regular env vars. Passing secrets via a 0600 `--env-file` instead of `-e` keeps them out of `docker inspect` output and process listings — a real security improvement. Secret aliasing removes per-app secret duplication (one stored `DB_PASSWORD` mapped as `DATABASE_URL_PASSWORD`, `POSTGRES_PASSWORD` etc.). Complements the existing `config set --secret`/`secret set` commands: `tengiz secret set myapp PRODUCTION_KEY ...` + `.tengiz.yaml`'da `env.secret: ["APP_KEY:PRODUCTION_KEY"]`. Low effort — generate the env file in the deploy pipeline, escape values, pass `--env-file`.
- **Detected:** 2026-07-31

## Git-Clone Build Context (Build from Committed State)
- **Source:** Kamal
- **Description:** When a git repo is detected and no custom build `context` is configured, Kamal builds from a fresh git clone in a temp dir (`builder.git_clone?` + `clone_directory`, `lib/kamal/configuration/builder.rb`), not from the working directory. Uncommitted changes are ignored with a yellow warning listing them (`cli/build.rb`). If dirty files are present in a non-clone build, the image version gets an `_uncommitted_<hex>` suffix. The clone path is derived from the git root + relative pwd + pwd hash for cache stability.
- **Why add to Tengiz:** Building from a clone guarantees the image matches committed state — dirty-working-directory builds silently produce unreproducible images that can't be recreated or safely rolled back. This is the correctness foundation for Git-Based Image Version Tagging (#148), rollback (#8, implemented), and Deploy Source Metadata (#49). `tengiz deploy` from a git dir would clone to a temp cache dir and build there (with the `_uncommitted_` suffix warning preserved), or offer `build.context: git-clone` explicitly. Low effort: `git clone` via `os/exec` + tmpdir cache keyed by app+pwd.
- **Detected:** 2026-07-31

## Version-Clash Container Renaming (Idempotent Zero-Downtime Redeploy)
- **Source:** Kamal
- **Description:** During `kamal app boot`, if a container for the target version already exists on the host, Kamal renames it to `<version>_replaced_<hex>` (`lib/kamal/cli/app/boot.rb` — `old_version_renamed_if_clashing`) before starting the new container with the same version. The renamed container keeps serving while the new one boots and passes health checks; only then is the old running version stopped. This makes redeploying the *same* version idempotent and collision-free.
- **Why add to Tengiz:** Re-running `tengiz deploy` on an already-deployed version currently risks a "container name already in use" failure or a forced stop of the old container before the new one is ready — both break zero-downtime (#1, implemented). Rename-before-boot gives retry-safety (a crashed first deploy can be re-run) and graceful drain of the old version while the new one warms up. `docker rename tengiz-myapp-<sha> tengiz-myapp-<sha>_replaced_<hex>` before `docker run`. Complements Stale Container Detection (#47) which then reports/cleans these `_replaced_` leftovers. Low effort, high reliability value.
- **Detected:** 2026-07-31

## Resource Template System (User-Marked Reusable Resources)
- **Source:** Komodo
- **Description:** Every Komodo resource (Deployment, Stack, Build, Repo, Server, etc.) carries a `template: bool` flag (`client/core/rs/src/entities/resource.rs:49-52`). Queries support a `TemplatesQueryBehavior` (`Include` / `Exclude` / `Only`), letting the UI surface only templates (`resource.rs:137-162`). Template resources get a "T" badge and are meant to be copied via the generic `Copy*` write ops (`CopyDeployment`, `CopyStack`, `CopyServer`, `CopyBuild`, `CopyRepo`, `CopyProcedure`, `CopyBuilder`, `CopyAlerter` — one per resource type in `api/write/*`). `ResourceToml` also carries `template` for GitOps-managed template definitions (`entities/toml.rs:165-167`).
- **Why add to Tengiz:** Distinct from One-Click Service Templates (#23/#64, curated Docker Compose catalog) and Project Scaffolding (#88, source-code starters): this lets a user mark *any working configuration* as a reusable base and clone it with one command. `tengiz template mark myapp` + `tengiz app create --from-template myapp` gives teams a self-maintained golden-config library (production-grade app setup shared across dev/staging/prod) without maintaining starter repos. The `Only` query filter turns `tengiz ps --templates` into a browsable template catalog. Complements App Cloning (#123) — cloning copies live state, templates are the declarative base. Low effort: add a `template` bool to `AppEntry` + a `--from-template` flag on app create.
- **Detected:** 2026-07-31

## Adopt Existing Container (CreateDeploymentFromContainer)
- **Source:** Komodo
- **Description:** `CreateDeploymentFromContainer` (write op, `client/core/rs/src/api/write/deployment.rs:75-105`) takes an existing running container name + server and creates a full managed Deployment resource from it — importing its image, ports, volumes, env, and labels into the platform's resource model. The container becomes a first-class managed deployment with normal lifecycle operations.
- **Why add to Tengiz:** Tengiz can only manage apps it deployed itself. Real servers accumulate hand-run containers (a Postgres started via raw `docker run`, a CI-launched job, a legacy service) that operators want under Tengiz's idle/health/proxy umbrella without redeploying. `tengiz adopt <container>` introspects via `docker inspect` (existing exec runtime), synthesizes an `AppEntry` (image, port, env, volumes) and adopts the container. High onboarding/migration value — "bring your own container" is the fastest path to a managed platform. Low-medium effort: `docker inspect` parse → `AppEntry` build → store registration + proxy route add.
- **Detected:** 2026-07-31

## Scheduled Maintenance Windows (Alert Suppression)
- **Source:** Komodo
- **Description:** `MaintenanceWindow` (`client/core/rs/src/entities/mod.rs:532`) is a first-class entity: name, description, schedule type (`Daily` / `Weekly` / `OneTime`), `day_of_week`, ISO `date`, start `hour`/`minute`, `duration_minutes`, `timezone`, `enabled`. Alerters reference a list of windows (`entities/alerter.rs:87`) and suppress alert delivery while an active window is in effect — planned maintenance doesn't page operators.
- **Why add to Tengiz:** Notification System (#5) and Alert System (#201) fire on every event with no notion of "this failure is expected right now." A 02:00 migration or a deploy window currently triggers Slack/Discord/Email noise that trains operators to ignore alerts. `tengiz maintenance-window add --daily 02:00-03:00` or `--one-time 2026-08-15 22:00 --duration 90` silences non-critical alerts during the window while still recording them. Distinct from Maintenance Mode (#25, proxy draining — traffic-side) — this is notification-side suppression. Low effort: time-range check in the `notify` dispatch path.
- **Detected:** 2026-07-31

## WebAuthn Passkey 2FA (Beyond TOTP)
- **Source:** Komodo
- **Description:** The User entity carries both `totp: UserTotpConfig` and `passkey: UserPasskeyConfig` (`client/core/rs/src/entities/user.rs:75`) — WebAuthn/FIDO2 passkeys as a second 2FA method alongside TOTP. `external_skip_2fa` allows third-party (OIDC/OAuth) logins to skip 2FA, and DB helpers `clear_user_2fa_methods` provide admin lockout recovery (`lib/database`). Passkeys are stored as `UserPasskeyConfig` with credential IDs for challenge verification.
- **Why add to Tengiz:** Recorded 2FA (#159, Coolify) covers TOTP only. Passkeys (WebAuthn) are phishing-resistant, don't require a shared secret or typing codes, and work with hardware keys / platform authenticators — the enterprise security baseline. For a self-hosted admin surface, adding `tengiz auth add-passkey` (generates the FIDO2 ceremony) alongside the existing TOTP flow covers the current best-practice MFA. Go: `github.com/go-webauthn/webauthn` + store credential in the existing `encrypt`-protected auth store. Medium effort, complements OIDC SSO (#157) and 2FA (#159).
- **Detected:** 2026-07-31

## Regex Multi-Term Log Search (AND/OR Combinator + Invert)
- **Source:** Komodo
- **Description:** `SearchContainerLog` / `SearchDeploymentLog` / `SearchStackLog` (`client/core/rs/src/api/read/docker.rs:291-333`, mirrored for stacks/deployments) search logs with **multiple terms** plus a `SearchCombinator` (`All` = every term must match a line, `Any` = one or more), an `invert` flag (return lines that do NOT match), and `timestamps` toggle. Results come back as a `Log` with matched-line highlighting.
- **Why add to Tengiz:** Tengiz `logs --grep` (implemented) supports a single pattern. Debugging needs compound queries: "lines containing ERROR AND payment" or "all lines EXCEPT health-check pings" (`--invert`) to see the real error in a noisy stream. Extends the existing `--grep` flag with `--and/--or`, `--invert`, and multiple patterns. Complements Log Filtering (#17) and Log Drains (#84) with an interactive debugging primitive. Low effort: extend the log scanner in `internal/cli/logs.go` with term+combinator matching.
- **Detected:** 2026-07-31

## Git-Commit-Based Stack Update Detection (DeployStackIfChanged / CheckStackForUpdate)
- **Source:** Komodo
- **Description:** Stacks track `deployed_commit_hash`/`deployed_commit_message` vs `latest_commit_hash`/`latest_commit_message` (`client/core/rs/src/entities/stack.rs:277-305`) plus the deployed vs remote compose file contents. `DeployStackIfChanged` (execute op, `api/execute/stack.rs:96`) redeploys **only when the git commit or compose content changed**; `CheckStackForUpdate` / `BatchCheckStackForUpdate` (`api/write/stack.rs:231-305`) report update availability, and deployments mirror this with `CheckDeploymentForUpdate` (with `skip_auto_update` and `wait_for_auto_update` controls).
- **Why add to Tengiz:** This is git-source-based update detection, complementary to Image Digest Change Detection (#73, image-based). For Tengiz's gitdeploy pipeline it answers "is there a new commit I should deploy?" without a webhook or cron rebuild — a manual `tengiz stack check-update` or scheduled sweep (`monitor` + notify) surfaces drift before it becomes a prod incident. `DeployStackIfChanged` also gives idempotent deploy (skip when nothing changed), saving build cycles on re-runs. Complements Scheduled Deployments (#42) and Git-Sync (#113). Low-medium effort: compare `deployed_commit` in the deploy record vs `git rev-parse HEAD` in the cached repo.
- **Detected:** 2026-07-31

## Config Change History with Diff (prev/current TOML in Update Records)
- **Source:** Komodo
- **Description:** Every operation produces an `Update` record (`client/core/rs/src/entities/update.rs`); for config-update operations the record stores `prev_toml` and `current_toml` (`update.rs:76-81`), giving a before/after snapshot of the resource config. The UI renders this as a config diff. `operator` is attributed (user id, or sentinel values `Procedure` / `Github` / `Auto Redeploy` for automated runs), with `version`, `commit_hash`, and `other_data`.
- **Why add to Tengiz:** Event Logging & Audit Trail (#7) is recorded conceptually but without a diffable before/after config snapshot. For a multi-developer team, "who changed what in this app's config, and when" with a visible diff is the audit primitive that makes config mistakes debuggable ("this domain was removed on Tuesday"). Tengiz's `~/.tengiz/apps.json` already snapshots per-app state — recording `prev_toml`/`current_toml` on each config-mutating CLI op and exposing `tengiz app history <app>` / `tengiz config diff <app> <op-id>` closes the loop. Low effort: persist the pre/post `AppEntry` JSON on write ops. Complements Config Diff & Impact (#1646, pre-deploy) with a post-change audit trail.
- **Detected:** 2026-07-31

## GitHub Actions OIDC Token Authentication (Tokenless CI/CD)
- **Source:** Juno
- **Description:** Juno's satellite supports CI/CD authentication without pre-shared static tokens. When a GitHub Actions workflow runs, GitHub issues an OIDC `id_token` (subject `repo:owner/repo:ref:refs/heads/main`) signed by GitHub's JWKS. The platform (`src/libs/auth/src/openid/credentials/automation/`, `src/libs/satellite/src/automation/`) fetches and caches the JWKS, verifies the JWT's issuer/audience, validates the repository/ref against registered automation rules, then mints a short-lived, scoped access key (`Write`/`Submit`) with expiry. Each consumed token's `jti` is persisted (`#automation-token` collection) to prevent JWT replay. Workflow metadata (run_id, run_number, run_attempt, sha, actor, workflow, event_name) is upserted per `provider#repository#run_id` as an audit trail read back by the console. A template generator produces ready-to-paste GitHub Actions YAML for deploy/publish across npm/yarn/pnpm (`src/frontend/src/lib/templates/actions-*.yml`).
- **Why add to Tengiz:** Recorded CI/CD features (#16 App Deploy Tokens, #17 Headless CI/CD Mode) rely on static pre-shared tokens — rotation, leakage, and per-user credential sprawl. OIDC tokenless auth is strictly better for GitHub Actions: the workflow presents a short-lived JWT bound to the exact repo/ref, Tengiz verifies it via JWKS, and no secret is ever stored in CI settings. Implementation in Go is a small OIDC verifier (`github.com/coreos/go-oidc`) in the webhook server: CI's `tengiz deploy` sends the `actions/oidc-token`; `.tengiz.yaml`'da `automation.github: {allowed_repos: [owner/repo], refs: [refs/heads/main]}`. Replay protection via a jti store, workflow audit trail in the deployment record, and a `tengiz automation yaml` generator for the workflow file. Medium effort, high security value — removes an entire class of credential-management incidents.
- **Detected:** 2026-07-31

## Built-in Auth User Directory & Administration (Users, Quotas, Banning, Passkeys)
- **Source:** Juno
- **Description:** Juno's satellite keeps a first-class registered-user directory. Every user who authenticates is stored as a doc in the `#user` system collection keyed by self-authenticating principal (`src/libs/satellite/src/user/core/`), with rich provider data (OIDC email/name/picture, or WebAuthn AAGUID). Controllers get an administration layer: list/filter/paginate users, view profile + provider + device info, delete, and ban/unban (`BannedReason::Indefinite`) — banned users are rejected at the assert layer on all datastore/storage access while controllers can still clean up their records. A per-user per-collection usage tracker (`#user-usage`, `src/libs/satellite/src/user/usage/`) counts every create/set/delete and enforces `max_changes_per_user` quotas inside the same write transaction (race-safe). WebAuthn credentials are registered per user (`#user-webauthn` + `#user-webauthn-index`) with an AAGUID→"friendly device name" mapping and per-credential usage counters.
- **Why add to Tengiz:** The recorded Built-in Auth Service covers login flows, but not "who are my users". Once apps use Tengiz auth, operators need an admin layer on a shared platform: `tengiz user list <app>`, `tengiz user ban <app> <email>`, quotas so one user can't exhaust datastore capacity, and visibility into sign-in methods/devices. The per-user change quota maps directly to Tengiz's planned NoSQL datastore and is the multi-tenant abuse control that rate limiting alone doesn't provide. For Tengiz: users stored in `~/.tengiz/auth/users-<app>.json` (or the datastore), `max_changes_per_user` in collection rules, ban flag enforced by the proxy's `/__tengiz/` middleware. Low-medium effort on top of the auth service, high value for real deployments.
- **Detected:** 2026-07-31

## Serverless Function Runtime Capabilities (Outbound HTTP + Platform Service SDK)
- **Source:** Juno
- **Description:** Juno's Sputnik embedded JS runtime exposes a full server-side SDK and outbound capabilities to serverless functions. The `ic_cdk` API surface provides `call_raw` (inter-canister calls) and `http_request` (HTTPS outcalls) with BigInt/principal helpers (`src/sputnik/src/js/apis/`), so functions can reach external HTTP services. A purpose-built server SDK (`src/sputnik/src/sdk/js/`) lets functions perform datastore CRUD (`DB.count/set/get/list/delete`), storage asset operations with chunked content, `setAssetToken`, access-key management, and `callerHasWritePermission` guards. Arguments and results cross the Rust↔JS boundary as JSON via a `JsonData` derive (`src/libs/macros/src/functions/derive.rs`), and each invocation gets a fresh QuickJS runtime — no cross-request state leakage.
- **Why add to Tengiz:** The recorded Embedded Serverless Functions Runtime (#34) establishes the goja runtime but not the *capability surface* that makes functions useful. Functions that can't reach the outside world or the platform's own datastore are toy functions. Adding outbound HTTP (Go `net/http` client in the runner) and a server-side SDK (datastore/storage/auth calls with the caller's permission context, mirroring Juno's `callerHasWritePermission`) makes Tengiz functions real: webhooks that call Stripe, form handlers that write to the datastore, image transforms fetching remote assets. Fresh-runtime-per-invocation isolation and typed args/results (JSON via `encoding/json`) are low effort in Go and important for safety. Medium effort, directly extends #34.
- **Detected:** 2026-07-31

## Analytics Collection Toggles & Restricted-Origin Ingestion (Privacy/Ops Layer)
- **Source:** Juno
- **Description:** Juno's Orbiter analytics enforces per-satellite feature flags and origin isolation on the ingestion API. Each satellite config carries `OrbiterSatelliteFeatures` booleans — `page_views`, `track_events`, `performance_metrics` — and every write is gated on the enabled flags (`src/orbiter/src/assert/config.rs`); disabled features are rejected rather than stored. A per-satellite `restricted_origin` is applied dynamically as the `Access-Control-Allow-Origin` response header, giving multi-tenant CORS isolation. All ingestion responses set security headers (HSTS, X-Content-Type-Options, Referrer-Policy, `cache-control: no-store`). Ingestion endpoints support batch operations with per-item error reporting (`src/orbiter/src/api/`), so a partially-failing batch reports exactly which records failed instead of aborting wholesale.
- **Why add to Tengiz:** The recorded Built-in Platform Analytics (#91) describes collection but not the compliance/ops controls. Apps differ wildly in what they may track (GDPR), so per-app feature toggles are table stakes: `tengiz analytics enable <app> --features page_views,performance_metrics`. Origin restriction prevents other sites from POSTing forged analytics and isolates multi-tenant traffic — the proxy sets the ACAO header from the app's own domains. Batch ingestion with per-item errors keeps high-volume telemetry reliable. Low-medium effort on top of #91, meaningful for production privacy compliance.
- **Detected:** 2026-07-31

## Upgrade Dock / Release Management Guardrails (Sequential Versions + Auto-Snapshot + Changelog Review)
- **Source:** Juno
- **Description:** Juno's console has a dedicated "Upgrade Dock" (`src/frontend/src/routes/(split)/upgrade-dock/`, `CanisterUpgradeWizard.svelte`) for platform release management. It lists every module with its current version vs. latest release (fetched from a release registry worker, including dev/next channels), each with a changelog. The upgrade wizard steps through: select version → review release changes → confirm (with an **auto-snapshot option** that snapshots the canister before upgrading) → download/upgrade → restart. It enforces **sequential upgrades** (next compatible version first), warns when serverless-function customizations would be lost on revert, and tracks upgrade history per module.
- **Why add to Tengiz:** Recorded Self-Upgrade (#165) is a bare `tengiz upgrade` that swaps the binary. Real deployments need guardrails around that: check for new versions against GitHub Releases, review the changelog before acting, refuse to skip intermediate versions, snapshot `~/.tengiz/` state before upgrading (reusing the backup/snapshot infrastructure), and warn when a revert would lose function deployments. `tengiz upgrade check` (no-op, prints versions+changelog), `tengiz upgrade --to <version>` with an auto-snapshot, and upgrade history in the audit log. Low effort on top of #165, high operational safety value for a self-hosted control plane.
- **Detected:** 2026-07-31

## One-Time Invitation Codes (Gated Platform Onboarding)
- **Source:** Juno
- **Description:** Juno's console gates signup with admin-issued, one-time invitation codes (`src/console/src/api/invitation_code.rs`, `src/console/src/store/heap/invitation_codes.rs`). An admin adds codes (`add_invitation_code`, `list_invitation_codes`); a prospective user redeems a code (`redeem_invitation_code`), which binds it to their user_id and makes it non-reusable. A simple yet effective closed-beta / controlled-access mechanism.
- **Why add to Tengiz:** Recorded Multi-Team invitations (Coolify) are per-team member invites inside an already-running platform. Juno's codes gate *platform access itself* — the closed-beta and capacity-control story for a self-hosted PaaS serving several companies: `tengiz invite create` prints codes, `tengiz invite list`, and account creation requires a valid code. In a CLI-first tool this doubles as the auth token for first-time `tengiz` login on a shared server (bind code → user identity → scoped access). Very low effort (a codes table in `~/.tengiz/invites.json` + a check in account creation), useful for controlled rollouts.
- **Detected:** 2026-07-31

## Optimistic Concurrency Control for Config Updates (Versioned CAS)
- **Source:** Juno
- **Description:** Juno's Orbiter stores satellite configs with an explicit `version` field and `assert_version`/`next_version` semantics (`src/orbiter/src/config/store.rs`). A config write must present the version it read; if another client already bumped the version, the write fails on the stale version rather than silently overwriting — a compare-and-swap (CAS) protocol that prevents lost updates.
- **Why add to Tengiz:** Recorded Concurrency Control (#101) is a file-based mutex preventing two operations from corrupting state files *simultaneously*. Versioned CAS solves a different problem: *stale writes*. In a team running `tengiz config set` from two shells (or via webhook + CLI), the last writer silently clobbers the other's change. Adding a `version` field to `AppEntry` and rejecting writes with a stale version (`"config version 4 != current 5"`), with `--force` to override, gives operators a clear stale-write signal. Very low effort (one int field + a check in the store), directly protects the multi-developer workflow the platform is targeting.
- **Detected:** 2026-07-31
