...
## Deployment Queue (Per-App Serialization for Deploy Pipeline)
- **Source:** Coolify
- **Description:** Coolify's `ApplicationDeploymentQueue` serializes deployment requests per app. If a deploy is already running, subsequent requests are queued and executed sequentially after the current one completes — not rejected (unlike a lock) and not deduplicated (unlike a build queue). Covers full pipeline (clone → build → push → deploy → healthcheck). Queue status: `queued`, `running`, `success`, `failure`. Newer queued requests replace older queued ones for same app (last-one-wins dedup within queue).
- **Why add to Tengiz:** Existing features address concurrency differently: Deploy Lock (#15, Kamal) rejects concurrent deploys, Build Queue with Dedup (#124, CapRover) covers only build phase. Neither serializes the full deploy pipeline. A deployment queue enables "deploy three times rapidly → all run sequentially" without manual retry. Critical for CI/CD with rapid-fire commits (GitHub Actions retries, quick pushes). Implementation: per-app channel-based queue in `internal/deploy/queue.go` with goroutine per app processing deployments sequentially. Queue state persisted in `~/.tengiz/queue.json`. Complements Deploy Lock (#15) and Build Tracking (#94).
- **Detected:** 2026-07-23

## Server-Level Resource Threshold Alerting with Automated Actions
- **Source:** Coolify
- **Description:** Coolify's `ServerStorageCheckJob` and `ServerLimitCheckJob` monitor host-level resources (disk, memory, CPU) at configurable intervals. When thresholds are crossed, automated actions trigger: disk >90% → Docker cleanup + notification, memory >85% → restart unhealthy containers. Tracks the Docker host itself, not individual containers. Configurable warning/critical levels with composable actions (cleanup, restart, notify, custom command).
- **Why add to Tengiz:** Existing monitoring focuses on per-container metrics. Server-level monitoring fills a gap: an app's resource growth can crowd out other containers on the same host. `tengiz monitor.thresholds.disk: { warning: 80%, critical: 90%, action: docker-cleanup }` in `.tengiz.yaml`. Implementation: `time.Ticker` in `internal/monitor/host.go` calling `df`, `free`, `uptime` with threshold comparison. Actions fire via the existing event/notification system. Distinct from Container Real-Time Metrics (#41, live per-container) — this is host-level and threshold-driven with automated action.
- **Detected:** 2026-07-23

## Docker Swarm Multi-Node Cluster Orchestration
- **Source:** Dokploy
- **Description:** Dokploy manages multi-node clusters using Docker Swarm mode (`services/cluster.ts`). Apps are deployed as Swarm services (not standalone containers), enabling native service discovery, built-in load balancing across nodes, rolling updates with health-check gating, and node-level high availability. Swarm stack support via `docker stack deploy` for Compose-based deployments. Node management: `docker node ls/promote/drain/rm`. Network overlay for multi-host communication.
- **Why add to Tengiz:** Existing features for multi-server (SSH Remote Deployment #105, Kamal; Multi-Server Architecture #194, Komodo) connect to individual Docker daemons — they don't provide HA, service discovery, or rolling updates. Docker Swarm is lighter than K3s (#199, Dokku), built into Docker Engine, and provides production-grade orchestration without Kubernetes complexity. `tengiz swarm init/join/leave` commands manage the cluster; `tengiz deploy --mode swarm` deploys as a replicated service. Service discovery via Swarm's internal DNS means apps find each other by service name. Rolling updates with health check gating prevent bad deploys from taking down the entire cluster. Works with Tengiz's existing `os/exec` Docker CLI pattern. Composes with existing compose lifecycle management — stacks deploy as Swarm stacks via `docker stack deploy`. Medium-high effort but adds a major capability that differentiates from Coolify/Dokku single-server model.
- **Detected:** 2026-07-23

## Threshold-Based Monitoring with External Callback Alerts
- **Source:** Dokploy
- **Description:** Dokploy's monitoring service (`apps/monitoring/`, a standalone Go app) collects real-time server metrics (CPU %, memory %, disk usage, network I/O) and per-container metrics (CPU, memory with percentage/used/total, network, block I/O) at configurable refresh rates (default 25s). Metrics stored with configurable retention days. When CPU or memory thresholds are crossed, sends a structured notification to an external callback URL (HTTP POST with Type, Value, Threshold, Message, Timestamp, Token). Supports `include`/`exclude` container name filtering. Cron-based daily cleanup of old metrics.
- **Why add to Tengiz:** Existing features cover Prometheus Metrics (#26, Kamal, pull-based), System Stats Recording (#72, Komodo, historical), and Container Real-Time Metrics (#41, Coolify, live). None combine: (1) configurable thresholds, (2) push-based external callback, (3) retention-managed metric storage, and (4) per-container include/exclude filtering — all in one system. Dokploy's monitoring model is designed for single-server setups with external alert routing, which is exactly Tengiz's target architecture. Implementation: a `tengiz monitoring` command group that starts a metric collection goroutine in the proxy process. `tengiz monitor threshold set --metric cpu --warning 80 --critical 95` configures alerting. Callback URL receives POST alerts that can route to PagerDuty, Slack, or custom endpoints. `tengiz monitor metrics <app>` shows latest container metrics. Low-medium effort since Go already has `docker stats` parsing, `gopsutil` for host metrics, and `net/http` for callbacks. Complements existing event/notification system.
- **Detected:** 2026-07-23

## Server Onboarding Validation & Audit Trail
- **Source:** Dokploy
- **Description:** Dokploy has structured server registration (`services/server.ts`, `setup/server-setup.ts`, `setup/server-validate.ts`) that validates: Docker version meets minimum requirement, OS compatibility (Linux distro, kernel version), port availability (required ports 80, 443, 3000), disk space above threshold, architecture support (amd64/arm64), and system resource adequacy (RAM > 512MB). After validation, generates a server audit entry (`setup/server-audit.ts`) recording: server ID, hostname, IP, Docker version, OS details, validation timestamp, and any warnings. Servers failing validation are rejected with specific error messages and remediation steps.
- **Why add to Tengiz:** Existing `tengiz doctor` (#114, CapRover) checks system prerequisites but is a one-time diagnostic, not a structured onboarding workflow. For multi-server deployments (Swarm or SSH-based), a validated onboarding process prevents: incompatible Docker versions causing deploy failures, low-disk servers blocking image pulls, ARM/AMD64 architecture mismatches causing runtime errors. `tengiz server add <host>` runs validation before registering the server. `tengiz server list` shows audit history per server. `tengiz server validate <host>` re-runs validation on an existing server. Implementation: `internal/server/validate.go` with pluggable checks (Docker ping, version parsing, disk stat, arch detection), results stored in `~/.tengiz/servers/<id>/audit.json`. Complements Server Bootstrap (#31, Kamal) which installs Docker — this validates after installation.
- **Detected:** 2026-07-23

## Traefik Dynamic Configuration with Hot-Reload
- **Source:** Dokploy
- **Description:** Dokploy uses Traefik as its reverse proxy with fully dynamic configuration (`setup/traefik-setup.ts`, ~950 lines). When apps, domains, SSL certs, middlewares (security headers, IP allow/deny, rate limiting), redirect rules, or forward auth configs change, Traefik's dynamic config is updated via file provider or API — no restart required. Middleware chains are composed dynamically per route. SSL certificates are auto-obtained via Let's Encrypt and stored as Traefik certificate resolvers. Dashboard enabled at configurable path with auth.
- **Why add to Tengiz:** Existing "Pluggable Reverse Proxy Backend" (#63, Dokku) covers switching proxy backends. "Gelişmiş Proxy Konfigürasyonu" (#76, Kamal) covers per-app config. Neither addresses live config reload — currently Tengiz requires a proxy restart for any routing change, causing traffic interruption. Adding hot-reload to Tengiz's Go proxy (or integrating Traefik) means: domain add/remove takes effect immediately, SSL cert renewal is seamless, middleware changes don't drop connections. Implementation for the built-in Go proxy: use `sync.RWMutex`-protected route/middleware maps, reloaded atomically on config change events. `sigs.k8s.io/yaml` watches the config file for changes and swaps maps without stopping the HTTP server. Medium effort, high operational value — eliminates restart-related downtime from config changes.
- **Detected:** 2026-07-23

## Forward Auth Integration (External Authentication Gate)
- **Source:** Dokploy
- **Description:** Dokploy implements Traefik forward auth (`services/proprietary/forward-auth.ts`, `setup/forward-auth-setup.ts`) — an external authentication service that intercepts all requests before they reach the app. The auth service validates session tokens, cookies, or JWTs, and either allows the request through (with injected headers for user identity) or returns 401/302 redirect to login. Supports: OIDC/OAuth providers (Google, GitHub, GitLab), session-based auth with cookies, JWT validation, configurable auth exclusion paths (public routes), and header injection for backend services to identify authenticated users.
- **Why add to Tengiz:** Existing "HTTP Basic Auth (Staging Protection)" (#32, CapRover) provides simple username/password gates. "Built-in Authentication Service" (#116, Juno-style) provides platform-level auth-as-a-service. Forward auth fills a different need: an authentication gate at the proxy level that authenticates requests BEFORE they reach any app, with no app code changes required. This enables: (1) add SSO to legacy apps without auth support, (2) centralize auth across multiple apps behind Tengiz, (3) inject authenticated user identity into app via headers (`X-Forwarded-User`, `X-Forwarded-Email`). `.tengiz.yaml`'da `auth.forward:` bölümü: provider, client ID, client secret, session secret, public paths. Implementation: Go middleware in the proxy that checks for a valid session cookie/JWT, redirects to OIDC provider if missing, and injects user headers on success. Standalone auth endpoint at `/__tengiz/auth/` for the OIDC callback flow. Medium effort, high value for teams that want to add auth to existing apps without modifying them. Complements both basic auth (simpler case for staging) and built-in auth (for new apps that want platform-managed auth).
- **Detected:** 2026-07-23

## libsql/SQLite Edge Database Support (Serverless Database)
- **Source:** Dokploy
- **Description:** Dokploy supports libsql (`services/libsql.ts`) as a managed database type alongside Postgres, MySQL, MongoDB, Redis, and MariaDB. libsql is an open-source, SQLite-compatible database designed for edge/serverless deployments with: low latency (no separate server process), high concurrency via WAL mode, HTTP-based protocol (optional), bi-directional replication to Turso's cloud platform, and SQL syntax compatibility with SQLite. Deployment runs as a Docker container with persistent volume, exposed port, and health checks.
- **Why add to Tengiz:** Tengiz's scale-to-zero model makes libsql an ideal database choice. Unlike Postgres/MySQL (which need a running server process and take seconds to cold-start), SQLite/libsql is file-based — opens in milliseconds, needs no background process, and data persists on disk. A `tengiz db create libsql --name mydb` provisions a libsql container with persistent volume. Apps connect via SQLite client library or libsql HTTP protocol. For the embedded NoSQL Datastore (#1, Juno-style) that uses SQLite internally, libsql provides the user-facing database equivalent — the same engine, user-managed. Implementation: Docker container with `ghcr.io/tursodatabase/libsql-server` image, auto-generated connection string, persistent volume mount. `resources.databases.libsql: true` in `.tengiz.yaml` auto-provisions alongside the app. Complements Managed Database Provisioning (#63, Coolify) which covers Postgres/MySQL/Redis/Mongo — libsql fills the edge/serverless gap. Low effort (single Docker image, standard volume+port pattern), high value as the database best suited to scale-to-zero architectures.
- **Detected:** 2026-07-23

---

## Plugin System (Extensible Architecture via Lifecycle Hooks)
- **Source:** Dokku
- **Description:** Everything in Dokku is a plugin — core features (git, domains, ps, proxy, certs) and community extensions use the exact same mechanism: directories of scripts triggered by lifecycle hooks via `plugn`. Plugins can be written in any language (Bash, Go, Python, Ruby). 40+ trigger points across the app lifecycle (`pre-deploy`, `post-deploy`, `post-stop`, `post-config-set`, `post-scale`, etc.). 100+ community plugins for databases, SSL, monitoring, CI/CD, and more. `dokku plugin:install <url>` installs from any git URL. Plugin trigger functions receive JSON context on stdin with app name, phase, and phase-specific data. Plugins declare dependencies, version constraints, and scheduling phases in `plugin.toml`.
- **Why add to Tengiz:** Tengiz has no extension mechanism — every feature must be added to core, requiring forking or PRs. A plugin system enables: community ecosystem (users build and share plugins without modifying Tengiz), third-party integration without core changes, per-site custom logic without code forks, and a platform for advanced users to extend Tengiz arbitrarily. Implementation: define trigger points at key lifecycle phases (deploy, config change, domain add, container start/stop, proxy update, health check result). At each trigger point, Tengiz scans `~/.tengiz/plugins/<name>/` for executable files named by trigger name and runs them with JSON context on stdin. `tengiz plugin install gh:user/repo` clones the repo into the plugins directory. Each trigger script receives structured JSON (app name, environment, event-specific data) and returns exit code (0=success, non-zero=failure with error message). The existing hook system (#12, #24, #28) provides the foundation — this generalizes it into a discoverable, installable ecosystem with a standard interface. High effort but transformative: it's the architectural feature that made Dokku's ecosystem successful.
- **Detected:** 2026-07-23

## APT System Dependencies (Project-Level apt-packages File)
- **Source:** Dokku
- **Description:** Dokku's `dokku-apt` plugin reads `apt-packages` and `apt-repositories` files from the project root during build. Packages are installed via `apt-get` in the build container before the app build runs. Supports: `apt-packages` (package list, one per line), `apt-repositories` (extra APT repos), `apt-keys` (repository signing keys via URL), `apt-sources-list` (full override of sources.list), `apt-preferences` (APT pinning config), `apt-debconf` (pre-seed debconf answers for EULA acceptance), `dpkg-packages` directory (local `.deb` files to install). APT repositories and packages automatically trigger `apt-get update`. The build image is cached to skip repeated installs if files haven't changed.
- **Why add to Tengiz:** Many apps need system packages that can't be installed via language-level package managers: `nginx` for reverse proxy within the app, `imagemagick` or `ffmpeg` for media processing, `libxml2-dev` for Ruby Nokogiri, `unifont` for PDF generation, `ca-certificates` for custom CAs. Currently users must write a custom Dockerfile just to add these. A `apt-packages` file in the project root is zero-config: place the file, next build installs packages automatically. Implementation: during build phase, check for `apt-packages` in the source directory, prepend `RUN apt-get update && apt-get install -y <packages>` to the generated Dockerfile. Cache-bust using file content hash. `.tengiz.yaml`'da `apt.packages:` listesi alternatifi. Low-medium effort, eliminates the most common reason users need custom Dockerfiles.
- **Detected:** 2026-07-23

## Copy Files to Image (Build-Time Asset Injection)
- **Source:** Dokku
- **Description:** Dokku's `dokku-copyfiles-to-image` plugin copies additional files from the host or project into the Docker image during the build phase. Files are specified as source/destination pairs. Useful for: injecting environment-specific configuration files without modifying the repo, adding SSL certificates or CA bundles for internal services, including host-specific runtime config (datadog agent config, log shipping config, monitoring agent config). Files are copied after the buildpack/build step completes, so they can override generated files. Supports directory copy with recursive flag.
- **Why add to Tengiz:** Existing Patches (#63, Dokploy) covers file overrides at build time, and Build-Time Secrets (#139, Kamal) covers secret injection. Neither explicitly handles copying arbitrary files from the Tengiz host filesystem into the image — which is a different use case from either source-level patching or secret injection. Use case: the operator's server has a custom CA cert at `/etc/ssl/certs/internal-ca.crt` that the app needs to trust. Or a monitoring agent config at `/etc/datadog-agent/datadog.yaml` that should be baked into the image. Or a `nginx.conf` template that references internal service names resolved differently per-host. `.tengiz.yaml`'da `copy_files: [{source: "/etc/tengiz/myapp/nginx.conf", dest: "/etc/nginx/nginx.conf"}]`. Implementation: `COPY` instruction in the generated Dockerfile or `docker cp` after build. Low effort, fills a gap between source-level patches (which modify repo files) and build-time secrets (which are runtime env vars).
- **Detected:** 2026-07-23

## ACL-Based User Access Control (Per-User App Permissions)
- **Source:** Dokku
- **Description:** Dokku-community's `dokku-acl` plugin implements per-user and per-user-group access control over app operations. Users are identified by their SSH key fingerprint. Permissions are granular: `deploy`, `logs`, `run`, `config`, `domains`, `proxy`, `scale`, `enter`. ACL entries are app-scoped or global (all apps). Commands: `acl:add-user <app> <key-name>`, `acl:remove-user <app> <key-name>`, `acl:add-global <key-name>`, `acl:list`. Unauthorized operations return a permission denied error. Global permissions allow admin-level access without per-app configuration. The `dokku-access` plugin provides a simpler variant focused on per-app deploy access.
- **Why add to Tengiz:** Existing features cover API-level auth (SSO #128, API keys #76, RBAC groups #140) but assume CLI access means full access. Tengiz currently has no user concept at the CLI level — anyone with the binary and server access can deploy to any app. For team deployments (shared server), ACLs provide: per-developer access (Alice can deploy `myapp`, Bob can only view logs), protection against accidental cross-app operations, and audit trail with real user identities. Implementation: SSH key management (#99, already detected) provides user identity. ACLs add a permission layer: `tengiz acl add myapp --user alice --permission deploy`. Permissions stored in `~/.tengiz/acl.json` as `{app: {user: [permissions]}}`. CLI commands check permissions before executing. Complements RBAC Groups (#140, Komodo) — ACLs are simpler and more granular (per-app, per-user). Groups cover role-based access for larger teams.
- **Detected:** 2026-07-23

## UFW Firewall Management (Host Firewall Integration)
- **Source:** Dokku
- **Description:** Dokku-community's `dokku-ufw` plugin manages UFW (Uncomplicated Firewall) rules directly from Dokku commands: `ufw:enable/disable`, `ufw:allow/deny <port>[/protocol]`, `ufw:allow/deny-from <ip> <port>`, `ufw:status`, `ufw:logging`. Rules are app-aware: when an app is deployed, the plugin auto-adds UFW rules for the app's assigned port. When an app is removed, rules are cleaned up. Supports IPv4/IPv6, rate limiting, and custom rule descriptions tagged with app name for audit.
- **Why add to Tengiz:** Existing Server Security Hardening (#181, Coolify) mentions UFW setup during `tengiz server init` but doesn't manage ongoing rules per app. When Tengiz auto-assigns ports from the 9000-9999 range, those ports should be UFW-closed by default — only the proxy port (8080) should be open. If a user adds a custom port mapping for direct container access, UFW should be auto-configured. Implementation: `tengiz ufw allow 9001/tcp --app myapp` integrates with UFW CLI via `os/exec`. A post-deploy hook reads the app's port mapping and configures UFW accordingly. `tengiz server init --secure` enables UFW with a default-deny policy except for SSH, HTTP, HTTPS, and the Tengiz proxy port. Low effort (UFW is a thin wrapper around iptables, CLI commands are simple), high security value — the most common server hardening task.
- **Detected:** 2026-07-23

## Monit Process Monitoring (Host-Level Process Supervision)
- **Source:** Dokku
- **Description:** Dokku-community's `dokku-monit` plugin integrates Monit (system process supervisor) for host-level monitoring of: Docker daemon health, Tengiz process uptime, system resource thresholds (CPU, memory, disk), container crash detection beyond Docker's restart policy. Monit configuration is auto-generated per-app and per-service. Alerting via Monit's built-in email/SMTP. Monit status available via `monit status` and optional web interface.
- **Why add to Tengiz:** Existing health checks (#4) are container-level — Docker's restart policy handles container crashes. But what if Docker itself crashes? Or the Tengiz process? Or the system runs out of disk space? Monit provides host-level supervision: restart Docker if it stops, restart Tengiz if it crashes, alert on low disk space before Docker fails, and provide a second layer of supervision beyond Docker's restart policy. Implementation: `tengiz server init --monit` installs and configures Monit with auto-generated checks for: Docker daemon, Tengiz proxy, system resources (disk >90%, memory >90%, CPU load >5). `tengiz monit status` shows Monit dashboard. Complements Platform Self-Health Check (#157, CapRover) — Monit runs as a system-level supervisor (survives Tengiz crashes), self-health check runs in-process (faster but fails with Tengiz). Low effort (Monit config generation via Go templates + `apt-get install monit`), high reliability value for production deployments.
- **Detected:** 2026-07-23

## Auto-Sync (Periodic Remote Repository Polling)
- **Source:** Dokku
- **Description:** Dokku-community's `dokku-autosync` plugin periodically polls a remote git repository for changes. Unlike `git:sync` (one-time manual sync), autosync runs on a configurable interval (default 5 minutes), fetches the remote, compares HEAD commit SHA, and triggers a rebuild only when changes are detected. Supports: per-app interval configuration, branch-specific sync, email notification on sync failure, and opt-out for specific apps. Different from Scheduled Deployments (cron-based rebuild from source) — this polls a remote repo and rebuilds only on actual changes.
- **Why add to Tengiz:** Existing Git-Sync Deployment (#113, Dokku) covers one-time `git:sync`. Webhook-based deploy (#13, Coolify) covers push-triggered deploy. Neither covers pull-based deployment with periodic polling. Autosync enables: "deploy from this GitHub repo without setting up webhooks" (ideal for internal/sandbox environments), "keep staging in sync with main branch automatically", and "pull-based deploy behind firewalls that can't receive webhooks." Implementation: a goroutine per app in `internal/git/autosync.go` that runs `git ls-remote` at intervals, compares HEAD, triggers deploy on change. `.tengiz.yaml`'da `git.autosync.enabled: true`, `git.autosync.interval: 5m`, `git.autosync.branch: main`. Queue with dedup prevents overlapping syncs. Low-medium effort (glue between existing git and deploy pipelines), fills the gap between webhook and scheduled deployments.
- **Detected:** 2026-07-23

## Multi-Buildpack Support (Multiple Buildpacks Per App)
- **Source:** Dokku
- **Description:** Beyond single-buildpack detection, Dokku supports multiple buildpacks per app via a `.buildpacks` file or `BUILDPACK_URL` env var. Each buildpack runs sequentially during the build phase. Example `.buildpacks` file: `https://github.com/heroku/heroku-buildpack-nodejs.git\nhttps://github.com/heroku/heroku-buildpack-python.git`. Use cases: Node.js frontend + Python backend in the same deploy, adding the Heroku multi-buildpack for sidecar dependencies (e.g., nginx buildpack + app buildpack), or applying a buildpack for static asset compilation before the main app build.
- **Why add to Tengiz:** Existing Herokuish Buildpacks (#191, Dokku) covers single-buildpack detection. Many real-world apps need multiple buildpacks: a Node.js app with native C extensions (nodejs + apt), a Python app that needs nginx (python + nginx), or a Ruby app with Node dependencies for asset compilation (ruby + nodejs). `.tengiz.yaml`'da `buildpacks: [heroku/nodejs, heroku/python]` enables multi-buildpack composition. Implementation: extend `builder/herokuish.go` to iterate over buildpacks in order, each writing to the same image. Low-medium effort on top of single-buildpack support, enables complex app configurations that were previously Dockerfile-only.
- **Detected:** 2026-07-23

## Pre-Deploy JavaScript Functions (Runtime Build Pipeline Modification)
- **Source:** CapRover
- **Description:** Beyond shell-based pre-deploy hooks, CapRover supports a `preDeployFunction` property in the app definition that contains JavaScript code. This JavaScript executes inside the platform process during the build pipeline and can modify Docker service update objects at runtime — enabling dynamic deployment configuration based on environment state (node availability, resource pressure, time-of-day). The function receives the full Docker service spec as a JSON object and returns a modified version. Use cases: dynamically add placement constraints based on node labels, inject environment-specific volumes based on the deployment target, modify resource limits based on time-of-day traffic patterns, or conditionally skip health checks during maintenance windows.
- **Why add to Tengiz:** Tengiz's existing pre-deploy hooks (#15) run shell commands before deploy — they're pipeline gates, not configuration transformers. Pre-deploy JS functions extend the concept: JavaScript runs inside Tengiz's process (not as a subprocess), can inspect and modify the Docker run config before it's applied, and has full access to the platform's state. Implementation: embed `goja` (pure Go JS runtime) in the deploy pipeline. `.tengiz.yaml`'da `pre_deploy_function: "function(spec) { if (context.env === 'production') spec.Memory = 1024; return spec; }"`. The function receives a well-defined spec object (image, cmd, env, ports, volumes, resources, restart policy, labels) and returns a modified copy. This is a power-user feature that makes Tengiz programmable without modifying the Go codebase. Complements Extended Hook System (#46) and TypeScript Action Automation (#142) — JS functions are lighter (no Deno runtime, no container), purpose-built for deploy-time config transformation. Medium effort (goja integration + spec schema definition), high value for advanced deployment workflows.
- **Detected:** 2026-07-23

## Service Update Override (Raw Docker Configuration Escape Hatch)
- **Source:** CapRover
- **Description:** CapRover's `serviceUpdateOverride` property lets expert users specify raw JSON/YAML Docker service update spec that overrides any generated configuration. This is an escape hatch for Docker configurations that CapRover's model doesn't expose: placement constraints, resource reservations, update parallelism, delay between updates, failure action, restart policy with max-attempts, network endpoint spec, and custom labels. The override is deep-merged with the generated spec — users only need to specify the fields they want to override. Complements the existing `customNginxConfig` for proxy-level overrides.
- **Why add to Tengiz:** No matter how many Docker options Tengiz exposes via its config model, there will always be edge cases. A service override escape hatch lets expert users specify ANY Docker `docker run` flag or `docker-compose.yml` property that Tengiz doesn't natively support. `.tengiz.yaml`'da `docker_override.run: { shm_size: "256m", sysctls: { net.core.somaxconn: "1024" }, cap_add: ["SYS_PTRACE"] }` veya `docker_override.compose: { deploy: { resources: { reservations: { memory: "512M" } } } }`. Implementation: a `map[string]interface{}` field in AppEntry that gets marshaled into the Docker CLI args or Compose config. The runtime package's `Run()` function merges these overrides before executing the Docker command. Low effort (map merge + flag generation), high value — prevents feature requests for every possible Docker flag. Complements Custom Docker Options (#36) by adding a raw override alongside structured options. Like CapRover's approach: structured config for common cases, raw override for edge cases.
- **Detected:** 2026-07-23

## Configuration Override File (Environment-Specific Config Overlays)
- **Source:** CapRover
- **Description:** CapRover supports `config-override.json` files at build-time and in the user-data directory. These JSON files can override any configuration value — server settings, registry URLs, default resource limits, proxy config — without modifying the main config. The override system is separate from the main data store: overrides are applied on top of defaults and written config. Similar to CapRover's env var override system (e.g., `CAPTAIN_IS_DEBUG`, `DEFAULT_PASSWORD`, `CAPTAIN_BASE_DIRECTORY`) which provides environment-level config without file changes.
- **Why add to Tengiz:** Tengiz already supports `.tengiz.{env}.yaml` environment overrides via `LoadForEnvironment`. A config override file extends this: `~/.tengiz/override.json` or `/etc/tengiz/override.json` can override any Tengiz setting without modifying the config files managed by tools like Ansible or Salt. Use cases: Docker-in-Docker installations need different volume paths, air-gapped environments need custom registry mirrors, CI/CD runners override the proxy port to avoid conflicts. Implementation: a second config layer loaded AFTER the main config merge, applied via `viper.MergeConfig`. The override file path is configurable via `TENGIZ_OVERRIDE` env var. Low effort (viper already supports merge), high operational value for managed installations.
- **Detected:** 2026-07-23

## Graceful Self-Restart (Platform Update Without Downtime)
- **Source:** CapRover
- **Description:** CapRover's main `CaptainManager` includes a `resetCaptain` method that updates the CapRover Captain Docker service specification, causing Docker Swarm to gracefully restart the container. The restart process: saves current state, signals the HTTP server to stop accepting new connections, drains active requests (configurable drain timeout), updates the Docker service with the new image tag, and Docker Swarm starts the new container while stopping the old one — zero-downtime self-update. All state is persisted in Docker volumes, so no data is lost.
- **Why add to Tengiz:** Existing Self-Upgrade (#165) covers binary download but not the actual restart. For Tengiz (Go binary, not a Docker container), graceful self-restart means: parent process spawns child with the new binary, child signals parent via SIGUSR1, parent completes in-flight requests (drain), parent exits, child continues serving. `tengiz upgrade --restart` or auto-restart after binary download. Use cases: zero-downtime Tengiz upgrades, crash recovery with bounded restart backoff, config reload without full restart (SIGHUP). Implementation: `internal/self/restart.go` using Go's `os/exec` self-exec pattern. The existing binary checks the `TENGIZ_UPGRADE` env var to detect restart mode. State (in-memory caches, proxy routing table, idle timers) is serialized to `~/.tengiz/state.json` before restart and reloaded after. Signal handling (SIGUSR1 = graceful restart, SIGHUP = config reload, SIGTERM = drain+stop) follows the standard Go server pattern. Medium effort, high production reliability value.
- **Detected:** 2026-07-23

## Cron Expression Validation (Schedule Format Verification)
- **Source:** CapRover
- **Description:** CapRover validates cron expressions before accepting them in scheduled task configurations. An invalid cron expression is rejected with a clear error message explaining the syntax issue. Validation covers: standard 5-field cron (minute hour day month weekday), optional 6-field with seconds, range and step syntax (`*/5`, `1-5`, `1,3,5`), month and weekday name abbreviations (`JAN`, `MON`), and special strings (`@yearly`, `@daily`, `@hourly`). Invalid expressions are caught at config-write time, not at execution time.
- **Why add to Tengiz:** Scheduled Tasks / Cron Jobs (#54) and Scheduled Deployments (#172) both accept cron expressions. Invalid expressions silently fail at execution time — a `* * * * *` vs `0 * * * *` typo can go unnoticed for days. Validation at config-set time catches errors immediately. Implementation: use `robfig/cron`'s built-in parser (the same library used for scheduling) in the config validation path. `tengiz config validate` and `tengiz cron validate "*/5 * * * *"` commands. The cron expression field in `.tengiz.yaml` is validated during `tengiz deploy` and `tengiz config set`. Low effort (existing library's parse function + error message formatting), high operational reliability value — prevents "cron job didn't run because expression was invalid" bugs.
- **Detected:** 2026-07-23

## Public IP Auto-Detection (Server Installation Helper)
- **Source:** CapRover
- **Description:** During installation (`captain installer`), CapRover automatically detects the server's public IP address using multiple methods: querying `ifconfig.co`, `checkip.amazonaws.com`, `api.ipify.org`, and parsing interface addresses. The detected IP is used for Docker Swarm advertisement address and display in the dashboard UI. Falls back to manual input if auto-detection fails.
- **Why add to Tengiz:** `tengiz server init` currently doesn't detect the server's public IP. Knowing the public IP is needed for: root domain setup, Let's Encrypt SSL validation, webhook configuration guidance, and displaying the server URL post-installation. Adding auto-detection improves the onboarding UX: `tengiz server init` detects the public IP, prints "Your Tengiz instance will be accessible at http://203.0.113.42:8080", and pre-fills the domain configuration. Implementation: Go's `net/http` calls to well-known IP echo services (with fallback chain), plus `net.InterfaceAddrs()` for local network display. No binary size increase (stdlib only). Low effort, high onboarding UX value.
- **Detected:** 2026-07-23

---

## Config Template Evaluation (Go Template Support for .tengiz.yaml)
- **Source:** Kamal
- **Description:** Kamal evaluates Ruby ERB templates in `deploy.yml`, enabling dynamic values from shell commands, env vars, and computed expressions (`<%= \`hostname\`.strip %>`, `<%= ENV["DOCKER_REGISTRY"] %>`). Tengiz could support Go `text/template` evaluation in `.tengiz.yaml`: `{{ env "DATABASE_URL" }}`, `{{ shell "git rev-parse HEAD" }}`, `{{ secret "DB_PASSWORD" }}`. Template evaluation runs during config load, AFTER environment-specific merge but BEFORE validation. Users get dynamic config without shell wrapper scripts.
- **Why add to Tengiz:** Eliminates the most common reason users write scripts around Tengiz. Dynamic config values for: registry URL (env-based), image tags (git SHA), resource limits (env-based per environment), domain names (computed from app name + env). Fits Tengiz's CLI-first model perfectly — users write `.tengiz.yaml` once with templates, deploy across environments without modification. Implementation: Go `text/template` with custom functions (`env`, `shell`, `secret`, `default`) in the config loading pipeline. Template functions are sandboxed (no arbitrary Go code execution). Complements Config Display Command (#48) — templates are shown as resolved values in the merged config output. Low-medium effort, high power-user value.
- **Detected:** 2026-07-23

## SSH Agent Forwarding During Docker Build
- **Source:** Kamal
- **Description:** Kamal forwards the local SSH agent socket during Docker builds via `builder.ssh: default=$SSH_AUTH_SOCK`. This exposes the SSH agent to the build context, allowing `RUN --mount=type=ssh` commands in the Dockerfile to clone private git repos, install private npm packages, or pull private Go modules during build — without embedding SSH keys in the image layers or using build secrets.
- **Why add to Tengiz:** Private dependency access during build is the #1 reason users need custom Dockerfiles with embedded SSH keys (a security anti-pattern). SSH agent forwarding is zero-config for developers (they already have SSH keys loaded), works with 1Password/Bitwarden SSH agents, and leaves no trace in the final image. Implementation: `docker build --ssh default=$SSH_AUTH_SOCK` flag in `builder.go`, configured via `.tengiz.yaml`'da `build.ssh_agent: true`. The user's `~/.ssh/id_ed25519` or ssh-agent socket is forwarded only during build. Low effort (one Docker flag), eliminates a major security concern with private dependency builds. Complements Build-Time Secrets (#139) for the SSH-specific use case.
- **Detected:** 2026-07-23

## Drain Timeout & Stop Timeout (Per-Operation Graceful Shutdown)
- **Source:** Kamal
- **Description:** Kamal provides configurable `drain_timeout` (default 30s) — how long to wait for in-flight requests to drain from old container before deploy returns, and `stop_timeout` — how long to wait for container stop after SIGTERM before sending SIGKILL. Separate from `readiness_delay` (container startup wait). These timeouts are per-operation: deploy waits for drain, maintenance mode waits for drain, container stop waits for graceful shutdown.
- **Why add to Tengiz:** Tengiz has a generic "deploy timeout" concept but doesn't differentiate between container startup wait, connection draining, and graceful shutdown. Different apps have different needs: a WebSocket-heavy app needs longer drain timeout (60s), a stateless Go API can drain in 1s, a database migration container needs longer stop timeout (120s). Without per-operation timeouts, operators face: premature SIGKILL killing in-flight requests, or unnecessarily long deploy times. Implementation: add `drain_timeout` and `stop_timeout` fields to `DeployConfig` in `.tengiz.yaml`. The proxy's traffic migration drains old connections for `drain_timeout`, and `runtime.Stop()` uses `stop_timeout` for `docker stop -t <timeout>`. Complements existing Readiness Delay feature with a complete timeout model. Low effort (Docker CLI already supports `-t` flag), high reliability value.
- **Detected:** 2026-07-23

## Build Provenance (Attestations) & SBOM Generation
- **Source:** Kamal
- **Description:** Kamal supports Docker BuildKit's build provenance and SBOM (Software Bill of Materials) features: `builder.provenance: mode=max` generates in-toto attestations (SLSA Level 2+), `builder.sbom: true` generates SPDX/Syft-format SBOMs attached to the image as attestation manifests. These are stored in the registry alongside the image, enabling verifiable supply chain metadata: who built it, with what config, what source code, what dependencies.
- **Why add to Tengiz:** Supply chain security is increasingly mandatory for enterprise deployments (EO 14028, SSDF, SLSA). Without provenance/SBOM, Tengiz deployments produce unverifiable images — users can't prove what code and dependencies are running in production. Implementation: `docker build --provenance=true --sbom=true` flags in `builder.go`. `.tengiz.yaml`'da `build.provenance: true` ve `build.sbom: true`. Attestations are stored in the registry alongside the image (compatible with Docker Hub, GHCR, any OCI-compliant registry). Low effort (Docker CLI already supports these flags since Buildx v0.10+). Complements Image Digest Pinning (#76) for a complete software supply chain story.
- **Detected:** 2026-07-23

## Docker Build Cloud Integration
- **Source:** Kamal
- **Description:** Kamal supports Docker Build Cloud (`builder.driver: cloud org-name/builder-name`) — Docker's managed build service that provides remote build infrastructure with native multi-architecture support (amd64 + arm64), large build cache, and no local resource consumption. Kamal users add one line to config and builds shift to Docker's cloud infrastructure automatically.
- **Why add to Tengiz:** Heavy builds (large monorepos, native dependencies, multi-architecture) consume significant local CPU/RAM, competing with production containers on single-server deployments. Docker Build Cloud offloads builds to Docker's infrastructure: no local build resource contention, native multi-arch (build arm64 images from an amd64 server), shared build cache across team members, and faster builds with cloud credentials caching. Implementation: `docker buildx create --driver cloud <name>` + `docker buildx use <name>` before build. `.tengiz.yaml`'da `build.driver: cloud` ve `build.cloud_builder: myorg/main`. Medium-low effort (buildx driver selection + credential check), high value for teams with complex builds. Complements Remote Docker Builder (#69) and Multi-Architecture Builds (#68) as additional build driver options.
- **Detected:** 2026-07-23

## Aliased Secrets (Environment Variable Name → Secret Name Mapping)
- **Source:** Kamal
- **Description:** Kamal supports aliased secret references: `DB_PASSWORD:MAIN_DB_PASSWORD` maps the environment variable `DB_PASSWORD` to the secret named `MAIN_DB_PASSWORD`. This allows different apps to use the same underlying secret with different env var names, or multiple env vars to reference different secrets from the same secret store. The alias syntax is `APP_SECRET:SECRET_NAME` — the env var gets the value of the named secret from the secrets file/password manager.
- **Why add to Tengiz:** In multi-app deployments, different apps may need the same secret value but with different env var names (e.g., `DATABASE_URL` vs `DB_URL`). Without aliasing, users duplicate secrets in the config. Also useful for: renaming env vars without changing secret values, mapping generic secrets (e.g., "stripe-api-key-prod") to app-specific env var names (`STRIPE_SECRET_KEY`), and sharing secrets across apps without duplication. Implementation: support `ENV_VAR:secret_name` syntax in `.tengiz.yaml`'s `env.secret` section. During deploy, resolve `secret_name` from the secrets store and assign to `ENV_VAR`. Low effort (string parsing + map lookup), simplifies multi-app config management. Complements Secrets Management (#32) and Secret Interpolation System (#54) with flexible naming.
- **Detected:** 2026-07-23

## Password Manager Secret Helpers (secrets fetch/extract CLI Commands)
- **Source:** Kamal
- **Description:** Kamal provides dedicated `kamal secrets fetch` and `kamal secrets extract` commands that integrate with password managers (1Password CLI, LastPass CLI, Bitwarden CLI, and custom commands). `kamal secrets fetch "op://vault/item/field"` outputs the secret value. `kamal secrets extract DB_PASSWORD $SECRETS` extracts a specific key from a JSON secret bundle. This is used in `.kamal/secrets` as: `KAMAL_REGISTRY_PASSWORD=$(kamal secrets fetch op://vault/docker/password)`.
- **Why add to Tengiz:** Existing Secrets Management (#32) describes vault integration conceptually but lacks the CLI workflow. The `secrets fetch/extract` pattern is elegant: it works with ANY CLI-based password manager via a simple stdout protocol, doesn't require a daemon or API server, and composes naturally with shell scripts and CI/CD pipelines. Implementation: `tengiz secrets fetch "<command>"` runs the command and captures stdout. `tengiz secrets extract <key> <json>` parses JSON. `.tengiz.yaml`'da `env.secret:` can reference these: `DATABASE_URL: "op://myvault/proddb/password"`. The fetch/extract pattern is simple but enables integration with 1Password, Bitwarden, Vault, pass, Doppler, and any custom CLI. Low effort (subcommand execution + JSON extraction), high security value — enables password manager integration without any library dependencies.
- **Detected:** 2026-07-23

## Per-Server-Tag Environment Variables
- **Source:** Kamal
- **Description:** Kamal supports per-tag environment variables via `env.tags`: servers can be tagged (`172.0.0.1: staging`, `172.0.0.2: [experiments, blue]`), and the `env.tags` section defines different env overrides per tag. A container running on a tagged server receives the tag-specific env values merged on top of the base env. Tags can be single strings or arrays — a server with `[blue, staging]` tags gets env from both tag configs.
- **Why add to Tengiz:** Multi-server deployments need env differentiation: staging servers should connect to staging databases, canary servers should use experimental feature flags, region-specific servers (US vs EU) need different API endpoints. Without per-tag env vars, operators must deploy separate app configurations per server group. Implementation: a `tags` field per server entry in `.tengiz.yaml`, and an `env.tags` section: `env.tags.staging: { DATABASE_URL: "postgres://staging/db" }`. During deploy, the server's tags are resolved and merged into the container env. Medium effort (tag resolution + merge logic), enables canary deployments and multi-region configurations without config duplication. Complements Role Tabanlı Sunucu Grupları (#35) — roles define what (web/worker), tags define where (staging/prod/canary).
- **Detected:** 2026-07-23

## Dedicated Docker Bridge Network for Tengiz Containers
- **Source:** Kamal
...
## Data Mutation Functions (Embedded JS Triggered by DB/Storage Operations)
- **Source:** Juno
- **Description:** Sputnik's core innovation: JavaScript/TypeScript functions that run inside the platform process (via embedded QuickJS/goja runtime), triggered by specific database and storage mutations — not by HTTP requests or lifecycle events. Four hook categories: **pre-auth hooks** (`assertSetDoc`, `assertDeleteDoc`, `assertUploadAsset`, `assertDeleteAsset`) that authorize or reject operations before they execute, and **post-operation hooks** (`onSetDoc`, `onDeleteDoc`, `onUploadAsset`, `onDeleteAsset`) that react after successful mutations. Hooks receive full operation context (document data, caller identity, collection metadata) and have access to platform APIs: read/write other collections, query storage, call auth services, make HTTP requests. Unlike generic serverless functions (#50, HTTP-triggered) or lifecycle hooks (#79, deploy/container events), these are tightly scoped to data operations with zero cold-start (run inside the existing process).
- **Why add to Tengiz:** Transforms the built-in datastore and storage from passive persistence into an active, programmable layer. Use cases: (1) `assertSetDoc` on a `comments` collection to validate comment content and reject spam before storage, (2) `onUploadAsset` on an `avatars` collection to generate thumbnail images, (3) `onDeleteDoc` on a `users` collection to cascade-delete related data, (4) `assertDeleteDoc` to prevent deletion of protected records. For Tengiz, `goja` provides a pure Go JS runtime with no cgo dependencies (matches Tengiz's zero-hard-dependency philosophy). Implementation: `internal/fn/` package with a JS runtime sandbox, hook registration via `.tengiz.yaml` or deploy-time: `tengiz function deploy --hook onSetDoc --collection comments validate-comment.ts`. Hooks compile to JS at deploy time, execute in a sandboxed goja VM with limited resource budget (memory, CPU, timeout). The existing proxy forwards mutation requests through the hook pipeline before reaching the datastore. This is Tengiz's biggest potential differentiator — no other Docker-based Vercel alternative (Coolify, Dokku, Kamal, CapRover) offers event-triggered embedded functions. It creates a platform that's programmable at every data access point.
- **Detected:** 2026-07-23

## Multi-Tenancy with User Isolation (Console/Mission Control/Satellite Model)
- **Source:** Juno
- **Description:** Juno's three-tier hierarchical architecture: **Console** (central management — accounts, billing, factory, system config), **Mission Control** (per-user management — satellites, orbiters, monitoring, deployment orchestration), **Satellites** (per-application containers — the actual deployed apps with auth/db/storage). Each tier is an isolated unit: Console manages the platform, Mission Control manages a user's resources, Satellites run the apps. Users cannot see or affect other users' resources. System-wide rate limits and quotas at the Console level prevent resource exhaustion. Invitation codes control user registration. Factory pattern spawns new instances on demand.
- **Why add to Tengiz:** Currently Tengiz operates as a single-user system — one CLI user manages all apps. For shared servers (teams, agencies, managed hosting), multi-tenancy is essential. The Console/Mission Control/Satellite model provides clean isolation without heavy multi-user infrastructure. Implementation: (1) **Console** — `tengiz console init` sets up the platform-level config, `tengiz user create/invite/remove` manages users, `tengiz console limits` sets per-user quotas, (2) **Mission Control** — `tengiz login <user>` switches context to a user's scope, `tengiz satellite list` shows apps within that scope, `tengiz satellite deploy` deploys to the user's isolated space, (3) **Isolation** — each user's data in `~/.tengiz/users/<user>/` (apps, ports, config separate from other users). Proxy routes based on user namespace as well as app name. Docker containers tagged with both user and app labels. In a team deployment, the Console operator creates user accounts, sets resource limits (max 5 apps per user, max 512MB RAM per app), and users manage their own satellites without interfering with each other. `.tengiz.yaml`'da `tenancy.user:` alanı. Complements existing multi-env support (#4, implemented) with multi-user support.
- **Detected:** 2026-07-23

## CI/CD Automation via GitHub Actions OIDC (Zero-Trust Deployment Auth)
- **Source:** Juno
- **Description:** Juno's auth automation system (`src/libs/auth/src/automation/`) implements zero-trust CI/CD authentication using GitHub Actions OIDC tokens. Instead of static API keys or deploy tokens (which can leak, expire, or be stolen), the automation flow uses GitHub Actions' OIDC provider: GitHub Actions workflow requests an OIDC token from GitHub's OIDC endpoint → the platform validates the token signature against GitHub's JWKS endpoint → extracts the repo/org/ref claims → maps to a pre-configured automation controller with scoped permissions (Write/Submit scopes available). The OIDC token is short-lived (minutes), tied to a specific workflow run, and signed by GitHub. No long-lived secrets stored in CI/CD. Prepares automation via `PrepareAutomation` which links a GitHub OIDC subject (e.g., `repo:myorg/myapp:environment:production`) to a platform controller identity with specific permissions.
- **Why add to Tengiz:** Existing deployment auth options (App Deploy Tokens #37, Headless CI/CD Mode #105, Granular Scoped API Keys #76) all use static secrets — they can leak via CI/CD logs, need rotation, and add key management overhead. OIDC-based CI/CD auth eliminates static secrets entirely: the CI/CD pipeline proves its identity via signed tokens from a trusted issuer (GitHub, GitLab, or any OIDC provider). Implementation: (1) Tengiz exposes an OIDC verification endpoint at `/__tengiz/auth/automation`, (2) GitHub Actions workflow POSTs its OIDC token to this endpoint, (3) Tengiz validates the token signature against GitHub's JWKS, (4) extracts the `repository` and `ref` claims, (5) checks against configured automation rules: `tengiz automation create --provider github --repo myorg/myapp --branch main --permission deploy`, (6) returns a short-lived session token for the deployment. `.tengiz.yaml`'da `automation.rules: [{provider: github, repo: myorg/myapp, branch: main, permission: deploy}]`. The existing GitHub webhook handler (#13) already receives GitHub events — OIDC auth adds verification that the CI/CD request is genuine. Medium effort (JWT verification + JWKS caching + claim mapping), high security value — eliminates the #1 CI/CD security risk (leaked credentials). Complements Commit Status Reporting (#146) for a complete CI/CD feedback loop.
- **Detected:** 2026-07-23
