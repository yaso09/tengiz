...
✅ Implemented, ⬜ Pending. Re-evaluated on **Impact (I)** — how many users benefit / criticality for Vercel alternative; **Effort (E)** — complexity given Tengiz's architecture (Go, CLI-first, Docker exec, scale-to-zero); **Alignment (A)** — fit with existing patterns.

Highest priority: features that are **table-stakes for any PaaS** combined with **low effort** due to strong architectural alignment. Lower priority: niche/enterprise features or those requiring significant new infrastructure.
...
| 5 | **Event Logging & Audit Trail** ⬜ | Yüksek | Düşük | Mükemmel | Multi-developer audit: who deployed what, when? `log/slog` + JSON Lines. |
| 6 | **REST API + OpenAPI Spec** ⬜ | Yüksek | Yüksek | Orta | Unlocks CI/CD, programmatic access, future web UI. Must-have despite deviating from CLI-first. |
| 7 | **Pre-Deploy Hooks** ⬜ | Yüksek | Düşük | Mükemmel | Migration runner before deploy is table stakes. `.tengiz.yaml` `pre_deploy` command list. |
| 8 | **Otomatik SSL/TLS (Let's Encrypt)** ⬜ | Çok Yüksek | Orta | Orta | Production HTTPS is non-negotiable. `golang.org/x/crypto/acme/autocert`. External proxy alternative exists. |
| 9 | **App Report (Detailed Status)** ⬜ | Yüksek | Düşük | Mükemmel | `tengiz ps` too minimal. Deploy history, image tags, env vars, resource limits, domains in one command. |
| 10 | **Monorepo Support (Base Directory)** ⬜ | Yüksek | Düşük | Mükemmel | Turborepo/Nx/Lerna users are a large segment. Simple `base_dir` override. |
| 11 | **Custom Build Commands** ⬜ | Yüksek | Düşük | Mükemmel | Framework detection must be overridable. `commands.install/build/start` in `.tengiz.yaml`. |
| 12 | **Headless CI/CD Mode** ⬜ | Yüksek | Düşük | Mükemmel | `TENGIZ_TOKEN` + `--headless` for GitHub Actions, GitLab CI. Non-interactive, no TTY. |
| 13 | **One-Line Install Script** ⬜ | Yüksek | Düşük | Mükemmel | `curl -fsSL https://tengiz.dev/install.sh | bash`. Cross-compile binaries, verify checksum. Massive adoption impact. |
| 14 | **Commit Status Reporting** ⬜ | Yüksek | Düşük | Mükemmel | Report deploy result to GitHub/GitLab commit status API. Green checkmark/red X on PRs. Essential feedback loop. |
| 15 | **App Auto-Creation on Git Push** ⬜ | Yüksek | Düşük | Mükemmel | Zero-setup deploy: `git push tengiz main` auto-creates app. Every PaaS does this. |
| 16 | **HMAC-Signed Webhook Payloads** ⬜ | Yüksek | Düşük | Mükemmel | Verify GitHub/GitLab webhook signatures. Prevent unauthorized deploy triggers. Security prerequisite for webhooks. |
| 17 | **Explicit Image Name Deploy** ⬜ | Orta | Düşük | Mükemmel | Deploy pre-built images (Postgres, Redis, CI/CD output) without build step. `tengiz deploy --image nginx:alpine`. |
| 18 | **Build Arguments from Env** ⬜ | Orta | Düşük | Mükemmel | NEXT_PUBLIC_*, NPM_TOKEN need `--build-arg` passthrough. Critical for framework builds. |
| 19 | **Deploy Lock** ⬜ | Orta | Düşük | Mükemmel | Prevent concurrent deploy collisions in team environments. File-based lock + `--lock-wait`. |
| 20 | **App Deploy Tokens** ⬜ | Orta | Düşük | Mükemmel | CI/CD authentication without user credentials. `tengiz token create --app myapp`. |
| 21 | **Config Export/Import** ⬜ | Orta | Düşük | Mükemmel | Disaster recovery and app migration. Export env vars as shell/dotenv/Docker args/JSON. |
| 22 | **Environment Variable Locking** ⬜ | Orta | Düşük | Mükemmel | Prevent accidental `tengiz config unset DATABASE_URL`. Locked vars require confirmation. |
| 23 | **Container Restart Policy Management** ⬜ | Orta | Düşük | Mükemmel | Per-app Docker restart policy for crash behavior control. `no/always/unless-stopped/on-failure`. |
| 24 | **Server Reboot Recovery** ⬜ | Orta | Düşük | Mükemmel | Auto-restart all apps after host reboot. Systemd unit + `ps:restore` on boot. Essential for unattended operation. |
| 25 | **Build-Time Secrets (Docker Build Secrets)** ⬜ | Orta | Düşük | Mükemmel | NPM_TOKEN, signing keys via `--secret` not `--build-arg`. Excluded from image history. |
| 26 | **Well-Known Paths Automatic Handling** ⬜ | Orta | Düşük | Mükemmel | `/.well-known/` for ACME HTTP-01, Apple Universal Links, security.txt. Critical for domain verification. |
| 27 | **Custom HTTP Headers per URL Path** ⬜ | Orta | Düşük | Mükemmel | Per-path Cache-Control, CORS, security headers. Vercel/Netlify `_headers` style. |
| 28 | **Auth Rate Limiting** ⬜ | Orta | Düşük | Mükemmel | Per-IP brute force protection on auth/webhook endpoints. 5 attempts/300s window. |
| 29 | **Deploy Branch Configuration** ⬜ | Orta | Düşük | Mükemmel | Only push to specific branch triggers deploy. `git.deploy_branch: main`. Standard CI/CD hygiene. |

### P1 — High (Production-Ready Platform)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 30 | **Container Registry Integration** ⬜ | Yüksek | Düşük-Orta | Mükemmel | Build → push pipeline. Image storage for rollback + multi-node. `docker tag && docker push`. |
| 31 | **Private Registry Authentication** ⬜ | Orta | Düşük | Mükemmel | GHCR, GitLab Registry, AWS ECR pull support. Enterprise prerequisite. |
| 32 | **Error Pages** ⬜ | Orta | Düşük | Mükemmel | Cold start or container-down UX: friendly pages instead of raw HTTP errors. Proxy middleware. |
| 33 | **Container Retention Policy** ⬜ | Orta | Düşük | Mükemmel | Rollback companion. Keep N old containers, prune rest. |
| 34 | **Full System Backup & Restore** ⬜ | Orta | Orta | Mükemmel | `tengiz backup create/restore` archives `~/.tengiz/` state. DR essential. |
| 35 | **Extended Hook System** ⬜ | Orta | Düşük-Orta | Mükemmel | Pre-build, post-deploy, app-boot hooks with rich env context. |
| 36 | **Maintenance Mode** ⬜ | Orta | Düşük | Mükemmel | Proxy draining for planned maintenance. `tengiz maintenance:on --message "Upgrading..."`. |
| 37 | **Prometheus Metrics** ⬜ | Orta | Düşük | Mükemmel | Proxy HTTP metrics: request count, latency, error rate, cold starts. Grafana + alerting. |
| 38 | **Readiness Delay & Deploy Timeouts** ⬜ | Orta | Düşük | Mükemmel | Per-operation timeouts. Accommodates both fast Go apps and slow Node.js. |
| 39 | **Zero-Downtime Deploy Health Checks** ⬜ | Orta | Düşük | Mükemmel | App-level health verification before traffic migration. Auto-rollback on failure. |
| 40 | **Node.js Multi-Core Scaling (PM2/Cluster)** ⬜ | Orta | Düşük | Mükemmel | 4-8x Node.js perf via PM2 cluster mode. `.tengiz.yaml` toggle. |
| 41 | **Custom Docker Network** ⬜ | Orta | Düşük | Mükemmel | Isolated networks for multi-service apps. `docker run --network` flag. |
| 42 | **Server Bootstrap** ⬜ | Orta | Orta | Mükemmel | `tengiz server init` + `tengiz setup` — one-command Docker + Tengiz installation. |
| 43 | **HTTP Basic Auth (Staging Protection)** ⬜ | Orta | Düşük | Mükemmel | Password-protect staging/pre-production. Proxy middleware. |
| 44 | **Per-Container Resource Metrics** ⬜ | Orta | Düşük | Mükemmel | Live CPU/memory/network from `docker stats`. `tengiz ps --stats`. |
| 45 | **Scheduled Deployments** ⬜ | Orta | Düşük | Mükemmel | Cron-based auto-deploy. Nightly rebuilds, dependency updates. `robfig/cron`. |
| 46 | **Stale Container Detection** ⬜ | Orta | Düşük | Mükemmel | Detect orphaned old-version containers after zero-downtime deploy. |
| 47 | **Config Display Command** ⬜ | Orta | Düşük | Mükemmel | `tengiz config` shows merged config after env override + template resolution. |
| 48 | **Deploy Source Metadata Recording** ⬜ | Orta | Düşük | Mükemmel | Record GIT_SHA, branch, deploy source. Env vars + build record. |
| 49 | **Webhook Event Filtering** ⬜ | Orta | Düşük | Mükemmel | Branch/tag/path filtering. Prevent wasteful deploys from docs/WIP branches. |
| 50 | **App Repository Lifecycle Management** ⬜ | Orta | Düşük | Mükemmel | Per-app git repo management: `git:lock/unlock/status`. |
| 51 | **Custom Image Repository Naming** ⬜ | Orta | Düşük | Mükemmel | Go template-based naming. `ghcr.io/myorg/{{ .AppName }}`. |
| 52 | **Secret Interpolation System** ⬜ | Orta | Düşük | Mükemmel | `[[secret.name]]` syntax. AES-GCM encrypted. No vault needed for single-server. |
| 53 | **Variable Resource (Global Interpolation)** ⬜ | Orta | Düşük | Mükemmel | Define `[[database_url]]` once, reference everywhere. Eliminates duplication. |
| 54 | **Parallel Bulk Operations** ⬜ | Orta | Düşük | Mükemmel | `tengiz rebuild --all`, `tengiz restart --all`. Goroutine pool. |
| 55 | **Granular Docker Prune Operations** ⬜ | Orta | Düşük | Mükemmel | Per-category prune: containers/networks/images/volumes/buildx cache. |
| 56 | **Background Monitoring Scheduler** ⬜ | Orta | Düşük | Mükemmel | Proactive health checks, disk usage, container status. Feeds alert system. |
| 57 | **Accessory Services (Sidecar Containers)** ⬜ | Orta | Orta | Mükemmel | Postgres/Redis/Search alongside app. Scale-to-zero only affects app. |
| 58 | **Build Pipeline with Auto-Versioning** ⬜ | Orta | Orta | Mükemmel | Source → versioned image → multi-registry push. Auto-tag: semver/sha/timestamp. |
| 59 | **Build-to-Deploy Trigger Chain** ⬜ | Orta | Orta | Mükemmel | Build completes → linked deployment auto-redeploys. Full CI/CD without external tools. |
| 60 | **Herokuish Buildpacks** ⬜ | Orta | Orta | Mükemmel | Heroku's entire buildpack ecosystem. Zero-config Heroku migration. |
| 61 | **Multi-Architecture Builds (Buildx)** ⬜ | Orta | Orta | Mükemmel | `linux/amd64 + linux/arm64` manifests. Apple Silicon → Intel server. |
| 62 | **Remote Docker Builder** ⬜ | Orta | Orta | Mükemmel | SSH-based remote buildx builder. Offload heavy builds. |
| 63 | **GitOps / Declarative ResourceSync** ⬜ | Yüksek | Yüksek | Mükemmel | Declare apps/env/domains as git-managed YAML. `tengiz sync` reconciles. IaC for Tengiz. |
| 64 | **Embedded Serverless Functions (goja)** ⬜ | Yüksek | Yüksek | Mükemmel | Biggest differentiator. <10ms function runtime, no container. No other Docker-based alt has this. |
| 65 | **App Renaming** ⬜ | Düşük | Düşük | Mükemmel | `tengiz rename <old> <new>`. Full state migration. |
| 66 | **Custom Docker Options** ⬜ | Düşük-Orta | Düşük | Mükemmel | Power user escape hatch: `--shm-size`, `--sysctl`, `--cap-add`. |
| 67 | **KEDA-based Autoscaling** ⬜ | Orta-Yüksek | Yüksek | Mükemmel | Scale 0→N based on HTTP rate, queue depth, CPU. Transforms idle timer into real serverless. |

### P2 — Medium (Significant Differentiators)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 68 | **Process Scaling (Multi-Container)** ⬜ | Orta | Yüksek | Orta | HA + background workers. Combines with idle timeout for serverless model. |
| 69 | **Managed Database Provisioning** ⬜ | Yüksek | Çok Yüksek | Orta | Vercel Postgres/KV equivalent. Very high effort but high impact. |
| 70 | **One-Click Service Templates** ⬜ | Yüksek | Yüksek | Orta | 361 Docker Compose templates. `tengiz service create <template>`. |
| 71 | **Server Monitoring** ⬜ | Orta | Orta | Mükemmel | Disk, container states, backup status. `tengiz status` + threshold alert. |
| 72 | **Scheduled Tasks / Cron Jobs** ⬜ | Düşük-Orta | Orta | Mükemmel | Vercel Cron Jobs equivalent. `.tengiz.yaml` `cron:` + `robfig/cron`. |
| 73 | **Force HTTPS Redirect** ⬜ | Orta | Düşük | Mükemmel | HTTP→HTTPS 301 redirect in proxy. Basic security hygiene. |
| 74 | **Gelişmiş Proxy Konfigürasyonu** ⬜ | Orta | Orta | Mükemmel | Path prefix, buffering, timeout, X-Forwarded-* control. |
| 75 | **Pattern-Based Watch Paths** ⬜ | Orta | Düşük-Orta | Mükemmel | `tengiz deploy --watch` with glob patterns. `fsnotify`. |
| 76 | **WebSocket Support Per App** ⬜ | Orta | Düşük | Mükemmel | Per-app WS toggle. `.tengiz.yaml` `proxy.websocket: true`. |
| 77 | **Event-Driven Data Hooks (Trigger System)** ⬜ | Orta | Orta | Mükemmel | `container:start`, `deploy:success`, `idle:timeout` events. |
| 78 | **Container Snapshot System** ⬜ | Orta | Düşük | Mükemmel | `docker commit`-based stateful snapshot before risky deploys. |
| 79 | **Built-in Platform Analytics** ⬜ | Orta | Orta | Mükemmel | HTML injection + Web Vitals tracking. SQLite storage. Vercel Analytics-level. |
| 80 | **Built-in Authentication Service** ⬜ | Yüksek | Yüksek | Mükemmel | Platform-level auth-as-a-service. Google/GitHub/Passkey via proxy intercept. |
| 81 | **Built-in NoSQL Datastore** ⬜ | Yüksek | Yüksek | Mükemmel | Zero-config document store. Embedded SQLite + proxy `/__tengiz/db/`. |
| 82 | **Built-in File/Blob Storage** ⬜ | Yüksek | Yüksek | Mükemmel | Platform-level asset hosting. URL-based access control. |
| 83 | **Framework Plugins (Next.js/Vite)** ⬜ | Orta | Orta | Mükemmel | `@tengiz/nextjs` npm package. Auto-inject env + API routes. |
| 84 | **Build Precompression** ⬜ | Orta | Düşük | Mükemmel | Gzip/Brotli pre-compression. Zero-CPU asset serving. |
| 85 | **Staged Deployments (Change Sets)** ⬜ | Orta | Orta | Mükemmel | `tengiz deploy --no-apply` → `tengiz changes apply <id>`. |
| 86 | **Project Scaffolding with Starter Templates** ⬜ | Orta | Orta | Mükemmel | `tengiz create <template>`. Minutes to first deploy. |
| 87 | **Change Approval Workflow** ⬜ | Orta | Orta | Mükemmel | Submit → Review → Apply. Team governance. |
| 88 | **Procfile Support** ⬜ | Orta | Düşük | Mükemmel | Heroku-style process type definition. Zero-config migration. |
| 89 | **Docker Compose Import** ⬜ | Orta | Orta | Mükemmel | Convert existing `docker-compose.yml` to Tengiz containers. |
| 90 | **Global/Per-App Property Cascade** ⬜ | Orta | Orta | Mükemmel | Global defaults → app overrides. Reduce per-app config overhead. |
| 91 | **Per-Process-Type Resource Limits** ⬜ | Orta | Düşük | Mükemmel | Separate CPU/memory for web/worker/scheduler. |
| 92 | **Build Tracking with Retention** ⬜ | Orta | Orta | Mükemmel | Structured deploy history: JSON records, status tracking, retention. |
| 93 | **Git Provider OAuth App Integration** ⬜ | Orta | Yüksek | Mükemmel | One-click GitHub/GitLab App connection. Auto-configure webhooks. |
| 94 | **Magic Environment Variables** ⬜ | Orta | Orta | Mükemmel | Auto-generated service URLs, DB connection strings for linked services. |
| 95 | **Centralized Multi-Server Management** ⬜ | Orta | Yüksek | Mükemmel | Control-plane model managing remote Docker hosts via SSH. |
| 96 | **MCP Server for AI Integration** ⬜ | Orta | Orta | Mükemmel | Model Context Protocol server for AI assistants. Strategic value. |
| 97 | **Local Development Emulator** ⬜ | Yüksek | Orta | Mükemmel | `tengiz emulator start` — full platform stack locally. Match Vercel dev experience. |
| 98 | **Client SDK Ecosystem** ⬜ | Yüksek | Orta | Mükemmel | `@tengiz/core` npm package. Makes platform services feel built-in. |
| 99 | **Concurrency Control (Operation Locking)** ⬜ | Orta | Düşük | Mükemmel | File-based mutex per app. Prevents corrupting state files. |
| 100 | **Docker Network & Volume CRUD** ⬜ | Orta | Düşük | Mükemmel | `tengiz network/create/ls/rm`. No raw Docker CLI needed. |
| 101 | **Build Cache Management & Git GC** ⬜ | Orta | Düşük | Mükemmel | `tengiz cleanup --cache --gc`. Per-app build cache + git repo pruning. |
| 102 | **URL Redirect & Rewrite Rules** ⬜ | Orta | Düşük | Mükemmel | Per-app 301/302 redirects, rewrites at proxy level. SEO migration. |
| 103 | **Proxy Security Middleware** ⬜ | Orta | Düşük | Mükemmel | IP allow/deny (CIDR), security headers (HSTS, CSP), per-app basic auth. |
| 104 | **CDN Provider Detection** ⬜ | Orta | Düşük | Mükemmel | Cloudflare/Fastly IP range detection. Correct client IP extraction. |
| 105 | **Email Notification Engine** ⬜ | Orta | Düşük | Mükemmel | SMTP alerts for deploy failure, SSL expiry, backup notification. |
| 106 | **Real-Time WebSocket for Deploy Logs** ⬜ | Orta | Orta | Mükemmel | Live deploy log streaming. `tengiz deploy --stream`. Web UI foundation. |
| 107 | **Automated Database Backups** ⬜ | Orta | Orta | Mükemmel | `docker exec pg_dump` cron-based, S3 storage. |
| 108 | **SSH Key Management** ⬜ | Orta | Orta | Mükemmel | Per-server SSH key pairs. `tengiz ssh-key generate/add/list/remove`. |
| 109 | **Rate Limiting** ⬜ | Orta | Düşük | Mükemmel | Webhook/API rate limiting. `golang.org/x/time/rate`. HTTP 429. |
| 110 | **Log Drains (External Log Streaming)** ⬜ | Orta | Orta | Mükemmel | Axiom, New Relic, Loki log forwarding. |
| 111 | **AI-Powered Deployment Assistant** ⬜ | Orta | Düşük | Mükemmel | `tengiz ai "deploy WordPress with Redis"` → LLM-generated compose. |
| 112 | **GPU Passthrough (NVIDIA/CUDA)** ⬜ | Orta | Orta | Mükemmel | `--gpus all`. AI/ML workloads (Ollama, vLLM). |
| 113 | **TypeScript Action Automation (Deno)** ⬜ | Orta | Orta | Mükemmel | Embedded TS runtime for platform automation. Custom deploy logic. |
| 114 | **OIDC/OAuth Single Sign-On** ⬜ | Orta | Orta | Mükemmel | Google/GitHub OAuth + generic OIDC. Team auth. |
| 115 | **Inter-Container Network Aliases** ⬜ | Orta | Düşük | Mükemmel | DNS-based service discovery via Docker network aliases. |
| 116 | **Static Web Listener (Cold-Start Fallback)** ⬜ | Orta | Düşük | Mükemmel | Graceful cold-start page instead of raw 502/503. |

### P3 — Lower (Niche / Enhancement / Enterprise)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 117 | **SSH Tabanlı Remote Deployment** | Orta | Yüksek | Orta | Multi-server via SSH. High effort, Tengiz built as single-node. |
| 118 | **Role Tabanlı Sunucu Grupları** | Orta | Orta | Orta | Web/worker/job role separation. Different cmd/env per role. |
| 119 | **Redeploy** | Düşük | Düşük | Mükemmel | Skip bootstrap/prune, just build/push/restart. |
| 120 | **Rolling Boot / Canary Deployment** | Düşük | Yüksek | Düşük | Multi-server only. Gradual rollout. |
| 121 | **Encryption at Rest** | Orta | Orta | Mükemmel | AES-256 encryption of env vars. Enterprise security. |
| 122 | **Safe Volume Deletion** | Düşük | Düşük | Mükemmel | Cross-app check before volume deletion. |
| 123 | **Port Mapping Protocol Selection** | Düşük-Orta | Düşük | Mükemmel | TCP/UDP/both for non-HTTP services. |
| 124 | **Project-Based App Organization** | Düşük | Düşük | Mükemmel | `tengiz project create`, `tengiz ps --project`. |
| 125 | **App Tags** | Düşük | Düşük | Mükemmel | Ad-hoc grouping via tags. |
| 126 | **Pre-Install Env Validation (tengiz doctor)** | Düşük | Düşük | Mükemmel | Docker version, port availability, disk space check. |
| 127 | **Git Commit Hash Auto-Injection** | Düşük | Düşük | Mükemmel | `TENGIZ_COMMIT_SHA` env var. |
| 128 | **Root Domain Change** | Düşük | Düşük | Mükemmel | `tengiz proxy --domain production.com`. |
| 129 | **Interactive Env Prompts** | Düşük | Düşük | Mükemmel | TTY prompts for required env vars on first deploy. |
| 130 | **Patches (Build-Time File Overrides)** | Düşük | Düşük-Orta | Mükemmel | Environment-specific `.env`, `robots.txt` injected during build. |
| 131 | **Cloudflare Tunnel Support** | Düşük-Orta | Orta | Mükemmel | Zero-trust exposure via Cloudflare edge. |
| 132 | **S3-Compatible Backup Storage** | Orta | Orta | Mükemmel | DB backups to S3. Scheduled + retention. |
| 133 | **Outgoing Webhook Payloads** | Düşük | Düşük | Mükemmel | POST deploy events to external URLs. |
| 134 | **Custom Compose Overrides** | Düşük | Düşük | Mükemmel | `docker-compose.override.yml` merge. |
| 135 | **App Cloning** | Düşük | Düşük | Mükemmel | `tengiz apps:clone <old> <new>`. Full config copy. |
| 136 | **Build Queue with Dedup** | Düşük | Düşük | Mükemmel | Per-app channel-based queue. Last-one-wins. |
| 137 | **GoAccess Real-Time Log Analytics** | Düşük | Düşük | Mükemmel | Optional analytics container. |
| 138 | **Service Template Registry** | Orta | Orta | Mükemmel | Central template registry with CDN auto-update. |
| 139 | **Lambda Builder (Docker-Based FaaS)** | Düşük | Orta | Mükemmel | AWS Lambda-compatible functions. |
| 140 | **Container Entering (tengiz enter)** | Düşük | Düşük | Mükemmel | `tengiz enter <app>` → `docker exec -it`. |
| 141 | **Trace/Debug Mode** | Düşük | Düşük | Mükemmel | `--debug` flag → slog LevelDebug. |
| 142 | **Git-Sync Deployment** | Düşük | Düşük | Mükemmel | Pull-based deployment from remote repo. |
...
| 145 | **Failed Deploy Logs** | Düşük | Düşük | Mükemmel | `tengiz logs --failed <app>`. |
| 146 | **Vector Log Shipping** | Düşük | Orta | Mükemmel | Log aggregator companion container. |
| 147 | **Config Validation** | Düşük | Düşük | Mükemmel | `tengiz config validate`. Pre-deploy sanity check. |
| 148 | **Git-Based Image Version Tagging** | Düşük | Düşük | Mükemmel | Auto-tag images with commit SHA. |
| 149 | **SSH Key Management for Deploy Access** | Düşük | Düşük | Mükemmel | Per-developer SSH key deploy access control. |
| 150 | **Web Dashboard (Admin UI)** | Yüksek | Yüksek | Orta | Highest impact for non-CLI users but high effort, deviates from CLI-first. |
| 151 | **NetData Integration** | Düşük | Düşük | Mükemmel | Real-time system monitoring companion. |
| 152 | **Platform Self-Health Check** | Düşük | Düşük | Mükemmel | `/healthz` endpoint + auto-restart on failure. |
| 153 | **Self-Hosted Docker Registry** | Düşük | Düşük | Mükemmel | Built-in `registry:2` container. |
| 154 | **Service Update Strategy** | Düşük | Düşük | Mükemmel | `startFirst` vs `stopFirst` deploy. |
| 155 | **Persistent Docker BuildKit Cache** | Düşük | Düşük | Mükemmel | 60-90% build time reduction. |
| 156 | **Output/Telemetry Loggers** | Düşük | Orta | Orta | OpenTelemetry/file logger. |
| 157 | **CLI Alias Tanımlama** | Çok Düşük | Çok Düşük | Mükemmel | `.tengiz.yaml` command shortcuts. |
| 158 | **Alternative ACME Providers** | Düşük | Düşük | Mükemmel | ZeroSSL, BuyPass, Google. |
| 159 | **Staging Mode for SSL Testing** | Düşük | Düşük | Mükemmel | ACME staging endpoints. |
| 160 | **Pluggable Multi-Scheduler (Docker → K3s)** | Düşük | Çok Yüksek | Orta | Scheduler abstraction. Huge architectural change. |
| 161 | **Pluggable Reverse Proxy** | Düşük | Yüksek | Orta | nginx/Caddy/HAProxy backend option. |
| 162 | **Custom Build Server** | Düşük | Yüksek | Orta | Separate build/deploy servers. |
| 163 | **Self-Upgrade / Auto-Update** | Düşük | Düşük-Orta | Mükemmel | `tengiz upgrade`. GitHub Releases download. |
| 164 | **app.json Manifest (Heroku Compatible)** | Düşük | Orta | Mükemmel | Zero-config Heroku migration manifest. |
| 165 | **Git Submodules & Git LFS** | Düşük | Düşük | Mükemmel | `git submodule update --init --recursive`. |
| 166 | **App-Level Lifecycle Data Hooks** | Orta | Orta | Mükemmel | `onSetDoc`, `onDeleteDoc` triggers. |
| 167 | **Docker Logging Konfigürasyonu** | Düşük | Düşük | Mükemmel | Per-app log driver config. |
| 168 | **Asset Path / Asset Bridging** | Düşük | Düşük | Mükemmel | Zero-downtime asset co-existence. |
| 169 | **Gelişmiş Docker Build** | Düşük | Düşük | Mükemmel | Multi-arch, build cache, driver config. |
| 170 | **Server Exec (Host-Level Commands)** | Düşük | Düşük | Mükemmel | `tengiz server exec "df -h"`. |
| 171 | **Version Targeting** | Düşük | Düşük | Mükemmel | Deploy/exec specific previous version. |
| 172 | **Cloud Native Buildpacks (pack CLI)** | Düşük | Düşük | Mükemmel | Heroku-style buildpacks via `pack`. |
| 173 | **App Images Command** | Düşük | Düşük | Mükemmel | List Docker images per app. |
| 174 | **System Stats Recording** | Düşük | Düşük | Mükemmel | Historical per-container stats. JSON Lines. |
| 175 | **Image Digest Change Detection** | Düşük | Düşük | Mükemmel | Auto-redeploy on image digest change. |
| 176 | **Procedure Automation** | Düşük | Düşük | Mükemmel | Multi-step workflows: deploy→migrate→healthcheck→notify. |
| 177 | **Image Digest Pinning** | Düşük | Düşük | Mükemmel | Pin deployment to specific SHA digest. |
| 178 | **Granular Scoped API Keys** | Düşük | Düşük | Mükemmel | Read/Write/Execute per resource type. |
| 179 | **Stack/Compose Lifecycle Management** | Düşük | Orta | Mükemmel | First-class Compose stack resource. |
| 180 | **Server Security Hardening** | Orta | Orta | Mükemmel | UFW, Fail2ban, Docker TLS during init. |
| 181 | **Database Connection String Auto-Injection** | Orta | Orta | Mükemmel | Auto-inject `DATABASE_URL` when linked. |
| 182 | **Database Backup Import/Upload** | Düşük | Düşük | Mükemmel | Import external SQL dump files. |
| 183 | **Protected Service Deletion** | Düşük | Düşük | Mükemmel | Confirmation, backup-before-delete. |
| 184 | **Reusable External Storage Destinations** | Düşük | Düşük | Mükemmel | First-class S3 destination management. |
| 185 | **Platform Admin Settings** | Düşük | Düşük | Mükemmel | Server URL, Docker config, defaults. |
| 186 | **Manual SSL Certificate Management** | Orta | Orta | Mükemmel | Import/generate/inspect/remove SSL certs. |
| 187 | **Per-App Proxy Toggle** | Düşük | Düşük | Mükemmel | `tengiz proxy:disable/enable <app>`. |
| 188 | **Linux Capabilities Management** | Düşük | Düşük | Mükemmel | `--cap-add`/`--cap-drop`. Least privilege. |
| 189 | **Pluggable Event/Trigger Architecture** | Orta | Yüksek | Mükemmel | 40+ lifecycle trigger points. Plugin ecosystem. |
| 190 | **Config Format Self-Documentation** | Düşük | Düşük | Mükemmel | `tengiz config docs` shows valid keys. |
| 191 | **Multi-Server Architecture w/ Periphery Agent** | Düşük | Çok Yüksek | Mükemmel | Core↔Periphery distributed model. Transformative. |
| 192 | **Builder Resource (URL/Server/AWS EC2)** | Düşük | Yüksek | Mükemmel | Decoupled build targets. Ephemeral EC2 builders. |
| 193 | **User Group Resource (RBAC)** | Düşük | Orta | Mükemmel | Group-based permissions. |
| 194 | **Alert System with Severity Levels** | Orta | Orta | Mükemmel | 5 levels, resolved/unresolved tracking. |
| 195 | **Multi-Channel Alerters** | Düşük | Düşük | Mükemmel | Slack/Discord/Pushover/ntfy formatting. |
| 196 | **Docker Swarm Resource Management** | Düşük | Yüksek | Mükemmel | Multi-node HA via Docker Swarm. |
| 197 | **Git Provider Account Management** | Düşük | Düşük | Mükemmel | Store GitHub/GitLab tokens. |
| 198 | **Docker Registry Account Management** | Düşük | Düşük | Mükemmel | Multi-registry auth management. |
| 199 | **WebSocket Interactive Terminal** | Düşük | Orta | Mükemmel | PTY + WebSocket streaming. |
| 200 | **Batch Operations Across Resource Types** | Düşük | Düşük | Mükemmel | Filter by tag/project/status. |
| 201 | **Two-Factor Authentication (2FA)** | Orta | Orta | Mükemmel | TOTP for admin accounts. |
| 202 | **Private Asset Access Tokens** | Düşük | Düşük | Mükemmel | Token-gated private file URLs. |
| 203 | **Granular Per-Operation Rate Limiting** | Düşük | Orta | Mükemmel | Per-operation-type limits. |
| 204 | **Collection Memory Type Configuration** | Düşük | Düşük | Mükemmel | Ephemeral vs persistent per datastore collection. |
| 205 | **Teams & Invitations** | Orta | Düşük-Orta | Mükemmel | Multi-user team management with invitations. |
| 206 | **Shared Environment Variables** | Orta | Düşük | Mükemmel | Team/project-wide env inheritance. |
| 207 | **Cloud Provider Integration** | Orta | Yüksek | Mükemmel | VPS provisioning from Hetzner/AWS/DO APIs. |
| 208 | **Forward Auth (External Auth Middleware)** | Orta | Orta | Mükemmel | Proxy-level auth delegation to Authentik/Authelia. |
| 209 | **Bind-All-Interfaces Toggle** | Düşük | Düşük | Mükemmel | Container network binding control. |
| 210 | **Orphaned SSL Certificate Detection** | Düşük | Düşük | Mükemmel | Detect expiring certs for unused domains. |
| 211 | **Password Change Token Versioning** | Düşük | Düşük | Mükemmel | Immediate token invalidation on password change. |
| 212 | **Download Token for Backup Files** | Düşük | Düşük | Mükemmel | Short-lived JWT URLs for backup downloads. |
| 213 | **JavaScript-Based Pre-Deploy Hooks** | Düşük | Düşük-Orta | Mükemmel | Programmatic Docker config modification via goja. |
| 214 | **Service Update Override** | Düşük | Düşük | Mükemmel | Raw Docker config JSON injection. |
| 215 | **Configurable App Log Retention** | Düşük | Düşük | Mükemmel | Per-app log size limits. Prevents disk bloat. |
| 216 | **Docker Secrets for Encryption Key Storage** | Düşük | Düşük-Orta | Mükemmel | Secure key management via Docker secrets. |
| 217 | **Multiple Named Data Stores** | Düşük | Orta | Mükemmel | Namespace isolation for multi-tenancy. |
| 218 | **Server Info API** | Düşük | Düşük | Mükemmel | Docker infrastructure visibility endpoint. |
| 219 | **Server Onboarding Key System** | Düşük | Orta | Mükemmel | One-time registration tokens for multi-server. |
| 220 | **Resource Abstraction (Uniform Resource Model)** | Düşük | Yüksek | Mükemmel | Generic CRUD for all resource types. Architectural refactor. |
| 221 | **Variable Interpolation in All Config Fields** | Düşük | Düşük-Orta | Mükemmel | `[[variable]]` in compose, hooks, Docker args. |
| 222 | **Alert Buffer with 2-Check Requirement** | Düşük | Düşük | Mükemmel | Prevent alert storms from transient issues. |
| 223 | **Platform Version Management & Upgrade Pipeline** | Düşük | Orta | Mükemmel | Enterprise-grade upgrade with pre/post hooks, rollback. |
| 224 | **Automated Resource Monitoring with Self-Healing** | Orta | Orta | Mükemmel | Auto-remediate disk/memory pressure. Prevents #1 failure mode. |
| 225 | **OpenID Provider Management** | Düşük | Düşük-Orta | Mükemmel | Dynamic JWKS caching for OIDC. |
| 226 | **Rich Hook Environment Context** | Düşük | Düşük | Mükemmel | Contextual env vars injected into hooks. |
| 227 | **Hook Output & Error Handling Configuration** | Düşük | Düşük | Mükemmel | Per-hook verbose/quiet/only-on-error output. |
| 228 | **Docker Host Architecture Detection** | Düşük | Düşük | Mükemmel | Auto-detect amd64/arm64 for platform-aware builds. |
| 229 | **Deploy Lock Configuration** | Düşük | Düşük | Mükemmel | Configurable wait timeout and retry interval. |
| 230 | **Healthcheck Barrier for Multi-Service Coordination** | Düşük | Orta | Mükemmel | Startup order: web healthy → workers start. |
| 231 | **Dev Build Command** | Düşük | Düşük | Mükemmel | Test build locally without registry push. |
| 232 | **Uncommitted Changes Detection** | Düşük | Düşük | Mükemmel | Dirty version tag warning before deploy. |
| 233 | **Sensitive Value Redaction** | Orta | Düşük | Mükemmel | Auto-redact secrets from logs and error output. |
...
## Sensitive Value Redaction (Secrets in Logs & Error Messages)
- **Source:** Kamal
- **Description:** Kamal's `Kamal::Utils::Sensitive` wraps secret values to automatically redact them from all log output, error messages, and command traces. When a secret value inadvertently appears in a log line or error message, it's replaced with `[REDACTED]` before the output is displayed to the user or written to a file. This protects against accidental credential leakage in CI/CD output, debug logs, and support tickets. Kamal implements this as a custom string class that overrides `inspect` and `to_s` to return the redacted value — any output containing a secret is automatically sanitized.
- **Why add to Tengiz:** Production security hygiene requires that secrets (DB passwords, API keys, tokens) never appear in log output, CLI output, or error messages. A single `tengiz config get` or deploy failure dumping env vars can leak production credentials. Currently Tengiz has log filtering (#9) which filters log content, but no system-wide secret redaction. Implementation: a `SecretString` type in `internal/types/` that implements `fmt.Stringer` returning `[REDACTED]`, used for all env var values and secret config fields. A `slog` handler wrapper that scans log output for known secret patterns (TENGIZ_SECRET_* env vars) and redacts them. `Sensitive` struct wrapping all credential values in config, store, runtime, and proxy packages. Low effort (Go `Stringer` interface + slog middleware), critical for production security. Complements Secrets Management (#4) — secrets handling isn't complete without protection against accidental leakage. Kamal's `lib/kamal/utils/sensitive.rb` is the reference implementation (~30 lines).
- **Detected:** 2026-07-21

## Uncommitted Changes Detection & Dirty Build Warning
- **Source:** Kamal
- **Description:** Before deploying, Kamal checks the git working tree for uncommitted changes using `git status --porcelain`. If uncommitted changes are detected, Kamal appends `-dirty` to the version tag and displays a prominent warning: "Warning: you have uncommitted changes. Deploying with dirty version tag." The deploy proceeds but the dirty tag makes it visible in the deployment history that the code was not clean. The version string becomes `<sha>-dirty` instead of `<sha>`. Users can suppress the warning via a config flag.
- **Why add to Tengiz:** Deploying uncommitted code is a common mistake that leads to unreproducible builds and "works on my machine" problems. When debugging production issues, knowing that the deployed code included uncommitted changes (non-reproducible state) is critical context. Implementation: `git rev-parse HEAD` for the commit SHA, `git status --porcelain` to check for changes, version string `<sha>-dirty` when changes are present. Warning displayed via `log/slog.Warn()`. Integration with the existing deploy pipeline in `internal/cli/deploy.go`. Complements Git Commit Hash Auto-Injection (#115) and Git-Based Image Version Tagging (#148) by adding the dirty-state signal. Low effort (two git commands + string concatenation), high debugging value. Kamal's version detection is in `lib/kamal/version.rb`.
- **Detected:** 2026-07-21

## Dev Build Command (Local Testing Without Registry Push)
- **Source:** Kamal
- **Description:** `kamal build dev` builds and tags the Docker image as dirty (appending `-dirty` to the tag) and stores it in the local Docker image store without pushing to a registry. This enables developers to test the build process and verify the image locally without affecting the registry or triggering a deploy. The dirty tag prevents accidental production deployment of a development build. Kamal also supports `kamal build` for the standard build+push cycle. The dev build uses the same build configuration (Dockerfile, build args, secrets) as production but skips the push step.
- **Why add to Tengiz:** Currently `tengiz deploy` always builds AND deploys. There's no way to verify the build step independently. A `tengiz build` or `tengiz build dev` command lets developers: test build configuration changes (Dockerfile, build args, buildpacks) without deploying, verify Dockerfile generation works, inspect the produced image locally, and cache layers between builds. Implementation: a `tengiz build` command that runs `builder.RunBuild()` without calling `runtime.Create()` or proxy updates. Tag the image with `dev-<timestamp>` to distinguish from production images. Complements Build Tracking (#94) and the existing builder package. Kamal's `lib/kamal/cli/build.rb` separates build subcommands (deliver, push, pull, create, dev) for different build lifecycle stages. Low-medium effort (extract build phase from deploy pipeline into standalone command), high developer experience value.
- **Detected:** 2026-07-21

## Rich Hook Environment Context (Contextual Env Vars for Hook Scripts)
- **Source:** Kamal
- **Description:** Kamal injects a rich set of environment variables into every hook script execution: `KAMAL_HOSTS`, `KAMAL_ROLES`, `KAMAL_VERSION` (git SHA), `KAMAL_LOCK` (who holds the lock), `KAMAL_COMMAND` (deploy/rollback/setup), `KAMAL_SUBCOMMAND` (app/accessory/proxy), `KAMAL_RUNTIME` (execution duration), and `KAMAL_SECRET_*` (specific secrets for the hook to use). Hook scripts read these env vars to make context-dependent decisions: branch-based notification routing, version-tagged Slack messages, hosts-aware deployment scripts. Pre-deploy hooks receive the future version before the deploy starts. Post-deploy hooks receive the actual deployed version plus the deploy duration for performance monitoring.
- **Why add to Tengiz:** Existing hooks (#9, pre-deploy) and the extended hook system (#24) mention hooks exist but don't specify what context they receive. Without rich context, hooks are limited to static scripts. With environment context, hooks become powerful: `TENGIZ_APP_NAME`, `TENGIZ_DEPLOY_VERSION`, `TENGIZ_COMMIT_SHA`, `TENGIZ_ENV`, `TENGIZ_DEPLOY_SOURCE` (webhook/cli/git), `TENGIZ_HOSTS`, `TENGIZ_PREVIOUS_VERSION`, `TENGIZ_DEPLOY_DURATION_SECONDS`. A Slack notification hook can use `$TENGIZ_DEPLOY_SOURCE` to prefix messages with "🚀 Webhook deploy" vs "🖥️ Manual deploy". A post-deploy hook can use `$TENGIZ_DEPLOY_DURATION_SECONDS` to alert on slow deploys (>120s). Implementation: populate a `HookContext` struct in `internal/hooks/` with all relevant state from the deploy pipeline, pass as env vars to hook execution (`osexec.Command.Env`). Kamal's hook context is set in `lib/kamal/cli/base.rb` and `lib/kamal/configuration/hooks.rb`. Low effort (struct + env injection), high hook value multiplier.
- **Detected:** 2026-07-21

## Hook Output & Error Handling Configuration (Verbose/Quiet Per Hook)
- **Source:** Kamal
- **Description:** Kamal's `hooks_output` per-hook output configuration: `false` (quiet — only show hook name and exit code), `true` (verbose — show all stdout/stderr from the hook), or `"only_on_error"` (show output only when the hook fails). Output is captured per-hook and displayed alongside the deploy output at the appropriate verbosity level. Hook failures are captured with exit codes and error messages. Combined with the `--skip-hooks` flag for emergency deploys.
- **Why add to Tengiz:** Hook scripts can be noisy — a post-deploy notification hook that echoes "Sending to Slack... Message sent!" adds clutter to the deploy output. Per-hook output control lets hooks choose their verbosity: pre-deploy migration hooks are `verbose` (show migration output), post-deploy notifications are `quiet` (no output unless they fail). Implementation: `hooks_output` config field per hook in `.tengiz.yaml` (or a global default). A `HookRunner` that captures stdout/stderr, checks the verbosity setting, and displays output accordingly. Kamal's output handling is in `lib/kamal/configuration/hooks.rb`. Low effort (io.MultiWriter + config parsing), improves the hook UX dramatically.
- **Detected:** 2026-07-21

## Docker Host Architecture Detection (Platform-Aware Builds)
- **Source:** Kamal
- **Description:** Kamal auto-detects the Docker host CPU architecture (amd64/arm64) using `docker info --format '{{.Architecture}}'` and selects the appropriate build platform. If the host is ARM64, Kamal builds for `linux/arm64`; if AMD64, it builds for `linux/amd64`. Combined with multi-arch build configuration (`builder.arch: [amd64, arm64]`), Kamal can build native arch locally and cross-compile other archs via buildx. Architecture detection also validates that the builder supports the target platform before starting the build.
- **Why add to Tengiz:** Without architecture detection, Tengiz assumes the build host architecture matches the deploy host. This breaks when developers on Apple Silicon (ARM64) trigger a deploy to an Intel (AMD64) server — the built image won't run. Architecture detection enables: automatic `--platform` selection for builds, validation that buildx driver supports the target platform, and correct `FROM --platform=$TARGETPLATFORM` in generated Dockerfiles. Implementation: `docker info --format '{{.Architecture}}'` parsed and stored in a `HostArch` field in the builder config. Multi-arch builds (#69) require this as a prerequisite. Kamal's detection is in `lib/kamal/configuration/validator.rb` and `lib/kamal/configuration/builder.rb`. Low effort (docker info parsing + string matching), prerequisite for cross-platform deployment. Complements Multi-Architecture Builds (#69) and Remote Docker Builder (#70).
- **Detected:** 2026-07-21

## Deploy Lock Configuration (Wait Timeout & Retry Interval)
- **Source:** Kamal
- **Description:** Kamal's deploy lock supports configurable wait behavior: `--lock-wait-timeout` (max total time to wait for the lock, default 0 = no wait) and `--lock-wait-interval` (how often to retry acquiring the lock, default 5s). When a deploy encounters a held lock with `--lock-wait-timeout 60`, it polls every `--lock-wait-interval` seconds until the lock is released or the timeout expires. This prevents deploy failures in CI/CD when a previous deploy is still running — the CI job waits instead of immediately failing. Lock messages include the deployer identity and timestamp for debugging. `kamal lock status` shows who holds the lock, when it was acquired, and the lock file path.
- **Why add to Tengiz:** Existing Deploy Lock (#15) describes the basic mechanism but lacks configuration knobs. In CI/CD environments, immediate lock failure causes pipeline failures that need manual retry. Configurable wait transforms lock contention from a hard failure into a soft delay. `.tengiz.yaml`'da `deploy.lock_timeout: 60` ve `deploy.lock_interval: 5` ile yapılandırılır. `tengiz lock status` shows lock details. Implementation: in `internal/deploy/lock.go`, add a polling loop with configurable timeout and interval, atomic mkdir as the lock primitive (filesystem atomicity). Kamal's lock implementation is in `lib/kamal/cli/lock.rb`. Low effort (timer + polling loop), high CI/CD reliability value.
- **Detected:** 2026-07-21

## Healthcheck Barrier for Multi-Service Coordination
- **Source:** Kamal
- **Description:** Kamal's `Healthcheck::Barrier` uses `Concurrent::IVar` to synchronize container startup across roles. When `primary_role` (default: web) is defined, Kamal boots all primary role containers first and waits for their health checks to pass before booting secondary roles (workers, jobs). If any primary container fails the health check, the barrier closes and no secondary containers are booted — the deploy is aborted. The barrier timeout matches the `deploy_timeout` configuration. Failed container logs are automatically captured for debugging. This ensures that traffic-serving infrastructure is healthy before background workers start.
- **Why add to Tengiz:** For multi-process apps (web + worker + scheduler), deploying the worker before the web server is healthy can cause processing failures (worker connects to a not-yet-ready app). A healthcheck barrier enforces startup order: database accessories start first, then web containers, then workers. Extends Tengiz's existing health check (#4) from per-container to cross-container coordination. `.tengiz.yaml`'da `deploy.barrier.enabled: true` ve `deploy.barrier.primary_role: web` ile yapılandırılır. Implementation: a `Barrier` struct in `internal/deploy/` using Go channels + `context.Context` with timeout. Primary group containers are started and health-checked in a goroutine; secondary group goroutines block on a channel until the primary group reports healthy or the timeout fires. Kamal's barrier is in `lib/kamal/cli/healthcheck/barrier.rb`. Medium effort (channel-based synchronization + health check integration), important for multi-process/accessory deployments.
- **Detected:** 2026-07-21
...
## Teams & Invitations (Multi-User Team Management)
- **Source:** Coolify
- **Description:** Coolify supports full team-based multi-user access with email invitations, role-based roles (admin/owner/member), team switching, and team-scoped resource isolation. The `TeamInvitation` model handles invite lifecycle (pending/accepted/declined/revoked), with expiry and resend support. Resources (apps, servers, deployments) are scoped to a team — users see only their team's resources. Admins manage team membership, roles, and billing. Team-switching lets users belong to multiple teams and context-switch between them.
- **Why add to Tengiz:** Currently Tengiz has no user management — anyone with CLI or proxy access controls all apps. For teams using Tengiz on a shared server, multi-user team management provides: per-developer access control (admins vs members), audit trail with real user identities, team-scoped resource isolation (Team A can't see Team B's apps), and an invitation workflow for onboarding. Complements existing OIDC/OAuth SSO (#157) which handles auth — this handles authorization + team lifecycle. Implementation: lightweight team store in `~/.tengiz/teams.json`, `tengiz team create/invite/leave/list` commands, role checks in CLI middleware before state-modifying operations. Team IDs stored in `AppEntry.TeamID`. Email invitations via SMTP notification engine. Low-medium effort for the basic model, higher for email-based invite flow. Essential for any multi-developer Tengiz deployment.
- **Detected:** 2026-07-21

## Shared Environment Variables (Team/Project-Wide Env Inheritance)
- **Source:** Coolify
- **Description:** Coolify's `SharedEnvironmentVariable` model defines environment variables at the team/project level that are automatically inherited by all apps in that scope. When a shared env var is updated, all apps that reference it receive the new value on their next deploy (or immediately if hot-reload is configured). Override precedence: app-level > project-level > team-level > global. Enables defining `DATABASE_URL`, `SENTRY_DSN`, `STRIPE_PUBLIC_KEY` once for all apps in a project, eliminating duplication and ensuring consistency.
- **Why add to Tengiz:** Without shared env vars, operators managing 10+ apps must copy-paste the same env vars (DB URLs, API keys, logging endpoints) into every app's config. This is error-prone (typos, stale values, inconsistency across apps) and violates DRY principles. Shared env vars complement existing env var management (#19, implemented) and the Variable Resource (#1462, planned for interpolation). Unlike variables which use `[[var.name]]` syntax for interpolation, shared env vars are direct value inheritance — the app literally receives `DATABASE_URL=postgres://...` without any template syntax. Implementation: a `shared_envs` section in `.tengiz.yaml` or a separate `~/.tengiz/shared-envs.json` store. Apps declare which shared envs they inherit via `inherit_envs: [production-db, shared-cache]`. The deploy pipeline merges inherited envs with app-specific ones (app-level wins). Low-medium effort, high operational efficiency value for multi-app deployments.
- **Detected:** 2026-07-21

## Cloud Provider Integration (VPS Provisioning from Cloud APIs)
- **Source:** Coolify
- **Description:** Coolify integrates with cloud provider APIs (Hetzner, AWS EC2, DigitalOcean, Vultr, Linode) to provision new server instances directly from the platform. The `CloudProviderToken` model stores API tokens per provider. Users select a provider, configure instance specs (type, region, OS image, volume size), and Coolify auto-provisions the server, installs Docker, connects it as a Coolify-managed server — all without leaving the dashboard. Supports listing available server types, regions, and images per provider. Provisioned servers are automatically added to the server pool for deployments.
- **Why add to Tengiz:** Multi-server deployment requires server infrastructure. Currently users must manually provision VPS instances, install Docker, and configure SSH access — a significant friction point for self-hosting. Cloud provider integration makes Tengiz an end-to-end platform: select Hetzner CX22 → Tengiz provisions the server → installs Docker → registers it as a deployment target → deploy apps. This closes the loop between "I want a server" and "my app is deployed." Implementation: a `tengiz server provision` command family with provider-specific subcommands (`tengiz server provision hetzner --plan cx22 --region fsn1`). Go libraries: `hetznercloud/hcloud-go` for Hetzner, `aws/aws-sdk-go-v2` for EC2. Provider credentials stored encrypted in `~/.tengiz/providers/`. The provision flow: call cloud API → wait for instance → SSH in → install Docker → register as managed server. High effort (multiple provider API integrations, SSH bootstrap), but transformative for the user experience — eliminates the "you need a server before you can use Tengiz" prerequisite. Long-term differentiation from Coolify/Dokku/Kamal which require pre-existing servers.
- **Detected:** 2026-07-21

## Forward Auth (External Authentication Middleware)
- **Source:** Dokploy
- **Description:** Proxy-level authentication delegation to an external auth service (Authentik, Authelia, OAuth2 Proxy, Cloudflare Access). Before requests reach the application, they are intercepted by the forward auth middleware, which checks for a valid session cookie or token. If unauthenticated, the request is redirected to the auth service's login page. After successful authentication, the auth service injects identity headers (X-Forwarded-User, X-Forwarded-Email, X-Forwarded-Groups) and the request continues to the app. Dokploy implements this via Traefik ForwardAuth middleware in packages/server/src/setup/forward-auth-setup.ts. Per-app enable/disable, configurable auth service URL, and header passthrough settings.
- **Why add to Tengiz:** Distinct from HTTP Basic Auth (simple static credentials) and OIDC/SSO (user-facing login). Forward Auth is the industry-standard pattern for adding authentication to ANY app without modifying app code. Users can protect staging/preview environments, add single sign-on to internal tools, or integrate enterprise identity providers — all at the proxy level. Tengiz's Go reverse proxy can implement this with a middleware that calls an external auth endpoint, caches the result briefly, and injects identity headers. `.tengiz.yaml`'da `auth.forward_auth.url: https://auth.example.com/verify` ve `auth.forward_auth.headers: [X-Forwarded-User, X-Forwarded-Email]` ile yapilandirilir. Complements Proxy Security Middleware with identity-aware access control. Medium effort (HTTP middleware + caching), high value for zero-code app authentication.
- **Detected:** 2026-07-21

## Deploy Branch Configuration (Git Branch Filtering)
- **Source:** Dokku
- **Description:** Per-app configuration for which git branch triggers deployments (`git:set deploy-branch`). Dokku's default is `master`; users can set it to `main`, `develop`, `prod`, or any branch. Only pushes to the configured branch trigger the deploy pipeline — pushes to other branches are accepted but don't trigger a build. The configured branch is persisted per app. `git push dokku <other-branch>:master` can force-deploy a different branch. Multi-branch support via custom receive hooks.
- **Why add to Tengiz:** Git-based deployment is already implemented, but currently any push to the tracked repo triggers a deploy. Teams need to control which branch triggers production deploys vs which are ignored (feature branches, WIP commits). This is standard in Vercel/Netlify/Coolify — preventing wasteful builds is a basic CI/CD hygiene feature. `.tengiz.yaml`'da `git.deploy_branch: main` ile yapılandırılır. A mismatch between pushed branch and deploy_branch skips the entire deploy pipeline (no build, no container start). The webhook handler checks the branch from the push event payload before proceeding. Low effort (string comparison in git hook/webhook handler), high CI/CD value. Complements Webhook Event Filtering (branch/tag/path filtering, added 2026-07-16).
- **Detected:** 2026-07-21

## Inter-Container Network Aliases (DNS-Based Service Discovery)
- **Source:** Dokku
- **Description:** Dokku auto-creates Docker network aliases for every app container in the format `<appname>.<processtype>` (e.g., `myapp.web`, `myapp.worker`). These aliases are attached to the Docker bridge network, enabling containers to discover and communicate with each other by name — no hardcoded IPs or `--link` needed. The `network` plugin manages alias attachment with three phases: `initial-network` (before container start), `attach-post-create` (after creation but before start), and `attach-post-deploy` (after health checks pass). Multi-container apps (web + worker) automatically discover each other via these aliases. Works across Docker bridge networks and DNS resolution is handled by Docker's embedded DNS server (`127.0.0.11`).
- **Why add to Tengiz:** Many Tengiz apps will need inter-container communication: a web process talking to a background worker, an accessory Postgres being accessed by the main app, or preview deployments that need to resolve each other. Currently containers would need hardcoded IPs or manual `--link` flags. Network aliases provide zero-config service discovery that works transparently with Docker's built-in DNS. Implementation: `docker run --network-alias <app>.<type>` flag in `runtime.Create()`. For single-process apps, alias is `<appname>.web`. For multi-process (future), each process type gets its own alias. Also enables custom TLD configuration (`svc.cluster.local`) and static web listener (separate feature) for cold-start fallback. Complements the upcoming Custom Docker Network (#30) — aliases enable DNS-based discovery within those networks. Low effort (one Docker CLI flag per container), high value for multi-service architectures. Differentiates Tengiz from Coolify/Kamal which lack built-in service discovery.
- **Detected:** 2026-07-21

## Static Web Listener (Cold-Start Fallback Address for Proxy)
- **Source:** Dokku
- **Description:** Dokku's `network:set <app> static-web-listener <host:port>` provides a fallback network address for the proxy to use when no container is running for an app. During cold start (scale-to-zero → container booting) or after a failed deploy, instead of returning a connection refused error, the proxy can forward traffic to a static listener that serves a "starting..." status page or a maintenance message. The static listener is declared per-app and persists regardless of container state. When a container starts and passes health checks, the proxy switches from the static listener to the real container. This enables graceful cold-start UX.
- **Why add to Tengiz:** Tengiz's scale-to-zero and cold-start architecture means containers frequently transition between stopped and running states. During the ~1-10 second cold-start window, requests to the app return raw HTTP 502/503 errors or connection refused — a poor user experience. A static web listener provides a configurable fallback: a simple Go HTTP server that returns a user-friendly "App is starting..." page (with optional branding, estimated wait time, or retry-after header). Implementation: when an app is stopped or starting, Tengiz's proxy routes to the static listener instead of returning an error. The static listener is a lightweight embedded HTTP server that serves a configurable HTML page. `.tengiz.yaml`'da `proxy.static_listener.enabled: true` ve `proxy.static_listener.page: "custom-starting.html"` ile yapılandırılır. Complements Error Pages (#21, custom 404/500 pages) — this covers the cold-start case specifically. Low-medium effort (lightweight embedded HTTP server + proxy routing logic), high UX impact.
- **Detected:** 2026-07-21

## Bind-All-Interfaces Toggle (Container Network Binding Control)
- **Source:** Dokku
- **Description:** Dokku's `network:set <app> bind-all-interfaces <true|false>` controls whether the app's Docker container binds to `0.0.0.0` (all interfaces) or only to the Docker bridge IP. When `false` (default in Dokku), containers bind only to the Docker bridge network (`172.17.0.x`), preventing accidental exposure of internal services to the host network. When `true`, containers bind to `0.0.0.0`, making them accessible from the host and other networks. This provides a security boundary between internal app containers and external network access.
- **Why add to Tengiz:** Currently Tengiz containers bind to `0.0.0.0` by default (Docker default), meaning every container is potentially accessible from the host network. For accessory services (Postgres, Redis), background workers, or internal APIs, this is unnecessary network exposure. A per-app `bind_all_interfaces: false` setting makes containers bind only to Docker's internal bridge, adding a security layer. Implementation: when `bind_all_interfaces` is `false`, Tengiz runs `docker run` without `-p` port mapping (containers communicate via Docker DNS/network aliases) and uses Docker bridge IP for proxy routing. When `true` (default for web apps), normal port mapping applies. `.tengiz.yaml`'da `network.bind_all_interfaces: false` ile yapılandırılır. Complements Inter-Container Network Aliases (service discovery) — containers on the bridge network can find each other by name without host-network exposure. Low effort (Docker run flag selection + proxy routing), high security value.
- **Detected:** 2026-07-21

## Orphaned SSL Certificate Detection (Expiring Unused Certs)
- **Source:** CapRover
- **Description:** CapRover's `CertbotManager.isExpiringOrphanedCertificate()` detects SSL certificates for domains no longer in active use that are about to expire (within 48 hours). `logExpiringOrphanedCertificates()` reads the cert renewal config directory, checks each certificate against the active domain set, and logs orphaned near-expiry certs for administrative action. Called during the cert renewal cycle (~20.3h interval). Prevents certificate-related outages from stale certs that users forgot to remove.
- **Why add to Tengiz:** As apps are deleted or domains change, old SSL certificates accumulate. These orphaned certs eventually expire, causing certificate monitoring alerts and confusion. Tengiz's SSL management should detect and report certs for domains no longer active, enabling proactive cleanup before expiry. Implementation: during the cert renewal check, iterate stored certs, compare against active domain list per app, flag any domain-not-found certs expiring within 48h. Log to audit trail, optionally auto-remove. Low effort (domain list + cert expiry comparison), prevents a class of certificate-related incidents.
- **Detected:** 2026-07-21

## Password Change Token Versioning (Immediate Token Invalidation)
- **Source:** CapRover
- **Description:** When a user changes their password, all existing auth tokens are immediately invalidated via a `tokenVersion` counter. CapRover's `Authenticator.changepass()` generates a new `tokenVersion` (secure random 64-char string) on password change. All subsequent token validation checks the token's stored version against the current `tokenVersion` — mismatches are rejected. This prevents compromised tokens from working after a password rotation.
- **Why add to Tengiz:** If an admin password is compromised, the first action is to change it. But without token versioning, any previously issued tokens (CI/CD deploy tokens, API keys) remain valid. Token versioning: immediately invalidates all tokens on password change, forces re-authentication for all sessions, and provides a clear security boundary — password change = full session reset. Implementation: `tokenVersion` field in AuthStore, incremented (or regenerated) on password change, checked on every authenticated request. Complements existing App Deploy Tokens (#62) — deploy tokens would use a separate version counter scoped per token, not the user password version. Low effort (version field + comparison in auth middleware), critical production security hygiene.
- **Detected:** 2026-07-21

## Download Token for Backup Files (Short-Lived Secure URLs)
- **Source:** CapRover
- **Description:** CapRover protects backup file downloads with short-lived JWT tokens (2-minute expiry) instead of requiring full auth tokens. `Authenticator.getDownloadToken()` generates a JWT with claim `{data: 'download', expiresIn: '2m'}`. `decodeDownloadToken()` validates the token on download request. This allows secure backup sharing without exposing admin credentials — tokens are time-limited and scope-restricted.
- **Why add to Tengiz:** `tengiz backup create` and `tengiz backup restore` need a mechanism to transfer backup files securely. Currently any user with the file path can download backups. Download tokens provide: time-limited access (2min default, configurable), scope-restricted (can only download, not modify), shareable without exposing credentials (email a download link), and revocable (tokens are validated per-request). Implementation: `tengiz backup download <app> --token <token>` or `tengiz backup share <app> --duration 5m` generates a JWT-signed download URL. Go's `github.com/golang-jwt/jwt` for token creation/validation. Tokens stored in `~/.tengiz/tokens/` for validation. Low effort (JWT generation + middleware), high security value for admin operations.
- **Detected:** 2026-07-21

## JavaScript-Based Pre-Deploy Hooks (Programmatic Docker Config Modification)
- **Source:** CapRover
- **Description:** Beyond simple shell-command pre-deploy hooks, CapRover supports JavaScript functions as pre-deploy hooks. `ServiceManager.createPreDeployFunctionIfExist()` loads JS from a string via `require-from-string`, wraps it in `module.exports = function(captainAppObj, dockerUpdateObject)`. The JS function receives the full Docker service update object and the app definition, and can programmatically modify deployment parameters (add volumes, set environment vars, configure health checks, modify Docker labels). This enables dynamic, context-aware pre-deploy logic impossible with static shell commands.
- **Why add to Tengiz:** Existing Pre-Deploy Hooks (#15, shell commands) are limited to running scripts. JavaScript-based hooks enable: programmatic Docker config manipulation based on deploy context, conditional logic (skip if env=staging), dynamic value generation (compute resource limits based on app type), integration with external APIs (fetch config from remote service during deploy). Implementation: embed `goja` (pure Go JS runtime, already referenced for embedded serverless functions) to execute user-provided JS functions during deploy. The JS context receives the `AppEntry` and the Docker run command args, and can return modified config. `.tengiz.yaml`'da `hooks.pre_deploy_js: "function(ctx) { ctx.setEnv('COMMIT_SHA', ctx.gitSha); }"` veya ayrı bir `.js` dosyası referansı. Low-medium effort (goja embedding), enables significantly more powerful deploy automation.
- **Detected:** 2026-07-21

## Service Update Override (Raw Docker Config Injection)
- **Source:** CapRover
- **Description:** CapRover allows injecting arbitrary Docker service update JSON/YAML to override specific deployment parameters. `ServiceManager` validates and parses `serviceUpdateOverride` as JSON/YAML and passes it to `dockerApi.updateService()` as the final parameter. This provides a power-user escape hatch for any Docker configuration not directly supported by CapRover's UI — resource limits, restart policies, placement constraints, logging config, label changes, environment overrides, etc.
- **Why add to Tengiz:** Not every Docker configuration flag can or should have a first-class Tengiz config option. A raw Docker config override provides: future-proofing (new Docker features work immediately), power-user flexibility (configure anything Docker supports), migration path (users migrating from Docker Compose can paste their service config directly), and debugging (apply experimental Docker settings). Implementation: `~/.tengiz.yaml`'da `docker.service_update: {...}` veya `tengiz config set myapp docker.service_update '{"limit":{"memory":"512M"}}'`. The deploy pipeline merges the override JSON with the computed Docker run config before execution. Different from Custom Docker Options (#23) which adds CLI flags to `docker run` — this injects into the Docker API call at the JSON level. Low effort (JSON merge in deploy pipeline), high value for power users.
- **Detected:** 2026-07-21

## Configurable App Log Retention (Per-App Log Size Limits)
- **Source:** CapRover
- **Description:** CapRover maintains configurable log size limits for both build logs and runtime logs via `CaptainConstants.buildLogSize (50KB)` and `appLogSize (500KB)`. Build logs use a circular buffer — when the log exceeds the limit, older entries are discarded. This prevents unbounded log growth on disk from verbose applications or frequent deployments.
- **Why add to Tengiz:** Tengiz stores build logs (Build Logs feature, implemented) and container logs (`tengiz logs`). Without size limits, verbose apps or frequent deploys can generate gigabytes of logs, filling disk and causing platform failures. Per-app configurable log retention gives operators control: set 100KB for build logs (enough for most builds), 1MB for runtime logs. Implementation: `tail -n <limit>` truncation when storing, or circular buffer in log file writer. `.tengiz.yaml`'da `build.log_size_kb: 100` ve `logs.max_size_mb: 10` ile yapılandırılır. Complements Docker Housekeeping (#6) and Granular Docker Prune (#145) — those clean Docker artifacts; this cleans Tengiz-managed log files. Complements Docker Logging Config (#31, log driver selection). Low effort (size check + truncation in log writer), prevents a common production disk-full incident.
- **Detected:** 2026-07-21

## Docker Secrets for Encryption Key Storage (Secure Key Management)
- **Source:** CapRover
- **Description:** CapRover stores its master encryption salt as a Docker secret (`captain-salt`) rather than in a config file. `CaptainManager.ensureSecret()` creates the Docker secret at initialization if it doesn't exist. `getSecretValue()` reads it from `/run/secrets/captain-salt` at runtime. Docker secrets are encrypted during transit and at rest by Docker Swarm, and are never written to disk unencrypted (stored in Swarm's Raft log). This provides hardware-backed security for the master encryption key.
- **Why add to Tengiz:** Existing Encryption at Rest (#78) describes AES-GCM encrypting env vars and secrets. But where is the encryption key stored? File-based keys (`~/.tengiz/.key`) are vulnerable to filesystem compromise. Docker secrets provide a more secure storage mechanism: keys are managed by Docker, never written to disk unencrypted, and can be rotated via `docker secret create/rm`. Implementation: on `tengiz init`, generate a 256-bit key and store it as a Docker secret. At runtime, read from `/run/secrets/tengiz-key`. Fall back to `~/.tengiz/.key` if Docker secrets aren't available (single-server without Swarm mode). The key is used for AES-GCM encryption of env vars, API tokens, registry credentials, and backup encryption. Low-medium effort (Docker secret API via `os/exec` + file fallback), significantly improves encryption security.
- **Detected:** 2026-07-21

## Multiple Named Data Stores (Namespace Isolation for Multi-Tenancy)
- **Source:** CapRover
- **Description:** CapRover uses a namespace-based data store system where each namespace has its own isolated config file (`config-{namespace}.json`). `DataStoreProvider` creates per-namespace store instances. The root namespace is `captain`. This provides complete config isolation between tenants — namespace A's apps, domains, and settings are stored in a separate file from namespace B's. Prevents cross-tenant data leaks and enables per-tenant backup/restore.
- **Why add to Tengiz:** Tengiz currently stores all data in a flat `~/.tengiz/apps.json`, `ports.json`, etc. For multi-tenant deployments (hosting provider scenario, multiple teams on one server), namespace isolation is essential: Team A's app configs are in `apps-team-a.json`, Team B's in `apps-team-b.json`. Each namespace has its own port allocation, domain list, and config. Implementation: `tengiz namespace create <name>` creates an isolated store. `tengiz --namespace team-a deploy .` deploys into that namespace. Store files: `~/.tengiz/{namespace}/apps.json`. CLI global flag `--namespace` scopes all operations. Complements Teams & Invitations (team-scoped resource isolation) — namespaces provide the data isolation layer, teams provide the user management layer. Medium effort (store refactoring + namespace CLI flag), foundational for multi-tenant Tengiz deployments.
- **Detected:** 2026-07-21

## Server Info API (Docker Infrastructure Visibility Endpoint)
- **Source:** CapRover
- **Description:** CapRover exposes a `GET /nodes/` endpoint returning Docker Swarm cluster node information: node ID, role (manager/worker), architecture, OS, Docker engine version, CPU count, total memory, IP address. `ServerDockerInfo` model stores this data. `CaptainManager.getNodesInfo()` queries Docker API for node info. This provides infrastructure visibility critical for capacity planning and debugging.
- **Why add to Tengiz:** Tengiz operators need to answer: what Docker version is running, how much RAM/CPU is available, is it amd64 or arm64, what OS is the host running? Currently users must fall back to `docker info` or `uname -a`. A `tengiz server info` command and `GET /api/server` endpoint would surface this info. Implementation: `docker info --format '{{json .}}'` parsed into a Go struct, displayed as a table. Fields: Docker version, OS/arch, total CPUs, total memory, storage driver, Docker root dir, OS type. `tengiz doctor` could run this as part of system validation. Low effort (Docker CLI passthrough + JSON parsing), provides essential infra visibility. Complements Server Monitoring (#73) with a snapshot view.
- **Detected:** 2026-07-21

---

## Server Onboarding Key System (One-Time Registration Tokens)
- **Source:** Komodo
- **Description:** When a new server agent connects to the core, it presents a one-time onboarding key generated during core initialization. The key authenticates the server and auto-configures a public/private key pair for all subsequent communication. Keys are single-use — after the first successful authentication, they are invalidated and replaced with persistent per-server credentials. Onboarding keys eliminate the need for manual SSH key exchange or shared secrets when adding servers to a multi-server deployment. Komodo's implementation (`bin/core/src/server_key.rs`) generates a 32-byte random seed, derives an ed25519 key pair, and stores the public key. The server presents this key on its initial connection request. The core validates it, creates a persistent server identity, and rotates the key for ongoing use.
- **Why add to Tengiz:** As Tengiz expands to multi-server deployments (Multi-Server Architecture with Periphery Agent, #163), onboarding becomes a critical friction point. Manual SSH key exchange, shared credential management, and server identity bootstrap are complex and error-prone. Onboarding keys provide: zero-config server registration (start the agent with the key, it auto-registers), security (keys are single-use, cannot be replayed), rotation (persistent keys replace the onboarding key after first use), and audit trail (every server registration is logged). Implementation: `tengiz onboard-key generate` creates a key printed to stdout. `tengiz agent --onboard-key <key>` registers the server. The key is stored in `~/.tengiz/onboard-keys/` and deleted after first use. Cryptographic identity uses ed25519 (Go's `crypto/ed25519`). Complements SSH Key Management (#99) — onboarding keys handle initial registration, SSH keys handle ongoing authentication. Medium effort (key generation + connection handshake + rotation), unblocks the multi-server architecture.
- **Detected:** 2026-07-21

## Resource Abstraction (Uniform Resource Model for All Types)
- **Source:** Komodo
- **Description:** All Komodo resources (Server, Deployment, Stack, Build, Builder, Repo, Procedure, Action, ResourceSync, Alerter, Swarm, User Group, Variable) follow the same uniform pattern: each has a `Config` struct (user-settable parameters), an `Info` struct (computed/read-only state), and a `PartialConfig` struct (for partial updates). This consistency provides: auto-generated OpenAPI specs from a single code path, uniform permission inheritance (e.g., deployments inherit from servers), generic CRUD routes that work for ALL resource types without per-type boilerplate, consistent audit trail and update records, and trivial addition of new resource types. The resource system (`bin/core/src/resource/`) defines ~20 lines of trait per type — config fields, info fields, and permission checks. Everything else (CRUD, API, serialization, validation) is auto-generated from the trait implementation.
- **Why add to Tengiz:** Tengiz currently has no resource abstraction — apps, domains, volumes, env vars, and build records are all stored in separate ad-hoc JSON files with inconsistent read/write patterns. Adding a new feature requires: new store functions, new CLI handlers, new API endpoints, new permission checks — all custom code. A uniform resource model would: reduce new feature development time by 60%+, enable auto-generated OpenAPI spec for the REST API (P0 #8), provide consistent permission inheritance across all types, make the codebase more maintainable and testable. Implementation: a `Resource[T any]` generic struct in Go 1.26 (`internal/resource/resource.go`) with `Config()`, `Info()`, `Validate()` methods. Each resource type (App, Domain, Volume, Variable, Build) implements the `Resource` interface. A generic `Store[T Resource]` provides CRUD. Permission checks chain through parent resources automatically. This is an architectural refactor, not a user-facing feature — but it enables all other features to be added faster and more consistently. High effort (refactor existing store, types, CLI handlers) but transformative for the codebase velocity. Komodo's `bin/core/src/resource/` with its `CreateResource`, `UpdateResource` derives and `ResourceTarget` enum is the model — ~30 trait impls, each ~20 lines, supporting all resource operations.
- **Detected:** 2026-07-21

## Variable Interpolation in All Config Fields (Beyond Env Vars)
- **Source:** Komodo
- **Description:** Komodo's `[[variable.name]]` and `[[secret.name]]` interpolation syntax works in ALL resource configuration fields — not just environment variables and build args. Interpolation is supported in: compose file environment directives (`services.app.environment`), compose extra arguments (`docker compose -f ...`), repo git hooks (pre/post receive commands), build extra Docker args, deployment docker options, stack run command, and any other free-text config field. The interpolator scans all string fields recursively for `[[...]]` patterns and replaces them with the variable or secret value. Variables can reference other variables (`[[db_url]]` → `postgres://user:pass@[[db_host]]:5432/mydb`). Secret-flagged values are masked in logs and API responses.
- **Why add to Tengiz:** Existing Variable Resource (#1462) and Secret Interpolation System (#1468) limit interpolation to deployment env vars and build args. Komodo's approach is more powerful — variables injected into compose configs, custom Docker options, hook commands, and stack run commands enable: dynamic compose files (database hostnames auto-injected at stack deploy), consistent secret injection across all resource types (same `[[npm_token]]` used in build args AND `.npmrc` content), template-based configuration (compose files with `[[version]]` that auto-updates across all stacks). Implementation: a recursive string interpolator in `internal/variables/` that walks all `map[string]any` and `[]any` fields in any config struct, finds `[[...]]` patterns, and resolves them against the Variables and Secrets stores. The interpolator runs before any operation that uses the config (build, deploy, stack up, hook exec). `.tengiz.yaml`'da `variables:` and `secrets:` sections feed the interpolator. Low-medium effort (recursive string replacement + store lookup), high power-user value. Complements the existing Variable/Secret features by expanding their scope from env-only to all-config.
- **Detected:** 2026-07-21

## Alert Buffer with 2-Check Requirement (Transient Alert Suppression)
- **Source:** Komodo
- **Description:** Komodo's alert system does not fire immediately when a problem is detected. Instead, it maintains an alert buffer that requires the same alert condition to be detected 2 consecutive check cycles before the alert transitions from `Ok` to `Warning`/`Error`/`Critical`. This prevents alert storms from transient issues: momentary network blips, brief CPU spikes, health check timeout due to GC pause, or a single failed deploy followed by a successful retry. The buffer is per-alert-source — if the second check returns `Ok`, the alert counter resets. After firing, the alert transitions to `Resolved` on a single successful check (no double-verify for resolution — a single OK clears it). This design balances avoiding false positives (double-verify to fire) against rapid recovery (single-verify to resolve). Komodo's `bin/core/src/alert.rs` implements this: `alert_buffer: Vec<(ResourceTarget, Vec<AlertVariant>)>` stores pending alerts, checked every monitoring cycle.
- **Why add to Tengiz:** The existing Alert System (#1480) and Background Monitoring Scheduler (#1534) describe when alerts should fire but don't address the false positive problem. In a scale-to-zero platform, transient issues are common: cold starts take a few seconds (health check may briefly fail), Docker daemon may stutter during GC, network partitions are brief. Without an alert buffer, every transient issue generates an alert notification — leading to alert fatigue and missed real incidents. Implementation: `AlertBuffer` struct in `internal/alerts/` with a `map[ResourceTarget][]AlertVariant` storing pending alerts across monitoring cycles. On each check cycle: if the condition exists and is in buffer, increment severity; if condition exists and not in buffer, add to buffer with `Warning` severity; if condition is gone and in buffer, clear from buffer. After two consecutive detections at the same or higher severity, fire. Low effort (map + counter + time check), prevents a specific operational pain point (alert fatigue). Complements the Alert System by making it production-usable.
- **Detected:** 2026-07-21

## Platform Version Management & Upgrade Pipeline
- **Source:** Juno
- **Description:** Comprehensive platform version system beyond simple binary download. Juno tracks versions for all canister types (satellite, orbiter, mission control, console, observatory) via CDN-hosted release metadata (`releases.json` with semver, changelog, download URLs). Each canister stores its current version and checks against the latest available. Upgrades follow a pipeline: download new WASM binary from CDN → verify checksum → pre-upgrade hooks (backup state, drain connections) → install new version → post-upgrade hooks (run migrations, reinitialize services). Central Upgrade Dock UI shows all components, current vs latest versions, and upgrade status. Version rollback available via previous binary retention. Upgrade notifications when new versions are available.
- **Why add to Tengiz:** Existing Self-Upgrade/Auto-Update (#165) is a simple `go install` replacement of the Tengiz binary. Juno's system is enterprise-grade: it tracks versions for ALL platform components (not just the core binary), manages upgrade lifecycle with safety hooks, provides rollback when upgrades fail, and surfaces version information in the CLI. For Tengiz: `tengiz version check` shows current + latest across all components. `tengiz version upgrade [component]` downloads, verifies, applies with pre/post hooks. `tengiz version history` shows upgrade log. Implementation: a `~/.tengiz/versions.json` registry, release metadata URL, version checking goroutine, and hook integration in the upgrade pipeline. `.tengiz.yaml`'da `upgrade.channel: stable|beta`, `upgrade.auto_check: true`. Medium effort, transforms Tengiz from a static binary to a self-managing platform.
- **Detected:** 2026-07-21

## Automated Resource Monitoring with Self-Healing
- **Source:** Juno
- **Description:** Mission Control's monitoring engine continuously tracks canister resource consumption (cycles — the compute/storage resource on IC). Configurable thresholds trigger automated remediation: when cycles drop below `funding.threshold`, the monitoring scheduler converts ICP tokens to cycles and deposits them into the canister. This happens without user intervention — the platform self-heals resource depletion. Email notifications for deposit success/failure. Monitoring history records every deposit. Funding strategies configurable: target balance, minimum threshold, max deposit, notification preferences.
- **Why add to Tengiz:** Disk space is the #1 production issue on single-server deployments. Tengiz needs automated resource monitoring that goes beyond alerting (#57) into self-healing. Translated to Tengiz: a background goroutine monitors disk usage, container count, Docker image storage, memory pressure. Configurable thresholds trigger automated actions: disk > 80% → prune old images, disk > 90% → stop idle containers, memory > 85% → restart high-usage containers. Each action logged and notified. `.tengiz.yaml`'da `monitor.disk.warning: 80`, `monitor.disk.critical: 90`, `monitor.disk.action: prune_images`. `tengiz monitor status` shows current resource levels. Implementation: a `Monitor` goroutine in `internal/monitor/` with resource check interface and action executors. Different from Docker Housekeeping (#6, manual/timer-based) — this is reactive, threshold-based auto-remediation. Medium effort, prevents the most common production failure mode on Tengiz.
- **Detected:** 2026-07-21

## OpenID Provider Management (Dynamic JWKS Caching)
- **Source:** Juno
- **Description:** Juno's Observatory canister manages OpenID Connect providers with full lifecycle: add provider with issuer URL + client ID + scopes, automatic JWKS (JSON Web Key Set) certificate fetching and caching, background scheduler for periodic JWKS refresh, certificate expiry monitoring. This enables dynamic OIDC provider configuration without restarting any platform component. The Observatory maintains a JWKS cache in stable storage, refreshes keys before they expire (configurable interval), and exposes the certificate set for JWT validation.
- **Why add to Tengiz:** Existing OIDC/SSO (#128) describes OIDC integration but assumes static provider configuration. Juno's approach is more robust — dynamic JWKS management ensures provider key rotations don't break auth. Without JWKS caching, each request fetches keys from provider (latency, dependency) or uses stale keys (validation failures after key rotation). For Tengiz: `tengiz auth openid add <name> --issuer https://accounts.google.com --client-id <id>` configures a provider. Platform auto-fetches JWKS, caches in `~/.tengiz/openid/jwks.json`, refreshes before expiry. Multiple providers supported. `.tengiz.yaml`'da `auth.openid.providers: [{name: google, issuer: ..., client_id: ...}]`. Background scheduler for key refresh runs every hour. Low-medium effort (HTTP JWKS fetcher + file cache + scheduler), transforms OIDC from fragile-single-provider to robust-enterprise-auth.
- **Detected:** 2026-07-21
