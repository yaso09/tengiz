...
| 208 | **Concurrent Build Limits (Per-Server Throttling)** ⬜ | Orta-Yüksek | Düşük | Mükemmel | Semaphore-based global build throttle. Prevents Docker daemon overload from concurrent `tengiz deploy` calls. |
| 209 | **Docker Compose Preview Deployments** ⬜ | Yüksek | Orta | Mükemmel | Extend preview deployments to multi-service stacks. Each PR gets isolated DB + cache + web containers. |
| 210 | **Cloud Provider Auto-Provisioning (Hetzner)** ⬜ | Orta-Yüksek | Yüksek | Orta | `tengiz server create --provider hetzner`. API-based server creation with cloud-init Docker install. |
| 211 | **Container Escape Prevention** ⬜ | Orta | Çok Düşük | Mükemmel | `--security-opt=no-new-privileges:true` toggle per app. CIS Docker Benchmark recommended. |
| 212 | **Service Version Auto-Updates** ⬜ | Orta | Orta | Mükemmel | Auto-detect new template versions from registry, trigger zero-downtime redeploy of managed services. |
| 213 | **SSH Multiplexing for Remote Operations** ⬜ | Orta | Düşük | Mükemmel | SSH ControlMaster persistence eliminates per-command handshake overhead in multi-server deploys. |
| 214 | **Guided Onboarding Workflow** ⬜ | Yüksek | Düşük | Mükemmel | `tengiz setup` interactive wizard: check Docker → set URL → deploy sample app. First-impression critical. |
| 215 | **Literal Variables Mode** ⬜ | Düşük-Orta | Çok Düşük | Mükemmel | Per-var toggle to disable `$VAR` interpolation. Apps with `$` in values work correctly. |
...
## Concurrent Build Limits (Per-Server Throttling)
- **Source:** Coolify
- **Description:** Per-server limit on the number of concurrent builds (`concurrent_builds` property on `Server.php`). When a deployment is triggered and the server is at capacity, it queues rather than starting a new build. Prevents Docker daemon from being overwhelmed when multiple apps deploy simultaneously, especially on resource-constrained servers. Configurable per-server, with sensible defaults (default: 3 concurrent builds). Builds exceeding the limit wait in queue and execute FIFO when a slot opens.
- **Why add to Tengiz:** Tengiz's single-server architecture means all builds share one Docker daemon. Running 10 concurrent builds (from rapid git pushes or webhooks) can exhaust disk I/O, memory, and CPU — causing all builds to fail or time out. A concurrent build limit prevents resource thrashing and ensures reliable builds. Different from Build Queue with Dedup (#124) which deduplicates per-app queue entries — this is a global/server-level throttle that caps total concurrent builds. `.tengiz.yaml`'da `server.concurrent_builds: 5` ile yapılandırılır. Implementation: a counting semaphore (Go channel or `sync.Semaphore`) in the deploy pipeline that each build acquires before starting and releases on completion. Low effort, high operational stability value.
- **Detected:** 2026-07-19

## Container Escape Prevention (Security Hardening with no-new-privileges)
- **Source:** Coolify
- **Description:** Per-app toggle to prevent container escape via `--security-opt=no-new-privileges:true`. When enabled, processes inside the container cannot gain new privileges (e.g., via `suid` binaries, `setuid` syscalls, or kernel vulnerabilities). Coolify stores this as `escape_prevention` boolean on the Application model. Prevents a compromised container from escalating privileges to the host. A fundamental Docker security best practice (CIS Docker Benchmark recommends it).
- **Why add to Tengiz:** Container security is a layered defense. Tengiz already supports Resource Limits (#6, CPU/memory) and will support Linux Capabilities Management (#155, cap-drop/cap-add). no-new-privileges adds a critical layer: even if an attacker exploits a kernel vulnerability inside the container, they cannot escalate to root on the host. `.tengiz.yaml`'da `security.no_new_privileges: true` ile yapılandırılır. Implementation: add `--security-opt=no-new-privileges:true` to `docker run` flags in `runtime.Run()`. Default: enabled for production apps, toggleable for apps that legitimately need privilege escalation (e.g., container-based build systems, certain init processes). Low effort (single Docker CLI flag), high security value. Complements Container Security Bench (#148) for a comprehensive security posture.
- **Detected:** 2026-07-19

## Literal Variables Mode (Env Var Interpolation Toggle)
- **Source:** Coolify
- **Description:** Per-environment-variable toggle to prevent interpolation of `$VAR` and `${VAR}` syntax. Coolify's `EnvironmentVariable.php` model includes an `is_literal` boolean. When enabled, the env var value is used as-is, with no dollar-sign expansion. Essential for apps whose configuration values legitimately contain `$` characters (e.g., shell scripts stored as env vars, passwords containing `$`, JDBC connection strings, Prometheus relabel configs with `$1` regex backreferences).
- **Why add to Tengiz:** Tengiz interpolates env vars at deploy time — a feature for referencing other values. But this breaks apps that use `$` in their config. A literal mode toggle lets users opt out of interpolation per variable. CLI: `tengiz config set myapp DOLLAR_SIGN '$value' --literal` stores the value as-is. `.tengiz.yaml`'da `env.literals: [MY_VAR]` veya per-var `env: { MY_VAR: { value: "$original", literal: true } }`. Implementation: an `IsLiteral` boolean per env var in `AppEntry.Config.Env`, checked during interpolation pass. Low effort (boolean check in interpolation logic), eliminates a common user pain point.
- **Detected:** 2026-07-19

## Docker Compose Preview Deployments
- **Source:** Coolify
- **Description:** Preview deployments (PR-based ephemeral environments) extended to Docker Compose-based service applications — not just single-container apps. When a PR is opened against a Docker Compose project, Coolify deploys the entire Compose stack (web + db + redis + worker) as a preview environment with isolated containers and auto-generated preview domains. The same auto-cleanup applies: PR close → stack teardown. Preview databases get unique names to prevent data collision between preview environments.
- **Why add to Tengiz:** Existing Preview Deployments (#18, implemented) support single-container apps. But many real-world projects use Docker Compose for multi-service setups. A PR preview of just the web container without its database and cache is non-functional. Docker Compose previews mean: each PR gets a complete, isolated stack. Implementation: extend the preview deployment pipeline to handle Compose projects — generate unique service names (`pr-42-myapp-web`, `pr-42-myapp-db`), create an isolated Docker network per preview, auto-generate env vars for inter-service links, and clean up the entire stack on PR close. `.tengiz.yaml`'da `preview.compose: true` flag. Medium effort (extends existing preview + compose infrastructure), high value for multi-service applications.
- **Detected:** 2026-07-19

## Cloud Provider Auto-Provisioning (Hetzner Integration)
- **Source:** Coolify
- **Description:** Coolify integrates deeply with Hetzner Cloud to auto-provision new servers directly from the platform UI. Users select a location (nbg1, fsn1, hel1), server type (CX22, CX32, CX42), image (Ubuntu, Debian), and SSH key — Coolify then creates the server via Hetzner API, installs Docker via cloud-init, and registers it as a managed server. Cloud provider tokens (`CloudProviderToken.php`) are stored encrypted. `CloudInitScript.php` provides customizable initialization scripts for server setup. The entire process is automated with `ValidateAndInstallServerJob`.
- **Why add to Tengiz:** Adding a new server to a Tengiz multi-server setup currently requires: provision a VM manually, SSH in, install Docker, configure Tengiz. Auto-provisioning reduces this to: `tengiz server create --provider hetzner --location nbg1 --type cx22`. This is a major differentiator for Tengiz vs. other Vercel alternatives that only configure existing servers. Implementation: Go HTTP client for Hetzner Cloud API (`api.hetzner.cloud/v1/servers`), cloud-init script generation that installs Docker + Tengiz agent, SSH key injection via Hetzner API. `tengiz server providers` shows supported cloud providers. Start with Hetzner (simplest API, generous free tier), extend to DigitalOcean and Vultr later. Medium-high effort (cloud provider API client + cloud-init generation), transformative for the onboarding experience. Complements Server Bootstrap (#40) and One-Line Install Script (#140).
- **Detected:** 2026-07-19

## Service Version Auto-Updates (Template Registry → Running Service)
- **Source:** Coolify
- **Description:** Coolify tracks latest versions of one-click service templates from its CDN registry and can auto-update running services. When a service template gets a new version (e.g., WordPress 6.5 → 6.6), Coolify detects the change, builds a new image with updated Docker Compose, and optionally triggers a zero-downtime redeploy. Version tracking is per-service, with configurable auto-update (`auto_update` toggle) and scheduled update windows. Template version metadata is synced from the CDN via `PullTemplatesFromCDN` job.
- **Why add to Tengiz:** Existing Service Template Registry (#84, Coolify) covers template discovery and one-click deploy. But once a service is deployed, keeping it updated is manual — users must recreate the service with the new template version. Auto-updates close this gap: Tengiz periodically checks the template registry for newer versions of deployed services, and either notifies the user or auto-updates (configurable). Implementation: a background goroutine in `internal/templates/` that compares current vs latest version for each deployed service, generates a diff, and optionally triggers the deploy pipeline. `.tengiz.yaml`'da `service.auto_update: true` with `service.update_window: "02:00-04:00"` for scheduled maintenance windows. Medium effort (version comparison + safe update orchestration), high operational value for managed services. Complements Image Digest Change Detection (#73, Komodo) which watches arbitrary Docker images — this specifically targets Tengiz template-managed services.
- **Detected:** 2026-07-19

## SSH Multiplexing for Remote Operations
- **Source:** Coolify
- **Description:** SSH multiplexing with control socket persistence for efficient remote server connections. When Coolify connects to a remote Docker host via SSH, it creates a persistent control socket (`~/.tengiz/ssh-mux/<server-id>/control`) via SSH's `ControlMaster` feature. Subsequent connections reuse the existing TCP connection, eliminating SSH handshake overhead (TCP connect + key exchange + auth). Health checks monitor control socket validity — stale sockets are cleaned up by `CleanupStaleMultiplexedConnections` job. Configurable retry logic with exponential backoff for transient SSH failures.
- **Why add to Tengiz:** SSH Remote Deployment (#73) and Centralized Multi-Server Management (#143) require frequent SSH connections for Docker commands. Without multiplexing, each `docker pull`, `docker run`, `docker ps` opens a new SSH connection — the overhead adds 1-3 seconds per command. On a deploy that runs 10+ Docker commands, this is 10-30 seconds of pure SSH overhead. Multiplexing eliminates this: first connection establishes the tunnel, all subsequent commands reuse it. Implementation: Go's `os/exec` with `ssh -o ControlMaster=auto -o ControlPath=...` flags. Stale connection cleanup via `ssh -O stop` on control socket. `.tengiz.yaml`'da `ssh.multiplex: true` (default: enabled). Low-medium effort (Docker/SSH CLI flag passthrough + socket management), high performance impact for multi-server deployments.
- **Detected:** 2026-07-19

## Guided Onboarding Workflow (First-Run Setup Experience)
- **Source:** Coolify
- **Description:** Coolify provides a multi-step onboarding wizard on first launch: set admin credentials → configure instance name → connect Git provider → create first project → deploy a sample application. Each step has clear instructions and validation. Users are guided through the entire setup rather than presented with an empty dashboard. The onboarding state is tracked so users can resume where they left off. Sample application templates (Hello World in various frameworks) are available to demonstrate platform capabilities with one click.
- **Why add to Tengiz:** Current first-run experience: `go build -o tengiz . && ./tengiz init` — users get a minimal config and must figure out the rest. A guided onboarding flow via `tengiz setup` would: (1) check Docker availability, (2) prompt for instance URL, (3) optionally connect a Git provider, (4) create a sample app and deploy it, (5) print a summary with next steps. Implementation: interactive CLI wizard using Go's `bufio.Scanner` or `survey` library (already compatible with cobra). State persisted in `~/.tengiz/setup.json` to track progress. Sample app deployment: `tengiz setup --with-sample` clones a hello-world template, configures it, and deploys it to demonstrate the full pipeline. Low-medium effort (CLI prompts + template clone + deploy), high impact on new user adoption and first-impression.
- **Detected:** 2026-07-19
...
## Forward Auth / SSO Proxy for App Protection (oauth2-proxy)
- **Source:** Dokploy
- **Description:** Deploy an `oauth2-proxy` service that protects apps behind Single Sign-On authentication. Users must authenticate via an OIDC provider (Google, GitHub, Okta, Azure AD) before accessing proxied apps. The forward auth middleware intercepts requests, redirects unauthenticated users to the SSO login page, and injects authenticated user info via headers (`X-Auth-Request-User`, `X-Auth-Request-Email`). Configurable auth domain, cookie secrets, email domain whitelist, and session lifetime. Per-app toggle so internal services (databases, monitoring) can skip auth.
- **Why add to Tengiz:** Different from OIDC/OAuth SSO (#157, #128) which covers admin/CLI authentication — this protects the deployed apps themselves from unauthorized access. Staging environments, internal tools, and admin panels should not be publicly accessible without auth. Implementation: deploy an `oauth2-proxy` container managed by Tengiz (`tengiz forward-auth enable --app myapp --provider google`), proxy middleware intercepts requests and delegates auth. Complements HTTP Basic Auth (#27) with enterprise-grade SSO. `.tengiz.yaml`'da `security.forward_auth: { provider: google, allowed_domains: ["example.com"] }`. Medium effort (oauth2-proxy container lifecycle + proxy middleware), high security value for team deployments.
- **Detected:** 2026-07-19

## Rich Notification Providers (Mattermost, Gotify, Ntfy, Lark)
- **Source:** Dokploy
- **Description:** Multi-channel notification support extending beyond Discord/Slack/Telegram: Mattermost (self-hosted Slack alternative), Gotify (self-hosted push notifications), Ntfy (HTTP-based push, open source), Lark/Feishu (ByteDance's enterprise messaging). Each provider has a dedicated notification handler with platform-specific formatting (Mattermost uses Slack-compatible webhooks, Gotify uses simple POST JSON, Ntfy uses HTTP PUT with topic-based routing, Lark uses card messages). Per-provider configuration: webhook URL, auth token, channel/room ID, rate limits.
- **Why add to Tengiz:** Existing notification system (#24) supports Discord/Slack/Telegram/Email. Adding self-hosted/open-source providers (Mattermost, Gotify, Ntfy) aligns with Tengiz's self-hosted philosophy — users who run their own infrastructure want self-hosted notifications. Lark/Feishu covers enterprise users in Asia/China. Implementation: each provider is a Go struct implementing a `Notifier` interface with `Send(event Event) error`. Config stored in `.tengiz.yaml` under `notifications.providers`. The existing event system (#46) provides the event payloads. Low-medium effort per provider (each is a templated HTTP POST), high coverage value for diverse team communication preferences.
- **Detected:** 2026-07-19

## Granular Notification Event Toggles (Per-Event-Type Control)
- **Source:** Dokploy
- **Description:** Independent enable/disable toggles per notification event type: `appDeploy`, `serverThreshold`, `dokployRestart`, `dockerCleanup`, `backup`, `containerCrash`, `sslExpiry`, `deployRollback`. Each notification provider has its own event filter configuration. Event types include metadata (app name, environment, timestamp, status) for rich, context-aware notifications. Users can subscribe only to events they care about — e.g., get deploy failure notifications but ignore daily cleanup summaries.
- **Why add to Tengiz:** Current notification model is all-or-nothing — users get every platform event or none. Granular toggles let operators filter noise: production deploy failures are critical (alert immediately), backup completions are informational (daily summary), Docker cleanup is operational (ignore unless error). Implementation: each event carries a `EventType` string, notification dispatch checks a per-channel `SubscribedEvents` set before sending. `.tengiz.yaml`'da `notifications.events: { deploy_failure: true, backup_success: false, container_crash: true }`. Low effort (event type filter in dispatch loop), high operational value for reducing alert fatigue.
- **Detected:** 2026-07-19

## File Content Mounts (Inline Config Files in Containers)
- **Source:** Dokploy
- **Description:** Mount inline file content directly into containers without needing host-side files or Docker volumes. Supports create/update/delete of files with specified path, content, and permissions inside the container. Content can be defined as a plain text string or base64-encoded binary in the app configuration. Files are written to the container via Docker bind mounts or entrypoint injection. Useful for: Nginx config files, SSL certs, `.env` files not ready for env vars, custom `nginx.conf`, `Caddyfile`, `robots.txt`, and application config files that must exist as files on disk.
- **Why add to Tengiz:** Many apps need config files at specific paths that can't be replaced by env vars. Currently users must either commit these files to their repo (bad for secrets), mount host volumes (requires host-side files), or bake them into the Docker image (inflexible). File content mounts solve this: define `nginx.conf` inline in `.tengiz.yaml` → Tengiz writes it to the correct path inside the container. `.tengiz.yaml`'da `mounts: [{ type: file, path: /etc/nginx/conf.d/default.conf, content: "server { listen 80; }" }]`. Different from Persistent Storage (#7) which is for runtime data — this is for static configuration files. Implementation: a pre-start hook that writes content to a temp directory and mounts it via Docker `--volume`. Low-medium effort, high flexibility value for power users.
- **Detected:** 2026-07-19

## Cancel Running Deployments (Abort In-Progress Deploy)
- **Source:** Dokploy
- **Description:** Abort an in-progress deployment with a single command or API call. When cancelled: the current build step is interrupted (SIGTERM to Docker build), the deploy pipeline halts, partial state is cleaned up (intermediate images, temporary containers), and the app stays on its previous working version. Deployment status is set to `cancelled` in history. Multiple sources can trigger cancel: CLI (`tengiz deploy --cancel` or `tengiz deploy:cancel <app>`), webhook API, or automatic cleanup of stale builds on Tengiz startup (cancel-deployments.ts equivalent).
- **Why add to Tengiz:** Deployments can get stuck (slow build, hanging health check, network timeout) or become obsolete (a newer push arrived). Currently the only option is to wait or kill the Tengiz process. Cancel provides a clean abort mechanism: `tengiz deploy:cancel myapp` immediately stops the build and clean up partial state. The startup cancel (cancel stale deployments from crashed sessions) prevents orphaned build containers from accumulating. Implementation: a context.Context with cancel function passed through the deploy pipeline. The cancel signal propagates to Docker build (SIGTERM), health check (stop polling), and proxy update (skip). A `DeploymentStatus` of `cancelled` is recorded in build history. Medium effort (context plumbing through the entire deploy pipeline), high operational value for CI/CD and team environments.
- **Detected:** 2026-07-19

## Port Conflict Detection (Pre-Deploy Port Availability Check)
- **Source:** Dokploy
- **Description:** Before starting a container, check if the assigned port (from port allocation or app config) is already in use by another Docker container or a host-level process. Dokploy's `checkPortInUse` queries `docker ps --format` for container port bindings and also checks host port occupancy via netstat equivalent. If a conflict is detected, the deploy fails with a clear error message showing which container/process occupies the port. Optionally, auto-reallocate to a different port within the 9000-9999 range.
- **Why add to Tengiz:** Port conflicts can occur when: a non-Tengiz Docker container uses a Tengiz-allocated port, a manual `docker run` collides with Tengiz's allocation range, or the port store (`ports-{env}.json`) gets out of sync with actual Docker state. Today these failures manifest as cryptic Docker errors ("port is already allocated"). A pre-flight port check catches these early with actionable messages. Implementation: `docker ps --format '{{.Ports}}'` parsed to extract host port bindings, plus `net.Listen("tcp", ":port")` probe for non-Docker processes. Run before `docker run` in the deploy pipeline. Low effort (~50 lines of Go), eliminates a common source of deployment failure.
- **Detected:** 2026-07-19

## Server Security Audit (UFW / SSH / Fail2Ban Scanner)
- **Source:** Dokploy
- **Description:** Automated security configuration scanner that checks the host server's security posture. Scans: UFW firewall status and rules (is it enabled? are ports 22/80/443 allowed?), SSH daemon configuration (is root login disabled? is password auth disabled? is PAM enabled?), fail2ban status and jail configuration (is SSH jail active?). Results are presented as a security report with pass/fail/warning per check and remediation instructions. Designed to run periodically and alert on configuration drift (e.g., UFW was disabled during troubleshooting and not re-enabled).
- **Why add to Tengiz:** Production servers need ongoing security validation. `tengiz doctor` (#88) checks system prerequisites for installation — this checks ongoing security posture. A server might pass the initial bootstrap with UFW enabled, but a maintenance session could disable it. Periodic security audits catch drift early. Implementation: `tengiz server audit` runs `ufw status`, `sshd -T`, `fail2ban-client status` via `os/exec`, parses outputs, and generates a structured report with severity levels. `.tengiz.yaml`'da `audit.interval: 24h` enables scheduled scans. Complements Server Security Hardening (#141) which configures these tools — this verifies they stay configured. Low-medium effort (command parsing + report generation), high security compliance value.
- **Detected:** 2026-07-19

## Hostname/Domain Format Validation
- **Source:** Dokploy
- **Description:** Validate domain name format before accepting custom domain configuration. Checks: DNS label length (max 63 chars per label), total length (max 253 chars), allowed characters (alphanumeric, hyphens, dots), no leading/trailing hyphens per label, no consecutive dots, no IP addresses as domains (reject `192.168.1.1`), punycode support for internationalized domain names. Returns a clear validation error with the specific rule violated. Applied when: `tengiz domain add`, `.tengiz.yaml` domain entries, and webhook payload domain fields.
- **Why add to Tengiz:** Invalid domain names cause SSL certificate issuance failures, DNS resolution errors, and proxy configuration problems that are hard to debug retroactively. Front-end validation at config entry time prevents these failures. Go's `net/http` and `golang.org/x/net/idna` provide the validation primitives. Complements existing Custom Domain Management (implemented) and SSL/TLS (#51). Low effort (Go std library), prevents a class of subtle configuration errors.
- **Detected:** 2026-07-19

## Template Variables System (Configurable Service Template Parameters)
- **Source:** Dokploy
...
## Captain Definition File (Multi-Method Deploy Manifest)
- **Source:** CapRover
- **Description:** A JSON-based `captain-definition` file at the project root serving as the deployment manifest. Schema supports four mutually exclusive deployment methods: `dockerfileLines` (inline Dockerfile content — no separate file needed), `dockerfilePath` (path to existing Dockerfile), `imageName` (deploy a pre-built image directly, no build step), and `templateId` (deploy from a one-click app template). The `schemaVersion` field enables forward compatibility. At deploy time, exactly one method must be set — validated by CapRover's `ImageMaker.getCaptainDefinition()`.
- **Why add to Tengiz:** Tengiz currently has `.tengiz.yaml` with framework auto-detection as the only deploy mode. A `captain-definition`-style manifest adds three new deploy methods: (1) inline Dockerfile — useful for quick deploys without creating a `Dockerfile` file, (2) pre-built image deploy — deploy CI/CD-built images, third-party services (Postgres, Redis), or registry-mirrored images without any build step, (3) template ID — one-command deploy from a template library. Tengiz's `builder` package already handles Dockerfile and Nixpacks — adding `imageName` and `templateId` methods fills the deploy menu. The inline Dockerfile approach is particularly valuable: `tengiz deploy --dockerfile-lines "FROM node:20\nCOPY . /app\nCMD node index.js"` deploys without touching the filesystem. `.tengiz.yaml`'da zaten `image:` alanı var (Explicit Image Name), ama bir manifest dosyası tüm metodları tek yerde toplar. CapRover'ın `captain-definition`'ı esin kaynağı: Tengiz'de `.tengiz.yaml` içinde `deploy.method: image|dockerfile|dockerfile_lines|template` alanı ve `deploy.dockerfile_lines: [...]` ile uygulanabilir. Bu, framework auto-detection'ın yanında alternatif deploy stratejileri sunar.
- **Detected:** 2026-07-19

## Platform JWT Authentication & Password Protection
- **Source:** CapRover
- **Description:** Full authentication system for the platform itself (not app-level auth). CapRover uses JWT tokens (`jsonwebtoken`) with separate token types: auth cookies (long-lived, for dashboard sessions), app push webhook tokens (per-app, for CI/CD triggering), and download tokens (time-limited, 2-minute expiry for backup downloads). Password hashing via `bcryptjs`. Token versioning (`tokenVersion` field) enables global invalidation — increment the version and all existing tokens become invalid. OTP-based two-factor authentication available in the Pro version. All auth operations go through `Authenticator.ts` which wraps JWT with platform-specific logic: `getAuthToken()`, `decodeAuthToken()`, `changepass()`, `isPasswordCorrect()`, `getAppPushWebhookToken()`, `getDownloadToken()`.
- **Why add to Tengiz:** Tengiz currently has zero authentication — anyone with CLI access or network access to the proxy/webhook can manage any app. A JWT-based auth system enables: (1) `tengiz login` with password for CLI sessions, (2) token-based auth for CI/CD (`TENGIZ_TOKEN`), (3) per-app webhook tokens for git deploy verification (HMAC-alternative), (4) time-limited download tokens for backup retrieval, (5) token versioning to revoke all sessions during password rotation. Implementation: Go's `github.com/golang-jwt/jwt/v5` for tokens, `golang.org/x/crypto/bcrypt` for password hashing. Tokens stored in `~/.tengiz/auth/tokens.json` with hashed secrets. CLI commands: `tengiz auth login/password/token/create/token/revoke`. Complements existing OIDC/SSO (#157) and App Deploy Tokens (#16) — this is the foundational auth layer that both build on. Without platform auth, all other security features (app-level auth, API keys, RBAC) lack a user identity model to anchor against. P1 feature — fundamental for team/multi-tenant deployments.
- **Detected:** 2026-07-19

## DH Param Generation for SSL Hardening (Perfect Forward Secrecy)
- **Source:** CapRover
...
## Alert Filtering with Resource Whitelist/Blacklist
- **Source:** Komodo
- **Description:** Each alerter (Slack, Discord, Pushover, Ntfy, custom webhook) supports per-resource filtering: whitelist (only send alerts for specific resources or resource types) and blacklist (exclude specific resources or resource types). Also filterable by alert type (deploy failure, container crash, disk threshold, SSL expiry, etc.). Configurable rate limits per alerter prevent notification floods. Komodo's `Alerter` resource stores `include_resources`, `exclude_resources`, `include_alert_types`, `exclude_alert_types`, and `rate_limit` per alerter. Combined, these filters prevent alert fatigue while ensuring critical alerts reach the right channels.
- **Why add to Tengiz:** Existing notification system covers multi-channel delivery but sends all platform events to all configured channels — creating noise that causes operators to ignore alerts. Alert filtering via whitelist/blacklist gives surgical control: send critical deploy failures to PagerDuty/Slack emergency channel, send backup completions to a daily digest channel, suppress routine Docker prune notifications entirely. `.tengiz.yaml`'da `notifications.alerters.slack.filter: { include: [deploy_failure, container_crash], exclude: [backup_success, prune_done] }` ile yapılandırılır. Implementation: each alerter has a `ShouldSend(alert)` method that checks filter rules before dispatching. Complements the Alert System for a complete notification hierarchy. Low-medium effort, high operational value for reducing alert fatigue in production.
- **Detected:** 2026-07-19

## Maintenance Windows for Alert Suppression
- **Source:** Komodo
- **Description:** Time-based recurring or one-time maintenance windows that suppress alerts during known maintenance periods. Komodo's `MaintenanceWindow` resource supports: `type: Daily | Weekly | OneTime`, day of week, start/end time, timezone, resource whitelist (which resources' alerts are suppressed), and optional reason. During a maintenance window, alerts for specified resources are automatically acknowledged or suppressed rather than dispatched to alerters. Windows can overlap — the most restrictive window applies. Auto-cleanup of expired one-time windows via background goroutine.
- **Why add to Tengiz:** Planned maintenance (DB migration, server upgrade, app redeploy) generates predictable alerts (container restart, temporary unavailability, high CPU during migration). Without maintenance windows, these alerts cause unnecessary pages and erode trust in the alerting system. Operators resort to manually disabling alerters during maintenance — a dangerous practice that risks missing real incidents. `.tengiz.yaml`'da `maintenance_windows: [{ schedule: "sun 02:00-04:00", timezone: UTC, suppress: [deploy_failure, container_crash] }]` ile yapılandırılır. Implementation: `~/.tengiz/maintenance.json` stores window definitions, a goroutine checks active windows during alert dispatch. Low-medium effort, essential companion to the alert system for production reliability. Different from Maintenance Mode which drains proxy traffic — this suppresses alerts, not traffic.
- **Detected:** 2026-07-19

## Service Users for Automated Operations (System-Level Accounts)
- **Source:** Komodo
- **Description:** Non-human system accounts used for automated platform operations: webhook handlers, auto-redeploy, resource sync, scheduled procedures, and internal platform automation. Komodo's `User` entity supports `service` user type with specific system roles: `GitHubWebhook`, `GitLabWebhook`, `AutoRedeploy`, `ResourceSync`, `ProcedureRunner`. Each service user has a unique API key with scoped permissions and no login credentials (no username/password). Service user actions are recorded in the audit trail separately from human actions. They are auto-created during system initialization and managed by the platform itself.
- **Why add to Tengiz:** Current Tengiz has no identity model — all operations run under the CLI user's authority. As Tengiz gains automation features (webhooks, scheduled deploys, auto-redeploy, sync, procedures), automation actions need their own identity: (1) audit trail distinguishes "human deploy" from "webhook-triggered deploy", (2) permission scoping prevents automation accounts from exceeding their purpose, (3) credential rotation doesn't affect automation flows. Implementation: `tengiz service-user create --name github-webhook --role deploy-only` generates an API key. Service users stored in `~/.tengiz/service-users.json` with hashed tokens. Actions performed by service users carry their identity in the audit log. Low-medium effort, foundational for any automated workflow system.
- **Detected:** 2026-07-19

## Login Credential Locking (Admin-Controlled Accounts)
- **Source:** Komodo
- **Description:** Administrative setting that locks users from changing their own username or password. Komodo's `lock_login_credentials_for` config option accepts user groups or specific user IDs. Locked users see a notification in their profile that credentials are managed by the system administrator. Admin users can change credentials on behalf of locked users via backend operations. Used in enterprise environments where: (1) credentials are provisioned via identity management (IdP sync, HR system), (2) compliance requires credential rotation to be centrally managed, (3) shared service accounts must have non-changeable passwords.
- **Why add to Tengiz:** As Tengiz gains multi-user support (OIDC/SSO, RBAC, User Groups), enterprise deployments need admin-controlled user management. Without credential locking, a user could change their password to something non-compliant or disable their account via self-service. Implementation: a `credentials_locked` bool on the User model, checked during password change and profile update operations. `tengiz user lock <username>` and `tengiz user unlock <username>` commands. Only admin-level users can issue lock/unlock. Low effort (boolean flag + access control), important for enterprise compliance scenarios. Complements OIDC/SSO which delegates credential management to the identity provider.
- **Detected:** 2026-07-19

## Dockerfile Editing within Platform (No Git Commit Needed)
- **Source:** Komodo
- **Description:** Edit Dockerfile directly through the platform's configuration without needing to commit to a Git repository and trigger a new build. Komodo's `Build` resource has a `dockerfile` field that stores inline Dockerfile content. When set, this content is written to `Dockerfile` in the build context before `docker build` runs — overriding any existing `Dockerfile` in the source. Changes take effect on the next build without a separate git commit. The field can be unset to revert to the source repository's Dockerfile. Combined with `files_on_host` mode, users can edit and deploy entirely within the platform without any Git repository.
- **Why add to Tengiz:** Quick Dockerfile fixes (wrong base image, missing `RUN` step, incorrect `CMD`) currently require: edit file → git commit → git push → build → deploy — a slow loop. Inline Dockerfile editing enables rapid iteration: `tengiz config set myapp dockerfile "FROM node:20\nWORKDIR /app\nCOPY . .\nCMD ['node', 'server.js']"` → `tengiz deploy myapp`. Also useful for: (1) users who don't control the source repository (deploying a public project with custom Dockerfile), (2) testing Dockerfile changes before committing to source, (3) platform-managed apps where the Dockerfile is a platform concern, not a source concern. `.tengiz.yaml`'da `dockerfile: |` block for multi-line content. Implementation: builder writes the inline content to `<build-dir>/Dockerfile` before build. Low effort (~30 lines in builder.go), increases deploy flexibility.
- **Detected:** 2026-07-19

## Compose Stack Configuration File Management (Config Files as Resources)
- **Source:** Komodo
- **Description:** Manage configuration files (nginx.conf, .env, custom configs) as first-class resources for Docker Compose stacks. These config files are tracked separately from the app's source code: they are stored in the platform state, versioned with the stack configuration, and survive stack redeployments. When the stack is deployed, these files are mounted into the appropriate containers. Changes to config files can trigger optional stack redeployment. Komodo's Stack resource stores a `config_files` map where keys are paths inside the container and values are file content (plain text or base64 for binaries).
- **Why add to Tengiz:** Docker Compose stacks often need companion config files that don't live in the app's source repository: environment-specific `.env` files with secrets, custom `nginx.conf` for reverse proxy behavior, `Caddyfile` for TLS configuration, `logrotate.conf` for log management. Currently users must either (1) commit these to the repo (bad for secrets), (2) mount host volumes (requires host-side presence), or (3) bake them into the base image (inflexible). First-class config file management solves all three: `.tengiz.yaml`'da `stack.config_files: { "/etc/nginx/conf.d/default.conf": "server { listen 80; }", ".env": "DATABASE_URL=postgres://..." }`. Implementation: files written to `~/.tengiz/stacks/<name>/configs/` and mounted via Docker volumes. Different from File Content Mounts which targets single-container apps — this targets Docker Compose stacks specifically.
- **Detected:** 2026-07-19

## Service-Level Status Tracking in Compose Stacks
- **Source:** Komodo
- **Description:** Per-service granular status tracking within a multi-service Docker Compose stack. Beyond the binary "stack is up/down" status, individual service status is tracked: running, updating, failed, stopped, paused. Komodo's Stack resource tracks `service_statuses` as a map of service name → status with last status change timestamp. `tengiz stack ps <name>` displays a service-level table showing each service name, image, status, uptime, port mappings, and resource usage. Useful for debugging: if a stack deploy reports success but one service fails to start, service-level status surfaces the failure immediately.
- **Why add to Tengiz:** Current Compose support treats stacks as atomic units. A 5-service stack with one failed service shows as "partial" or "degraded" — operators must inspect each service individually to find the problem. Service-level status surfaces the issue immediately: `tengiz stack ps myapp` shows `web: running (2h), db: running (2h), cache: failed (30s ago)`. Implementation: `docker compose ps --format json` parsed per service, stored in `~/.tengiz/stacks/<name>/status.json`. Low effort (~100 lines), significantly improves stack troubleshooting experience.
- **Detected:** 2026-07-19

## CRON-to-English Schedule Display (Human-Readable Schedules)
- **Source:** Komodo
- **Description:** Convert cron expressions to human-readable English descriptions in CLI output and configuration display. Komodo's Schedule resource stores both a CRON expression (for execution) and an English description (for display). The description is auto-generated from the CRON expression using `croner` Rust crate, or user-supplied as an override. Examples: `"0 3 * * *"` → `"At 03:00 AM every day"`, `"*/15 * * * *"` → `"Every 15 minutes"`, `"0 0 1 1 *"` → `"At 12:00 AM on day 1 of January, every year"`. Descriptions are shown in `tengiz schedule list`, `tengiz config show`, and deploy confirmation prompts.
- **Why add to Tengiz:** Cron expressions are notoriously hard to read and easy to misconfigure. A schedule that should run "every hour" but has a typo (`0 * * * *` → `0 * * *` — missing day-of-week field) could run silently at incorrect times. Human-readable display catches these errors immediately. Implementation: use one of the existing Go cron-description libraries or a lookup table of common patterns. Display in `tengiz cron list`, `tengiz config show`, and deploy confirm prompts. Low effort (~100 lines for basic patterns), prevents a class of scheduling misconfigurations. Complements Scheduled Tasks and Scheduled Deployments.
- **Detected:** 2026-07-19

## Docker Compose Pull Operation (Refresh Stack Images Without Redeploy)
- **Source:** Komodo
- **Description:** Pull the latest versions of all Docker images referenced in a Compose stack without redeploying the stack. `tengiz stack pull <name>` runs `docker compose pull` for all services in the stack. Services continue running on their current images until `tengiz stack redeploy <name>` is issued, which recreates containers using the freshly pulled images. This decouples image download (which may be slow and bandwidth-intensive) from container restart (which causes brief downtime). Useful for: preparing stacks before maintenance windows, downloading images during off-peak hours, and validating image availability before scheduled redeploy.
- **Why add to Tengiz:** Currently updating a stack's images requires full redeploy — downloading new images AND restarting containers in one operation. This couples image download latency (significant for large images) with container restart (which users may want to schedule). Separating pull from redeploy gives operators finer control. `tengiz stack pull myapp` runs image download in the background; `tengiz stack redeploy myapp` uses fresh images with minimal downtime. Implementation: `docker compose pull` via `os/exec`. Low effort (~30 lines), high operational flexibility value. Complements Stack Lifecycle Management with a pre-deploy image preparation step.
- **Detected:** 2026-07-19

## Global Auto-Update Procedure (System-Wide Image Refresh)
- **Source:** Komodo
- **Description:** Single system-wide operation that checks all deployments and stacks for newer versions of their referenced Docker images and automatically redeploys stale images. Komodo's `GlobalAutoUpdate` operation iterates over all Deployments and Stacks with `redeploy_on_new_image: true` or `auto_update: true`, checks registry image digests, and triggers zero-downtime redeploy for any that have changed. Configurable per-app and per-stack opt-out via `auto_update: false`. Runs on a schedule (default: daily) or on-demand via `tengiz auto-update`. Reports results: N images updated, M unchanged, F failures.
- **Why add to Tengiz:** Docker base images (nginx, postgres, redis, node) receive frequent security patches. Without auto-update, users must manually check and redeploy each app — a backlog of outdated base images is the most common security finding in container audits. A global auto-update procedure keeps all app containers running up-to-date base images with minimal operator effort. Implementation: a system `tengiz auto-update` command in the procedure/automation framework, scheduled via Cron. `.tengiz.yaml`'da `auto_update: { enabled: true, schedule: "0 4 * * *" }` with per-app `auto_update: false` opt-out. Complements per-app Image Digest Change Detection which triggers on individual image changes — this is the system-wide orchestration of that feature. Medium effort (iteration over all apps + digest comparison + parallel redeploy), high security maintenance value.
- **Detected:** 2026-07-19

## Stats Retention Policy (Historical Data Auto-Pruning)
- **Source:** Komodo
- **Description:** Automatic deletion of historical system and container stats data after a configurable retention period. Komodo's `keep_stats_for_days` configuration option (default: 30) controls how long per-container CPU, memory, network I/O, and disk usage history is retained. A background goroutine periodically scans the stats store and deletes entries older than the retention threshold. Pruned data is permanently removed (not soft-deleted). Separate retention configs for per-container stats vs system-level stats. Alerts are generated when disk usage approaches the retention limit to prevent data loss before pruning runs.
- **Why add to Tengiz:** Historical stats collection records data without bounds — on a busy server with many apps, stats JSON Lines files grow unbounded, eventually consuming all available disk space. A retention policy is the safety valve: stats older than N days are automatically deleted. Implementation: cron-based cleanup in `internal/stats/cleanup.go`, configurable via `.tengiz.yaml`'da `stats.retention_days: 30`. Default: 30 days for container stats, 90 days for system stats. Low effort (~100 lines), critical for long-running Tengiz instances. Complements System Stats Recording and Prune Operations for comprehensive disk management.
- **Detected:** 2026-07-19

## Transparent Mode (All Resources Readable by Default)
- **Source:** Komodo
- **Description:** Simplified permission mode where all authenticated users can view (read) all resources without explicit permission assignment. Komodo's `transparent_mode` configuration option (default: false) overrides read-level permission checks — any authenticated user can read any resource. Write operations still require explicit permissions. Designed for small teams where: (1) there's no need to hide resource visibility between team members, (2) the overhead of maintaining per-resource read permissions outweighs the security benefit, (3) the platform serves as a collaborative dashboard where everyone sees everything.
- **Why add to Tengiz:** As Tengiz gains multi-user support with RBAC, the permission system can become a significant management burden for small teams. A 5-person startup doesn't need per-resource read permissions — everyone should see all apps, deployments, and servers. Transparent mode eliminates this overhead while preserving write-level access control. Implementation: boolean check in the permission middleware — if transparent mode is enabled, skip read-level permission checks. `.tengiz.yaml`'da `auth.transparent_mode: true` with a warning about security implications. Low effort (~20 lines), crucial for making multi-user features adoptable by small teams without an admin dedicated to permission management.
- **Detected:** 2026-07-19
