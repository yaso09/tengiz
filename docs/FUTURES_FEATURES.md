...
- **Source:** Coolify
- **Description:** Coolify has a community-maintained Terraform provider (`coolify-terraform`) that enables managing Coolify resources (projects, environments, services, deployments, databases) declaratively via Terraform/HCL. Users define their entire platform configuration as code. Terraform state tracks drift between declared and actual state. Plan/apply workflow enables PR-based infrastructure changes.
- **Why add to Tengiz:** Tengiz's planned REST API (#8 on roadmap) is the prerequisite — a Terraform provider wraps the REST API as a Terraform resource. Once the API exists, a `tengiz-terraform` provider enables: (1) declarative management of all Tengiz resources (apps, env vars, domains, databases, secrets), (2) GitOps-style infrastructure via Terraform Cloud/Enterprise, (3) drift detection between `.tengiz.yaml` and live state, (4) integration with existing Terraform-managed infrastructure (VPC, DNS, Kubernetes). Follows the standard Terraform provider pattern (CRUD per resource, `terraform-plugin-sdk`). Low-medium effort once REST API is stable. High value for enterprise teams already using Terraform.
- **Detected:** 2026-07-26

## Database SSL/TLS Certificate Management (Auto-Generated CA Certs)
- **Source:** Coolify
- **Description:** Coolify auto-generates a Certificate Authority (CA) for each managed database instance and configures SSL/TLS connections between apps and databases. Multiple SSL modes: `allow`, `prefer`, `require`, `verify-full`. The CA certificate is generated at database creation time, stored securely, and automatically mounted into app containers linked to the database. Connection strings include SSL parameters. Distinct from platform-level TLS — it is database-level transport encryption.
- **Why add to Tengiz:** Managed Database Provisioning (#63) would be incomplete without built-in SSL/TLS. Users deploying databases need encrypted connections for defense in depth. Manual SSL configuration for databases is error-prone. Implementation: during `tengiz db create`, generate a CA cert + server cert via Go's `crypto/x509`, store certs in `~/.tengiz/certs/db/<db-id>/`, mount into the database container, inject `sslmode` into the auto-generated `DATABASE_URL`. Per-database SSL mode config in `.tengiz.yaml`: `database.ssl.mode: verify-full`. Medium effort, fills a critical security gap in database provisioning.
- **Detected:** 2026-07-26

## Wildcard SSL Certificates via DNS-01 Challenge
- **Source:** Coolify
- **Description:** Beyond basic Let's Encrypt HTTP-01 (cannot issue wildcard `*.example.com` certificates), Coolify supports DNS-01 challenge via Traefik with 30+ DNS provider integrations (Route53, Cloudflare, Google Cloud DNS, DigitalOcean, Hetzner, etc.). DNS-01 places a TXT record to prove ownership, enabling wildcard certificates that cover all subdomains. Auto-renewed before expiry.
- **Why add to Tengiz:** "Otomatik SSL/TLS (Let's Encrypt)" (#65) and "Manual SSL Certificate Management" (#188) cover HTTP-01 and manual import but not wildcard certs. Wildcard certs are the industry standard for multi-app platforms — a single `*.tengiz.example.com` covers all `<app>.tengiz.example.com` deployments, avoiding the Let's Encrypt 50-certs/week rate limit. DNS-01 is the only way to get wildcard certs from Let's Encrypt. Implementation: integrate `go-acme/lego` DNS-01 solvers into `tengiz domain add --wildcard`. Providers configured in `.tengiz.yaml`: `ssl.dns_provider: cloudflare`. Auto-renewal via background goroutine. Medium effort, high value for multi-app instances.
- **Detected:** 2026-07-26

## HTTP Load Balancing Across Multiple Containers (Round-Robin)
- **Source:** Coolify
- **Description:** Coolify's Traefik proxy load-balances HTTP traffic across multiple containers of the same application (horizontal scaling). When an app is scaled to N replicas, Traefik distributes requests using round-robin with optional session stickiness. Health checks determine which containers receive traffic. Enables horizontal scaling within a single server and across multiple servers.
- **Why add to Tengiz:** "Process Scaling (Multi-Container)" (#72) covers running multiple process types but not distributing traffic across them. Tengiz's proxy routes to a single backend per app. For multi-container apps (via scaling), the proxy should distribute across all healthy instances. Essential for: (1) utilizing multi-core for single-threaded frameworks (Node.js, Python), (2) zero-downtime traffic migration, (3) absorbing traffic spikes. Implementation: extend the Go `httputil.ReverseProxy` with a weighted round-robin pool per route key. Container health feeds into the pool. `.tengiz.yaml`'da `scaling.min: 1`, `scaling.max: 4`, `proxy.load_balance: round-robin`. Medium effort, unblocks horizontal scaling without Docker Swarm/Kubernetes.
- **Detected:** 2026-07-26
