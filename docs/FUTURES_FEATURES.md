...
- **Source:** Coolify
- **Description:** Create and manage cloud VPS instances directly from the platform via provider APIs. Coolify supports Hetzner, Vultr, and DigitalOcean — users select region, server type, image, SSH keys, and firewall from the UI, and Coolify provisions the server, installs Docker, and registers it as a managed server. Full lifecycle: create, status tracking (server ID, status, IP assignment), and delete.
- **Why add to Tengiz:** Tengiz's `tengiz server init` currently requires manual server setup (install Docker, configure SSH). Cloud provisioning automates this: `tengiz server create --provider hetzner --region nbg1 --type cx22` provisions a new VPS, installs Docker, and registers it. Existing SSH-based Remote Deployment (#105) is the execution layer — cloud provisioning is the automation layer above it. Essential for users who want to scale from a single server to a fleet without manual setup.
- **Detected:** 2026-07-22

## Crash Loop Protection (Max Restart Count with Rate Limiting)
- **Source:** Coolify
- **Description:** Rate-limited auto-restart for crashing containers: if a container crashes N times within a configurable window (e.g., 5 restarts in 60 seconds), stop trying and mark the deployment as failed. Coolify tracks `restart_count`, `max_restart_count`, `last_restart_at`, and `last_restart_type` per application. Prevents resource exhaustion from crash-looping apps consuming CPU/memory on repeated restart attempts. Health check failures are excluded from the counter (those follow a different retry policy).
- **Why add to Tengiz:** Container Health Check + Auto Restart (implemented) restarts unhealthy containers indefinitely, which can mask persistent failures and waste resources. Crash loop protection catches the "app crashes on boot → restart → crash → restart" death spiral. Implementation: `health.Checker` tracks restart timestamps per app, computes rate in a sliding window, and escalates to "crash-looping" status instead of restarting. Complements existing health check system. Low effort, prevents a common production failure pattern.
- **Detected:** 2026-07-22

## Configuration Diff/Snapshots per Deployment
- **Source:** Coolify
- **Description:** Before each deployment, Coolify snapshots the current application configuration (env vars, build commands, domains, resource limits, health check settings) and stores it alongside the deployment record. After deployment, the diff between old and new config is computed and displayed. Enables answering "what changed between the working deploy and this broken one?" during incident response.
- **Why add to Tengiz:** Existing Build Tracking with Retention (#94) records build metadata and logs but not configuration snapshots. When a deploy breaks, operators need to know "what env var changed?" or "was the build command modified?". A `tengiz deploy diff <app> --from <deploy-id> --to <deploy-id>` command shows the config delta. Implementation: before each deploy, serialize `AppEntry` to JSON, store as `deployment-config-<id>.json` alongside build logs. After deploy, compare old and new config, attach diff to deployment record. Low effort, high debugging value.
- **Detected:** 2026-07-22

## Deployment Cancellation (Cancel In-Progress Deploy)
- **Source:** Coolify
- **Description:** Cancel a running deployment via API/UI. Useful when a newer commit makes the current build obsolete, or when a deploy is stuck (e.g., hanging `npm install`). Coolify tracks deployment queue statuses (queued, in_progress, finished, failed, cancelled) and allows cancelling from any non-terminal state. Cancelled deployments clean up any partially-built images and temporary resources.
- **Why add to Tengiz:** Rapid-fire CI/CD pipelines (e.g., multiple git pushes in quick succession) can queue up deployments that become obsolete. `tengiz deploy --cancel <app>` or `tengiz deploy:cancel <app>` stops the current build, cleans up temp resources, and marks the deployment as cancelled. For the webhook server, automatically cancel in-progress deploy when a newer push arrives for the same app. Implementation: per-app context cancellation via `context.WithCancel`, stored in a `sync.Map`. The build goroutine checks `ctx.Done()` between build steps. Already partially covered by "Build Queue with Dedup" (#72) — cancellation complements dedup (which prevents starting duplicate builds) with the ability to stop a running build.
- **Detected:** 2026-07-22

## Multi-Team Management with Invitations
- **Source:** Coolify
- **Description:** Full team management system: create teams, invite members via email/link, assign roles (admin, member), scope resources (servers, projects, apps) to teams. Each team has its own view of resources — team A cannot see team B's servers or apps. Team invitations with accept/decline flow. Supports team-scoped API tokens.
- **Why add to Tengiz:** Currently every Tengiz user has access to all resources on a server (no isolation). For team-hosted Tengiz instances, this means anyone with CLI access can see/modify all apps. Team-scoped resource access enables: a shared Tengiz server hosting multiple teams' apps with full isolation, delegated management (team leads manage their own team), and audit trail with real user identities. Implementation: `teams.json` in `~/.tengiz/` with team membership, `team_id` field in `AppEntry`/`ServerEntry`, filtering in `ListApps`/`ListServers`. CLI: `tengiz team create/invite/leave/list`, `tengiz ps --team <name>`. Complements OIDC/SSO (#111) for authentication and Granular Scoped API Keys (#180) for authorization.
- **Detected:** 2026-07-22

## Shared Environment Variables (Server/Project/Environment Scoped)
- **Source:** Coolify
- **Description:** Environment variables defined at three levels: server-level (shared by all apps on a server), project-level (shared by all apps in a project), and environment-level (shared by all apps in a specific environment like staging or production). Variables cascade: app-level vars override environment-level, which override project-level, which override server-level. Coolify's `SharedEnvironmentVariable` model supports this with a `type` field (`server`, `project`, `environment`) and polymorphic relationships.
- **Why add to Tengiz:** Currently every app's env vars are defined independently. On a Tengiz instance with 20 apps all using the same `DATABASE_URL` or `REDIS_URL`, operators must update 20 separate configs when credentials change. Shared env vars eliminate this duplication. Implementation: extend `config.Store` with `SetSharedEnv(scope, key, value)`, `GetSharedEnv(scope, key)`, and merge shared vars with app-level vars at deploy time. The merge order: app vars > environment vars > project vars > server vars. `tengiz config set --scope project myapp DATABASE_URL=...` sets at project level. Low effort, high operational value for multi-app instances.
- **Detected:** 2026-07-22

## Public Database Access Toggle
- **Source:** Coolify
- **Description:** Toggle external network access for managed databases. When enabled, the database container's port is published to the host network (`-p hostPort:containerPort`), making it accessible from outside the Docker host. Coolify shows a prominent security warning when enabling this. When disabled, the database is only reachable from within the Docker network (other apps on the same server).
- **Why add to Tengiz:** Managed Database Provisioning (#63) creates databases that are only accessible from within the Docker network by default (127.0.0.1 binding). Some use cases require external access: connecting from a remote app, using with external BI tools (Metabase, Grafana), or manual migration. `tengiz db public <app> --enable --port 5432` exposes the database port. `.tengiz.yaml`'da `database.public_access: {enabled: true, port: 5432}`. A security warning should be printed or require `--force` flag. Low effort (adds `-p` flag to runtime.Create for database containers), important for managed DB flexibility.
- **Detected:** 2026-07-22

## Database SSL Auto-Configuration
- **Source:** Coolify
- **Description:** Automatically generate and manage SSL certificates for managed database connections. Coolify provisions self-signed or Let's Encrypt certificates for PostgreSQL, MySQL, MongoDB, and ClickHouse. The CA certificate, client certificate, and client key are generated per database instance and made available as environment variables (`SSL_CERT`, `SSL_KEY`, `SSL_CA`) to connected apps. Certificate rotation with minimal downtime.
- **Why add to Tengiz:** Production databases should never use unencrypted connections, but manually configuring SSL certificates for each database is tedious. Auto-SSL for managed databases ensures every database connection is encrypted by default. Implementation: during database provisioning, generate a self-signed CA + server cert via `openssl`, mount cert files as Docker volumes, configure the database container to require SSL, and inject connection string with `sslmode=require`. Certificates stored in `~/.tengiz/certs/db-<app>/`. `tengiz db ssl renew <app>` regenerates on expiry. Complements Managed Database Provisioning (#63) with production-grade security.
- **Detected:** 2026-07-22

## Server File System Browsing
- **Source:** Coolify
- **Description:** Browse the Docker host's file system from the platform UI/API. Coolify's `ServerFilesFromServerJob` allows navigating directories, viewing file contents, listing files with metadata (size, permissions, modification time). Useful for checking deployment artifacts, verifying config files, debugging volume mounts, and inspecting build output without SSH access.
- **Why add to Tengiz:** Operators often need to check files on the server: "Was the build output placed correctly?", "Does the .env file exist?", "What's in the volume mount directory?". Currently requires SSH or `docker exec`. `tengiz server ls <path>` lists directory contents, `tengiz server cat <path>` shows file contents, `tengiz server stat <path>` shows file metadata. Implementation: Go's `os.Open`/`os.ReadDir`/`os.Stat` — Tengiz already runs on the host, so this is a simple filesystem operation. Complements `tengiz server exec` (#113) for a non-interactive server file inspection workflow. Low effort, useful for quick debugging without full SSH.
- **Detected:** 2026-07-22

## Per-Provider Webhook Secrets
- **Source:** Coolify
- **Description:** Configure separate webhook secrets per Git provider (GitHub, GitLab, Bitbucket, Gitea) rather than a single shared secret. Coolify stores `manual_webhook_secret_github`, `manual_webhook_secret_gitlab`, `manual_webhook_secret_bitbucket`, and `manual_webhook_secret_gitea` on each application. This allows different secrets for different providers, supports provider-specific rotation without affecting others, and simplifies webhook setup when multiple providers are used for the same app.
- **Why add to Tengiz:** Current webhook system stores a single global secret. If a team uses both GitHub and GitLab for different branches of the same app, they either share one secret (security risk) or can't properly verify webhooks from one provider. Per-provider secrets also simplify rotation: rotate GitHub secret today, GitLab secret next week. Implementation: extend `WebhookConfig` with per-provider secret fields. Webhook server selects the correct secret based on detected provider. `.tengiz.yaml`'da `webhook.secrets.github`, `webhook.secrets.gitlab`, etc. Low effort, good security hygiene.
- **Detected:** 2026-07-22

## Server Validation & OS Detection on Add
- **Source:** Coolify
- **Description:** When a new server is added, Coolify validates: SSH connectivity, OS compatibility (supported distros only), Docker installation and version, system prerequisites (curl, git), kernel version, architecture. Validation results are displayed with clear pass/fail/hints. OS detection is used to offer distro-specific setup commands.
- **Why add to Tengiz:** `tengiz server init` should validate the target server before proceeding. A `tengiz server validate <host>` command would: test SSH connection, check OS compatibility, verify Docker is installed (or install it), check available disk space, verify port availability. Implementation: SSH to target, run detection scripts, parse output. For `tengiz server init --auto`, only proceed if all checks pass. Complements Pre-Install Env Validation (#87, `tengiz doctor`) which checks local system — this checks remote servers. Low effort, prevents frustrating "server setup fails halfway through" scenarios.
- **Detected:** 2026-07-22

## Resource Move Between Servers/Environments
- **Source:** Coolify
- **Description:** Move an application or database from one server to another, or from one environment to another, without redeploying from source. Coolify's `POST /v1/applications/{uuid}/move` and `POST /v1/databases/{uuid}/move` APIs handle: saving current state, pushing Docker image to registry, pulling on target server, creating container on target, updating DNS/proxy, cleaning up source container. Configuration (env vars, domains, volumes) is preserved.
- **Why add to Tengiz:** Enables common operational workflows: promote a staging app to production by moving it (vs rebuilding), migrate an app to a larger server without downtime, rebalance apps across servers. Implementation: `tengiz app move <app> --target-server <host> --target-env production`. Uses `docker save`/`docker load` or registry push/pull to transfer the image, then `runtime.Create` on target. `tengiz db move <app> --target-server <host>` for databases. Depends on SSH-based Remote Deployment (#105) for target server access. High operational value for multi-server deployments.
- **Detected:** 2026-07-22
