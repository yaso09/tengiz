...
## sslip.io Dynamic DNS Auto-Assignment (Domainless HTTPS Preview URLs)
- **Source:** Coolify
- **Description:** Coolify auto-generates `*.sslip.io` subdomains for all deployed services, providing instant HTTPS access without requiring users to own or configure a real domain. The `SERVICE_FQDN` env var is automatically injected into Docker Compose services with the generated sslip.io URL. No DNS propagation wait, no domain purchase needed — the service is accessible at `https://<random>.sslip.io` moments after deployment. sslip.io is a DNS service that resolves hostnames with embedded IP addresses (e.g., `192-168-1-1.sslip.io` → `192.168.1.1`). Works for both apps and databases/services.
- **Why add to Tengiz:** Lowers the barrier to first deployment dramatically. Users currently must have a domain and configure DNS to use Tengiz's proxy with custom domains. With sslip.io, `tengiz deploy` would auto-assign a `https://<app>-<random>.sslip.io` URL instantly — no domain, no DNS, no SSL config. Perfect for: development/testing environments, demo deployments, CI/CD preview URLs, and first-time user onboarding. Implementation: the proxy detects requests to `*.sslip.io` hostnames, extracts the server IP from the hostname, and routes accordingly. Combined with the built-in proxy, this is a small addition to the existing hostname routing logic. `.tengiz.yaml`'da `proxy.sslip_io: true` ile etkinleştirilir.
- **Detected:** 2026-07-28

## Concurrent Build Limit Per Server (Build Throttling)
- **Source:** Coolify
- **Description:** Coolify v4.0+ configures how many Docker builds can run simultaneously on a server (`concurrent_builds_per_server`). When the limit is reached, additional build requests are queued and executed sequentially. Prevents server resource exhaustion from too many concurrent `docker build` processes competing for CPU, memory, and disk I/O. Configurable per-server with a sensible default (typically 2-4 concurrent builds).
- **Why add to Tengiz:** On a single-server Tengiz instance, multiple simultaneous deploys can overwhelm the Docker daemon with concurrent builds, causing OOM kills, disk I/O starvation, and build failures. Without throttling, a CI/CD pipeline triggering 5 simultaneous deploys can bring the server to its knees. `.tengiz.yaml`'da `server.concurrent_builds: 3` ile yapılandırılır. Implementation: a buffered Go channel (`chan struct{}`) as a semaphore in the `builder` package — each build acquires a token before starting `docker build`, releases on completion. Complements Build Queue with Dedup (#124) — dedup prevents duplicate builds, concurrency limit prevents overload. Low effort (channel-based semaphore), high production reliability value.
- **Detected:** 2026-07-28

## www <-> non-www Redirect (Canonical Domain Enforcement)
- **Source:** Coolify
- **Description:** Coolify provides per-app www <-> non-www URL redirection: users choose whether `www.example.com` redirects to `example.com` or vice versa. Generates proper 301 redirects at the proxy level and provisions SSL certificates for both variants automatically. Traefik middleware handles the redirect with configurable permanent/temporary redirect type.
- **Why add to Tengiz:** This is the single most common URL redirection pattern — every production website needs to pick a canonical domain. While general URL redirect rules (#134) cover custom redirects, www normalization is common enough to warrant first-class support. `.tengiz.yaml`'da `domains.www_redirect: non-www` (or `www`) ile yapılandırılır. Implementation: proxy middleware that checks the request hostname and issues a 301 redirect if it doesn't match the configured canonical form. Automatically provisions SSL for both variants when Let's Encrypt is enabled. Low effort (hostname prefix check + redirect), standard for all production deployments.
- **Detected:** 2026-07-28

## Temporary Auto-Deploy Pause (Maintenance Mode for Git Hooks)
- **Source:** Coolify
- **Description:** Coolify's "Autodeploy pause" temporarily suspends automatic deployments triggered by git push/webhook events. When paused, incoming webhook events are logged but do not trigger a build. Useful during: production incident debugging (prevent deploys while investigating), infrastructure maintenance, database migrations that require deployment freeze, and CI/CD pipeline testing. Resume with a single toggle — queued events during pause can optionally be discarded or processed on resume.
- **Why add to Tengiz:** Without a pause mechanism, the only way to stop auto-deploys during maintenance is to disable the webhook at the Git provider or stop the Tengiz webhook server. `tengiz deploy pause` and `tengiz deploy resume` provide fine-grained control. `.tengiz.yaml`'da `deploy.auto_deploy: true/false` toggle with CLI override. Implementation: a boolean flag in `AppEntry.Config` checked by the webhook handler before triggering a build. When paused, webhook events are logged to `~/.tengiz/paused-events.log` with the option to replay on resume. Low effort, high operational control value. Complements Maintenance Mode (#45) which is proxy-level — this is deploy-level.
- **Detected:** 2026-07-28

## Custom Docker Labels on Applications (Metadata Tagging for Containers)
- **Source:** Coolify
- **Description:** Coolify allows per-app custom Docker labels via the UI. Labels are applied as `docker run --label key=value` and can be used by external tools (Traefik, Prometheus, cAdvisor, log shippers) for container discovery, routing decisions, and metadata enrichment. Labels persist across container restarts and redeploys. Coolify also auto-generates its own labels (`coolify.applicationId`, `coolify.type`, etc.) for platform management.
- **Why add to Tengiz:** Docker labels are the standard mechanism for container metadata in the Docker ecosystem. External tools like Traefik (routing rules), Prometheus (scrape targets), and cAdvisor (container grouping) rely on custom labels. Currently Tengiz only sets its own labels (`tengiz-app=`, `tengiz-env=`). `.tengiz.yaml`'da `docker_labels: { traefik.enable: "true", prometheus.scrape: "true" }` ile yapılandırılır. Implementation: pass `--label` flags to `docker create/run` in `runtime.Create()`, merge user-defined labels with Tengiz's own internal labels. Low effort (key-value passthrough to Docker CLI), enables any external tool integration that relies on Docker labels.
- **Detected:** 2026-07-28

## Webhook Event Queue During Maintenance (Lossless Deploy Freeze)
- **Source:** Coolify
- **Description:** Coolify collects incoming webhook events during maintenance mode and queues them for later processing instead of dropping them. When maintenance ends, queued events are replayed in order, triggering the deferred deployments. Prevents webhook loss during: server maintenance windows, auto-deploy pause periods, proxy restarts, or unplanned downtime. Queue storage uses a simple file-based append log with replay tracking.
- **Why add to Tengiz:** Without a webhook queue, any webhook event received while Tengiz is in maintenance (auto-deploy paused, or server under load) is silently dropped. CI/CD pipelines that fire and forget will not retry — resulting in missed deployments. Implementation: when deploy is paused or maintenance mode is active, incoming webhook payloads are serialized to `~/.tengiz/webhook-queue/<app>/<timestamp>.json`. On resume, a goroutine replays queued events sequentially. Configurable max queue size to prevent disk exhaustion. Low effort (JSON file append + replay loop), high reliability value for CI/CD workflows. Complements Auto-Deploy Pause (above) — pause is the control, webhook queue is the safety net.
- **Detected:** 2026-07-28

## Cross-Environment Resource Cloning (Clone Apps Between Environments)
- **Source:** Coolify
- **Description:** Coolify v4.0+ allows cloning any resource (app, service, database) not just within the same project but across different projects and environments. Cloning copies all configuration (env vars, domains, resource limits, Docker options, storage mounts, secrets) to the target environment. Useful for: promoting apps from staging → production with identical configuration, creating preview environments from production template, and duplicating complex service configurations across teams.
- **Why add to Tengiz:** Existing App Cloning (#43) covers same-environment cloning. Cross-environment cloning is essential for promotion workflows: `tengiz clone myapp --from staging --to production` creates a production app with the exact same config as staging. The user can then deploy the production version. Implementation: `tengiz clone <app> <new-app> --env <target-env>` reads the source AppEntry, applies env-specific overrides (registry, domain, resource limits differ between staging and production), and writes a new AppEntry. Medium effort (config merge logic + dependency resolution), high value for environment promotion workflows. Complements Multi-Environment Desteği (implemented) — envs exist, cloning connects them.
- **Detected:** 2026-07-28

## Preview Deployment-Specific Logs (Separated Log Views)
- **Source:** Coolify
- **Description:** Coolify separates preview deployment logs from production deployment logs. Each PR preview has its own log stream, build history, and deployment status. The UI shows preview logs separately from the main app's logs. Preview deployments have independent log retention and can be filtered by PR number. Build logs for previews are stored separately to avoid cluttering the main app's build history.
- **Why add to Tengiz:** Preview Deployments (#18, implemented) already create isolated containers but share the same `tengiz logs` and `tengiz build-logs` namespace. A `tengiz preview logs <app> <pr>` command would show only the preview's container logs, while `tengiz preview build-logs <app> <pr>` shows the preview's build output. Implementation: log files stored under `~/.tengiz/logs/<app>/previews/<pr>/` and `~/.tengiz/builds/<app>/previews/<pr>/`. Low-medium effort on top of existing preview deployment infrastructure, high operational value for teams reviewing PR deployments.
- **Detected:** 2026-07-28
