...
Features re-evaluated Jul 2026: Priority = f(Impact × Alignment / Effort), where Impact weights very-high=5, high=4, medium=3, low=2, very-low=1.

### P0 — Critical Path (Must-Have for Vercel Alternative)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 1 | **Nixpacks Build System** ⬜ | 5 | 3 | 5 | Expands framework support from 6 to hundreds (Ruby, Rust, PHP, Java, Elixir). Single `builder` package integration. Largest single user-base expansion. |
| 2 | **Secrets Management** ⬜ | 5 | 3 | 5 | Production security fundamental. No platform without encrypted DB passwords, API keys. Vault/1Password/Doppler integration. |
| 3 | **Secret Interpolation System** ⬜ | 4 | 4 | 5 | `[[secret.name]]` syntax. AES-GCM encrypted storage. No external vault needed for single-server. Ships alongside secrets management. |
| 4 | **Notification System** ⬜ | 4 | 3 | 5 | Production ops require alerts for deploy failures, SSL expiry, disk filling. Discord/Slack/Telegram/Email. |
| 5 | **Docker Housekeeping** ⬜ | 5 | 5 | 5 | Disk space is #1 production issue on single-server. Label-based `docker system prune`. `tengiz cleanup`. Highest ROI per line of code. |
| 6 | **Event Logging & Audit Trail** ⬜ | 4 | 5 | 5 | Who deployed what, when? Why did container stop? Multi-developer audit trail via `log/slog` + JSON Lines. Low effort, high ops value. |
| 7 | **Pre-Deploy Hooks** ⬜ | 4 | 5 | 5 | Migration runner before deploy is table stakes. `.tengiz.yaml` `pre_deploy` command list. Failed hook aborts deploy. |
| 8 | **Monorepo Support (Base Directory)** ⬜ | 4 | 5 | 5 | Turborepo/Nx/Lerna users are a large segment locked out without it. Simple `base_dir` override. |
| 9 | **Custom Build Commands** ⬜ | 4 | 5 | 5 | Framework detection must be overridable. `commands.install/build/start` in `.tengiz.yaml`. Essential for custom toolchains. |
| 10 | **App Report (Detailed Status)** ⬜ | 4 | 5 | 5 | `tengiz ps` too minimal. Need deploy history, image tags, env vars, resource limits, domains in one command. |
| 11 | **Build Arguments from Env** ⬜ | 3 | 5 | 5 | NEXT_PUBLIC_*, NPM_TOKEN need `--build-arg` passthrough during build. Critical for framework builds. |
| 12 | **Zero-Downtime Deploy Health Checks** ⬜ | 4 | 5 | 5 | App-level health verification before traffic migration. Auto-rollback on failure. Prevents "deployed but broken" scenarios. |
| 13 | **Commit Status Reporting** ⬜ | 4 | 5 | 5 | Report deploy result back to GitHub/GitLab commit status API. Green checkmark/red X on PRs. Closes git push → deploy → confidence loop. |
| 14 | **One-Line Install Script** ⬜ | 5 | 5 | 5 | `curl -fsSL https://tengiz.dev/install.sh \| bash`. Cross-compile binaries, detect platform, verify checksum. #1 adoption friction reducer. |
| 15 | **Container Registry Integration** ⬜ | 4 | 4 | 5 | Build → push pipeline. Image storage for rollback + multi-node deployment. `docker tag && docker push`. |
| 16 | **Variable Resource (Global Interpolation)** ⬜ | 4 | 5 | 5 | Define `[[database_url]]` once, reference across all apps. Eliminates env var duplication. Ships well with secrets interpolation. |
| 17 | **Explicit Image Name Deploy** ⬜ | 3 | 5 | 5 | Deploy pre-built images (Postgres, Redis, CI/CD output) without build step. `tengiz deploy --image nginx:alpine`. |
| 18 | **App Deploy Tokens** ⬜ | 3 | 5 | 5 | CI/CD authentication without user credentials. `tengiz token create --app myapp`. Token rotation. |
| 19 | **Headless CI/CD Mode** ⬜ | 3 | 5 | 5 | `TENGIZ_TOKEN` + `--headless` for GitHub Actions, GitLab CI. Non-interactive, no TTY. |
| 20 | **Deploy Lock Mekanizması** ⬜ | 3 | 5 | 5 | Prevent concurrent deploy collisions in team environments. File-based lock + `--lock-wait`. |
| 21 | **Config Export/Import** ⬜ | 3 | 5 | 5 | Disaster recovery and app migration. Export env vars as shell/dotenv/Docker args/JSON. |
| 22 | **HMAC-Signed Webhook Payloads** ⬜ | 3 | 5 | 5 | Verify GitHub/GitLab webhook signatures. Prevent unauthorized deploy triggers. Low effort, high security impact. |
| 23 | **App Auto-Creation on Git Push** ⬜ | 3 | 5 | 5 | Zero-setup deploy: `git push tengiz main` auto-creates app. Removes `tengiz create` step. |
| 24 | **Server Reboot Recovery** ⬜ | 4 | 4 | 5 | Auto-restart all apps after host reboot. Systemd unit + `ps:restore` on boot. Without this, host reboot = manual recovery. |
| 25 | **Container Restart Policy Management** ⬜ | 3 | 5 | 5 | Per-app Docker restart policy: `no/always/unless-stopped/on-failure`. Crash behavior control. Essential for production. |
| 26 | **Config Display Command** ⬜ | 3 | 5 | 5 | `tengiz config` shows merged config after env override + template resolution. Debugging essential. |
| 27 | **Deploy Source Metadata Recording** ⬜ | 3 | 5 | 5 | Record GIT_SHA, branch, deploy source (git-hook/cli/webhook). Env vars + build record. |
| 28 | **Webhook Event Filtering** ⬜ | 3 | 5 | 5 | Branch/tag/path filtering. Prevents wasteful deploys from docs changes or WIP branches. |
| 29 | **Per-Container Resource Metrics** ⬜ | 3 | 5 | 5 | Live CPU/memory/network from `docker stats`. `tengiz ps --stats` or `tengiz stats <app>`. |
| 30 | **Parallel Bulk Operations** ⬜ | 3 | 5 | 5 | `tengiz rebuild --all`, `tengiz restart --all` with `--parallelism N`. Goroutine pool. |
| 31 | **Granular Docker Prune Operations** ⬜ | 3 | 5 | 5 | Per-category prune: containers/networks/images/volumes/buildx cache. Surgical disk management. |
| 32 | **Auth Rate Limiting** ⬜ | 3 | 5 | 5 | Per-IP brute force protection on auth/webhook endpoints. Low effort, essential security. |
| 33 | **Well-Known Paths Automatic Handling** ⬜ | 3 | 5 | 5 | `/.well-known/` for ACME HTTP-01, Apple Universal Links, security.txt. Critical for domain verification. |
| 34 | **Custom HTTP Headers per URL Path** ⬜ | 3 | 5 | 5 | Per-path Cache-Control, CORS, security headers. Vercel/Netlify `_headers` style. Proxy middleware. |
| 35 | **Private Registry Authentication** ⬜ | 3 | 5 | 5 | GHCR, GitLab Registry, AWS ECR pull support. Enterprise prerequisite. |
| 36 | **Error Pages** ⬜ | 3 | 5 | 5 | Cold start or container-down UX: user-friendly error pages instead of raw HTTP errors. Proxy middleware. |
| 37 | **Container Retention Policy** ⬜ | 3 | 5 | 5 | Rollback companion. Keep N old containers, prune rest. Default: retain 5. |
| 38 | **Scheduled Deployments** ⬜ | 3 | 4 | 5 | Cron-based auto-deploy. Nightly rebuilds, dependency updates. Built on `robfig/cron`. |

### P1 — Production Platform (Team-Ready)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 39 | **Extended Hook System** ⬜ | 3 | 4 | 5 | Pre-build (secret injection), post-deploy (notifications), app-boot (cache warming). Rich hook env context. |
| 40 | **Maintenance Mode** ⬜ | 3 | 5 | 5 | Proxy draining for planned maintenance. `tengiz maintenance:on --message "Upgrading..."`. |
| 41 | **Prometheus Metrics** ⬜ | 3 | 5 | 5 | Proxy HTTP metrics: request count, latency, error rate, cold start count. Grafana + alerting foundation. |
| 42 | **Readiness Delay & Deploy Timeouts** ⬜ | 3 | 5 | 5 | Per-operation timeouts for deploy/drain/stop. Accommodates both fast Go apps and slow Node.js. |
| 43 | **Node.js Multi-Core Scaling (PM2/Cluster)** ⬜ | 3 | 5 | 5 | 4-8x performance improvement for Node.js apps via PM2 cluster mode. `.tengiz.yaml` toggle. |
| 44 | **Custom Docker Network** ⬜ | 3 | 5 | 5 | Isolated networks for multi-service apps. `docker run --network` flag support. |
| 45 | **Server Bootstrap** ⬜ | 3 | 4 | 5 | `tengiz server init` + `tengiz setup` — one-command Docker + Tengiz installation. |
| 46 | **HTTP Basic Auth (Staging Protection)** ⬜ | 3 | 5 | 5 | Password-protect staging/pre-production environments. Proxy middleware. |
| 47 | **Build-Time Secrets (Docker Build Secrets)** ⬜ | 3 | 5 | 5 | NPM_TOKEN, signing keys passed via `--secret` (not `--build-arg`). Excluded from image history. |
| 48 | **Stale Container Detection** ⬜ | 3 | 5 | 5 | Detect orphaned old-version containers after zero-downtime deploy. Auto-cleanup or report. |
| 49 | **Environment Variable Locking** ⬜ | 3 | 5 | 5 | Prevent accidental `tengiz config unset DATABASE_URL`. Locked vars require confirmation. |
| 50 | **Background Monitoring Scheduler** ⬜ | 3 | 5 | 5 | Proactive health checks, disk usage tracking, container status monitoring. Feeds alert system. |
| 51 | **Full System Backup & Restore** ⬜ | 3 | 4 | 5 | `tengiz backup create` archives `~/.tengiz/` state. `tengiz backup restore` for DR. |
| 52 | **App Repository Lifecycle Management** ⬜ | 3 | 5 | 5 | Per-app git repo management: `git:lock/unlock/status`. Prevent pushes during maintenance. |
| 53 | **Custom Image Repository Naming** ⬜ | 3 | 5 | 5 | Go template-based image naming. `ghcr.io/myorg/{{ .AppName }}`. Collision avoidance. |
| 54 | **Concurrency Control (Operation Locking)** ⬜ | 3 | 5 | 5 | File-based mutex per app for state-modifying ops. Prevents state file corruption. |
| 55 | **Per-App Proxy Toggle** ⬜ | 3 | 5 | 5 | `tengiz proxy:disable/enable <app>`. Internal-only services that shouldn't be internet-facing. |
| 56 | **Linux Capabilities Management** ⬜ | 3 | 5 | 5 | Per-app `--cap-add`/`--cap-drop`. Principle of least privilege for containers. |
| 57 | **Persistent App Mode (Stateful vs Stateless)** ⬜ | 4 | 5 | 5 | Mark DBs/caches as persistent → skip scale-to-zero, idle timeout, multi-replica. Prevents data-loss. |
| 58 | **Auto-SSL/TLS (Let's Encrypt)** ⬜ | 4 | 4 | 4 | Important but external proxy (Caddy/Nginx) handles this today. `autocert` integration medium effort. |
| 59 | **One-Click Service Templates** ⬜ | 4 | 3 | 4 | 361 Docker Compose templates (WordPress, N8N, MinIO). `tengiz service create <template>`. Medium alignment due to Docker Compose dependency. |
| 60 | **Managed Database Provisioning** ⬜ | 4 | 2 | 4 | Vercel Postgres/KV equivalent. `tengiz db create postgres --name mydb`. Very high effort but high impact. |
| 61 | **Accessory Services (Sidecar Containers)** ⬜ | 3 | 4 | 5 | Postgres/Redis/Search alongside app. Scale-to-zero only affects app, not accessories. Pragmatic alternative to managed DBs. |
| 62 | **Procfile Support** ⬜ | 3 | 5 | 5 | Heroku-style process type definition. Zero-config Heroku migration. `web/worker/clock` process types. |
| 63 | **Force HTTPS Redirect** ⬜ | 3 | 5 | 5 | HTTP→HTTPS 301 redirect in proxy. `.tengiz.yaml` `force_https: true`. SEO + security. |
| 64 | **Build Tracking with Retention** ⬜ | 3 | 4 | 5 | Structured deploy history: JSON records, status tracking, build logs retention. |
| 65 | **Git-Based Image Version Tagging** ⬜ | 3 | 5 | 5 | Auto-tag images with git commit SHA. Foundation for rollback and deploy history. |
| 66 | **Config Validation** ⬜ | 3 | 5 | 5 | `tengiz config validate`. Pre-deploy config sanity check. Catches errors before build. |
| 67 | **Magic Environment Variables** ⬜ | 3 | 4 | 5 | Auto-generated service URLs, DB connection strings, passwords for linked services. |
| 68 | **Log Drains (External Log Streaming)** ⬜ | 3 | 4 | 5 | Axiom, New Relic, Loki log forwarding. Structured metadata. Production observability. |
| 69 | **Rate Limiting** ⬜ | 3 | 5 | 5 | Webhook/API rate limiting. `golang.org/x/time/rate`. HTTP 429. |
| 70 | **Server Security Hardening** ⬜ | 3 | 4 | 5 | UFW firewall, Fail2ban, Docker TLS during `tengiz server init`. |
| 71 | **Database Connection String Auto-Injection** ⬜ | 3 | 4 | 5 | Auto-inject `DATABASE_URL` when DB accessory linked to app. |
| 72 | **Docker Network & Volume CRUD** ⬜ | 3 | 5 | 5 | `tengiz network create/ls/rm`, `tengiz volume create/ls/rm`. No raw Docker CLI needed. |
| 73 | **Build Cache Management & Git GC** ⬜ | 3 | 5 | 5 | `tengiz cleanup --cache --gc`. Per-app build cache + git repo pruning. |
| 74 | **Herokuish Buildpacks** ⬜ | 3 | 4 | 5 | Heroku's entire buildpack ecosystem. Zero-config migration from Heroku. Complements Nixpacks. |
| 75 | **Multi-Architecture Builds (Buildx)** ⬜ | 3 | 4 | 5 | `linux/amd64 + linux/arm64` multi-arch manifests. Apple Silicon → Intel server. |
| 76 | **Docker Buildx Builder Lifecycle Management** ⬜ | 3 | 4 | 5 | Auto-create/inspect/remove buildx builders for multi-arch and build cache. Prerequisite for #75. |
| 77 | **Network Aliases for Container Connectivity** ⬜ | 3 | 5 | 5 | Custom Docker network aliases for inter-container communication in multi-service apps. |
| 78 | **Deploy Source Metadata Recording** ⬜ | 3 | 5 | 5 | Record GIT_SHA, branch, deploy source (git-hook/cli/webhook). Env vars + build record. |
| 79 | **Zip Archive Direct Deployment** ⬜ | 3 | 5 | 5 | Deploy from .zip archive without git. CI/CD artifacts, quick demos. |
| 80 | **Free Development Domain Provisioning** ⬜ | 4 | 5 | 5 | `*.sslip.io` or `*.tengiz.dev` subdomain for instant public URLs. Core PaaS promise: "deploy and get a URL." |
| 81 | **Healthcheck Barrier for Coordinated Multi-Server Boot** ⬜ | 3 | 4 | 5 | Batch health gating during rolling deployment. Prevents cascading failures in multi-server rollout. |

### P2 — Differentiators (Vercel-Level Features)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 82 | **REST API + OpenAPI Spec** ⬜ | 4 | 3 | 4 | Unlocks CI/CD integration, programmatic access, and future web UI. Must-have for growth despite deviating from CLI-first. Higher effort. |
| 83 | **Embedded Serverless Functions (goja)** ⬜ | 4 | 3 | 5 | Biggest potential differentiator. Dockerless <10ms function runtime. TypeScript → Go `goja`. No other Docker-based alt offers this. |
| 84 | **GitOps / Declarative ResourceSync** ⬜ | 4 | 3 | 5 | Declare apps/env/domains as git-managed YAML. `tengiz sync` reconciles. IaC for Tengiz. High effort but transformative for team ops. |
| 85 | **Local Development Emulator** ⬜ | 4 | 4 | 5 | `tengiz emulator start` runs full platform stack locally. Hot-reload. Match Vercel local dev experience. |
| 86 | **Client SDK Ecosystem** ⬜ | 4 | 4 | 5 | `@tengiz/core` npm package for auth/datastore/storage. Makes platform services feel built-in. |
| 87 | **Web Dashboard (Admin UI)** ⬜ | 4 | 3 | 4 | Highest impact for non-CLI users but high effort and deviates from CLI-first. Embed SPA in binary via `embed.FS`. |
| 88 | **WebSocket Support Per App** ⬜ | 3 | 5 | 5 | Per-app WebSocket toggle in proxy. `.tengiz.yaml` `proxy.websocket: true`. |
| 89 | **Pattern-Based Watch Paths** ⬜ | 3 | 4 | 5 | `tengiz deploy --watch` with glob patterns. Auto-redeploy on file changes. `fsnotify`. |
| 90 | **Event-Driven Data Hooks (Trigger System)** ⬜ | 3 | 4 | 5 | `container:start`, `deploy:success`, `idle:timeout` events. Makes Tengiz programmable. |
| 91 | **Container Snapshot System** ⬜ | 3 | 5 | 5 | `docker commit`-based stateful snapshot. Risk mitigation before risky deploys. |
| 92 | **Built-in Platform Analytics** ⬜ | 3 | 4 | 5 | HTML injection + Web Vitals tracking. SQLite storage. Vercel Analytics-level feature. |
| 93 | **Built-in Authentication Service** ⬜ | 4 | 3 | 5 | Platform-level auth-as-a-service. Google/GitHub/Passkey. Proxy auth intercept + header injection. |
| 94 | **Built-in NoSQL Datastore** ⬜ | 4 | 3 | 5 | Zero-config document store. Embedded SQLite + proxy `/__tengiz/db/` API. |
| 95 | **Built-in File/Blob Storage** ⬜ | 4 | 3 | 5 | Platform-level asset hosting. URL-based access control. Upload/serve/delete API. |
| 96 | **Framework Plugins (Next.js/Vite)** ⬜ | 3 | 4 | 5 | `@tengiz/nextjs` npm package. Auto-inject env + API routes. Differentiator from Coolify/Dokku. |
| 97 | **Build Precompression** ⬜ | 3 | 5 | 5 | Gzip/Brotli pre-compression at build time. Zero-CPU asset serving. |
| 98 | **Staged Deployments (Change Sets)** ⬜ | 3 | 4 | 5 | `tengiz deploy --no-apply` → `tengiz changes apply <id>`. Deploy on Friday, apply on Monday. |
| 99 | **Project Scaffolding with Starter Templates** ⬜ | 3 | 4 | 5 | `tengiz create <template>`. React/Vite/Next.js/Go API templates. Minutes to first deploy. |
| 100 | **Change Approval Workflow** ⬜ | 3 | 4 | 5 | Submit → Review → Apply. Team governance for deployments. |
| 101 | **Docker Compose Import** ⬜ | 3 | 4 | 5 | Convert existing `docker-compose.yml` to Tengiz containers. Multi-service deploy. |
| 102 | **Global/Per-App Property Cascade** ⬜ | 3 | 4 | 5 | Global defaults → app overrides. Reduce per-app config overhead for multi-app instances. |
| 103 | **Per-Process-Type Resource Limits** ⬜ | 3 | 5 | 5 | Separate CPU/memory for web/worker/scheduler. Extends existing resource limits. |
| 104 | **Git Provider OAuth App Integration** ⬜ | 3 | 3 | 5 | One-click GitHub/GitLab App connection. Auto-configure webhooks + deploy keys. |
| 105 | **Centralized Multi-Server Management** ⬜ | 3 | 3 | 5 | Control-plane model: one Tengiz instance manages remote Docker hosts via SSH. |
| 106 | **MCP Server for AI Integration** ⬜ | 3 | 4 | 5 | Model Context Protocol server. AI assistants query/manage Tengiz via natural language. |
| 107 | **Process Scaling (Multi-Container)** ⬜ | 3 | 3 | 4 | HA + background workers (Sidekiq, Celery). Combines with idle timeout for serverless model. |
| 108 | **Server Monitoring** ⬜ | 3 | 4 | 5 | Disk, container states, backup status. `tengiz status` + threshold alert. |
| 109 | **Scheduled Tasks / Cron Jobs** ⬜ | 3 | 4 | 5 | Vercel Cron Jobs equivalent. `.tengiz.yaml` `cron:` + `robfig/cron`. `docker exec` execution. |
| 110 | **Gelişmiş Proxy Konfigürasyonu** ⬜ | 3 | 4 | 5 | Path prefix, buffering, timeout, X-Forwarded-* header control. Production-grade proxy. |
| 111 | **KEDA-based Autoscaling** ⬜ | 3 | 3 | 5 | Scale 0→N based on HTTP rate, queue depth, CPU. Transforms idle timer into real serverless. |
| 112 | **Build Pipeline with Auto-Versioning** ⬜ | 3 | 4 | 5 | Source → versioned image → multi-registry push. Auto-tag: semver/commit-sha/timestamp. |
| 113 | **Build-to-Deploy Trigger Chain** ⬜ | 3 | 4 | 5 | Build completes → linked deployment auto-redeploys. Full CI/CD without external tools. |
| 114 | **Remote Docker Builder** ⬜ | 3 | 4 | 5 | SSH-based remote buildx builder. Offload heavy builds from production server. |
| 115 | **Forward Auth / SSO Gate for Deployed Apps** ⬜ | 4 | 4 | 5 | Protect deployed apps behind enterprise SSO (Google/GitHub/OIDC) without app code changes. Proxy-level auth intercept. |
| 116 | **Multi-Tenancy with Team Management & RBAC** ⬜ | 4 | 3 | 5 | Team scoping, invitations, MEMBER/ADMIN/OWNER roles. Required for shared server deployments. |
| 117 | **Service Users (Built-in System Automation Accounts)** ⬜ | 3 | 5 | 5 | Internal identities for webhooks/procedures/cron. Audit trail with actor attribution. |
| 118 | **TypeScript Action Automation (Deno)** ⬜ | 3 | 4 | 5 | Embedded TS runtime for platform automation. Custom deploy logic, webhook transforms. |
| 119 | **OIDC/OAuth Single Sign-On** ⬜ | 3 | 4 | 5 | Google/GitHub OAuth + generic OIDC. Team auth for shared servers. |
| 120 | **Two-Factor Authentication (2FA)** ⬜ | 3 | 4 | 5 | TOTP-based 2FA for admin accounts. `github.com/pquerna/otp`. |
| 121 | **Granular Scoped API Keys** ⬜ | 3 | 5 | 5 | API keys with Read/Write/Execute permissions per resource type. |
| 122 | **Collection Memory Type Configuration** ⬜ | 3 | 5 | 5 | Ephemeral (Go map) vs persistent (SQLite) per datastore collection. |
| 123 | **Persistent Docker BuildKit Cache** ⬜ | 3 | 5 | 5 | Per-app build cache volume. 60-90% build time reduction. |
| 124 | **Private Asset Access Tokens** ⬜ | 3 | 5 | 5 | Token-gated private file URLs. Shareable without auth. |
| 125 | **Granular Per-Operation Rate Limiting** ⬜ | 3 | 4 | 5 | Per-operation-type limits: reads vs writes, uploads vs downloads. |
| 126 | **Platform Self-Health Check** ⬜ | 3 | 5 | 5 | Background goroutine + `/healthz` endpoint. Auto-restart on failure. |
| 127 | **Server Exec (Host-Level Commands)** ⬜ | 3 | 5 | 5 | `tengiz server exec "df -h"`. Controlled host access without SSH. |
| 128 | **Self-Upgrade / Auto-Update** ⬜ | 3 | 4 | 5 | `tengiz upgrade`. GitHub Releases binary download. |
| 129 | **Trace/Debug Mode** ⬜ | 3 | 5 | 5 | `--debug` flag → slog LevelDebug. Verbose logging across all packages. |
| 130 | **Failed Deploy Logs** ⬜ | 3 | 5 | 5 | `tengiz logs --failed <app>`. Retrieve last failed deploy logs. |
| 131 | **Container Entering (tengiz enter)** ⬜ | 2 | 5 | 5 | `tengiz enter <app>` → `docker exec -it`. Interactive debug shell. |
| 132 | **Git-Sync Deployment** ⬜ | 3 | 5 | 5 | `tengiz deploy --sync <repo> --interval 5m`. Pull-based deployment. |
| 133 | **Scoped Shared Environment Variables** ⬜ | 3 | 4 | 5 | `{{team.*}}`, `{{project.*}}`, `{{environment.*}}`, `{{server.*}}` scope levels. Eliminates env var duplication. |
| 134 | **Environment Variable Advanced Flags** ⬜ | 3 | 5 | 5 | `is_buildtime`, `is_runtime`, `is_preview`, `is_shown_once`, `is_required` per env var. |
| 135 | **File-Based Storage Mounts** ⬜ | 3 | 5 | 5 | Mount individual files into containers with encrypted content tracking. |
| 136 | **Deployment Queue with Concurrency Control** ⬜ | 3 | 4 | 5 | Per-server concurrent builds limit + queued deployments. Provenance tracking with config snapshots. |
| 137 | **Resource Move Between Environments/Servers** ⬜ | 3 | 4 | 5 | `tengiz move myapp --from staging --to production`. Promote or migrate apps. |
| 138 | **Preview URL Template Customization** ⬜ | 2 | 5 | 5 | `{{domain}}`, `{{pr_id}}`, `{{random}}` in preview URL templates. |
| 139 | **DNS Validation with Custom DNS Servers** ⬜ | 3 | 5 | 5 | Validate DNS before SSL issuance. Custom DNS for air-gapped deployments. |
| 140 | **Custom Docker Labels Per App** ⬜ | 2 | 5 | 5 | Custom Docker labels for metadata tagging (service discovery, monitoring, billing). |
| 141 | **www ↔ non-www Redirect Configuration** ⬜ | 2 | 5 | 5 | Per-app canonical domain redirect. SEO best practice. |
| 142 | **API-Level Sensitive Data Masking** ⬜ | 3 | 5 | 5 | API responses mask sensitive fields unless elevated permissions. |
| 143 | **Multi-Provider Webhook Secrets** ⬜ | 3 | 5 | 5 | Per-provider webhook secrets (GitHub, GitLab, Bitbucket, Gitea). |
| 144 | **Application-Level Restart Tracking** ⬜ | 2 | 5 | 5 | Restart count, max allowed, type, timestamp. Prevents crash loops. |
| 145 | **Application Settings Toggles (SPA Mode, Gzip, Grace Period, etc.)** ⬜ | 3 | 5 | 5 | `is_spa`, `is_gzip_enabled`, `stop_grace_period`, etc. Power-user toggles, nearly zero cost. |
| 146 | **Procedure Automation (Multi-Step Workflows)** ⬜ | 3 | 5 | 5 | Define repeatable procedures: deploy → migrate → healthcheck → notify. |
| 147 | **Image Digest Change Detection & Auto-Redeploy** ⬜ | 2 | 5 | 5 | Auto-redeploy when tracked image digest changes. Keeps third-party images updated. |
| 148 | **Image Digest Pinning** ⬜ | 2 | 5 | 5 | Pin deployment to specific SHA digest. Deterministic deploys. |
| 149 | **System Stats Recording** ⬜ | 3 | 4 | 5 | Historical per-container CPU, memory, network stats. JSON Lines. |
| 150 | **Stack/Compose Lifecycle Management** ⬜ | 3 | 4 | 5 | First-class Compose stack resource with full lifecycle. Beyond one-time import. |
| 151 | **Alert System with Severity Levels** ⬜ | 3 | 4 | 5 | 5 severity levels, resolved/unresolved tracking, auto-pruning. |
| 152 | **Multi-Channel Alerters** ⬜ | 3 | 5 | 5 | Slack/Discord/Pushover/ntfy with severity-based formatting. |
| 153 | **Maintenance Windows for Alert Suppression** ⬜ | 3 | 4 | 5 | Suppress alerts during planned maintenance. Prevent alert fatigue. |
| 154 | **Stack File Dependency Tracking** ⬜ | 3 | 4 | 5 | Granular diff-based compose redeploy: only affected services restart. |

### P3 — Enterprise & Niche (Enhancement)

| # | Feature | I | E | A | Rationale |
|---|---------|---|---|---|-----------|
| 155 | **SSH Tabanlı Remote Deployment** ⬜ | 3 | 3 | 4 | Multi-server support via SSH. High effort, Tengiz built as single-node. `golang.org/x/crypto/ssh`. |
| 156 | **Multi-Server Architecture with Periphery Agent** ⬜ | 3 | 1 | 5 | Core↔Periphery distributed model via WebSocket. Transformative but very high effort. |
| 157 | **Docker Swarm Resource Management** ⬜ | 3 | 3 | 5 | Multi-node HA via Docker Swarm mode. Lighter than K3s. |
| 158 | **Onboarding Keys for Zero-Config Server Registration** ⬜ | 2 | 4 | 5 | Pre-shared keys for auto-registration of new servers. |
| 159 | **Role Tabanlı Sunucu Grupları** ⬜ | 3 | 4 | 4 | Web/worker/job role separation. Different cmd/env per role. |
| 160 | **Cloud Provider Server Provisioning** ⬜ | 3 | 2 | 3 | One-click Hetzner/Vultr/DO server creation. `tengiz server create --provider hetzner`. |
| 161 | **Cloud-Init Script Automation** ⬜ | 2 | 4 | 5 | Reusable cloud-init scripts for first-boot automation. |
| 162 | **Sentinel Push-Based Monitoring Agent** ⬜ | 3 | 4 | 5 | Dedicated monitoring agent container. Push-based metrics collection. |
| 163 | **Server Patch/Update Health Monitoring** ⬜ | 2 | 4 | 5 | Check for available OS package updates. Report via notifications. |
| 164 | **Version Mismatch Detection** ⬜ | 2 | 5 | 5 | Component compatibility checking between CLI, proxy, runtime. |
| 165 | **System Maintenance Operations** ⬜ | 2 | 4 | 5 | `ClearRepoCache`, `BackupCoreDatabase`, `RotateKeys`, `GlobalAutoUpdate`. |
| 166 | **User Group Resource (RBAC)** ⬜ | 3 | 4 | 5 | Group-based permissions for team management. |
| 167 | **Encryption at Rest** ⬜ | 3 | 4 | 5 | AES-256 encryption of env vars in `apps.json`. Enterprise security. |
| 168 | **Manual SSL Certificate Management** ⬜ | 3 | 4 | 5 | Import/generate/inspect/remove SSL certs. Enterprise cert management. |
| 169 | **MySQL/PostgreSQL Automated Database Backups** ⬜ | 3 | 4 | 5 | `docker exec <container> pg_dump`. Cron-based, S3 storage. |
| 170 | **Reusable External Storage Destinations** ⬜ | 3 | 4 | 5 | First-class S3 destination management for backups. |
| 171 | **Platform Admin Settings** ⬜ | 2 | 5 | 5 | Centralized server URL, Docker config, default resource limits. |
| 172 | **Git Provider Account Management** ⬜ | 3 | 5 | 5 | Store GitHub/GitLab tokens for private repo clone access. |
| 173 | **Docker Registry Account Management** ⬜ | 2 | 5 | 5 | Multi-registry auth management for push/pull. |
| 174 | **SSH Key Management for Servers and Git** ⬜ | 3 | 4 | 5 | Per-server SSH key pairs. `tengiz ssh-key generate/add/list/remove`. |
| 175 | **SSH Key Management for Deploy Access** ⬜ | 2 | 5 | 5 | Per-developer SSH key deploy access control. |
| 176 | **Pluggable Event/Trigger Architecture** ⬜ | 3 | 3 | 5 | 40+ lifecycle trigger points. Community plugin ecosystem. High effort, transformative. |
| 177 | **Pluggable Multi-Scheduler (Docker → K3s)** ⬜ | 2 | 1 | 4 | Scheduler abstraction. Very high architectural change. |
| 178 | **Pluggable Reverse Proxy** ⬜ | 2 | 3 | 4 | nginx/Caddy/HAProxy/Traefik backend option. |
| 179 | **Custom Build Server** ⬜ | 2 | 3 | 4 | Separate build/deploy servers. SSH + registry push/pull. |
| 180 | **Builder Resource (URL/Server/AWS EC2)** ⬜ | 2 | 3 | 5 | Decoupled build targets. Ephemeral AWS EC2 builders. |
| 181 | **Rolling Boot / Canary Deployment** ⬜ | 2 | 3 | 4 | Multi-server only. Gradual deployment to limit blast radius. |
| 182 | **Version Targeting** ⬜ | 2 | 5 | 5 | Deploy/exec specific previous version by tag or version ID. |
| 183 | **App Images Command** ⬜ | 2 | 5 | 5 | List Docker images per app: tag, creation time, size. |
| 184 | **App Cloning** ⬜ | 2 | 5 | 5 | `tengiz apps:clone <old> <new>`. Full config copy for staging/preview. |
| 185 | **App Renaming** ⬜ | 2 | 5 | 5 | `tengiz rename <old> <new>`. Full state migration. |
| 186 | **Docker Compose Service Update Strategy** ⬜ | 2 | 5 | 5 | `startFirst` vs `stopFirst` deploy strategy. Resource-constrained environments. |
| 187 | **Safe Volume Deletion** ⬜ | 2 | 5 | 5 | `tengiz volume rm` cross-app check. Protect shared volumes. |
| 188 | **Protected Service Deletion** ⬜ | 2 | 5 | 5 | Confirmation, backup-before-delete, linked resource protection. |
| 189 | **Port Mapping Protocol Selection** ⬜ | 2 | 5 | 5 | TCP/UDP/both protocol selection. Non-HTTP services (DNS, gRPC, DB). |
| 190 | **Project-Based App Organization** ⬜ | 2 | 5 | 5 | `tengiz project create`, `tengiz ps --project`. Group related apps. |
| 191 | **App Tags** ⬜ | 2 | 5 | 5 | `tengiz tag add myapp staging`. `tengiz ps --tag`. Ad-hoc grouping. |
| 192 | **Pre-Install Env Validation (tengiz doctor)** ⬜ | 2 | 5 | 5 | Docker version, port availability, disk space check before deploy. |
| 193 | **Interactive Env Prompts & Auto-Generated Secrets** ⬜ | 2 | 5 | 5 | TTY prompts for required env vars on first deploy. `"generator": "secret"`. |
| 194 | **Patches (Build-Time File Overrides)** ⬜ | 2 | 4 | 5 | Environment-specific `.env`, `robots.txt` injected during build. |
| 195 | **Cloudflare Tunnel Support** ⬜ | 2 | 4 | 5 | Zero-trust exposure via Cloudflare edge. `cloudflared` CLI wrapper. |
| 196 | **S3-Compatible Backup Storage** ⬜ | 3 | 4 | 5 | Database backups to S3. Scheduled + retention policy. |
| 197 | **Outgoing Webhook Payloads** ⬜ | 2 | 5 | 5 | POST deploy events to external URLs. CI/CD pipeline integration. |
| 198 | **Custom Compose Overrides** ⬜ | 2 | 5 | 5 | `docker-compose.override.yml` merge support for templates. |
| 199 | **Build Queue with Dedup** ⬜ | 2 | 5 | 5 | Per-app channel-based queue. Last-one-wins dedup for rapid-fire CI/CD. |
| 200 | **Deploy Cancellation (Build PID Kill & Lock Cleanup)** ⬜ | 2 | 5 | 5 | Cancel in-progress build by killing PID and cleaning lock. |
| 201 | **GoAccess Real-Time Log Analytics** ⬜ | 2 | 5 | 5 | Optional analytics container. `tengiz analytics enable` → dashboard. |
| 202 | **NetData Integration** ⬜ | 2 | 5 | 5 | Real-time system monitoring companion container. |
| 203 | **Self-Hosted Docker Registry** ⬜ | 2 | 5 | 5 | Built-in `registry:2` container. `tengiz registry enable`. |
| 204 | **Cloud Native Buildpacks (pack CLI)** ⬜ | 2 | 5 | 5 | Heroku-style buildpacks via `pack` CLI. Alternative to Nixpacks. |
| 205 | **Railpack Builder** ⬜ | 2 | 5 | 5 | Alternative build system alongside Nixpacks/CNB. |
| 206 | **Null Builder** ⬜ | 2 | 5 | 5 | Skip build permanently. Pre-built images only. |
| 207 | **Lambda Builder (Docker-Based FaaS)** ⬜ | 2 | 4 | 5 | AWS Lambda-compatible functions on Tengiz. `lambda.yml` manifest. |
| 208 | **WebSocket Interactive Terminal** ⬜ | 2 | 4 | 5 | PTY allocation + WebSocket streaming. Beyond simple `docker exec`. |
| 209 | **Real-Time WebSocket for Deploy Logs** ⬜ | 2 | 4 | 5 | Live deploy log streaming. `tengiz deploy --stream`. Web UI foundation. |
| 210 | **app.json Manifest (Heroku Compatible)** ⬜ | 2 | 4 | 5 | Zero-config Heroku migration manifest. Merge with `.tengiz.yaml`. |
| 211 | **Git Submodules & Git LFS Support** ⬜ | 2 | 5 | 5 | `git submodule update --init --recursive` + Git LFS during clone. |
| 212 | **App-Level Lifecycle Data Hooks** ⬜ | 3 | 4 | 5 | Data change triggers: `onSetDoc`, `onDeleteDoc`. General trigger system. |
| 213 | **Docker Logging Konfigürasyonu** ⬜ | 2 | 5 | 5 | Per-app Docker log driver config (json-file, loki, syslog). |
| 214 | **Asset Path / Asset Bridging** ⬜ | 2 | 5 | 5 | Zero-downtime deploy asset co-existence for hash-based filenames. |
| 215 | **Gelişmiş Docker Build** ⬜ | 2 | 5 | 5 | Multi-arch build, build cache, Docker driver config. |
| 216 | **Root Domain Change** ⬜ | 2 | 5 | 5 | `tengiz proxy --domain production.com`. Atomic domain + SSL update. |
| 217 | **Git Commit Hash Auto-Injection** ⬜ | 2 | 5 | 5 | `TENGIZ_COMMIT_SHA` env var. Shown in `tengiz ps --verbose`. |
| 218 | **Git Property Configuration** ⬜ | 2 | 5 | 5 | `deploy-branch`, `keep-git-dir`, `rev-env-var` per-app. |
| 219 | **Procfile Path Configuration & Dockerfile CMD Override** ⬜ | 2 | 5 | 5 | Custom Procfile path for monorepo. Dockerfile CMD override. |
| 220 | **App-Level Deploy Lock** ⬜ | 2 | 5 | 5 | `tengiz apps:lock/unlock <app>`. Prevent deploys during active debugging. |
| 221 | **Network Attachment & Security Properties** ⬜ | 2 | 4 | 5 | `bind-all-interfaces`, `tld`, `attach-post-create` per-app. |
| 222 | **Redeploy** ⬜ | 2 | 5 | 5 | Skip bootstrap/prune steps, just build/push/restart. |
| 223 | **Programmatic Pre-Deploy Hook** ⬜ | 2 | 4 | 5 | JavaScript hook (goja) to modify deployment object before container creation. |
| 224 | **Metrics Endpoint Security (Token Authentication)** ⬜ | 2 | 5 | 5 | Token-protected `/metrics` endpoint. Configurable port. |
| 226 | **Alternative Git Deploy Sources (Archive/Directory/Image)** ⬜ | 2 | 5 | 5 | Deploy from tar/tar.gz, local directory, or pre-built image. |
| 227 | **Alternative ACME Providers** ⬜ | 2 | 5 | 5 | ZeroSSL, BuyPass, Google. Let's Encrypt rate limit bypass. |
| 228 | **Staging Mode for SSL Testing** ⬜ | 2 | 5 | 5 | ACME staging endpoints for rate-limit-free SSL testing. |
| 229 | **CLI Alias Tanımlama** ⬜ | 1 | 5 | 5 | `.tengiz.yaml` `aliases:` section for command shortcuts. |
| 230 | **Output/Telemetry Loggers** ⬜ | 2 | 4 | 4 | OpenTelemetry/file logger. Centralized log collection (Loki, Datadog). |
| 231 | **Config Format Self-Documentation** ⬜ | 2 | 5 | 5 | `tengiz config docs` shows valid keys, types, defaults inline. |
| 232 | **Docker Helper Image Management** ⬜ | 2 | 5 | 5 | Custom helper Docker image for build operations. Auto-update tracking. |
| 233 | **Vector Log Shipping** ⬜ | 2 | 4 | 5 | Log aggregator companion container. Loki/Datadog/Axiom sinks. |
| 234 | **User-Facing Changelog/Activity Feed** ⬜ | 2 | 5 | 5 | Changelog from CDN + read tracking. Activity logging for all model changes. |
| 235 | **Email Notification Engine** ⬜ | 3 | 5 | 5 | SMTP-based alerts for deploy failure, SSL expiry, backup notification. |
| 236 | **CDN Provider Detection** ⬜ | 3 | 5 | 5 | Cloudflare/Fastly IP range detection. Correct client IP extraction. |
| 237 | **URL Redirect & Rewrite Rules** ⬜ | 3 | 5 | 5 | Per-app 301/302 redirects, URL rewrites at proxy level. |
| 238 | **Proxy Security Middleware** ⬜ | 3 | 5 | 5 | IP allow/deny (CIDR), security headers (HSTS, CSP), per-app basic auth. |
| 239 | **GPU Passthrough (NVIDIA/CUDA)** ⬜ | 3 | 4 | 5 | `--gpus all` flag. AI/ML workloads (Ollama, vLLM). |
| 240 | **AI-Powered Deployment Assistant** ⬜ | 3 | 5 | 5 | `tengiz ai "deploy WordPress with Redis"` → LLM-generated compose. |
| 241 | **Database Backup Import/Upload** ⬜ | 2 | 5 | 5 | `tengiz backup import` from external SQL dump files. |
| 242 | **ERB Template Support in .tengiz.yaml** ⬜ | 3 | 5 | 5 | Dynamic config via Go templates: `{{ env "DATABASE_URL" }}`. |
| 243 | **Build Provenance & SBOM Attestations** ⬜ | 2 | 5 | 5 | SLSA provenance + SBOM for supply chain security. |
| 244 | **Pre-Connect Hooks** ⬜ | 2 | 5 | 5 | Environment setup before SSH connections (VPN, bastion). |
| 245 | **Env Tag System (Server-Specific Env Vars via Tags)** ⬜ | 2 | 4 | 5 | Tag-based env var injection for canary/region-specific config. |
| 246 | **Git Clone-Based Clean Builds** ⬜ | 3 | 5 | 5 | Build from clean git clone, prevent uncommitted changes in image. |
| 247 | **Mirror-Seeded Image Pulling** ⬜ | 2 | 4 | 5 | Seed registry mirror cache first, then parallel pulls on remaining hosts. |
| 248 | **Procedure Stage-Level Enable/Disable** ⬜ | 2 | 5 | 5 | Enable/disable individual stages without removing from definition. |
| 249 | **Batch Operations Across Resource Types** ⬜ | 2 | 5 | 5 | Filter by tag/project/status, concurrent execution. |
| 250 | **Callable vs Event-Driven Serverless Function Architecture** ⬜ | 3 | 3 | 5 | Query/update/event-driven split for goja runtime. Queries sub-ms, updates write, hooks async. |
| 251 | **Function Guards (Pre-Execution Authorization)** ⬜ | 2 | 4 | 5 | Pre-execution guards for serverless functions. Auth + rate limit before function runs. |
| 252 | **Type-Safe Serverless Function Schema** ⬜ | 2 | 4 | 5 | Zod-style schema → auto-generated client bindings. |
| 253 | **Function Publish/Upgrade Independent Lifecycle** ⬜ | 2 | 5 | 5 | Hot-swap function source without app redeploy. Versioned, rollbackable. |
| 254 | **HTTPS Outcalls from Embedded Functions** ⬜ | 3 | 4 | 5 | Embedded functions make external HTTP requests. `fetch(url)` in goja context. |
| 255 | **Component-Based Platform Architecture** ⬜ | 3 | 1 | 5 | Composable service containers (satellite, orbiter, monitor). Independent lifecycle. Very high effort refactor. |
...
## Forward Auth / SSO Gate for Deployed Applications (oauth2-proxy)
- **Source:** Dokploy
- **Description:** Protect deployed applications behind an SSO login gate using oauth2-proxy + Traefik forward-auth. When enabled, all requests to the app are intercepted at the proxy level — unauthenticated users are redirected to the configured identity provider (Google, GitHub, Azure AD, Okta, Keycloak, any OIDC provider). After login, the proxy injects identity headers (`X-Forwarded-User`, `X-Forwarded-Email`, `X-Forwarded-Groups`) into requests forwarded to the app. The app itself doesn't need any auth code. Dokploy's `ApplicationAuth` model stores selected auth provider, session cookie config, and per-app toggle. Different from platform-level OIDC/SSO (#128, which protects Tengiz admin) and HTTP Basic Auth (#27, which is password-based) — this protects end-user apps behind enterprise SSO without any app code changes.
- **Why add to Tengiz:** Enterprise deployments need to restrict access to internal tools (admin panels, staging environments, internal dashboards) behind SSO without modifying app code. A `.tengiz.yaml` toggle (`auth.sso.enabled: true`, `auth.sso.provider: google`) enables SSO for any app in minutes. Implementation: oauth2-proxy as a sidecar container managed by Tengiz, or an embedded Go OIDC middleware in the proxy (lighter approach using `golang.org/x/oauth2` + `github.com/coreos/go-oidc/v3`). The proxy intercepts unauthenticated requests, performs the OIDC authorization code flow, and injects user identity headers. Complements existing OIDC/SSO (#128, admin auth) and Proxy Security Middleware (#107, IP allowlisting) for a complete security story. High value for enterprise and team deployments — every deployed app gets SSO for free.
- **Detected:** 2026-07-20

## Zip Archive Direct Deployment (Source Upload)
- **Source:** Dokploy
- **Description:** Deploy an application by uploading a .zip archive directly via CLI or UI. Dokploy accepts drag-and-drop zip uploads, extracts them, and runs the full deploy pipeline (detect framework, build, containerize, deploy). No git repository needed. Useful for: CI/CD artifacts from external build systems, quick demos from local builds, non-developer deployments where git isn't set up. Works with all build types (Dockerfile, Nixpacks, static).
- **Why add to Tengiz:** `tengiz deploy --archive ./build.zip` enables deploying pre-built artifacts from external CI/CD pipelines (GitHub Actions, Jenkins, CircleCI) without git integration. Also enables "deploy from your local build output" workflows where `npm run build` produces a `dist/` folder that gets zipped and deployed. Implementation: `archive` package that extracts zip to a temp directory, runs the standard `builder.Detect()` and `builder.RunBuild()`. For static sites, skips the Docker build and directly serves files from the archive via a minimal nginx container. Complements Explicit Image Name Deploy (#35, pre-built images) — archive deploy is for pre-built source code, not pre-built images. Low effort (Go `archive/zip` stdlib + temp dir + existing builder pipeline).
- **Detected:** 2026-07-20

## Free Development Domain Provisioning (Built-in Subdomain Service)
- **Source:** Dokploy
- **Description:** Dokploy integrates Traefik.me (free `*.traefik.me` wildcard DNS) and sslip.io (free `*.sslip.io` DNS pointing to any IP via hostname) for instant development/review domains. Any app deployed gets a publicly accessible subdomain without users configuring DNS. The domain resolves to the server's IP automatically. Works with Traefik's Let's Encrypt integration for auto-SSL on the wildcard domain. Different from Tengiz's existing `*.tengiz.local` which is local-only — this provides real public domains accessible from any device/browser.
- **Why add to Tengiz:** When setting up Tengiz on a VPS, users currently must configure custom DNS to test their deployed apps. A built-in free domain service means every app gets a public URL immediately: `myapp.203-0-113-42.sslip.io` or `myapp.tengiz-dev.example.com`. Implementation: integrate with sslip.io (no API key needed — it's DNS-based: `203-0-113-42.sslip.io` resolves to 203.0.113.42 automagically) or offer a `*.tengiz.dev` wildcard domain service. `tengiz deploy --free-domain` auto-assigns a free subdomain. Particularly valuable for preview deployments — each PR gets `pr-42-myapp.203-0-113-42.sslip.io`. Combined with Let's Encrypt auto-SSL, the free domain gets HTTPS automatically. Low effort (DNS-based, no infrastructure). High UX impact — "deploy and get a public URL instantly" is the core PaaS promise.
- **Detected:** 2026-07-20

## Metrics Endpoint Security (Token Authentication + Configurable Port)
- **Source:** Dokploy
- **Description:** Dokploy's monitoring system secures the metrics endpoint with: configurable metrics port (default 4500), authentication token (auto-generated, configurable), and per-service include/exclude selection. The metrics token is required to scrape the `/metrics` endpoint — unauthorized requests are rejected. The token is stored encrypted in the platform config. Include/exclude filters select which services' metrics are collected, reducing overhead for non-critical containers. Dokploy also supports a metrics callback URL pattern for forwarding metrics data to external aggregation endpoints.
- **Why add to Tengiz:** Prometheus Metrics (#47) covers exposing a `/metrics` endpoint, but without authentication anyone who can reach the server can scrape metrics — potentially leaking app-level traffic patterns and internal hostnames. Token auth prevents unauthorized scraping. Configurable port keeps metrics isolated from the main proxy port (security best practice for observability infrastructure). Include/exclude avoids collecting metrics for ephemeral or non-critical containers (build containers, helper containers). Implementation: `--metrics-token` flag auto-generates a token on first proxy start, stored in `~/.tengiz/config.json`. The Go prometheus HTTP handler checks `Authorization: Bearer <token>` header before serving. `.tengiz.yaml`'da `metrics.port: 9600`, `metrics.token: auto`, `metrics.include_services: [web]` ile yapılandırılır. Low-medium effort, security hardening for the observability stack.
- **Detected:** 2026-07-20

## Alternative Git Deploy Sources (Archive/Directory/Image)
- **Source:** Dokku
- **Description:** Beyond `git push`, Dokku supports three additional git-based deploy sources: `git:from-archive` (deploy from tar/tar.gz with configurable `archive-max-files=10000` and `archive-max-size=1GB` limits), `git:from-directory` (deploy from a local filesystem path), and `git:from-image` (deploy from a pre-built Docker image, tracking the source ref via `git-source-image` property). Each source runs the full deploy pipeline (build, release, deploy). The archive source is particularly useful for CI/CD artifacts that produce build tarballs.
- **Why add to Tengiz:** Tengiz currently deploys only from local directories or git clone via webhooks. Archive deploy enables: deploying pre-built CI/CD artifacts from GitHub Actions/GitLab CI without git integration, deploying from build outputs of external build systems, and one-shot deploys from release tarballs. `git:from-image` overlaps with Explicit Image Name Deploy (#13) but is integrated into Dokku's git-based deploy pipeline with source tracking. Implementation: add `--source archive --archive ./build.tar.gz` to `tengiz deploy`, extract to temp dir, run existing builder pipeline. Add `--source image` as an alias for `--image`. Configurable max size/files via `.tengiz.yaml`. Low-medium effort, expands deploy surface to non-git workflows.
- **Detected:** 2026-07-20

## Deploy Cancellation (Build PID Kill & Lock Cleanup)
- **Source:** Dokku
- **Description:** `tengiz builds:cancel <app>` cancels an in-progress build by: reading the deploy lock file to find the build PID, sending SIGTERM to the build process, waiting for graceful shutdown (with SIGKILL fallback), and cleaning up the deploy lock. The build record transitions to `canceled` status. If no build is in progress, it's a no-op. Works alongside Dokku's `builds:list` to identify running builds by status filter.
- **Why add to Tengiz:** Long-running builds (large npm install, Docker images) sometimes need manual cancellation — a misconfigured build that loops forever, a deploy triggered by accident, or a CI/CD pipeline that needs to abort. Currently the only way to stop a `tengiz deploy` is Ctrl+C (which leaves state inconsistent) or `docker kill` the build container. `tengiz build cancel` provides a clean, stateful cancellation. Implementation: store PID of deploy process in deploy lock file, `tengiz build cancel` reads PID, sends SIGTERM, waits 10s, then SIGKILL. Works with existing Deploy Lock (#15). Low effort (PID file + signal handling), important operational safety net.
- **Detected:** 2026-07-20

## Network Attachment & Security Properties (bind-all-interfaces, tld, attach-post-*)
- **Source:** Dokku
- **Description:** Dokku's `network:set` provides per-app network properties beyond basic Docker network CRUD: `bind-all-interfaces` (boolean — controls whether container ports bind to 0.0.0.0 or only 127.0.0.1, critical for security), `tld` (custom DNS TLD per app, e.g., `.internal` instead of `.tengiz.local`), `attach-post-create` (comma-separated network names to attach after container creation), `attach-post-deploy` (networks to attach after deploy completes), `initial-network` (override default network from `bridge`), and `static-web-listener` (hardcode the web listener address, bypassing dynamic IP/PORT file discovery). Each property has computed inheritance (app > global > default).
- **Why add to Tengiz:** Tengiz currently hardcodes container network binding. Key additions: `bind-all-interfaces: false` secures internal-only containers by not exposing ports to external interfaces; `tld: .prod.internal` enables DNS-based service discovery across a multi-server setup; `attach-post-create: [monitoring-net]` hooks new containers into a shared monitoring network automatically. `.tengiz.yaml`'da `network.bind_all_interfaces: false`, `network.tld: .internal`, `network.attach: [monitoring-net]` ile yapılandırılır. Complements Custom Docker Network (#30). Low-medium effort (Docker `--network` and `--publish` flag control in `runtime.Run()`), security + flexibility improvement.
- **Detected:** 2026-07-20

## Git Property Configuration (Deploy Branch, Keep .git, Rev Env Var)
- **Source:** Dokku
- **Description:** Per-app and global git property configuration: `deploy-branch` (which branch triggers deploy, default `master`, global override possible), `keep-git-dir` (whether `.git` directory stays in the deployed app's root — useful for version-displaying apps but a security concern), `rev-env-var` (custom env var name for git revision, default `GIT_REV`), `archive-max-files` (max files in archive deploy, default 10000), `archive-max-size` (max bytes in archive deploy, default 1GB). Properties cascade: app > global > built-in default.
- **Why add to Tengiz:** Today Tengiz has no configurable git behavior. `deploy-branch: main` lets CI/CD systems deploy from a specific branch while ignoring feature branch pushes. `keep-git-dir: false` removes `.git` from deployed containers (security best practice — prevents source code exposure if the web server serves hidden files). `rev-env-var: TENGIZ_COMMIT_SHA` customizes the automatically injected env var name (currently hardcoded). `.tengiz.yaml`'da `git.deploy_branch: main`, `git.keep_git_dir: false`. Low effort (properties stored in app config, applied during git clone/deploy). Complements Webhook Event Filtering (#50, branch filtering) with a complementary mechanism.
- **Detected:** 2026-07-20

## Procfile Path Configuration & Dockerfile CMD Override
- **Source:** Dokku
- **Description:** Dokku's `ps:set procfile-path` configures a custom Procfile path (supports monorepo structures where Procfile is not in the repo root — e.g., `apps/web/Procfile`). `ps:set dockerfile-start-cmd` overrides the default CMD in Dockerfile-based deploys. Both per-app properties with global defaults. The Procfile path is resolved during the deploy's extract phase — if the custom path doesn't exist, deployment fails with a clear error.
- **Why add to Tengiz:** Existing Procfile Support (#90) documents Procfile parsing but not path configurability. Monorepo users need `procfile-path: packages/api/Procfile` to point to the correct Procfile. Dockerfile users sometimes need to override CMD without modifying the Dockerfile (e.g., `dockerfile-start-cmd: "node dist/server.js"` for a Next.js standalone output). `.tengiz.yaml`'da `procfile_path: packages/api/Procfile` and `dockerfile_start_cmd: node dist/server.js`. Low effort (string property + path resolution in deploy pipeline). Complements Monorepo Support (#11).
- **Detected:** 2026-07-20

## App-Level Deploy Lock (Prevent Deployments to Specific Apps)
- **Source:** Dokku
- **Description:** `tengiz apps:lock <app>` creates a file-based lock that prevents any subsequent deployments to the app. `tengiz apps:unlock <app>` removes the lock. `tengiz apps:locked <app>` checks lock status and exits with non-zero if locked. The lock is checked at the beginning of `receive-app` trigger — if locked, the deploy is rejected immediately with a clear message. Different from the concurrent deploy lock (#15, prevents simultaneous deploys) and maintenance mode (#25, proxy-level traffic draining). This is specifically about blocking deploy operations to protect a production app during active debugging.
- **Why add to Tengiz:** When debugging a production issue on a running container, the last thing an operator wants is someone else's `tengiz deploy` restarting the container mid-investigation. `tengiz apps:lock myapp` prevents all deploys until explicitly unlocked. A lock file in `~/.tengiz/locks/<app>` is checked by `deploy.Run()` before any state-modifying operation. Complements Deploy Lock (#15) which targets concurrent deploy prevention — this targets operator-driven lockdown. Low effort (file-based lock + check in deploy pipeline), high operational safety value.
- **Detected:** 2026-07-20

---

## Persistent App Mode (Stateful vs Stateless Classification)
- **Source:** CapRover
- **Description:** CapRover classifies apps as **persistent** (stateful, single-instance, node-locked, data survives restarts) or **non-persistent** (stateless, horizontally scalable, can be freely moved/restarted). Persistent apps are excluded from Swarm's HA rescheduling (data corruption risk), pinned to a specific node, and exempt from rolling updates. Non-persistent apps get the full HA treatment. The toggle is set at app creation and stored per-app. This distinction is foundational — it determines: whether scale-to-zero applies, whether the app can run multiple replicas, whether the proxy treats it as a single backend, and whether volumes are mandatory.
- **Why add to Tengiz:** Tengiz's scale-to-zero kills idle containers — fine for stateless Node.js apps, catastrophic for PostgreSQL or Redis. Currently there's no concept of "this app MUST NOT be scaled to zero." Persistent app classification solves this: `.tengiz.yaml`'da `persistent: true` marks DBs, caches, and stateful services. Persistent apps: skip idle timeout, skip scale-to-zero, can only run 1 replica, get warning fly on rollback/deletion. Non-persistent (default): normal scale-to-zero, multi-replica, free to restart. This single toggle clarifies the entire deployment model and prevents data-loss incidents. Complements Persistent Storage Volume Management (which handles data) — this handles deployment behavior. Low effort (boolean field in AppEntry + conditional checks in idle/proxy/deploy), high safety impact.
- **Detected:** 2026-07-20

## Programmatic Pre-Deploy Hook (Container Spec Modification)
- **Source:** CapRover
- **Description:** CapRover's pre-deploy JavaScript hook runs before every container update and receives the raw Docker service update object (`dockerUpdateObject`) plus the current app config (`captainAppObj`). The hook can programmatically modify any field: environment variables, image tag, resource limits, health check config, network settings, custom nginx directives. For example, a hook could: append `--build-arg COMMIT_SHA=$(git rev-parse HEAD)` to Docker args based on config, inject a sidecar proxy container definition, or conditionally disable health checks during maintenance windows. Unlike shell hooks (which run commands), programmatic hooks have full access to the deployment object's data structure.
- **Why add to Tengiz:** Shell command hooks (#15) can run scripts but cannot modify the deployment itself. A programmatic hook (Go plugin or JS via `goja`) that receives and can mutate the `AppEntry` and deploy config before container creation enables powerful patterns: auto-inject `NODE_ENV=production` on all deploys, append `--restart=unless-stopped` for non-persistent apps, reject deploys without `DATABASE_URL` set (policy enforcement), insert custom health check paths per framework. `.tengiz.yaml`'da `hooks.pre_deploy_script: "validate-env.js"` ile referans edilir. The script receives JSON on stdin with the full deploy spec, outputs modified JSON on stdout, or exits non-zero to abort. Implementation: Go `plugin` package (native, compile-time) or `goja` (JS, runtime). Goja is preferred for dynamic loading without recompilation. Complements Extended Hook System (#26, more hook points) — this is deeper modification capability at each hook point. Medium effort (Goja integration + JSON pipe contract), enables a new class of platform extensions.
- **Detected:** 2026-07-20

---

## New Features from Kamal (2026-07-20 Analysis)

## Healthcheck Barrier for Coordinated Multi-Server Boot
- **Source:** Kamal
- **Description:** Kamal uses a `Healthcheck::Barrier` synchronization primitive during rolling boot. Primary hosts (first boot group) start their containers, pass health checks, and *then* open the barrier — unblocking the remaining hosts to proceed. This ensures that N hosts per boot group are healthy before the next batch begins. The barrier prevents: deploying a broken version to all servers simultaneously, traffic routing to unhealthy containers during rollout, and race conditions where a secondary host's health check completes before the primary host has finished booting. `boot.limit` controls batch size, `boot.wait` adds delay between groups.
- **Why add to Tengiz:** Existing rolling boot/canary (#47) covers batched deployment but lacks the health-verification gating between batches. Without a barrier, a broken deploy can cascade: server 1's container boots but fails health check → server 2's container still starts because there's no signal to wait. The barrier pattern ensures: boot group 1 is fully healthy before group 2 starts. For Tengiz (single-server today, multi-server future), this pattern is important architecture — the `Barrier` primitive (a Go channel / sync.Cond) can coordinate containers on the same host as well (cold-start health gating). `.tengiz.yaml`'da `deploy.batch_size: 2`, `deploy.batch_wait: 5` ile yapılandırılır. Medium effort (Go sync primitive + health check orchestration), critical for multi-server reliability.
- **Detected:** 2026-07-20

## Mirror-Seeded Image Pulling (Seeded Registry Pulls for Multi-Server Speed)
- **Source:** Kamal
- **Description:** When pulling images across multiple servers, Kamal first pulls on a *single host per Docker mirror* to seed the registry mirror's cache, then pulls on remaining hosts in parallel. The first pull populates the mirror cache; subsequent pulls from other hosts hit the cache and complete faster. `builder.first_mirror` identifies mirror servers. This reduces total deploy time from N×pullTime to 1×pullTime + (N-1)×cacheHitTime.
- **Why add to Tengiz:** For multi-server deployments (future SSH-based remote deployment), pulling large images (Next.js: ~1GB, ML models: ~5GB) on every server sequentially is painfully slow. Mirrored seeding cuts total pull time dramatically — only one server downloads the full image, the rest get cached copies. Implementation: in the multi-server pull step, identify mirror hosts, pull on one per mirror first, then pull on all remaining hosts in parallel. Even on single-server today, this pattern is useful for local registry architecture. Low effort (ordering of pull operations when multi-server support exists), high performance impact for multi-server.
- **Detected:** 2026-07-20

## Docker Buildx Builder Lifecycle Management (Auto-Create/Inspect/Remove)
- **Source:** Kamal
- **Description:** Kamal manages Docker buildx builders as part of the build lifecycle. `kamal build create` creates a builder using `docker buildx create` with configurable driver (`docker-container` by default for multi-arch, `docker` for simple, `cloud` for cloud builders). `kamal build details` inspects the builder and shows its status, driver, endpoints. `kamal build remove` tears down the builder. Kamal also auto-creates a builder if `docker buildx inspect` fails (handles first-run and builder-doesn't-exist errors). Builder name defaults to the app's Docker image name for isolation. Supports remote builders via SSH URL (`builder.remote`) for offloading builds. Cached builds persist across invocations via `builder.cache` (type: gha or registry).
- **Why add to Tengiz:** Existing "Gelişmiş Docker Build" (#32) covers multi-arch and build cache but not the builder lifecycle. Currently Tengiz runs `docker build` directly with no buildx management — users must manually create a buildx builder if they need multi-arch or build cache. For Tengiz: `tengiz build create` creates a `docker-container` builder for multi-arch support, `tengiz build details` shows builder status, builder auto-creation on first deploy. This especially matters for: multi-arch builds (#120), Remote Builder (#121), and Build Cache (#168). `.tengiz.yaml`'da `builder.driver: docker-container` (default) veya `builder.driver: docker` (simpler). Implementation: `docker buildx create --use` + `docker buildx inspect` + `docker buildx rm` via `os/exec`. Low-medium effort, prerequisite for advanced Docker Build features.
- **Detected:** 2026-07-20

## Git Clone-Based Clean Builds (Reproducible Builds from Committed Code)
- **Source:** Kamal
- **Description:** Kamal can clone the git repository into a temporary directory (`Dir.tmpdir/kamal-clones/`) and build from that clean checkout rather than from the local working directory. This ensures: no uncommitted changes leak into the image, no local build artifacts or node_modules are included, builds are deterministic (same commit = same image), and CI/CD environments build from the exact same source as local. The clone is cached and reused across builds (incremental pulls). Kamal's `builder.git_clone?` controls this behavior — enabled by default when a git repo is detected and no custom context is set.
- **Why add to Tengiz:** Currently `tengiz deploy .` builds from the local working directory — any uncommitted changes, stray files, build artifacts, or `.env` files with secrets get baked into the Docker image. A clean clone prevents: secret leakage (`.env` files in the image), inconsistent builds ("works on my machine" because local `node_modules` had a fixed version), and bloated images (build artifacts, `dist/` files from previous builds). `.tengiz.yaml`'da `build.clean_clone: true` enables this. Implementation: `git clone --depth 1` to `~/.tengiz/clones/<app>/` → build from there → remove after deploy (or cache with `git fetch` for incremental). Low effort (git clone + temp dir + builder context path), high security + reproducibility value.
- **Detected:** 2026-07-20

## ERB Template Support in .tengiz.yaml (Dynamic Configuration)
- **Source:** Kamal
- **Description:** Kamal's `config/deploy.yml` supports ERB (Embedded Ruby) template rendering before YAML parsing. This enables: reading environment variables (`<%= ENV['DOCKER_REGISTRY'] %>`), computed values (`<%= "app-#{ENV['RAILS_ENV']}" %>`), conditional config (`<% if ENV['STAGING'] %> ... <% end %>`), and including other files (`<%= File.read('config/shared.yml') %>`). The template is rendered first, then the result is parsed as YAML. This is a standard pattern in the Rails/deployment ecosystem (also used by Capistrano, mina, and others).
- **Why add to Tengiz:** Static YAML config is limiting for multi-environment and CI/CD workflows. Users currently duplicate config values or hardcode them. ERB (or Go templates/Terragrunt-style `include`) enables: single config file that adapts to environment, reading Docker registry URL from env var (not hardcoded), conditional sections for staging vs production, shared config includes for multi-app setups. Implementation: Go's `text/template` (safer than ERB, no arbitrary code execution) or `envsubst` for env var substitution. `.tengiz.yaml` can reference `{{ env "DATABASE_URL" }}` or `{{ file "base.yaml" }}`. This is a foundational enabler for: multi-environment (#31), GitOps (#49), and Variable Resource (#123). `.tengiz.yaml`'da `$template: true` (or auto-detect `{{ }}` syntax) enables template processing. Low effort (Go `text/template` with custom function map), high config flexibility value.
- **Detected:** 2026-07-20

## Build Provenance & SBOM Attestations (Supply Chain Security)
- **Source:** Kamal
- **Description:** Kamal's `builder` config supports generating build attestations for supply chain security: `builder.provenance` generates provenance attestations (SLSA provenance — who built what, when, how) and `builder.sbom` generates Software Bill of Materials (SBOM — list of all packages/dependencies in the image). Both are Docker BuildKit features passed via `--provenance` and `--sbom` flags to `docker buildx build`. Attestations are embedded in the image manifest and visible via `docker buildx imagetools inspect`. This is increasingly required for: SOC2 compliance, vulnerability scanning (SBOM feeds into security scanners), software supply chain audits (provenance proves the build pipeline wasn't tampered with), and enterprise procurement requirements.
- **Why add to Tengiz:** As Tengiz targets production and enterprise use, supply chain security is becoming table stakes. SBOM export enables: `tengiz sbom generate <app>` to list all packages in the deployed image, integration with vulnerability scanners (Grype, Trivy — "app has 3 critical CVEs in python packages"), and compliance reporting. Provenance attestation proves the image was built by Tengiz's pipeline (not tampered with). `.tengiz.yaml`'da `builder.provenance: true`, `builder.sbom: true` ile etkinleştirilir. Implementation: pass `--provenance=true` and `--sbom=true` to buildx build command. `tengiz sbom show <app>` downloads and displays the attestations. Low effort (BuildKit flags), differentiated value for enterprise security.
- **Detected:** 2026-07-20

## Pre-Connect Hooks (Environment Setup Before SSH Connections)
- **Source:** Kamal
- **Description:** Kamal runs hooks before SSH connections to servers are established. The `pre-connect` hook runs locally before any remote command execution. Use cases: setting up VPN tunnels (`openvpn --config work.ovpn`), establishing SSH bastion connections, sourcing environment variables (`source .env.production`), logging into cloud provider CLI for dynamic SSH key retrieval (`aws ssm start-session --target i-123`), or running pre-flight checks before deploy (`curl -s http://status.internal`). The hook receives no arguments and runs in the project directory. If the hook exits non-zero, the deploy is aborted.
- **Why add to Tengiz:** For multi-server deployments in corporate environments (SSH tunnels, VPN, bastion hosts), pre-connect hooks ensure network connectivity before any command runs. A single failed SSH attempt wastes time; a hook that fails fast provides better UX. For single-server Tengiz today, pre-connect hooks could: verify Docker daemon is running, check disk space before build, or establish VPN if deploying to a remote Docker socket. `.tengiz.yaml`'da `hooks.pre_connect: "./scripts/connect-vpn.sh"`. Implementation: extend existing hook runner to execute before any remote/stateful operation. Low effort (hook execution before SSH/Docker commands starts), important network reliability pattern.
- **Detected:** 2026-07-20

## Env Tag System (Server-Specific Environment Variables via Tags)
- **Source:** Kamal
- **Description:** Kamal allows tagging servers with arbitrary tags and setting environment variables per tag. In config: `servers: { web: { hosts: [{ "app1.internal": "primary" }, "app2.internal"], ... } }`. Then `env: { tags: { primary: { DATABASE_URL: "postgres://..." } } }`. Servers tagged `primary` get `DATABASE_URL` injected; untagged servers don't. Tags enable: canary servers (tagged `canary` get `CANARY: true`), region-specific config (tagged `us-east` get `REGION: us-east-1`), tier-specific config (tagged `high-mem` get `WORKER_CONCURRENCY: 25`). Tags cascade: server → role → global.
- **Why add to Tengiz:** Today env vars are global per app. With multi-server support, different servers need different config: primary database vs read-replica, high-memory vs standard workers, different regions needing different API endpoints. Server tags solve this elegantly: tag `canary` servers → inject `CANARY_DEPLOY: true` → only those servers run the new version. Implementation: `--tag` flag on server registration (`tengiz server add --tag primary`), `env.tag_map` in `.tengiz.yaml` mapping tags to env vars. Medium effort (tag storage + env resolution), enables sophisticated multi-server deployment patterns.
- **Detected:** 2026-07-20

---

## New Features from Komodo (2026-07-20 Analysis)

## Onboarding Keys for Zero-Config Server Registration
- **Source:** Komodo
- **Description:** Pre-shared onboarding keys allow new server agents (Periphery) to auto-register with the core control plane without manual configuration. An operator generates an onboarding key on the core, adds it to the Periphery config, and the Periphery automatically connects, registers itself, and begins accepting commands. Keys can be rotated for security. This eliminates the manual "add server → configure SSH → verify connectivity" workflow for every new machine.
- **Why add to Tengiz:** For future multi-server support, onboarding every new server manually (SSH keys, hostname, address, labels) is a pain point. Onboarding keys make it a one-step process: `tengiz server generate-onboarding-key` → paste into new server's config → server appears in `tengiz ps --servers`. Pattern is lightweight (`~/.tengiz/onboarding-keys.json` with HMAC verification) and works with Tengiz's `os/exec` + SSH approach. Complements SSH Key Management (#99) and Multi-Server Architecture (#194). Low-medium effort, high operational UX value for multi-server deployments.
- **Detected:** 2026-07-20

## Service Users (Built-in System Automation Accounts)
- **Source:** Komodo
- **Description:** Komodo creates and manages internal system users for automated operations: `System`, `Procedure`, `Action`, `Git Webhook`, `Auto Redeploy`, `Resource Sync`, `Stack Wizard`, `Build Manager`, `Repo Manager`. Each has pre-configured permissions scoped to its specific operation. Actions, procedures, webhooks, and automated deploys run under these service identities rather than a human user. This enables: audit trail distinguishing human action vs automated system action, permission scoping for automation (Git webhook can trigger deploys but not delete apps), and credential-free execution (no API key needed for internal operations).
- **Why add to Tengiz:** Currently Tengiz has no concept of "who triggered this deploy." Adding service users means: `tengiz ps --deployments` shows "deployed by git-webhook" vs "deployed by admin@example.com" vs "deployed by scheduled-cron". Service identities enable: audit trail with actor attribution, permission scoping for webhooks (webhook users can only deploy their app), and clean separation between human and automated operations. Implementation: built-in service accounts created on first run (stored in internal store, not in user-accessible state), auto-assigned to webhook/procedure/cron triggers. Low effort (internal user model + attribution), high audit value. Complements Event Logging (#7) and Granular Scoped API Keys (#76).
- **Detected:** 2026-07-20

## Procedure Stage-Level Enable/Disable (Granular Step Control)
- **Source:** Komodo
- **Description:** Multi-stage procedures allow individual stages and executions within a stage to be enabled or disabled independently. A procedure step can be temporarily disabled without removing it from the procedure definition. When a procedure runs, disabled stages are skipped with a clear log entry: "stage X: disabled, skipped." This allows operators to: disable a migration step during a rollback, skip a notification step for testing, or disable a slow health check stage during a fast iteration cycle. Stage state (enabled/disabled) is persisted and survives procedure re-runs.
- **Why add to Tengiz:** Existing procedure automation (#51) defines sequential steps but has no runtime control. Operators shouldn't need to edit `.tengiz.yaml` or remove steps to skip them temporarily. Stage-level enable/disable gives surgical control: "keep the deploy and health check steps, skip the migration and notification steps." `.tengiz.yaml`'da `procedures.<name>.stages.<name>.enabled: true/false` ile yapılandırılır. `tengiz procedure disable-step <name> <stage>` provides runtime control. Low effort (boolean field + conditional skip in runner), important operational flexibility for complex pipelines.
- **Detected:** 2026-07-20

## System Maintenance Operations (Global Platform Commands)
- **Source:** Komodo
- **Description:** First-class platform-level maintenance operations beyond app lifecycle: `ClearRepoCache` (delete cached git repos to reclaim disk space), `BackupCoreDatabase` (export all platform state to a portable file), `GlobalAutoUpdate` (check and update all deployments + stacks to their tracked image versions), `RotateAllServerKeys` (rotate cryptographic keys on all connected servers), `RotateCoreKeys` (rotate core identity keys). These are exposed as runnable operations with status tracking, log output, and result reporting. Each operation has appropriate confirmation and progress reporting.
- **Why add to Tengiz:** Today Tengiz has no concept of platform-level maintenance. `tengiz maintenance backup` exports `~/.tengiz/` state. `tengiz maintenance rotate-keys` rotates internal encryption keys. `tengiz maintenance clear-cache` frees disk space from stale git clones. These are the "janitorial" operations that every production platform needs but no one builds until they're urgently needed. `.tengiz.yaml`'da `maintenance.auto_backup.schedule: "0 4 * * *"` ile otomatik yedekleme planlanabilir. Low-medium effort (wraps existing operations + confirmation prompts), high operational hygiene value. Complements Full System Backup (#60), Build Cache Management (#41), and Image Digest Change Detection (#74).
- **Detected:** 2026-07-20

## Version Mismatch Detection (Component Compatibility Check)
- **Source:** Komodo
- **Description:** When a Periphery agent connects to Core, both sides report their software versions. If versions are incompatible (API contract changed), the connection is rejected with a clear error: "Periphery v1.2.0 is incompatible with Core v2.0.0 — please upgrade Periphery to >=2.0.0." This prevents silent failures from out-of-date agents executing operations with wrong assumptions. The version check is done at connection time and logged at both ends. Configurable compatibility policy: `strict` (reject mismatch), `warn` (allow but log warning), `off` (no check).
- **Why add to Tengiz:** As Tengiz evolves, the `tengiz` binary on different machines may be different versions. A `tengiz version` command that checks compatibility between the CLI, proxy, and Docker runtime prevents: deploy failures from breaking API changes, confusing error messages from incompatible state formats, and silent data corruption from version drift. Implementation: `internal/version` package with compiled-in version string, `/version` endpoint on proxy, version check middleware on state-modifying operations. `tengiz version --check` compares all components. `.tengiz.yaml`'da `version.policy: warn` (default) veya `strict`. Low effort (version string + comparison middleware), prevents a class of production failures that are very hard to diagnose.
- **Detected:** 2026-07-20

## Maintenance Windows for Alert Suppression (Scheduled Outage Planning)
- **Source:** Komodo
- **Description:** Configurable maintenance windows that suppress alerts during planned maintenance periods. Each window defines: recurrence (daily, weekly, one-time), start time + duration, timezone, optional resource scope (suppress alerts for specific apps/servers), and a description/reason. During a maintenance window, related alerts are silently suppressed (not sent to alerters) but still recorded in the audit log. After the window expires, alerting resumes automatically. This prevents alert fatigue during planned downtime (deploy windows, database migrations, server reboots) where the operator already knows alerts will fire.
- **Why add to Tengiz:** Without maintenance windows, any planned downtime (OS update, database migration, `tengiz server reboot`) triggers the full alert system — Slack messages, emails, push notifications — all for a known event. Operators start tuning out alerts, defeating the purpose. Maintenance windows let operators declare: "I'm upgrading Postgres from 2-3 AM, suppress all alerts for myapp and myapp-db." `.tengiz.yaml`'da `maintenance_windows:` listesi. `tengiz maintenance start "DB upgrade" --duration 1h` creates an ad-hoc window. Low-medium effort (time window matching in alert dispatch), high operational quality-of-life value. Complements Alert System (#147) and Notification System (#5).
- **Detected:** 2026-07-20

## Stack File Dependency Tracking (Smart Compose Redeploy)
- **Source:** Komodo
- **Description:** For multi-file Docker Compose stacks, Komodo tracks per-file dependencies with `StackFileRequires` to determine granular diff-based actions. Instead of redeploying the entire stack when a single file changes, Komodo compares file content hashes and only triggers `deploy`/`restart`/`none` per affected service. `stack_file_requires` explicitly maps which compose files affect which services. When only the monitoring stack's compose file changes, only monitoring services restart — the web app and database remain untouched. File dependencies can include environment files (`env_file` tracking), config files (`StackFileDependency`), and compose file fragments.
- **Why add to Tengiz:** Today `tengiz stack deploy` (when implemented) would redeploy everything on any change. In a typical stack (web + db + redis + worker + monitoring), a change to `monitoring.yml` shouldn't restart the database. Dependency tracking enables surgical restarts: only containers whose config actually changed get restarted. The diff output (`tengiz stack diff <name>`) shows exactly which services will be affected before applying. `.tengiz.yaml`'da `stack.dependencies: { web: [web.yml], db: [db.yml] }` ile tanımlanır. Medium effort (file hashing + dependency graph), significant time savings for large stacks. Complements Stack/Compose Lifecycle Management (#78) with intelligent diff-based deployment.
- **Detected:** 2026-07-20

---

## Callable vs Event-Driven Serverless Function Architecture (defineQuery/defineUpdate)
- **Source:** Juno
- **Description:** Juno's Sputnik canister separates serverless functions into three distinct types: **callable queries** (`defineQuery`, read-only, replicated across nodes, sub-millisecond execution, no state mutation), **callable updates** (`defineUpdate`, read-write, consensus-based, can mutate datastore/storage state), and **event-driven hooks** (`on_set_doc`, `on_delete_doc`, `on_upload_asset`, `on_delete_asset`, triggered automatically by data changes). Queries can be freely replicated for low-latency reads; updates go through consensus. Hooks fire asynchronously in response to data events. This architectural distinction is fundamental to Juno's performance model: queries are cheap and fast, updates are expensive and slow.
- **Why add to Tengiz:** The Embedded Serverless Functions Runtime (#50) describes running JS functions via `goja` but doesn't specify the execution model. Adding Juno's query/update/event-driven split gives Tengiz a structured serverless runtime: queries execute synchronously in the proxy (no container overhead, sub-ms latency), updates write to the embedded datastore (with concurrency control), event-driven hooks fire in goroutines in response to data changes. This enables a pricing/performance model: queries are free, updates cost, hooks are async. `.tengiz.yaml`'da `functions.query.timeout: 50ms`, `functions.update.timeout: 5s`, `functions.hooks.async: true` ile yapılandırılır. Implementation: extend `internal/fn/` with three execution paths — query (read-only, embedded goja, results cached), update (read-write, full goja context with commit), hook (async goroutine pool). Medium-high effort but transforms Tengiz from a container orchestrator to a full serverless platform. Complements the existing embedded functions implementation by adding architectural clarity.
- **Detected:** 2026-07-20

## Function Guards (Pre-Execution Authorization for Serverless Functions)
- **Source:** Juno
- **Description:** Juno's Sputnik supports guards — pre-execution functions that run before every callable function invocation. Guards receive the caller's identity (principal ID, auth provider) and the function context, and can reject the call with a custom error. Multiple guards can be chained (AND logic — all must pass). Guards are deployed as first-class functions alongside the functions they protect. Common patterns: `allowAdmin` (reject if caller is not the satellite controller), `allowOwner(docId)` (reject if caller didn't create the document), `rateLimit(10/min)` (reject if rate exceeded). Guards are authored in TypeScript/Rust just like regular functions, deployed via `juno functions publish`, and live-updated without downtime.
- **Why add to Tengiz:** The existing Embedded Serverless Functions (#50) need authorization — currently any HTTP request to the function endpoint executes the function. Guards provide a clean, function-level authorization model that separates auth logic from business logic. For Tengiz: define a guard function once, apply it to any function. Guards integrate with the Built-in Authentication Service (#0) to check caller identity. `.tengiz.yaml`'da `functions.<name>.guards: ["requireAuth", "rateLimit"]` ile yapılandırılır. Implementation: guards are just functions (same `goja` runtime) that receive `{caller, args, context}` and return `{allow: true}` or `{allow: false, error: "..."}`. The function runner checks guards before executing the target function. Low-medium effort on top of the existing function runtime. Complements the auth service and enables fine-grained access control at the function level.
- **Detected:** 2026-07-20

## Type-Safe Serverless Function Schema with Auto-Generated Client Bindings
- **Source:** Juno
- **Description:** Juno's `j` schema system (built on Zod) lets developers define a shared schema for their serverless functions — input parameters, return types, validation rules — in TypeScript. The schema is defined once in the function code and auto-generates: runtime validation (inputs validated before function execution), TypeScript type definitions (for the frontend SDK), and client API bindings (auto-complete, type checking in downstream code). Changes to the schema are propagated instantly — the client SDK is regenerated on `juno functions publish`. The schema supports: string/number/boolean primitives, optional fields, default values, custom validators, nested objects, arrays, and references to datastore document types.
- **Why add to Tengiz:** For Tengiz's goja-based functions, a schema system provides: type-safe function definitions (input validation, error messages don't leak implementation details), auto-generated documentation (function signatures visible via CLI), client SDK generation (TypeScript types for `@tengiz/core`), and runtime guards against malformed input. Implementation: a Go-based schema definition format (YAML/JSON in `.tengiz.yaml` or inline in JS via JSDoc annotations) that the function runtime parses at deploy time. `.tengiz.yaml`'da `functions.<name>.schema.input: { user_id: { type: string, required: true }, page: { type: number, default: 1 } }` `functions.<name>.schema.output: { id: string, name: string }`. The schema generates: validation middleware (pre-function), TypeScript typedef output file, and OpenAPI endpoint specifications. Low-medium effort (JSON schema validation + code generation), significant DX improvement.
- **Detected:** 2026-07-20

## Function Publish/Upgrade Independent Lifecycle (Separate from App Deploy)
- **Source:** Juno
- **Description:** Juno separates the serverless function lifecycle from the app/satellite lifecycle. Functions are built, published, and upgraded independently via `juno functions build`, `juno functions publish`, `juno functions upgrade`. A function can be updated without redeploying the entire satellite — the function WASM module is hot-swapped. This enables: rapid function iteration (seconds vs minutes), A/B function versions (traffic splitting between old/new), canary function deploys (rollback on error), and zero-downtime function updates (existing connections drain before swap). Each function deployment is versioned and individually rollbackable.
- **Why add to Tengiz:** Currently Tengiz has no concept of "update a function without redeploying the app." For the embedded function runtime, hot-swapping a function's JS source is orders of magnitude simpler than redeploying a container. `tengiz fn publish <name> <file>` replaces the function source in the store, triggers a goroutine to reload the goja runtime, and new invocations use the new code. Version history: `tengiz fn history <name>` shows published versions, `tengiz fn rollback <name> --version 2` reverts. Implementation: functions stored as JSON records in `~/.tengiz/functions/<name>/`, each version saved with timestamp+sha256, active version pointer in AppEntry. `goja` runtime recreated on version switch. Low effort (file storage + version pointer + runtime reload), enables rapid iteration without container rebuild.
- **Detected:** 2026-07-20

## HTTPS Outcalls from Embedded Serverless Functions
- **Source:** Juno
- **Description:** Juno's Sputnik canister supports HTTPS outcalls — serverless functions can make external HTTP requests to any internet endpoint. Each outcall has: configurable max response bytes (default 2MB, configurable per function), request transformation (header injection, body transformation), timeout (default 60s), and cycle cost calculation (bandwidth + compute). Outcalls are particularly valuable for: webhook forwarding (receive event → transform → forward to external service), API aggregation (call 3 APIs, merge results), OAuth token exchange (auth code → token), webhook signing (verify incoming webhook → compute HMAC → forward).
- **Why add to Tengiz:** Embedded functions need external HTTP access to be useful beyond simple data transformations. Use cases: fetch external API data and cache in datastore, forward webhook payloads to Slack/Discord, exchange OAuth codes for tokens, fetch deployment status from CI/CD systems, call AI/LLM APIs. Implementation: Go's `net/http` client exposed as `fetch(url, options)` in the embedded goja runtime context. Configurable per-function: `max_response_bytes`, `timeout`, `allowed_domains` (security — prevent SSRF). `.tengiz.yaml`'da `functions.<name>.outcalls.max_response_kb: 512`, `functions.<name>.outcalls.timeout: 30s`, `functions.<name>.outcalls.allowed_domains: ["api.github.com", "hooks.slack.com"]` ile yapılandırılır. Low-medium effort (HTTP client injection into goja context), high value for real-world function use cases.
- **Detected:** 2026-07-20

## Component-Based Platform Architecture (Composable Services Pattern)
- **Source:** Juno
- **Description:** Juno's platform is composed of multiple independent WASM containers ("canisters") that each serve a distinct purpose: **Satellite** (app hosting — serves frontend assets, runs serverless functions, manages datastore/storage), **Mission Control** (monitoring — tracks cycles/funding, auto-tops up, manages upgrades), and **Orbiter** (analytics — collects page views, events, web vitals). Each component is independently deployable, upgradeable, and scalable. They communicate via inter-canister calls (authenticated RPC). Users can opt into only the services they need, each component upgrades separately, and resource limits are per-component. This composable architecture provides natural separation of concerns with independent lifecycle management.
- **Why add to Tengiz:** Tengiz currently has a monolithic architecture — proxy, store, idle timer, health checker all live in the same Go binary. Juno's model suggests evolving toward composable service containers: a `tengiz-satellite` container (proxy + store + auth + functions), `tengiz-orbiter` container (analytics + metrics), `tengiz-monitor` container (health checks + alerts + background scheduling). Each is optional: `.tengiz.yaml`'da `components: [satellite, orbiter, monitor]`. Benefits: independent scaling (analytics doesn't affect proxy), independent upgrades (update monitor without touching proxy), per-component resource limits, and phased adoption. Also enables multi-server deployment (proxy on edge, monitor on management node). High effort (significant refactoring), but provides the foundational architecture for future platform expansion. The existing internal packages (proxy, runtime, config, health, idle) already reflect clean separation — packaging them as Docker containers with inter-container RPC is the next architectural step. This is a P1 architectural evolution that unblocks all composable service features (analytics, monitoring, auth as independent services).
- **Detected:** 2026-07-20
