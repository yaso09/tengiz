...
- **Framework auto-detection** — Next.js, Vite, Go, Node.js, Python, static sites. No config needed. Optional **Nixpacks** backend for 300+ frameworks (Rust, Ruby, PHP, Elixir, Deno, Bun, etc.).
...
Set `build.builder: nixpacks` in `.tengiz.yaml` to use Nixpacks as the build backend instead of the built-in Dockerfile generator. This supports hundreds of additional frameworks including Rust, Ruby, PHP, Java, Elixir, Deno, and Bun.

Requires the `nixpacks` CLI: `npm install -g nixpacks` or `brew install nixpacks`.
