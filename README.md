...
- **Framework auto-detection** — Next.js, Vite, Go, Node.js, Python, static sites (plus hundreds more via Nixpacks). No config needed.
...
| **Nixpacks** (hundreds of frameworks) | `.tengiz.yaml` `build.builder: nixpacks` | auto-detected |
...
For frameworks not natively supported (Rust, Ruby, PHP, Elixir, Deno, Bun, etc.), Tengiz can use [Nixpacks](https://nixpacks.com/) as the build backend:

```yaml
name: my-app
build:
  builder: nixpacks           # use Nixpacks instead of built-in Dockerfile generation
  nixpacks:
    packages: [ffmpeg]         # additional system packages (npm packages)
    apt_packages: [libpq-dev]  # additional apt packages
    cmd: "npm run start"       # override start command
```

When `build.builder: nixpacks` is set, Tengiz invokes `nixpacks build` instead of generating a Dockerfile. Requires `nixpacks` CLI to be installed (`npm install -g nixpacks` or `brew install nixpacks`).
