...
- **Framework auto-detection** — Next.js, Vite, Go, Node.js, Python, static sites. No config needed. Supports Nixpacks for 200+ frameworks (Rust, Ruby, PHP, Elixir, Deno, Bun, etc.).
...
- Nixpacks (optional, for `build.builder: nixpacks`): `npm install -g nixpacks` or `brew install nixpacks`
...
# build:
#   builder: nixpacks     # Use Nixpacks build backend (for Rust, Ruby, PHP, etc.)
#   nixpacks:
#     packages:            # Extra OS packages via apt
#       - ffmpeg
#     apt_packages:        # Alternative apt packages syntax
#       - curl
#     cmd: pnpm start      # Override start command
```
...
| **Nixpacks** | `build.builder: nixpacks` in config | Configurable |

When no Dockerfile exists, Tengiz auto-generates one with a multi-stage build optimized for each framework. For unsupported frameworks, enable the Nixpacks build backend with `build.builder: nixpacks` in your `.tengiz.yaml`.
