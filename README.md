...
- **Framework auto-detection** — Next.js, Vite, Go, Node.js, Python, static sites. No config needed. Nixpacks backend for 100+ frameworks (Ruby, Rust, PHP, Elixir, etc.).
...
# build:
#   builder: nixpacks    # use Nixpacks instead of auto-detection
#   nixpacks:
...
Set `build.builder: nixpacks` to use the Nixpacks build system for 100+ frameworks (Ruby, Rust, PHP, Elixir, Deno, Bun, etc.). The `nixpacks` CLI must be installed separately (`npm install -g nixpacks`). Nixpacks-specific options can be set under `build.nixpacks`: `packages` (OS packages), `apt_packages` (apt packages), and `cmd` (start command).
...
| **Nixpacks** (100+ frameworks) | `build.builder: nixpacks` in `.tengiz.yaml` | auto-detected |
