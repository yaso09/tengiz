...
- **Nixpacks build backend** — Optional `build.builder: nixpacks` in `.tengiz.yaml` unlocks hundreds of frameworks (Rust, Ruby, PHP, Elixir, Deno, Bun, etc.) via the Nixpacks CLI.
...
| **Nixpacks** (opt-in) | `build.builder: nixpacks` | auto-detected |

When no Dockerfile exists, Tengiz auto-generates one with a multi-stage build optimized for each framework. When `build.builder: nixpacks` is set, Tengiz delegates the build to the Nixpacks CLI, which supports hundreds of frameworks. Install Nixpacks with `npm install -g nixpacks`.
