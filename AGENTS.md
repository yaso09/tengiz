...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup; `Prune(PruneOptions)` for Docker housekeeping. `ContainerName(name, env)` helper. |
...
tengiz cleanup [--all/--containers/--images/--volumes/--networks/--build-cache] [--dry-run]  → free disk space by pruning unused Docker resources
