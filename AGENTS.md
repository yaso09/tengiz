...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static. Nixpacks backend via `build.builder: nixpacks` config. Env-aware image tags (`{env}-{deploymentID}`). |
...
- `.tengiz.yaml` `build.builder: nixpacks` enables Nixpacks build backend; `build.nixpacks:` block supports `packages`, `apt_packages`, `cmd`, `pkg_manager`, `app_directory`
