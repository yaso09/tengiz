...
| `cleanup` | Docker housekeeping via `PruneOptions`/`Prune()`/`DryRun()`. Label-based safety (`label!=tengiz-app`). Per-category pruning (containers/images/volumes/networks/build-cache). `DiskUsage()` + `PruneReport.Format()` for before/after comparison. Calls `docker <object> prune` via `os/exec`. |
...
tengiz cleanup [--dry-run] [--all] [--containers] [--images] [--volumes] [--networks] [--build-cache] [--keep-dangling] [-y] → remove unused Docker resources with label-based Tengiz protection
