...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (via `build.builder: nixpacks` in config). Env-aware image tags (`{env}-{deploymentID}`). |
...
- Nixpacks backend: configure via `build.builder: nixpacks` with optional `build.nixpacks` block (`packages`, `apt_packages`, `cmd`, `pkg_manager`, `app_directory`). Requires `nixpacks` CLI in PATH.
