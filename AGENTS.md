...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). `BuildStrategy` interface with `DockerfileStrategy` (default) and `NixpacksStrategy`. Supports: Docker, Next.js, Vite, Go, Node, Python, static, and 100s more via nixpacks. `StrategyFromName(name, dataDir)` selects strategy. `DetectWithBuilder(dir, builderType)` for nixpacks-aware detection. Env-aware image tags (`{env}-{deploymentID}`). |
...
tengiz deploy [dir] [--builder docker|nixpacks] → detect, build, run container (--builder overrides config build.builder)
