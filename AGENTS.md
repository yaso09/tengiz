...
| `builder` | Framework detection (`detect.go`, `nixpacks.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks (hundreds of frameworks). Env-aware image tags (`{env}-{deploymentID}`). |
...
tengiz deploy [dir] [--builder nixpacks] → detect, build, run container (--builder for nixpacks)
