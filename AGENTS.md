...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (hundreds of frameworks via `nixpacks` CLI). Env-aware image tags (`{env}-{deploymentID}`). `build.builder: nixpacks` in `.tengiz.yaml` selects Nixpacks. `build.nixpacks` block supports packages, apt_packages, cmd, etc. |
