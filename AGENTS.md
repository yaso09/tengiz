...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Configurable via `.tengiz.yaml` `build.builder: "nixpacks"` with optional `build.nixpacks` block for packages/apt_packages/cmd. |
