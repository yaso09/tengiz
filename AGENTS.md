...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Nixpacks backend selected via `build.builder: nixpacks` in `.tengiz.yaml`. Env-aware image tags (`{env}-{deploymentID}`). |
