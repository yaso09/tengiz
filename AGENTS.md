...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. `SetNixpacksConfig` enables Nixpacks build backend for 100+ frameworks. Env-aware image tags (`{env}-{deploymentID}`). |
