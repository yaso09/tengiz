...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks (via `build.builder: nixpacks` in `.tengiz.yaml`). Env-aware image tags (`{env}-{deploymentID}`). `buildWithNixpacks()` handles nixpacks CLI invocation; `SetNixpacksConfig()` passes config. |
