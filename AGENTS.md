...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). `Build.Builder` + `Build.NixpacksConfig` in `.tengiz.yaml` for Nixpacks backend selection. `SetNixpacksConfig()` setter. |
