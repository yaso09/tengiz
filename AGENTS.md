...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup, `Cleanup` for Docker housekeeping. `ContainerName(name, env)` helper. |
...
tengiz cleanup [--all] [--containers] [--images] [--volumes] [--networks] [--build-cache] [--keep N] [-f] → prune unused Docker resources
