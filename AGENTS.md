...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Set `build.builder: nixpacks` in `.tengiz.yaml` to use Nixpacks. Configure with `build.nixpacks` (packages, apt_packages, cmd, etc.). |
