...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static. Also supports Nixpacks build backend (`FrameworkNixpacks` + `buildWithNixpacks()`) via config `build.builder: nixpacks`. Env-aware image tags (`{env}-{deploymentID}`). |
