...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (via `build.builder: nixpacks` in config). `buildWithNixpacks()` dispatches to `nixpacks` CLI. `SetNixpacksConfig()` configures nixpacks packages. Env-aware image tags (`{env}-{deploymentID}`). |
