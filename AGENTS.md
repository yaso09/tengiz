...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (hundreds of frameworks via `nixpacks` CLI). Env-aware image tags (`{env}-{deploymentID}`). `.tengiz.yaml` `build.builder: nixpacks` selects the Nixpacks backend. |
