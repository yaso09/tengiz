...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static. Also supports Nixpacks via `buildWithNixpacks()` when `build.builder: nixpacks` is set in config. Env-aware image tags (`{env}-{deploymentID}`). |
