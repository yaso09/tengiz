...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup, `Prune` for docker housekeeping (`tengiz cleanup`). `ContainerName(name, env)` helper. |
...
tengiz cleanup [--containers] [--images] [--all-images] [--volumes] [--networks] [--build-cache] [--dry-run] → prune unused Docker resources (Tengiz-managed containers/images protected)
