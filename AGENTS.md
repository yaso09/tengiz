...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (external CLI). Env-aware image tags (`{env}-{deploymentID}`). Nixpacks config via `.tengiz.yaml` `build.builder: nixpacks` + `build.nixpacks.*`. |
