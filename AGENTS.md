...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). `buildWithNixpacks()` method dispatches to `nixpacks` CLI. Selected via `.tengiz.yaml` `build.builder: nixpacks`. |
