...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (external CLI). Env-aware image tags (`{env}-{deploymentID}`). `Builder.SetNixpacksConfig()` configures the Nixpacks backend. `Build()` dispatches to `buildWithNixpacks()` when `detection.Framework == FrameworkNixpacks`. |
