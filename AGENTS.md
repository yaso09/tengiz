...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages`, `Cleanup` (prunes unused Docker resources, always protecting `tengiz-app` containers) for rollback + image cleanup + housekeeping. `ContainerName(name, env)` helper. |
...
tengiz cleanup [-a] [-v] [-d] → prune unused Docker resources (always protects tengiz-app containers); -d shows disk usage without pruning
```
