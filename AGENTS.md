...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup. `Cleanup(ctx, CleanupOptions)` + `SystemDF(ctx)` for `tengiz cleanup` (label-protected pruning of containers/images/networks/volumes/build cache). `ContainerName(name, env)` helper. |
...
tengiz cleanup [--dry-run] [--containers] [--images] [--networks] [--volumes] [--build-cache] [--status] → remove unused Docker resources / show disk usage
