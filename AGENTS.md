...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup, `Cleanup` for pruning unused Docker resources. `ContainerName(name, env)` helper. |
...
tengiz cleanup [--containers] [--images] [--volumes] [--build-cache] [--all] [--yes] → prune unused Docker resources (dry-run by default; --yes to execute)
