...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Nixpacks is an alternative build backend (`.tengiz.yaml` `build.builder: nixpacks`). |
