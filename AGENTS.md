...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, and **Nixpacks** (external CLI for 100+ frameworks). Env-aware image tags (`{env}-{deploymentID}`). Nixpacks config in `.tengiz.yaml` via `build.builder: nixpacks` + `build.nixpacks.*`. |
