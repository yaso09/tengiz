...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static. Nixpacks build backend via `buildWithNixpacks()` when `build.builder: nixpacks` is set. Env-aware image tags (`{env}-{deploymentID}`). |
