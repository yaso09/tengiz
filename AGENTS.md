...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup. Also: `Prune` for `tengiz cleanup` (label-based docker prune, protects `tengiz-app` containers unless `--all`). `ContainerName(name, env)` helper. |
