...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup, `Clean` for Docker housekeeping. `ContainerName(name, env)` helper. |
...
tengiz cleanup [--containers|--images|--volumes|--networks|--cache|--all|--dry-run] → remove unused Docker resources
