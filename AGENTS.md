...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup. Prune methods: `PruneContainers`, `PruneImages`, `PruneVolumes`, `PruneNetworks`, `PruneBuildCache`. `ContainerName(name, env)` helper. |
...
tengiz cleanup [--all] [--dry-run] [--containers --images --volumes --networks --build-cache] → prune unused Docker resources
