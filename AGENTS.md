...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks (external CLI). Env-aware image tags (`{env}-{deploymentID}`). `build.builder: nixpacks` in `.tengiz.yaml` selects nixpacks backend. `build.nixpacks` config block for packages, apt packages, cmd. |
