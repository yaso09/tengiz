...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks. Env-aware image tags (`{env}-{deploymentID}`). `SetNixpacksConfig()` method to configure nixpacks CLI behavior. `buildWithNixpacks()` dispatches to `nixpacks build` subprocess. |
