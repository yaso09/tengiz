...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (via `nixpacks` CLI). Env-aware image tags (`{env}-{deploymentID}`). `SetNixpacksConfig()` to configure Nixpacks parameters. |
