...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (via `build.builder: nixpacks` config). Env-aware image tags (`{env}-{deploymentID}`). `buildWithNixpacks()` dispatches to `nixpacks build` CLI; `SetNixpacksConfig()` sets packages/apt/cmd options. |
