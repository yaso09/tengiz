...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static. Also supports Nixpacks via `build.builder: nixpacks` in config — dispatches to `buildWithNixpacks()` which runs `nixpacks build` as subprocess (covers Rust, Ruby, PHP, Deno, Bun, etc.). Env-aware image tags (`{env}-{deploymentID}`). |
