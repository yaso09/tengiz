...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages`, `Prune`, `PruneImages` for rollback + image cleanup + housekeeping. `ContainerName(name, env)` helper. |
