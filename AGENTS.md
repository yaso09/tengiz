...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (via `build.builder: nixpacks` in `.tengiz.yaml`). Env-aware image tags (`{env}-{deploymentID}`). `SetNixpacksConfig()` configures Nixpacks options (packages, apt_pkgs, cmd). |
