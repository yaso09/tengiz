...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). `BuildConfig.Builder` field selects backend (`docker`/`nixpacks`). `SetNixpacksConfig` passes `nixpacks:` config block to builder. |
