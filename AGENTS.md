...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static. Nixpacks backend via `.tengiz.yaml` `build.builder: nixpacks`. Env-aware image tags (`{env}-{deploymentID}`). |
