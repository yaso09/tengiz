...
|---------|---------------|
| `runtime.Manager` | Interface for container lifecycle. `NewDocker()` = exec-based impl, `NewStub()` = test mock. Also: `CreateFromImage`, `RemoveImage`, `KeepLastNImages` for rollback + image cleanup, and prune primitives (`ListContainers`, `ListImages`, `ListVolumes`, `ListNetworks`, `PruneBuildCache`, `RemoveVolume`, `RemoveNetwork`) for housekeeping. `ContainerName(name, env)` helper. |
