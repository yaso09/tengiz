...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Builder type dispatch (`auto`/`nixpacks`) via `NewWithType()`. |
...
tengiz deploy [dir] [--builder auto|nixpacks] → detect, build, run container
