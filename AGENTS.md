...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Nixpacks config via `SetNixpacksConfig()` + `buildWithNixpacks()` method for external CLI-based builds. |
