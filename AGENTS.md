...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). `Build()` dispatches to `buildWithNixpacks()` when `FrameworkNixpacks` is set or `buildWithDockerfile()` otherwise. `SetNixpacksConfig()` sets Nixpacks options (packages, apt_pkgs, cmd). |
