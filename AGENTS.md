...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Nixpacks builds (`nixpacks.go`) via CLI. |
...
tengiz deploy [dir] [--builder docker|nixpacks] → detect, build, run container
