...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks (external CLI for 100+ frameworks). Env-aware image tags (`{env}-{deploymentID}`). `BuildConfig.Builder` + `BuildConfig.NixpacksConfig` select the backend. `SetNixpacksConfig()` on `Builder`. |
