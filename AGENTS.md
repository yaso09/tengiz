...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Nixpacks selected via `cfg.Build.Builder == "nixpacks"` or `detection.Framework == FrameworkNixpacks`. |
...
| `types` | Shared: `AppConfig`, `AppStatus`, `AppEntry`, `PortEntry`, `DeploymentEntry`, `DeploymentStatus`. `AppConfig.Environment` field. `BuildConfig.Builder` + `NixpacksConfig` for nixpacks build backend. |
