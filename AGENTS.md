...
| `builder` | Framework detection (`detect.go`) + Dockerfile generation (`builder.go`). Supports: Docker, Next.js, Vite, Go, Node, Python, static, Nixpacks. Env-aware image tags (`{env}-{deploymentID}`). Nixpacks (`build.builder: nixpacks`) invokes `nixpacks build` as a subprocess for hundreds of frameworks. |
